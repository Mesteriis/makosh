...
use makosh_communication_task_candidate_api::{
    ...
    wire::CommunicationTaskCandidateStatusChangedV1,
};
...
use crate::{
    ...
    contracts::communication_task_candidate_realtime_contract_v1,
};

const REPLAY_WINDOW_V1: u16 = 1_024;

#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub(crate) struct CommunicationTaskCandidateClientRealtimePublisherV1 {
    last_sequence: Option<u64>,
}

impl CommunicationTaskCandidateClientRealtimePublisherV1 {
    ...
    ) -> Result<bool, CommunicationTaskCandidateClientRealtimeErrorV1> {
        let transitions = persistence
            .client_realtime_window(logical_owner_id, self.last_sequence, REPLAY_WINDOW_V1)
            .await
            .map_err(CommunicationTaskCandidateClientRealtimeErrorV1::Persistence)?;
        let published = !transitions.is_empty();
        for transition in transitions {
            let occurred_at_unix_millis = u64::try_from(transition.occurred_at_unix_millis)
                .map_err(|_| CommunicationTaskCandidateClientRealtimeErrorV1::InvalidTransition)?;
            let payload = CommunicationTaskCandidateStatusChangedV1 {
            ...
            let request = ManagedRuntimeClientRealtimePublishRequestV1 {
                ...
                cursor: format!("communication-task-candidate/{}", transition.sequence),
                event_kind: COMMUNICATION_TASK_CANDIDATE_REALTIME_EVENT_KIND_V1.to_owned(),
                occurred_at_unix_millis,
            ...
            let response = channel
                ...
                .map_err(|_| CommunicationTaskCandidateClientRealtimeErrorV1::Unavailable)?;
            if !response.error_code.is_empty() {
                return Err(CommunicationTaskCandidateClientRealtimeErrorV1::Unavailable);
            }
...
pub(crate) enum CommunicationTaskCandidateClientRealtimeErrorV1 {
    InvalidTransition,
