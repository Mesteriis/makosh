...
use crate::{
    client_port::{
        CommunicationTaskCandidateClientRuntimeContextV1,
...
pub struct CommunicationTaskCandidateRuntimeAdmissionV1 {
...
pub enum CommunicationTaskCandidateManagedRuntimeErrorV1 {
    Admission,
    EventContract,
    EventUnavailable,
    InvalidTransition,
...
pub struct CommunicationTaskCandidateManagedRuntimeV1 {
    admission: CommunicationTaskCandidateRuntimeAdmissionV1,
    ...
    client_realtime: CommunicationTaskCandidateClientRealtimePublisherV1,
}

impl CommunicationTaskCandidateManagedRuntimeV1 {
    pub async fn open(
    ...
    ) -> Result<Self, CommunicationTaskCandidateManagedRuntimeErrorV1> {
        validate_admission(admission)?;
        if event_hub_endpoint.trim().is_empty() || event_credential_revision == 0 {
            return Err(CommunicationTaskCandidateManagedRuntimeErrorV1::Admission);
        }
        ...
        let vault_public_key = storage_configuration
            ...
            .as_slice()
            .try_into()
            .map_err(|_| CommunicationTaskCandidateManagedRuntimeErrorV1::Admission)?;
        ...
        let password = std::str::from_utf8(&password)
            .map_err(|_| CommunicationTaskCandidateManagedRuntimeErrorV1::Admission)?;
        let persistence = CommunicationTaskCandidatePersistenceV1::connect_runtime(
            &binding,
        ...
        .await
        .map_err(CommunicationTaskCandidateManagedRuntimeErrorV1::Persistence)?;
        persistence
        ...
        .map_err(|_| CommunicationTaskCandidateManagedRuntimeErrorV1::EventUnavailable)?;
        ...
        .await
        .map_err(|_| CommunicationTaskCandidateManagedRuntimeErrorV1::EventUnavailable)?;
        let mut client_realtime = CommunicationTaskCandidateClientRealtimePublisherV1::default();
    ...
    pub async fn pump_control_once(
        ...
        if validate_module_client_response_v1(&response).is_err() {
            return Err(CommunicationTaskCandidateManagedRuntimeErrorV1::Unavailable);
        }
        self.control_channel
            .write_response(
                correlation_id,
    ...
    fn write_client_error(
        ...
        error_code: &str,
    ) -> Result<(), CommunicationTaskCandidateManagedRuntimeErrorV1> {
        self.control_channel
            .write_response(
                correlation_id,
                ManagedRuntimeControlResponseV1 {
...
async fn dispatch_client(
    ...
    let valid_identity = request.protocol_major == 1
        && request.module_id == COMMUNICATION_TASK_CANDIDATE_MODULE_ID_V1
        && request.owner_id == COMMUNICATION_TASK_CANDIDATE_OWNER_V1
        && request.logical_owner_id == admission.logical_owner_id;
    let (response_payload, accepted_route) = if valid_identity {
        if request.contract.as_ref() == Some(&communication_task_candidate_command_contract_v1()) {
            (
                start_communication_task_candidate_payload_v1(
                    persistence,
                    &admission.logical_owner_id,
                    &CommunicationTaskCandidateClientRuntimeContextV1 {
            ...
            == Some(&communication_task_candidate_query_contract_v1())
        {
            (
                get_communication_task_candidate_payload_v1(
                    persistence,
                    &admission.logical_owner_id,
                    &request.request_payload,
                )
...
fn bind_source_subscriptions(
    ...
    CommunicationTaskCandidateManagedRuntimeErrorV1,
> {
    if permits.len() != 2 {
        return Err(CommunicationTaskCandidateManagedRuntimeErrorV1::Admission);
    }
...
fn exact_permit(
...
) -> Result<RuntimeSubscribePermitV1, CommunicationTaskCandidateManagedRuntimeErrorV1> {
    ...
    let permit = matching
        .next()
        .cloned()
        .ok_or(CommunicationTaskCandidateManagedRuntimeErrorV1::Admission)?;
...
) -> Result<zeroize::Zeroizing<Vec<u8>>, CommunicationTaskCandidateManagedRuntimeErrorV1> {
    for attempt in 0..20 {
...
fn storage_binding(
    ...
    admission: &CommunicationTaskCandidateRuntimeAdmissionV1,
) -> Result<StorageBindingV1, CommunicationTaskCandidateManagedRuntimeErrorV1> {
...
fn storage_owner_is_exact(logical_owner_id: &str, owner: &str) -> bool {
    logical_owner_id == owner && owner == COMMUNICATION_TASK_CANDIDATE_OWNER_V1
}

fn event_relay_error(
    error: CommunicationTaskCandidateEventRelayErrorV1,
) -> CommunicationTaskCandidateManagedRuntimeErrorV1 {
    match error {
        CommunicationTaskCandidateEventRelayErrorV1::InvalidTimestamp => {
            CommunicationTaskCandidateManagedRuntimeErrorV1::EventContract
        }
        CommunicationTaskCandidateEventRelayErrorV1::Persistence(error) => {
            CommunicationTaskCandidateManagedRuntimeErrorV1::Persistence(error)
        }
        CommunicationTaskCandidateEventRelayErrorV1::EventUnavailable => {
            CommunicationTaskCandidateManagedRuntimeErrorV1::EventUnavailable
        }
    }
}

fn source_result_error(
    error: CommunicationTaskCandidateSourceResultErrorV1,
) -> CommunicationTaskCandidateManagedRuntimeErrorV1 {
    match error {
        CommunicationTaskCandidateSourceResultErrorV1::InvalidEnvelope
        | CommunicationTaskCandidateSourceResultErrorV1::InvalidPayload => {
            CommunicationTaskCandidateManagedRuntimeErrorV1::EventContract
        }
        CommunicationTaskCandidateSourceResultErrorV1::Blob(
            crate::CommunicationTaskCandidateBlobErrorV1::InvalidReceipt,
        ) => CommunicationTaskCandidateManagedRuntimeErrorV1::EventContract,
        CommunicationTaskCandidateSourceResultErrorV1::Blob(
            crate::CommunicationTaskCandidateBlobErrorV1::Unavailable,
        ) => CommunicationTaskCandidateManagedRuntimeErrorV1::Unavailable,
        CommunicationTaskCandidateSourceResultErrorV1::Extraction(error) => extraction_error(error),
        CommunicationTaskCandidateSourceResultErrorV1::Persistence(error) => {
            CommunicationTaskCandidateManagedRuntimeErrorV1::Persistence(error)
        }
...
fn extraction_error(
    error: CommunicationTaskCandidateExtractionErrorV1,
) -> CommunicationTaskCandidateManagedRuntimeErrorV1 {
    match error {
        CommunicationTaskCandidateExtractionErrorV1::InvalidState => {
            CommunicationTaskCandidateManagedRuntimeErrorV1::InvalidTransition
        }
...
fn client_realtime_error(
    error: CommunicationTaskCandidateClientRealtimeErrorV1,
) -> CommunicationTaskCandidateManagedRuntimeErrorV1 {
    match error {
        CommunicationTaskCandidateClientRealtimeErrorV1::InvalidTransition => {
            CommunicationTaskCandidateManagedRuntimeErrorV1::InvalidTransition
        }
        CommunicationTaskCandidateClientRealtimeErrorV1::Persistence(error) => {
            CommunicationTaskCandidateManagedRuntimeErrorV1::Persistence(error)
        }
        CommunicationTaskCandidateClientRealtimeErrorV1::Unavailable => {
            CommunicationTaskCandidateManagedRuntimeErrorV1::Unavailable
        }
    }
}
...
mod tests {
    ...
    fn admission_requires_exact_managed_fences() {
        assert_eq!(
            validate_admission(&CommunicationTaskCandidateRuntimeAdmissionV1 {
