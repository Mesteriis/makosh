//! Communication Delivery Intent workflow release assembly.
//!
...
pub const DELIVERY_INTENT_DESCRIPTOR_FILE: &str =
    "communication_delivery_intent.runtime.descriptor.pb";
pub const DELIVERY_INTENT_SETTINGS_FILE: &str = "communication_delivery_intent.runtime.settings.pb";
pub const DELIVERY_INTENT_STORAGE_BUNDLE_FILE: &str =
    "communication_delivery_intent.storage.bundle.pb";
...
const STORAGE_RELATIVE_PATH: &str = "storage/communication_delivery_intent.storage.bundle.pb";
...
#[derive(Clone, Debug, Eq, PartialEq, Deserialize, Serialize)]
#[serde(untagged)]
pub enum DeliveryIntentReleaseArtifactInputV1 {
    ModuleRuntime(ModuleRuntimeArtifactInputV1),
    StorageBundle(StorageBundleArtifactInputV1),
}

impl DeliveryIntentReleaseArtifactInputV1 {
    #[must_use]
...
#[derive(Clone, Debug, Eq, PartialEq, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
pub struct DeliveryIntentReleaseArtifactFragmentV1 {
    pub version: u32,
    pub owner_id: String,
    pub module_id: String,
    pub artifacts: Vec<DeliveryIntentReleaseArtifactInputV1>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct DeliveryIntentReleaseAssemblyPathsV1 {
...
pub enum DeliveryIntentReleaseAssemblyErrorV1 {
    InvalidInput,
...
pub fn materialize_delivery_intent_release_assembly_v1(
    output_directory: &Path,
    build_id: &str,
    runtime_source: &Path,
) -> Result<DeliveryIntentReleaseAssemblyPathsV1, DeliveryIntentReleaseAssemblyErrorV1> {
    validate_inputs(output_directory, build_id, runtime_source)?;
    let descriptor = communication_delivery_intent_module_descriptor_v1(build_id);
    let settings_schema = communication_delivery_intent_settings_schema_v1();
    let storage_bundle = communication_delivery_intent_storage_bundle_v1();
    ...
    {
        return Err(DeliveryIntentReleaseAssemblyErrorV1::InvalidCanonicalArtifact);
    }

    let paths = DeliveryIntentReleaseAssemblyPathsV1 {
        descriptor: output_directory.join(DELIVERY_INTENT_DESCRIPTOR_FILE),
        settings_schema: output_directory.join(DELIVERY_INTENT_SETTINGS_FILE),
        storage_bundle: output_directory.join(DELIVERY_INTENT_STORAGE_BUNDLE_FILE),
        artifact_fragment: output_directory.join(DELIVERY_INTENT_ARTIFACT_FRAGMENT_FILE),
    };
    ...
    let fragment_bytes = serde_json::to_vec_pretty(&fragment)
        .map_err(|_| DeliveryIntentReleaseAssemblyErrorV1::FragmentEncodingFailed)?;
    let writes = [
    ...
    builder
        ...
        .map_err(|_| DeliveryIntentReleaseAssemblyErrorV1::OutputUnavailable)?;
    for (path, bytes) in &writes {
        if write_new_private_file(path, bytes).is_err() {
            let _ = fs::remove_dir_all(output_directory);
            return Err(DeliveryIntentReleaseAssemblyErrorV1::ArtifactWriteFailed);
        }
    }
    Ok(paths)
}
...
fn validate_inputs(
    ...
    build_id: &str,
    runtime_source: &Path,
) -> Result<(), DeliveryIntentReleaseAssemblyErrorV1> {
    ...
    {
        return Err(DeliveryIntentReleaseAssemblyErrorV1::InvalidInput);
    }
    Ok(())
}
...
fn artifact_fragment(
    ...
    storage_bundle: &Path,
) -> Result<DeliveryIntentReleaseArtifactFragmentV1, DeliveryIntentReleaseAssemblyErrorV1> {
    let artifacts = vec![
        DeliveryIntentReleaseArtifactInputV1::ModuleRuntime(ModuleRuntimeArtifactInputV1 {
            artifact_kind: "module_runtime".to_owned(),
            artifact_id: DELIVERY_INTENT_RUNTIME_ARTIFACT_ID.to_owned(),
        ...
        DeliveryIntentReleaseArtifactInputV1::StorageBundle(StorageBundleArtifactInputV1 {
            artifact_kind: "storage_bundle".to_owned(),
            artifact_id: DELIVERY_INTENT_STORAGE_ARTIFACT_ID.to_owned(),
    ...
    Ok(DeliveryIntentReleaseArtifactFragmentV1 {
        version: DELIVERY_INTENT_ASSEMBLY_FRAGMENT_VERSION_V1,
        owner_id: DELIVERY_INTENT_ASSEMBLY_OWNER_ID.to_owned(),
        module_id: DELIVERY_INTENT_ASSEMBLY_MODULE_ID.to_owned(),
        artifacts,
    })
}
...
fn utf8_path(path: &Path) -> Result<String, DeliveryIntentReleaseAssemblyErrorV1> {
    path.to_str()
        .filter(|value| !value.is_empty())
        .map(str::to_owned)
        .ok_or(DeliveryIntentReleaseAssemblyErrorV1::InvalidInput)
}
...
mod tests {
    ...
    fn materializes_exact_unsigned_workflow_runtime_and_storage_fragment() {
        ...
        let fragment: DeliveryIntentReleaseArtifactFragmentV1 =
            serde_json::from_slice(&fs::read(paths.artifact_fragment).expect("fragment"))
                .expect("decode fragment");
        assert_eq!(descriptor.module_id, DELIVERY_INTENT_ASSEMBLY_MODULE_ID);
        assert_eq!(settings.major, 1);
        assert_eq!(storage.owner_id, DELIVERY_INTENT_ASSEMBLY_OWNER_ID);
        assert_eq!(
            fragment
                .artifacts
                .iter()
                .map(DeliveryIntentReleaseArtifactInputV1::artifact_id)
                .collect::<Vec<_>>(),
            [
    ...
    fn rejects_missing_symlinked_runtime_and_existing_output() {
        ...
        let output = root.join("assembly");
        assert_eq!(
            materialize_delivery_intent_release_assembly_v1(
                &output,
                "build-1",
                &root.join("missing"),
            ),
            Err(DeliveryIntentReleaseAssemblyErrorV1::InvalidInput)
        );
        assert_eq!(
            materialize_delivery_intent_release_assembly_v1(&output, "build-1", &runtime_link,),
            Err(DeliveryIntentReleaseAssemblyErrorV1::InvalidInput)
        );
        fs::create_dir(&output).expect("existing output");
        assert_eq!(
            materialize_delivery_intent_release_assembly_v1(&output, "build-1", &runtime),
            Err(DeliveryIntentReleaseAssemblyErrorV1::InvalidInput)
        );
        fs::remove_dir_all(root).expect("remove fixture");
    }
    ...
    fn temporary_directory() -> PathBuf {
        let root = std::env::current_dir()
            ...
            .join("target")
            .join(format!(
                "communication-delivery-intent-assembly-{}-{}",
                std::process::id(),
