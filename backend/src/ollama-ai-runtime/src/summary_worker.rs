...
pub(crate) async fn execute_summary_payload_v1(
    ...
    let request = AiProviderSummaryGenerationRequestV1::decode(payload)
        .map_err(|_| OllamaAiWorkerErrorV1::InvalidRequest)?;
    validate_provider_summary_generation_request_v1(&request)
        .map_err(|_| OllamaAiWorkerErrorV1::InvalidRequest)?;
    let accepted = accept_ollama_summary_request_v1(&request, settings)
        .map_err(|_| OllamaAiWorkerErrorV1::InvalidRequest)?;
    let persisted = persistence
        .accept_summary_run(logical_owner_id, accepted)
        .await
...
async fn drive_run(
    persistence: &OllamaAiPersistenceV1,
    port: &mut impl OllamaAiExecutionPortV1,
    mut persisted: PersistedOllamaSummaryRunV1,
    ...
    if persisted.run.state == OllamaAiRunStateV1::Executing {
        let uncertain = mark_ollama_summary_uncertain_v1(&persisted.run)
    ...
    let (executing, plan) =
        begin_ollama_summary_request_v1(&persisted.run, request, settings, model.digest)
            .map_err(|_| OllamaAiWorkerErrorV1::Conflict)?;
    persisted = persist(persistence, &persisted, executing).await?;
    let generated = match port.generate_summary(settings, &plan).await {
        Ok(value) => value,
    ...
    let ready = complete_ollama_summary_request_v1(&persisted.run, &plan, generated)
        .map_err(|_| OllamaAiWorkerErrorV1::InvalidRequest)?;
...
async fn reject(
    persistence: &OllamaAiPersistenceV1,
    persisted: PersistedOllamaSummaryRunV1,
    status: AiInferenceTerminalStatusV1,
) -> Result<Vec<u8>, OllamaAiWorkerErrorV1> {
    let next = reject_ollama_summary_request_v1(&persisted.run, status)
        .ok_or(OllamaAiWorkerErrorV1::Conflict)?;
...
async fn uncertain(
    ...
    let next =
        mark_ollama_summary_uncertain_v1(&persisted.run).ok_or(OllamaAiWorkerErrorV1::Conflict)?;
...
async fn persist(
    ...
    next_run: OllamaSummaryRunV1,
) -> Result<PersistedOllamaSummaryRunV1, OllamaAiWorkerErrorV1> {
    persistence
        .persist_summary_transition(OllamaSummaryTransitionV1 {
