use makosh_ai_contracts::{
    validate_provider_translation_request_v1,
    wire::{AiInferenceTerminalStatusV1, AiProviderTranslationRequestV1},
};
use makosh_ollama_ai_api::OllamaAiRuntimeSettingsV1;
use makosh_ollama_ai_core::{
    OllamaAiRunStateV1, OllamaTranslationRunV1, accept_ollama_translation_request_v1,
...
use makosh_ollama_ai_persistence::{
    OllamaAiPersistenceErrorV1, OllamaAiPersistenceV1, OllamaTranslationTransitionV1,
...
pub(crate) async fn execute_translation_payload_v1(
    ...
    let request = AiProviderTranslationRequestV1::decode(payload)
        .map_err(|_| OllamaAiWorkerErrorV1::InvalidRequest)?;
    validate_provider_translation_request_v1(&request)
        .map_err(|_| OllamaAiWorkerErrorV1::InvalidRequest)?;
    let accepted = accept_ollama_translation_request_v1(&request, settings)
        .map_err(|_| OllamaAiWorkerErrorV1::InvalidRequest)?;
    let persisted = persistence
        .accept_translation_run(logical_owner_id, accepted)
        .await
...
async fn drive_run(
    persistence: &OllamaAiPersistenceV1,
    port: &mut impl OllamaAiExecutionPortV1,
    mut persisted: PersistedOllamaTranslationRunV1,
    ...
    if persisted.run.state == OllamaAiRunStateV1::Executing {
        let uncertain = mark_ollama_translation_uncertain_v1(&persisted.run)
    ...
    let (executing, plan) =
        begin_ollama_translation_request_v1(&persisted.run, request, settings, model.digest)
            .map_err(|_| OllamaAiWorkerErrorV1::Conflict)?;
    persisted = persist(persistence, &persisted, executing).await?;
    let generated = match port.generate_translation(settings, &plan).await {
        Ok(value) => value,
    ...
    let ready = match complete_ollama_translation_request_v1(&persisted.run, &plan, generated) {
        Ok(value) => value,
        Err(_) => {
            return reject(
                persistence,
...
async fn reject(
    persistence: &OllamaAiPersistenceV1,
    persisted: PersistedOllamaTranslationRunV1,
    status: AiInferenceTerminalStatusV1,
) -> Result<Vec<u8>, OllamaAiWorkerErrorV1> {
    let next = reject_ollama_translation_request_v1(&persisted.run, status)
        .ok_or(OllamaAiWorkerErrorV1::Conflict)?;
...
async fn uncertain(
    ...
    let next = mark_ollama_translation_uncertain_v1(&persisted.run)
        .ok_or(OllamaAiWorkerErrorV1::Conflict)?;
...
async fn persist(
    ...
    next_run: OllamaTranslationRunV1,
) -> Result<PersistedOllamaTranslationRunV1, OllamaAiWorkerErrorV1> {
    persistence
        .persist_translation_transition(OllamaTranslationTransitionV1 {
