//! Unsigned Contacts release assembly.
//!
//! This build unit materializes canonical runtime contracts and Contacts-owned
//! Storage input. It never launches the runtime or receives signing authority.
...
pub const CONTACTS_ASSEMBLY_FRAGMENT_VERSION_V1: u32 = 1;
pub const CONTACTS_RUNTIME_ARTIFACT_ID_V1: &str = "contacts.runtime.v1";
pub const CONTACTS_STORAGE_ARTIFACT_ID_V1: &str = "contacts.storage.v1";
pub const CONTACTS_DESCRIPTOR_FILE_V1: &str = "contacts.runtime.descriptor.pb";
pub const CONTACTS_SETTINGS_FILE_V1: &str = "contacts.runtime.settings.pb";
...
#[derive(Clone, Debug, Eq, PartialEq, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
pub struct ContactsReleaseArtifactFragmentV1 {
    pub version: u32,
    pub owner_id: String,
    pub module_id: String,
    pub artifacts: Vec<ContactsReleaseArtifactInputV1>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ContactsReleaseAssemblyPathsV1 {
...
pub enum ContactsReleaseAssemblyErrorV1 {
    InvalidInput,
...
pub fn materialize_contacts_release_assembly_v1(
    output_directory: &Path,
    build_id: &str,
    runtime_source: &Path,
) -> Result<ContactsReleaseAssemblyPathsV1, ContactsReleaseAssemblyErrorV1> {
    validate_inputs(output_directory, build_id, runtime_source)?;
    let descriptor = contacts_module_descriptor_v1(build_id);
    let settings = contacts_settings_schema_v1();
    let storage = contacts_storage_bundle_v1();
    ...
    {
        return Err(ContactsReleaseAssemblyErrorV1::InvalidCanonicalArtifact);
    }
    let paths = ContactsReleaseAssemblyPathsV1 {
        descriptor: output_directory.join(CONTACTS_DESCRIPTOR_FILE_V1),
        settings_schema: output_directory.join(CONTACTS_SETTINGS_FILE_V1),
        storage_bundle: output_directory.join(CONTACTS_STORAGE_BUNDLE_FILE_V1),
        artifact_fragment: output_directory.join(CONTACTS_ARTIFACT_FRAGMENT_FILE_V1),
    };
    ...
    let fragment_bytes = serde_json::to_vec_pretty(&fragment)
        .map_err(|_| ContactsReleaseAssemblyErrorV1::FragmentEncodingFailed)?;

    let mut directory = DirBuilder::new();
    directory.mode(0o700);
    directory
        .create(output_directory)
        .map_err(|_| ContactsReleaseAssemblyErrorV1::OutputUnavailable)?;
    let writes = [
    ...
    for (path, bytes) in writes {
        if write_new_private_file(path, &bytes).is_err() {
            ...
            return Err(ContactsReleaseAssemblyErrorV1::ArtifactWriteFailed);
        }
    }
    Ok(paths)
}
...
fn validate_inputs(
    ...
    build_id: &str,
    runtime_source: &Path,
) -> Result<(), ContactsReleaseAssemblyErrorV1> {
    ...
    if !output_directory.is_absolute()
        ...
        || !valid_runtime
    {
        return Err(ContactsReleaseAssemblyErrorV1::InvalidInput);
    }
    Ok(())
}
...
fn artifact_fragment(
    ...
    module_id: &str,
    runtime_source: &Path,
    paths: &ContactsReleaseAssemblyPathsV1,
) -> Result<ContactsReleaseArtifactFragmentV1, ContactsReleaseAssemblyErrorV1> {
    let artifacts = vec![
        ContactsReleaseArtifactInputV1::ModuleRuntime(ModuleRuntimeArtifactInputV1 {
            artifact_kind: "module_runtime".to_owned(),
            artifact_id: CONTACTS_RUNTIME_ARTIFACT_ID_V1.to_owned(),
        ...
        ContactsReleaseArtifactInputV1::StorageBundle(StorageBundleArtifactInputV1 {
            artifact_kind: "storage_bundle".to_owned(),
            artifact_id: CONTACTS_STORAGE_ARTIFACT_ID_V1.to_owned(),
    ...
    Ok(ContactsReleaseArtifactFragmentV1 {
        version: CONTACTS_ASSEMBLY_FRAGMENT_VERSION_V1,
...
fn utf8_path(path: &Path) -> Result<String, ContactsReleaseAssemblyErrorV1> {
    path.to_str()
        .filter(|value| !value.is_empty())
        .map(str::to_owned)
        .ok_or(ContactsReleaseAssemblyErrorV1::InvalidInput)
}
...
mod tests {
    ...
    #[test]
    fn materializes_exact_unsigned_contacts_artifacts() {
        let root = temporary_directory();
        let runtime = root.join("runtime");
        fs::write(&runtime, b"runtime").expect("runtime fixture");
        let output = root.join("assembly");
        let paths = materialize_contacts_release_assembly_v1(&output, "build-1", &runtime)
            .expect("assembly");
        ...
        StorageBundleV1::decode(fs::read(paths.storage_bundle).expect("storage").as_slice())
            .expect("storage");
        let fragment: ContactsReleaseArtifactFragmentV1 =
            serde_json::from_slice(&fs::read(paths.artifact_fragment).expect("fragment"))
                .expect("fragment");
        assert_eq!(fragment.owner_id, "contacts");
    ...
    fn refuses_existing_output_and_symlink_runtime() {
        ...
        assert_eq!(
            materialize_contacts_release_assembly_v1(&output, "build-1", &runtime),
            Err(ContactsReleaseAssemblyErrorV1::InvalidInput)
        );
        let symlink = root.join("runtime-link");
        std::os::unix::fs::symlink(&runtime, &symlink).expect("symlink fixture");
        assert_eq!(
            materialize_contacts_release_assembly_v1(&root.join("other"), "build-1", &symlink),
            Err(ContactsReleaseAssemblyErrorV1::InvalidInput)
        );
        fs::remove_dir_all(root).expect("cleanup");
    }
