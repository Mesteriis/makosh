//! WhatsApp-owned reverse locator for provider-neutral delivery intents.
...
pub const WHATSAPP_DELIVERY_ROUTE_SCHEMA_V1: &str = r#"
CREATE TABLE IF NOT EXISTS makosh_data.whatsapp_delivery_route_accounts (
...
CREATE TABLE IF NOT EXISTS makosh_data.whatsapp_delivery_route_conversations (
...
CREATE TABLE IF NOT EXISTS makosh_data.whatsapp_delivery_route_messages (
...
pub struct WhatsAppDeliveryRouteLocatorV1 {
...
impl WhatsAppDeliveryRouteLocatorV1 {
    pub fn new(
        account_id: &str,
        ...
        Ok(Self {
            account_cursor: account_source_cursor_v1(ProviderProvenanceV1::WhatsAppWeb, account_id)
                .map_err(|_| WhatsAppDurablePersistenceError::InvalidRow)?,
            conversation_cursor: conversation_source_cursor_v1(
                ProviderProvenanceV1::WhatsAppWeb,
                account_id,
                provider_chat_id,
            )
            .map_err(|_| WhatsAppDurablePersistenceError::InvalidRow)?,
...
pub(crate) async fn upsert_delivery_route_locator(
    ...
    locator: &WhatsAppDeliveryRouteLocatorV1,
    ...
    let account = sqlx::query(
        "INSERT INTO makosh_data.whatsapp_delivery_route_accounts
         ...
         ON CONFLICT (account_cursor) DO UPDATE SET
            ...
            updated_at_unix_seconds = GREATEST(
                makosh_data.whatsapp_delivery_route_accounts.updated_at_unix_seconds,
                EXCLUDED.updated_at_unix_seconds)
         WHERE makosh_data.whatsapp_delivery_route_accounts.account_id = EXCLUDED.account_id",
    )
    ...
    if account.rows_affected() != 1 {
        return Err(WhatsAppDurablePersistenceError::ConflictingDeliveryRouteLocator);
    }
    let conversation = sqlx::query(
        "INSERT INTO makosh_data.whatsapp_delivery_route_conversations
            (conversation_cursor, account_cursor, account_id, provider_chat_id,
             updated_at_unix_seconds)
         VALUES ($1, $2, $3, $4, $5)
         ON CONFLICT (conversation_cursor) DO UPDATE SET
            updated_at_unix_seconds = GREATEST(
                makosh_data.whatsapp_delivery_route_conversations.updated_at_unix_seconds,
                EXCLUDED.updated_at_unix_seconds)
         WHERE makosh_data.whatsapp_delivery_route_conversations.account_cursor =
                   EXCLUDED.account_cursor
           AND makosh_data.whatsapp_delivery_route_conversations.account_id =
                   EXCLUDED.account_id
           AND makosh_data.whatsapp_delivery_route_conversations.provider_chat_id =
    ...
    let message = sqlx::query(
        "INSERT INTO makosh_data.whatsapp_delivery_route_messages
         ...
         ON CONFLICT (source_cursor) DO UPDATE SET
            updated_at_unix_seconds = GREATEST(
                makosh_data.whatsapp_delivery_route_messages.updated_at_unix_seconds,
                EXCLUDED.updated_at_unix_seconds)
         WHERE makosh_data.whatsapp_delivery_route_messages.account_cursor =
                   EXCLUDED.account_cursor
           AND makosh_data.whatsapp_delivery_route_messages.conversation_cursor =
                   EXCLUDED.conversation_cursor
           AND makosh_data.whatsapp_delivery_route_messages.account_id = EXCLUDED.account_id
           AND makosh_data.whatsapp_delivery_route_messages.provider_chat_id =
                   EXCLUDED.provider_chat_id
           AND makosh_data.whatsapp_delivery_route_messages.provider_message_id =
...
fn valid_locator(value: &WhatsAppDeliveryRouteLocatorV1) -> bool {
...
mod tests {
    ...
    fn locator_reproduces_the_exact_communications_scope_cursors() {
        let locator = WhatsAppDeliveryRouteLocatorV1::new("account-1", "chat-1", "message-1")
            .expect("locator");
        assert_eq!(
            locator.account_cursor,
            account_source_cursor_v1(ProviderProvenanceV1::WhatsAppWeb, "account-1")
    ...
    #[test]
    fn schema_is_whatsapp_owned_and_has_no_cross_owner_foreign_keys() {
        assert!(WHATSAPP_DELIVERY_ROUTE_SCHEMA_V1.contains("whatsapp_delivery_route_accounts"));
        assert!(!WHATSAPP_DELIVERY_ROUTE_SCHEMA_V1.contains("communications_"));
        assert!(!WHATSAPP_DELIVERY_ROUTE_SCHEMA_V1.contains("REFERENCES"));
    }
}
