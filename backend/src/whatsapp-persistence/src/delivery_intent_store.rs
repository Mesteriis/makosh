//! WhatsApp-owned atomic delivery-intent lifecycle store.

use makosh_events_protocol::delivery::OutboxRecordV1;
use sqlx::{PgPool, Row};

use crate::WhatsAppDurablePersistenceError;

pub const WHATSAPP_DELIVERY_INTENT_SCHEMA_V1: &str = r#"
CREATE TABLE IF NOT EXISTS makosh_data.whatsapp_delivery_intent_inbox (
...
CREATE TABLE IF NOT EXISTS makosh_data.whatsapp_delivery_intent_jobs (
...
CREATE INDEX IF NOT EXISTS whatsapp_delivery_intent_jobs_claim_idx
    ON makosh_data.whatsapp_delivery_intent_jobs
        (state, next_attempt_at_unix_seconds, intent_id);

CREATE TABLE IF NOT EXISTS makosh_data.whatsapp_delivery_intent_result_outbox (
    message_id BYTEA PRIMARY KEY,
    envelope_sha256 BYTEA NOT NULL,
...
pub const WHATSAPP_DELIVERY_INTENT_MAX_ATTEMPTS_V1: i32 = 12;
...
pub struct WhatsAppDeliveryIntentAdmissionV1 {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct WhatsAppDeliveryIntentJobV1 {
    pub intent_id: [u8; 16],
    pub command_message_id: [u8; 16],
...
#[repr(i16)]
pub enum WhatsAppDeliveryIntentJobStateV1 {
    PendingCustody = 1,
    BodyReady = 2,
    DeliveryQueued = 3,
    Succeeded = 4,
...
impl WhatsAppDeliveryIntentJobStateV1 {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ClaimedWhatsAppDeliveryIntentJobV1 {
    pub job: WhatsAppDeliveryIntentJobV1,
    pub state: WhatsAppDeliveryIntentJobStateV1,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum WhatsAppDeliveryIntentInboxOutcomeV1 {
    Pending,
    RouteNotFound,
    DuplicatePending,
...
impl WhatsAppDeliveryIntentStoreV1 {
    ...
    pub async fn accept_command(
        &self,
        admission: &WhatsAppDeliveryIntentAdmissionV1,
        route_not_found_result: &OutboxRecordV1,
        consumed_at_unix_seconds: i64,
    ) -> Result<WhatsAppDeliveryIntentInboxOutcomeV1, WhatsAppDurablePersistenceError> {
        ...
        let inserted = sqlx::query(
            "INSERT INTO makosh_data.whatsapp_delivery_intent_inbox
        ...
        if inserted.rows_affected() == 0 {
            let row = sqlx::query(
                "SELECT envelope_sha256, intent_id, logical_owner_id, state
                 FROM makosh_data.whatsapp_delivery_intent_inbox
                 WHERE message_id = $1",
            )
            .bind(admission.command_message_id.as_slice())
            ...
            {
                return Err(WhatsAppDurablePersistenceError::ConflictingDeliveryIntentInbox);
            }
            transaction
                .commit()
                .await
                .map_err(|_| WhatsAppDurablePersistenceError::Database)?;
            return match state {
                1 => Ok(WhatsAppDeliveryIntentInboxOutcomeV1::DuplicatePending),
                2 => Ok(WhatsAppDeliveryIntentInboxOutcomeV1::DuplicateRouteNotFound),
        ...
        let outcome = if let Some(route) = route {
            sqlx::query(
                "INSERT INTO makosh_data.whatsapp_delivery_intent_jobs
            ...
            .map_err(|_| WhatsAppDurablePersistenceError::Database)?;
            sqlx::query(
                "UPDATE makosh_data.whatsapp_delivery_intent_inbox SET state = 1
            ...
            .await?;
            sqlx::query(
                "UPDATE makosh_data.whatsapp_delivery_intent_inbox SET state = 2
                 WHERE message_id = $1 AND state = 0",
            )
            .bind(admission.command_message_id.as_slice())
            .execute(&mut *transaction)
            .await
            .map_err(|_| WhatsAppDurablePersistenceError::Database)?;
            WhatsAppDeliveryIntentInboxOutcomeV1::RouteNotFound
        };
        transaction
            .commit()
            .await
    ...
    pub async fn claim_next_job(
    ...
    ) -> Result<Option<ClaimedWhatsAppDeliveryIntentJobV1>, WhatsAppDurablePersistenceError> {
        ...
        let row = sqlx::query(
            "WITH next AS (
                SELECT intent_id
                FROM makosh_data.whatsapp_delivery_intent_jobs
             ...
             UPDATE makosh_data.whatsapp_delivery_intent_jobs job
             SET claimed_by = $3,
                 ...
                 END
             FROM next, makosh_data.whatsapp_delivery_intent_inbox inbox
        ...
        .bind(WHATSAPP_DELIVERY_INTENT_MAX_ATTEMPTS_V1)
        .bind(worker_id)
    ...
    pub async fn record_target_body_receipt(
        ...
        let updated = sqlx::query(
            "UPDATE makosh_data.whatsapp_delivery_intent_jobs
             SET state = $1,
        ...
        .bind(WhatsAppDeliveryIntentJobStateV1::BodyReady as i16)
        .bind(target_body_reference_id.as_slice())
        .bind(target_body_receipt_sha256.as_slice())
        .bind(now_unix_seconds)
        .bind(intent_id.as_slice())
        .bind(WhatsAppDeliveryIntentJobStateV1::PendingCustody as i16)
        .bind(worker_id)
        .execute(&self.pool)
        .await
        .map_err(|_| WhatsAppDurablePersistenceError::Database)?;
    ...
    pub async fn mark_delivery_queued(
        ...
        .bind(WhatsAppDeliveryIntentJobStateV1::DeliveryQueued as i16)
        .bind(now_unix_seconds)
        .bind(intent_id.as_slice())
        .bind(WhatsAppDeliveryIntentJobStateV1::BodyReady as i16)
        .bind(worker_id)
    ...
    pub async fn reschedule_claimed_job(
        ...
        intent_id: [u8; 16],
        worker_id: &str,
        state: WhatsAppDeliveryIntentJobStateV1,
...
fn claimed_job_from_row(
    ...
    worker_id: &str,
) -> Result<ClaimedWhatsAppDeliveryIntentJobV1, WhatsAppDurablePersistenceError> {
    ...
    Ok(ClaimedWhatsAppDeliveryIntentJobV1 {
        job: WhatsAppDeliveryIntentJobV1 {
        ...
        state: WhatsAppDeliveryIntentJobStateV1::from_i16(
            row.try_get("state")
                .map_err(|_| WhatsAppDurablePersistenceError::InvalidRow)?,
        )
...
fn required_id<const WIDTH: usize>(
    row: &sqlx::postgres::PgRow,
    column: &str,
) -> Result<[u8; WIDTH], WhatsAppDurablePersistenceError> {
...
fn optional_id<const WIDTH: usize>(
    row: &sqlx::postgres::PgRow,
    column: &str,
) -> Result<Option<[u8; WIDTH]>, WhatsAppDurablePersistenceError> {
...
struct ResolvedWhatsAppDeliveryRouteV1 {
    account_id: String,
...
async fn resolve_route(
    ...
    admission: &WhatsAppDeliveryIntentAdmissionV1,
) -> Result<Option<ResolvedWhatsAppDeliveryRouteV1>, WhatsAppDurablePersistenceError> {
    let row = if let Some(reply_cursor) = admission.reply_to_source_cursor {
        sqlx::query(
            "SELECT account.account_id, conversation.provider_chat_id,
             ...
             FROM makosh_data.whatsapp_delivery_route_accounts account
             JOIN makosh_data.whatsapp_delivery_route_conversations conversation
               ON conversation.account_cursor = account.account_cursor
             JOIN makosh_data.whatsapp_delivery_route_messages message
    ...
    .map_err(|_| WhatsAppDurablePersistenceError::Database)?;
    row.map(|row| {
        Ok(ResolvedWhatsAppDeliveryRouteV1 {
            account_id: required_string(&row, "account_id")?,
            provider_chat_id: required_string(&row, "provider_chat_id")?,
            reply_to_provider_message_id: row
                .try_get("reply_to_provider_message_id")
...
mod tests {
    ...
    fn schema_separates_inbox_jobs_and_result_outbox() {
        assert!(WHATSAPP_DELIVERY_INTENT_SCHEMA_V1.contains("whatsapp_delivery_intent_inbox"));
        assert!(WHATSAPP_DELIVERY_INTENT_SCHEMA_V1.contains("whatsapp_delivery_intent_jobs"));
        assert!(
            WHATSAPP_DELIVERY_INTENT_SCHEMA_V1.contains("whatsapp_delivery_intent_result_outbox")
        );
        assert!(!WHATSAPP_DELIVERY_INTENT_SCHEMA_V1.contains("communications_"));
    }
    ...
    fn provider_operation_identity_is_stable_and_private() {
        ...
        assert_eq!(
            value,
            "whatsapp-delivery-intent-07070707070707070707070707070707"
        );
