...
use crate::{
    REVIEW_OBLIGATION_CANDIDATE_APPROVED_CONTRACT_NAME_V1,
    ...
    REVIEW_OBLIGATION_CANDIDATE_SUBMISSION_REJECTED_CONTRACT_NAME_V1,
    REVIEW_OBLIGATION_CANDIDATE_SUBMIT_CONTRACT_NAME_V1,
    REVIEW_OBLIGATION_CANDIDATE_SUBMITTED_CONTRACT_NAME_V1,
    wire::{
        ObligationCandidateApprovedForPromotionV1, ObligationCandidateReviewSubmissionRejectedV1,
        ObligationCandidateReviewSubmittedV1, SubmitObligationCandidateForReviewCommandV1,
    },
};

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ReviewObligationCandidateEnvelopeContextV1 {
...
pub enum ReviewObligationCandidateEnvelopeBuildErrorV1 {
    InvalidContext,
    InvalidPayload,
    InvalidEnvelope,
    OutboxRejected,
...
pub fn build_submit_review_obligation_candidate_outbox_record_v1(
    payload: SubmitObligationCandidateForReviewCommandV1,
    deadline_unix_seconds: i64,
    context: &ReviewObligationCandidateEnvelopeContextV1,
) -> Result<OutboxRecordV1, ReviewObligationCandidateEnvelopeBuildErrorV1> {
    validate_context(context)?;
    let submission_id = validate_submission(&payload)?;
    if deadline_unix_seconds <= context.recorded_at_unix_seconds {
        return Err(ReviewObligationCandidateEnvelopeBuildErrorV1::InvalidPayload);
    }
    build(
        submission_id,
        submission_id,
        ...
        Semantics::Command(CommandMetadataV1 {
            ...
            target_capability: REVIEW_OBLIGATION_CANDIDATE_SUBMISSION_CAPABILITY_ID_V1.to_owned(),
            idempotency_key: digest(b"review-obligation-candidate-submit-v1", &submission_id)
                .to_vec(),
...
pub fn build_review_obligation_candidate_submitted_outbox_record_v1(
    command_message_id: [u8; 16],
    payload: ObligationCandidateReviewSubmittedV1,
    context: &ReviewObligationCandidateEnvelopeContextV1,
) -> Result<OutboxRecordV1, ReviewObligationCandidateEnvelopeBuildErrorV1> {
    validate_context(context)?;
...
pub fn build_review_obligation_candidate_submission_rejected_outbox_record_v1(
    command_message_id: [u8; 16],
    payload: ObligationCandidateReviewSubmissionRejectedV1,
    context: &ReviewObligationCandidateEnvelopeContextV1,
) -> Result<OutboxRecordV1, ReviewObligationCandidateEnvelopeBuildErrorV1> {
    validate_context(context)?;
    let submission_id = id16(&payload.submission_id)?;
...
pub fn build_review_obligation_candidate_approved_outbox_record_v1(
    ...
    let message_id = digest_with_revision(
        b"review-obligation-candidate-approved-v1",
        &review_id,
...
fn build(
    ...
    validate_envelope_v1(&envelope)
        .map_err(|_| ReviewObligationCandidateEnvelopeBuildErrorV1::InvalidEnvelope)?;
    OutboxRecordV1::accept(envelope.encode_to_vec()).map_err(outbox_error)
}

fn validate_submission(
    payload: &SubmitObligationCandidateForReviewCommandV1,
) -> Result<[u8; 16], ReviewObligationCandidateEnvelopeBuildErrorV1> {
    let submission_id = id16(&payload.submission_id)?;
    id16(&payload.candidate_id)?;
    id32(&payload.candidate_digest)?;
    id16(&payload.source_evidence_id)?;
...
fn validate_context(
    context: &ReviewObligationCandidateEnvelopeContextV1,
) -> Result<(), ReviewObligationCandidateEnvelopeBuildErrorV1> {
    ...
    {
        return Err(ReviewObligationCandidateEnvelopeBuildErrorV1::InvalidContext);
    }
    Ok(())
}
...
fn valid_receipt(reference: &[u8], bytes: u64, sha256: &[u8], proof: &[u8]) -> bool {
    reference.len() == 16
        ...
        && (1..=REVIEW_OBLIGATION_CANDIDATE_MAX_BLOB_BYTES_V1).contains(&bytes)
        && sha256.len() == 32
        && sha256.iter().any(|byte| *byte != 0)
        && !proof.is_empty()
        && proof.len() <= REVIEW_OBLIGATION_CANDIDATE_MAX_PROOF_BYTES_V1
}
...
fn id16(value: &[u8]) -> Result<[u8; 16], ReviewObligationCandidateEnvelopeBuildErrorV1> {
    value
        .try_into()
        .ok()
        .filter(valid_id)
        .ok_or(ReviewObligationCandidateEnvelopeBuildErrorV1::InvalidPayload)
}
fn id32(value: &[u8]) -> Result<[u8; 32], ReviewObligationCandidateEnvelopeBuildErrorV1> {
    value
        .try_into()
        .ok()
        .filter(valid_id)
...
fn timestamp(context: &ReviewObligationCandidateEnvelopeContextV1) -> Timestamp {
    Timestamp {
...
fn outbox_error(_: OutboxRecordError) -> ReviewObligationCandidateEnvelopeBuildErrorV1 {
    ReviewObligationCandidateEnvelopeBuildErrorV1::OutboxRejected
}
...
mod tests {
    use super::*;
    use crate::wire::{
        ReviewObligationCandidateSubmissionRejectCodeV1, ReviewTargetBoundCandidateReceiptV1,
    };

    fn context() -> ReviewObligationCandidateEnvelopeContextV1 {
        ReviewObligationCandidateEnvelopeContextV1 {
            module_id: "makosh-review-obligation-candidate-runtime".to_owned(),
    ...
    fn approval_is_human_actor_and_private_text_free() {
        ...
        assert!(!String::from_utf8_lossy(record.exact_bytes()).contains("private statement"));
    }

    #[test]
    fn rejection_is_an_exact_terminal_result() {
        let record = build_review_obligation_candidate_submission_rejected_outbox_record_v1([9; 16], ObligationCandidateReviewSubmissionRejectedV1 { submission_id: vec![1; 16], candidate_id: vec![2; 16], code: ReviewObligationCandidateSubmissionRejectCodeV1::ReviewObligationCandidateSubmissionRejectCodeConflict as i32, logical_owner_id: "owner-1".to_owned() }, &context()).expect("record");
        let envelope = DurableEnvelopeV1::decode(record.exact_bytes()).expect("envelope");
