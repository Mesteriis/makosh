//! Communication Summary workflow release assembly.
//!
...
pub const COMMUNICATION_SUMMARY_DESCRIPTOR_FILE: &str =
    "communication_summary.runtime.descriptor.pb";
pub const COMMUNICATION_SUMMARY_SETTINGS_FILE: &str = "communication_summary.runtime.settings.pb";
pub const COMMUNICATION_SUMMARY_STORAGE_BUNDLE_FILE: &str =
    "communication_summary.storage.bundle.pb";
...
#[derive(Clone, Debug, Eq, PartialEq, Deserialize, Serialize)]
#[serde(untagged)]
pub enum CommunicationSummaryReleaseArtifactInputV1 {
    ModuleRuntime(ModuleRuntimeArtifactInputV1),
    StorageBundle(StorageBundleArtifactInputV1),
}

impl CommunicationSummaryReleaseArtifactInputV1 {
    #[must_use]
...
#[derive(Clone, Debug, Eq, PartialEq, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
pub struct CommunicationSummaryReleaseArtifactFragmentV1 {
    pub version: u32,
    pub owner_id: String,
    pub module_id: String,
    pub artifacts: Vec<CommunicationSummaryReleaseArtifactInputV1>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct CommunicationSummaryReleaseAssemblyPathsV1 {
...
pub enum CommunicationSummaryReleaseAssemblyErrorV1 {
    InvalidInput,
...
pub fn materialize_communication_summary_release_assembly_v1(
    output_directory: &Path,
    build_id: &str,
    runtime_source: &Path,
) -> Result<CommunicationSummaryReleaseAssemblyPathsV1, CommunicationSummaryReleaseAssemblyErrorV1>
{
    validate_inputs(output_directory, build_id, runtime_source)?;
    let descriptor = communication_summary_module_descriptor_v1(build_id);
    let settings_schema = communication_summary_settings_schema_v1();
    let storage_bundle = communication_summary_storage_bundle_v1();
    ...
    {
        return Err(CommunicationSummaryReleaseAssemblyErrorV1::InvalidCanonicalArtifact);
    }

    let paths = CommunicationSummaryReleaseAssemblyPathsV1 {
        descriptor: output_directory.join(COMMUNICATION_SUMMARY_DESCRIPTOR_FILE),
        settings_schema: output_directory.join(COMMUNICATION_SUMMARY_SETTINGS_FILE),
        storage_bundle: output_directory.join(COMMUNICATION_SUMMARY_STORAGE_BUNDLE_FILE),
        artifact_fragment: output_directory.join(COMMUNICATION_SUMMARY_ARTIFACT_FRAGMENT_FILE),
    };
    ...
    let writes = [
        ...
        (
            ...
            serde_json::to_vec_pretty(&fragment)
                .map_err(|_| CommunicationSummaryReleaseAssemblyErrorV1::FragmentEncodingFailed)?,
        ),
    ];
    ...
    builder
        ...
        .map_err(|_| CommunicationSummaryReleaseAssemblyErrorV1::OutputUnavailable)?;
    for (path, bytes) in &writes {
        if write_new_private_file(path, bytes).is_err() {
            let _ = fs::remove_dir_all(output_directory);
            return Err(CommunicationSummaryReleaseAssemblyErrorV1::ArtifactWriteFailed);
        }
    }
    Ok(paths)
}
...
fn validate_inputs(
    ...
    build_id: &str,
    runtime_source: &Path,
) -> Result<(), CommunicationSummaryReleaseAssemblyErrorV1> {
    ...
    {
        return Err(CommunicationSummaryReleaseAssemblyErrorV1::InvalidInput);
    }
    Ok(())
}
...
fn artifact_fragment(
    ...
    storage_bundle: &Path,
) -> Result<CommunicationSummaryReleaseArtifactFragmentV1, CommunicationSummaryReleaseAssemblyErrorV1>
{
    let artifacts = vec![
        CommunicationSummaryReleaseArtifactInputV1::ModuleRuntime(ModuleRuntimeArtifactInputV1 {
            artifact_kind: "module_runtime".to_owned(),
            artifact_id: COMMUNICATION_SUMMARY_RUNTIME_ARTIFACT_ID.to_owned(),
        ...
        CommunicationSummaryReleaseArtifactInputV1::StorageBundle(StorageBundleArtifactInputV1 {
            artifact_kind: "storage_bundle".to_owned(),
            artifact_id: COMMUNICATION_SUMMARY_STORAGE_ARTIFACT_ID.to_owned(),
    ...
    Ok(CommunicationSummaryReleaseArtifactFragmentV1 {
        version: COMMUNICATION_SUMMARY_ASSEMBLY_FRAGMENT_VERSION_V1,
        owner_id: COMMUNICATION_SUMMARY_ASSEMBLY_OWNER_ID.to_owned(),
        module_id: COMMUNICATION_SUMMARY_ASSEMBLY_MODULE_ID.to_owned(),
        artifacts,
    })
}
...
fn utf8_path(path: &Path) -> Result<String, CommunicationSummaryReleaseAssemblyErrorV1> {
    path.to_str()
        .filter(|value| !value.is_empty())
        .map(str::to_owned)
        .ok_or(CommunicationSummaryReleaseAssemblyErrorV1::InvalidInput)
}
...
mod tests {
    ...
    fn materializes_exact_unsigned_runtime_and_storage_fragment() {
        ...
        let storage =
            StorageBundleV1::decode(fs::read(paths.storage_bundle).expect("storage").as_slice())
                .expect("storage");
        let fragment: CommunicationSummaryReleaseArtifactFragmentV1 =
            serde_json::from_slice(&fs::read(paths.artifact_fragment).expect("fragment"))
                .expect("fragment");
        assert_eq!(
            descriptor.module_id,
            COMMUNICATION_SUMMARY_ASSEMBLY_MODULE_ID
        );
        assert_eq!(storage.owner_id, COMMUNICATION_SUMMARY_ASSEMBLY_OWNER_ID);
        assert_eq!(
            fragment
                .artifacts
                .iter()
                .map(CommunicationSummaryReleaseArtifactInputV1::artifact_id)
                .collect::<Vec<_>>(),
            [
                COMMUNICATION_SUMMARY_RUNTIME_ARTIFACT_ID,
                COMMUNICATION_SUMMARY_STORAGE_ARTIFACT_ID,
            ]
        );
    ...
    fn rejects_missing_runtime_and_existing_output() {
        ...
        assert_eq!(
            materialize_communication_summary_release_assembly_v1(
                &output,
                "build-1",
                &root.join("missing"),
            ),
            Err(CommunicationSummaryReleaseAssemblyErrorV1::InvalidInput)
        );
        fs::create_dir(&output).expect("existing output");
        let runtime = root.join("runtime");
        fs::write(&runtime, b"runtime").expect("runtime");
        assert_eq!(
            materialize_communication_summary_release_assembly_v1(&output, "build-1", &runtime),
            Err(CommunicationSummaryReleaseAssemblyErrorV1::InvalidInput)
        );
        fs::remove_dir_all(root).expect("cleanup");
    }
    ...
    fn temporary_directory() -> PathBuf {
        let root = std::env::current_dir()
            ...
            .join("target")
            .join(format!(
                "communication-summary-assembly-{}-{}",
                std::process::id(),
