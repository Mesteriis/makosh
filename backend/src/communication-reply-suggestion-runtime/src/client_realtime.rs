...
use makosh_communication_reply_suggestion_api::{
    COMMUNICATION_REPLY_SUGGESTION_REALTIME_EVENT_KIND_V1, wire::ReplySuggestionStatusChangedV1,
};
...
use crate::{
    ...
    contracts::reply_suggestion_realtime_contract_v1,
};

const REPLAY_WINDOW_V1: u16 = 1_024;

#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub(crate) struct ReplySuggestionClientRealtimePublisherV1 {
    last_sequence: Option<u64>,
}

impl ReplySuggestionClientRealtimePublisherV1 {
    pub(crate) async fn publish_pending(
        ...
        for transition in transitions {
            ...
            let request = ManagedRuntimeClientRealtimePublishRequestV1 {
                ...
                cursor: format!("communication-reply-suggestion/{}", transition.sequence),
                event_kind: COMMUNICATION_REPLY_SUGGESTION_REALTIME_EVENT_KIND_V1.to_owned(),
                occurred_at_unix_millis,
            ...
            let response = channel
                ...
                .map_err(|_| ReplySuggestionClientRealtimeErrorV1::Unavailable)?;
            if !response.error_code.is_empty() {
                return Err(ReplySuggestionClientRealtimeErrorV1::Unavailable);
            }
...
pub(crate) enum ReplySuggestionClientRealtimeErrorV1 {
    InvalidTransition,
