...
use makosh_ai_contracts::{
    ...
    AI_OWNER_V1, encode_reply_source_content_v1,
    wire::{AiPrivateSourceReceiptV1, AiReplySourceContentV1},
};
...
use makosh_communication_reply_suggestion_persistence::ReplySuggestionBlobCleanupV1;
use makosh_communications_ai_source_api::decode_communication_reply_source_content_v1;
...
use crate::COMMUNICATION_REPLY_SUGGESTION_BLOB_CAPABILITY_ID_V1;

const MAX_PROOF_BYTES_V1: usize = 2_048;

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ReplySuggestionSourceBlobReceiptV1 {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) struct ReplySuggestionBlobMaterializationV1 {
    pub ai_source: AiPrivateSourceReceiptV1,
    pub source_cleanup: ReplySuggestionBlobCleanupV1,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum ReplySuggestionBlobErrorV1 {
    InvalidReceipt,
    Unavailable,
}

pub(crate) fn materialize_reply_source_for_ai_v1(
    channel: &mut ManagedControlChannelV2<UnixStream>,
    dispatcher: &mut dyn ManagedControlRequestDispatcherV2<UnixStream>,
    run_id: [u8; 16],
    source: &ReplySuggestionSourceBlobReceiptV1,
) -> Result<ReplySuggestionBlobMaterializationV1, ReplySuggestionBlobErrorV1> {
    ...
    let declared_bytes =
        u64::try_from(encoded.len()).map_err(|_| ReplySuggestionBlobErrorV1::InvalidReceipt)?;
    ...
    Ok(ReplySuggestionBlobMaterializationV1 {
...
pub(crate) fn release_reply_source_blobs_v1(
    channel: &mut ManagedControlChannelV2<UnixStream>,
    dispatcher: &mut dyn ManagedControlRequestDispatcherV2<UnixStream>,
    run_id: [u8; 16],
    ai_source: &AiPrivateSourceReceiptV1,
...
fn read_exact(
    ...
    declared_bytes: u64,
    sha256: &[u8; 32],
) -> Result<Zeroizing<Vec<u8>>, ReplySuggestionBlobErrorV1> {
...
fn ai_reference_id(run_id: [u8; 16], local_reference: [u8; 16], sha256: [u8; 32]) -> [u8; 16] {
    ...
    digest.update(b"makosh.communication_reply_suggestion.ai-source.v1\0");
...
fn release_operation_id(run_id: [u8; 16], label: &[u8]) -> [u8; 16] {
    let mut digest = Sha256::new();
    digest.update(b"makosh.communication_reply_suggestion.blob-release.v1\0");
    digest.update(label);
    digest.update(run_id);
    digest.finalize()[..16].try_into().expect("digest prefix")
}

fn id16(value: &[u8]) -> Result<[u8; 16], ReplySuggestionBlobErrorV1> {
    value
        .try_into()
        .ok()
        .filter(|value: &[u8; 16]| value.iter().any(|byte| *byte != 0))
        .ok_or(ReplySuggestionBlobErrorV1::InvalidReceipt)
}

fn id32(value: &[u8]) -> Result<[u8; 32], ReplySuggestionBlobErrorV1> {
    value
        .try_into()
        .ok()
