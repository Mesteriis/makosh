...
use crate::{
    client_port::{
        ReplySuggestionClientRuntimeContextV1, get_reply_suggestion_payload_v1,
        start_reply_suggestion_payload_v1,
    },
    client_realtime::{
        ReplySuggestionClientRealtimeErrorV1, ReplySuggestionClientRealtimePublisherV1,
    },
    consume_reply_source_prepared_once_v1, consume_reply_source_rejected_once_v1,
    contracts::{reply_suggestion_command_contract_v1, reply_suggestion_query_contract_v1},
    event_outbox::ReplySuggestionEventRelayErrorV1,
    inference::ReplySuggestionInferenceErrorV1,
    recover_accepted_reply_suggestion_once_v1, relay_source_prepare_outbox_once_v1,
    source_results::ReplySuggestionSourceResultErrorV1,
};

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ReplySuggestionRuntimeAdmissionV1 {
...
pub enum ReplySuggestionManagedRuntimeErrorV1 {
    Admission,
    EventContract,
    EventUnavailable,
    InvalidTransition,
...
pub struct ReplySuggestionManagedRuntimeV1 {
    ...
    client_realtime: ReplySuggestionClientRealtimePublisherV1,
}

impl ReplySuggestionManagedRuntimeV1 {
    pub async fn open(
    ...
    ) -> Result<Self, ReplySuggestionManagedRuntimeErrorV1> {
        validate_admission(admission)?;
        if event_hub_endpoint.trim().is_empty() || event_credential_revision == 0 {
            return Err(ReplySuggestionManagedRuntimeErrorV1::Admission);
        }
        ...
        let password = std::str::from_utf8(&password)
            .map_err(|_| ReplySuggestionManagedRuntimeErrorV1::Admission)?;
        let persistence = CommunicationReplySuggestionPersistenceV1::connect_runtime(
            &binding,
        ...
        .await
        .map_err(ReplySuggestionManagedRuntimeErrorV1::Persistence)?;
        persistence
        ...
        .map_err(|_| ReplySuggestionManagedRuntimeErrorV1::EventUnavailable)?;
        ...
        .await
        .map_err(|_| ReplySuggestionManagedRuntimeErrorV1::EventUnavailable)?;
        let mut client_realtime = ReplySuggestionClientRealtimePublisherV1::default();
        ...
        control_channel
            ...
            .map_err(|_| ReplySuggestionManagedRuntimeErrorV1::Unavailable)?;
        Ok(Self {
    ...
    pub async fn relay_source_prepare_outbox_once(
        &self,
        now_unix_millis: i64,
    ) -> Result<bool, ReplySuggestionManagedRuntimeErrorV1> {
    ...
    pub async fn consume_source_prepared_once(
        ...
        self.control_channel
            ...
            .map_err(|_| ReplySuggestionManagedRuntimeErrorV1::Unavailable)?;
        let mut dispatcher = RejectManagedControlRequestsV2;
        let result = consume_reply_source_prepared_once_v1(
            &self.persistence,
            &self.event_connection,
    ...
    pub async fn pump_control_once(
        ...
        if validate_module_client_response_v1(&response).is_err() {
            return Err(ReplySuggestionManagedRuntimeErrorV1::Unavailable);
        }
        self.control_channel
            .write_response(
                correlation_id,
    ...
    fn write_client_error(
        ...
        error_code: &str,
    ) -> Result<(), ReplySuggestionManagedRuntimeErrorV1> {
        self.control_channel
            .write_response(
                correlation_id,
                ManagedRuntimeControlResponseV1 {
...
async fn dispatch_client(
    ...
    let (response_payload, accepted_route) = if valid_identity {
        if request.contract.as_ref() == Some(&reply_suggestion_command_contract_v1()) {
            (
                start_reply_suggestion_payload_v1(
                    persistence,
                    &admission.logical_owner_id,
                    &ReplySuggestionClientRuntimeContextV1 {
        ...
        } else if request.contract.as_ref() == Some(&reply_suggestion_query_contract_v1()) {
            (
                get_reply_suggestion_payload_v1(
                    persistence,
                    &admission.logical_owner_id,
                    &request.request_payload,
                )
...
fn bind_source_subscriptions(
    ...
    ReplySuggestionManagedRuntimeErrorV1,
> {
    if permits.len() != 2 {
        return Err(ReplySuggestionManagedRuntimeErrorV1::Admission);
    }
...
fn exact_permit(
...
) -> Result<RuntimeSubscribePermitV1, ReplySuggestionManagedRuntimeErrorV1> {
    ...
    let permit = matching
        .next()
        .cloned()
        .ok_or(ReplySuggestionManagedRuntimeErrorV1::Admission)?;
...
) -> Result<zeroize::Zeroizing<Vec<u8>>, ReplySuggestionManagedRuntimeErrorV1> {
    for attempt in 0..20 {
...
fn event_relay_error(
    error: ReplySuggestionEventRelayErrorV1,
) -> ReplySuggestionManagedRuntimeErrorV1 {
    match error {
        ReplySuggestionEventRelayErrorV1::InvalidTimestamp => {
            ReplySuggestionManagedRuntimeErrorV1::EventContract
        }
        ReplySuggestionEventRelayErrorV1::Persistence(error) => {
            ReplySuggestionManagedRuntimeErrorV1::Persistence(error)
        }
        ReplySuggestionEventRelayErrorV1::EventUnavailable => {
            ReplySuggestionManagedRuntimeErrorV1::EventUnavailable
        }
    }
}

fn source_result_error(
    error: ReplySuggestionSourceResultErrorV1,
) -> ReplySuggestionManagedRuntimeErrorV1 {
    match error {
        ReplySuggestionSourceResultErrorV1::InvalidEnvelope
        | ReplySuggestionSourceResultErrorV1::InvalidPayload => {
            ReplySuggestionManagedRuntimeErrorV1::EventContract
        }
        ReplySuggestionSourceResultErrorV1::Blob(
            crate::ReplySuggestionBlobErrorV1::InvalidReceipt,
        ) => ReplySuggestionManagedRuntimeErrorV1::EventContract,
        ReplySuggestionSourceResultErrorV1::Blob(
            crate::ReplySuggestionBlobErrorV1::Unavailable,
        ) => ReplySuggestionManagedRuntimeErrorV1::Unavailable,
        ReplySuggestionSourceResultErrorV1::Inference(error) => inference_error(error),
        ReplySuggestionSourceResultErrorV1::Persistence(error) => {
            ReplySuggestionManagedRuntimeErrorV1::Persistence(error)
        }
...
fn inference_error(error: ReplySuggestionInferenceErrorV1) -> ReplySuggestionManagedRuntimeErrorV1 {
    match error {
        ReplySuggestionInferenceErrorV1::InvalidRequest
        | ReplySuggestionInferenceErrorV1::InvalidResult => {
            ReplySuggestionManagedRuntimeErrorV1::InvalidTransition
        }
        ReplySuggestionInferenceErrorV1::Persistence(error) => {
            ReplySuggestionManagedRuntimeErrorV1::Persistence(error)
        }
        ReplySuggestionInferenceErrorV1::Unavailable => {
            ReplySuggestionManagedRuntimeErrorV1::Unavailable
        }
    }
}
...
fn client_realtime_error(
    error: ReplySuggestionClientRealtimeErrorV1,
) -> ReplySuggestionManagedRuntimeErrorV1 {
    match error {
        ReplySuggestionClientRealtimeErrorV1::InvalidTransition => {
            ReplySuggestionManagedRuntimeErrorV1::InvalidTransition
        }
        ReplySuggestionClientRealtimeErrorV1::Persistence(error) => {
            ReplySuggestionManagedRuntimeErrorV1::Persistence(error)
        }
        ReplySuggestionClientRealtimeErrorV1::Unavailable => {
            ReplySuggestionManagedRuntimeErrorV1::Unavailable
        }
    }
}
...
mod tests {
    ...
    fn admission_requires_exact_managed_fences() {
        assert_eq!(
            validate_admission(&ReplySuggestionRuntimeAdmissionV1 {
