...
use makosh_attachment_security_assembly::materialize_attachment_security_release_assembly_v1;
...
fn main() -> ExitCode {
    match arguments(std::env::args().skip(1).collect()) {
        Some(arguments) => {
            if materialize_attachment_security_release_assembly_v1(
            ...
            println!("attachment-security-release-assembly: ok");
            ExitCode::SUCCESS
        }
        None => fail(
            "usage: makosh-attachment-security-assembly --build-id <id> --output-dir <absolute-path> \
...
fn fail(message: &str) -> ExitCode {
    eprintln!("attachment-security-release-assembly: {message}");
    ExitCode::FAILURE
}
...
mod tests {
    ...
    fn parses_each_exact_option_once_in_any_order() {
        let parsed = arguments(vec![
            "--runtime".to_owned(),
            "/tmp/makosh-attachment-security-runtime".to_owned(),
            "--build-id".to_owned(),
            "build-1".to_owned(),
            "--output-dir".to_owned(),
            "/tmp/attachment-security-assembly".to_owned(),
        ])
        ...
        assert_eq!(
            ...
            PathBuf::from("/tmp/attachment-security-assembly")
        );
        assert_eq!(
            parsed.runtime,
            PathBuf::from("/tmp/makosh-attachment-security-runtime")
        );
    }
