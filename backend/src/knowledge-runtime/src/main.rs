...
fn main() -> Result<(), String> {
    ...
    let Some(command) = arguments.next() else {
        return Err("Knowledge command is required".to_owned());
    };
    match command.to_str() {
        Some("export-storage-bundle") => export(
            &mut arguments,
            knowledge_storage_bundle_v1().encode_to_vec(),
        ),
        Some("export-module-descriptor") => {
            let build_id = required_string(&mut arguments, "build id")?;
            export(
                &mut arguments,
                knowledge_module_descriptor_v1(&build_id).encode_to_vec(),
            )
        }
        Some("export-settings-schema") => {
            export(&mut arguments, knowledge_settings_schema_bytes_v1())
        }
        Some("serve-inherited") => serve_inherited(&mut arguments),
        _ => Err("Knowledge command is invalid".to_owned()),
    }
}
    ...
    I: Iterator<Item = OsString>,
{
    ...
    decode_settings_schema_v1(&settings_schema)
        .map_err(|_| "Knowledge settings schema is invalid".to_owned())?;
    let configuration = ManagedDomainRuntimeConfigurationV1::decode(
        read_contract(&paths.runtime_configuration)?.as_slice(),
    )
    .map_err(|_| "Knowledge runtime configuration is invalid".to_owned())?;
    ...
    if configuration.runtime_instance_id != paths.runtime_instance_id {
        return Err("Knowledge runtime configuration is stale".to_owned());
    }
    let storage = configuration
        .storage
        .clone()
        .ok_or_else(|| "Knowledge storage is unavailable".to_owned())?;
    let admission = KnowledgeRuntimeAdmissionV1 {
    ...
    let executor = tokio::runtime::Runtime::new()
        .map_err(|_| "Knowledge executor is unavailable".to_owned())?;
    let mut runtime = executor
        .block_on(KnowledgeManagedRuntimeV1::open(
...
fn retry_runtime(result: Result<bool, KnowledgeManagedRuntimeErrorV1>) -> Result<(), String> {
    match result {
        Ok(_)
        | Err(KnowledgeManagedRuntimeErrorV1::EventUnavailable)
        | Err(KnowledgeManagedRuntimeErrorV1::Unavailable)
        | Err(KnowledgeManagedRuntimeErrorV1::Persistence(
...
fn runtime_error(error: KnowledgeManagedRuntimeErrorV1) -> String {
    let code = match error {
        KnowledgeManagedRuntimeErrorV1::Admission => "admission",
        KnowledgeManagedRuntimeErrorV1::EventContract => "event_contract",
        KnowledgeManagedRuntimeErrorV1::EventUnavailable => "event_unavailable",
        KnowledgeManagedRuntimeErrorV1::Persistence(_) => "persistence",
        KnowledgeManagedRuntimeErrorV1::Unavailable => "unavailable",
    };
    format!("Knowledge runtime failed: {code}")
}
    ...
    I: Iterator<Item = OsString>,
{
    let output = arguments
        .next()
        .map(PathBuf::from)
        .ok_or_else(|| "Knowledge output path is required".to_owned())?;
    require_no_arguments(arguments)?;
    fs::write(output, bytes).map_err(|_| "Knowledge export failed".to_owned())
}
    ...
    I: Iterator<Item = OsString>,
{
    if name.starts_with("--") && arguments.next().as_deref() != Some(OsStr::new(name)) {
        return Err("Knowledge arguments are invalid".to_owned());
    }
    arguments
        .next()
        .and_then(|value| value.into_string().ok())
        .filter(|value| !value.is_empty())
        .ok_or_else(|| format!("Knowledge {name} is required"))
}
...
fn inherited_control_channel() -> Result<UnixStream, String> {
    ...
    if duplicated < 0 {
        return Err("Knowledge control channel is unavailable".to_owned());
    }
...
fn read_contract(path: &Path) -> Result<Vec<u8>, String> {
    if !path.is_absolute() {
        return Err("Knowledge contract path is invalid".to_owned());
    }
    let bytes = fs::read(path).map_err(|_| "Knowledge contract is unavailable".to_owned())?;
    if bytes.is_empty() || bytes.len() > 2 * 1024 * 1024 {
        return Err("Knowledge contract is invalid".to_owned());
    }
    Ok(bytes)
}
