...
CREATE TABLE makosh_data.review_note_candidate_state (
    ...
    CHECK (
        ...
        OR (state = 2 AND promotion_status BETWEEN 2 AND 4
            ...
            AND ((promotion_status = 3 AND length(promoted_note_id) = 16)
                OR (promotion_status IN (2, 4) AND promoted_note_id IS NULL)))
