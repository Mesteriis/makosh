...
use crate::{
    ...
    model::{COMMUNICATION_EXPLANATION_REALTIME_LIMIT_V1, valid_identity, valid_timestamp},
    repository::state_from_code,
};

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct CommunicationExplanationRealtimeTransitionV1 {
    pub sequence: u64,
    pub run_id: [u8; 16],
...
impl crate::CommunicationExplanationPersistenceV1 {
    pub async fn client_realtime_window(
        ...
        Vec<CommunicationExplanationRealtimeTransitionV1>,
        CommunicationExplanationPersistenceErrorV1,
    > {
        if !valid_identity(logical_owner_id)
            || after_sequence == Some(0)
            || !(1..=COMMUNICATION_EXPLANATION_REALTIME_LIMIT_V1).contains(&limit)
        {
        ...
        let rows = if let Some(after) = after_sequence {
            ...
            sqlx::query(
                "SELECT realtime_sequence, run_id, state, state_revision,
                 ...
                 FROM makosh_data.communication_explanation_realtime
...
pub(crate) async fn insert_realtime_transition(
    ...
    let inserted = sqlx::query(
        "INSERT INTO makosh_data.communication_explanation_realtime (
    ...
    if inserted == 1 {
        ...
        Err(CommunicationExplanationPersistenceErrorV1::InvalidRow)
    }
}

fn transition_from_row(
    row: sqlx::postgres::PgRow,
) -> Result<CommunicationExplanationRealtimeTransitionV1, CommunicationExplanationPersistenceErrorV1>
{
    ...
    Ok(CommunicationExplanationRealtimeTransitionV1 {
...
fn rejection_from_code(
    value: i16,
) -> Result<CommunicationExplanationRejectionCodeV1, CommunicationExplanationPersistenceErrorV1> {
    match value {
