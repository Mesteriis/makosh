...
use crate::{
    ...
    UnpublishedCommunicationExplanationEventV1,
    model::{
        COMMUNICATION_EXPLANATION_MAX_EVENT_BYTES_V1, COMMUNICATION_EXPLANATION_OUTBOX_LIMIT_V1,
...
impl CommunicationExplanationPersistenceV1 {
    ...
    > {
        ...
        .map(|row| {
            ...
            let envelope_bytes: Vec<u8> = row
                ...
                .map_err(|_| CommunicationExplanationPersistenceErrorV1::InvalidRow)?;
            let event = UnpublishedCommunicationExplanationEventV1 {
            ...
            if !nonzero(&event.message_id)
                ...
                || event.envelope_bytes.len() > COMMUNICATION_EXPLANATION_MAX_EVENT_BYTES_V1
            {
    ...
    pub async fn mark_source_prepare_published(
        ...
        let updated = sqlx::query(
            "UPDATE makosh_data.communication_explanation_outbox
