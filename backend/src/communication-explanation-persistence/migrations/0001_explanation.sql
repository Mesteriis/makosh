CREATE TABLE makosh_data.communication_explanation_runs (
    ...
    CHECK (
        (state = 4 AND candidate_reasons_bytes IS NOT NULL
          AND length(candidate_reasons_bytes) BETWEEN 2 AND 8192
          AND candidate_completeness IN (1, 2)
          AND candidate_confidence_basis_points BETWEEN 0 AND 10000
          AND rejection_code IS NULL)
        OR
        (state = 5 AND candidate_reasons_bytes IS NULL AND rejection_code BETWEEN 1 AND 4)
        OR
        (state IN (1, 2, 3) AND candidate_reasons_bytes IS NULL AND rejection_code IS NULL)
    )
);
