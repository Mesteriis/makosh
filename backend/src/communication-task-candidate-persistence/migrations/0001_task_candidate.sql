CREATE TABLE makosh_data.communication_task_candidate_extraction_runs (
    ...
    source_sha256 BYTEA,
    source_read_receipt_bytes BYTEA,
    ...
    CHECK (
        ...
        OR
        (state = 5 AND (
          (source_evidence_id IS NULL AND source_evidence_revision IS NULL
            AND source_sha256 IS NULL)
          OR
          (length(source_evidence_id) = 16 AND source_evidence_revision > 0
            AND length(source_sha256) = 32)
        ))
    ),
    CHECK (
        (
            source_read_receipt_bytes IS NULL
        ...
        OR
        (
            length(source_read_receipt_bytes) BETWEEN 1 AND 16384
    ...
    CHECK (
        (state IN (1, 2) AND source_read_receipt_bytes IS NULL)
        OR (state = 3 AND source_read_receipt_bytes IS NOT NULL)
        OR state IN (4, 5)
    ),
    CHECK (
        state != 4
        OR source_read_receipt_bytes IS NOT NULL
    ...
    CHECK (
        (state = 4 AND candidate_bytes IS NOT NULL
          AND length(candidate_bytes) BETWEEN 2 AND 65536
...
CREATE INDEX communication_task_candidate_extraction_recoverable_idx
ON makosh_data.communication_task_candidate_extraction_runs (
    logical_owner_id,
    state,
    state_revision
);

CREATE TABLE makosh_data.communication_task_candidate_extraction_inbox (
    logical_owner_id TEXT NOT NULL,
...
CREATE TABLE makosh_data.communication_task_candidate_extraction_outbox (
    logical_owner_id TEXT NOT NULL,
...
CREATE INDEX communication_task_candidate_extraction_outbox_pending_idx
ON makosh_data.communication_task_candidate_extraction_outbox (
    logical_owner_id,
...
CREATE TABLE makosh_data.communication_task_candidate_extraction_realtime (
...
CREATE INDEX communication_task_candidate_extraction_realtime_owner_idx
ON makosh_data.communication_task_candidate_extraction_realtime (
    logical_owner_id,
    realtime_sequence
);
