...
use crate::{
    ...
    client_wire::KnowledgeNoteChangedV1,
...
pub fn build_knowledge_note_changed_outbox_record_v1(
    operation_id: [u8; 16],
    payload: KnowledgeNoteChangedV1,
    context: &KnowledgeCommandEnvelopeContextV1,
) -> Result<OutboxRecordV1, KnowledgeCommandEnvelopeBuildErrorV1> {
    validate_context(context)?;
    if !nonzero(&operation_id)
        || id16(&payload.event_id).is_err()
        || id16(&payload.note_id).is_err()
        || payload.note_revision == 0
        || payload.state == 0
        || !valid_owner(&payload.logical_owner_id)
    ...
    let note_id = id16(&payload.note_id)?;
    let occurred_at = payload
        ...
        .ok_or(KnowledgeCommandEnvelopeBuildErrorV1::InvalidPayload)?;
    let contract = knowledge_lifecycle_event_contract_reference_v1();
    let envelope = DurableEnvelopeV1 {
        ...
        partition_key: note_id.to_vec(),
        causation_message_id: operation_id.to_vec(),
        correlation_id: note_id.to_vec(),
