...
use makosh_timeline_persistence::{TimelinePersistenceErrorV1, timeline_storage_bundle_v1};
use makosh_timeline_runtime::{
    TimelineManagedRuntimeErrorV1, TimelineManagedRuntimeV1, TimelineRuntimeAdmissionV1,
    timeline_module_descriptor_v1, timeline_settings_schema_bytes_v1,
};
use prost::Message;
use std::{
    ffi::{OsStr, OsString},
...
fn main() -> Result<(), String> {
    ...
    match command.as_deref() {
        ...
        Some(v) if v == OsStr::new("export-storage-bundle") => {
            export(&mut args, timeline_storage_bundle_v1().encode_to_vec())
        }
        Some(v) if v == OsStr::new("export-module-descriptor") => {
            let build = args
                ...
                .filter(|v| !v.is_empty())
                .ok_or_else(|| "Timeline descriptor build id is required".to_owned())?;
            export(
                &mut args,
                timeline_module_descriptor_v1(&build).encode_to_vec(),
            )
        }
        Some(v) if v == OsStr::new("export-settings-schema") => {
            export(&mut args, timeline_settings_schema_bytes_v1())
        }
        _ => Err("Timeline runtime command is unavailable".into()),
    }
}
fn serve<I: Iterator<Item = OsString>>(args: &mut std::iter::Peekable<I>) -> Result<(), String> {
    ...
    let schema = decode_settings_schema_v1(&schema_bytes)
        .map_err(|_| "Timeline settings schema is invalid".to_owned())?;
    let snapshot = decode_settings_snapshot_v1(&read(&paths.settings_snapshot)?)
        .map_err(|_| "Timeline settings snapshot is invalid".to_owned())?;
    ...
    let config =
        ManagedEngineRuntimeConfigurationV1::decode(read(&paths.runtime_configuration)?.as_slice())
            .map_err(|_| "Timeline runtime configuration is invalid".to_owned())?;
    ...
    {
        return Err("Timeline runtime configuration is stale".into());
    }
    let storage = config
        .storage
        .clone()
        .ok_or_else(|| "Timeline storage is unavailable".to_owned())?;
    let admission = TimelineRuntimeAdmissionV1 {
    ...
    let executor = tokio::runtime::Runtime::new()
        .map_err(|_| "Timeline executor is unavailable".to_owned())?;
    let mut runtime = executor
        .block_on(TimelineManagedRuntimeV1::open(
            inherited()?,
            descriptor,
            schema_bytes,
            &admission,
    ...
    loop {
        match executor.block_on(runtime.service_once(now()?)) {
            Ok(_) => failures = 0,
            Err(TimelineManagedRuntimeErrorV1::ControlClosed) => return Ok(()),
            Err(TimelineManagedRuntimeErrorV1::EventUnavailable)
            | Err(TimelineManagedRuntimeErrorV1::Persistence(
...
fn runtime_error(error: TimelineManagedRuntimeErrorV1) -> String {
    let code = match error {
        TimelineManagedRuntimeErrorV1::Admission => "admission",
        TimelineManagedRuntimeErrorV1::EventContract => "event_contract",
        TimelineManagedRuntimeErrorV1::EventUnavailable => "event_unavailable",
        TimelineManagedRuntimeErrorV1::Persistence(_) => "persistence",
        TimelineManagedRuntimeErrorV1::ControlClosed => "control_closed",
        TimelineManagedRuntimeErrorV1::Unavailable => "unavailable",
    };
    format!("Timeline runtime failed: {code}")
}
fn now() -> Result<i64, String> {
    std::time::SystemTime::now()
        ...
        .ok_or_else(|| "Timeline clock is invalid".into())
}
fn export<I: Iterator<Item = OsString>>(args: &mut I, bytes: Vec<u8>) -> Result<(), String> {
    if args.next().is_some() {
        return Err("Timeline runtime command is unavailable".into());
    }
    std::io::Write::write_all(&mut std::io::stdout(), &bytes)
        .map_err(|_| "Timeline artifact is unavailable".into())
}
fn parse<I: Iterator<Item = OsString>>(args: &mut std::iter::Peekable<I>) -> Result<Paths, String> {
    ...
    if args.next().is_some() {
        return Err("Timeline runtime arguments are invalid".into());
    }
    Ok(Paths {
        descriptor,
        settings_schema,
...
fn required<I: Iterator<Item = OsString>>(args: &mut I, name: &str) -> Result<String, String> {
    ...
    args.next()
        .and_then(|v| v.into_string().ok())
        .filter(|v| !v.is_empty())
        .ok_or_else(|| "Timeline runtime arguments are invalid".into())
}
fn inherited() -> Result<UnixStream, String> {
    let fd = unsafe { libc::dup(std::io::stdin().as_raw_fd()) };
    if fd < 0 {
        return Err("Timeline control channel is unavailable".into());
    }
    Ok(unsafe { UnixStream::from_raw_fd(fd) })
}
fn read(path: &Path) -> Result<Vec<u8>, String> {
    let metadata = std::fs::symlink_metadata(path)
        .map_err(|_| "Timeline contract is unavailable".to_owned())?;
    if metadata.file_type().is_symlink() || !metadata.is_file() || metadata.len() > 512 * 1024 {
        return Err("Timeline contract is unavailable".into());
    }
    std::fs::read(path).map_err(|_| "Timeline contract is unavailable".into())
}
