use makosh_ai_contracts::{
    validate_explanation_inference_request_v1,
    wire::{
        AiInferenceTerminalStatusV1, AiProviderExplanationRequestV1,
        CommunicationExplanationInferenceRequestV1,
    },
};
use makosh_ai_inference_core::{
    AiExplanationExecutionPlanV1, AiInferenceCoreErrorV1, AiInferenceRunStateV1,
    ...
    explanation_inference_execution_plan_v1, reject_explanation_inference_v1,
};
use makosh_ai_inference_persistence::{
    AiExplanationTransitionV1, AiInferencePersistenceErrorV1, AiInferencePersistenceV1,
    PersistedAiExplanationRunV1,
};
use prost::Message;
...
pub(crate) async fn execute_explanation_payload_v1(
    ...
    let request = CommunicationExplanationInferenceRequestV1::decode(payload)
        .map_err(|_| AiInferenceWorkerErrorV1::InvalidRequest)?;
    if request.logical_owner_id != logical_owner_id
        || validate_explanation_inference_request_v1(&request).is_err()
    {
        return Err(AiInferenceWorkerErrorV1::InvalidRequest);
    }
    let accepted = accept_explanation_inference_v1(request).map_err(core_error)?;
    let persisted = persistence
        .accept_explanation_run(accepted)
        .await
        .map_err(persistence_error)?
        .persisted;
    let terminal = drive_explanation_run_v1(persistence, ports, persisted).await?;
    terminal
        .run
        .terminal_result
...
pub(crate) async fn recover_pending_explanations_v1(
    persistence: &AiInferencePersistenceV1,
    ports: &mut dyn AiInferenceExecutionPortsV1,
    logical_owner_id: &str,
) -> Result<u32, AiInferenceWorkerErrorV1> {
    let pending = persistence
        .load_recoverable_explanation_runs(logical_owner_id, 128)
        .await
        .map_err(persistence_error)?;
    let mut recovered = 0;
    for run in pending {
        match drive_explanation_run_v1(persistence, ports, run).await {
            Ok(_) => recovered += 1,
...
async fn drive_explanation_run_v1(
    persistence: &AiInferencePersistenceV1,
    ports: &mut dyn AiInferenceExecutionPortsV1,
    mut persisted: PersistedAiExplanationRunV1,
) -> Result<PersistedAiExplanationRunV1, AiInferenceWorkerErrorV1> {
    if matches!(
        persisted.run.state,
    ...
    let plan = explanation_inference_execution_plan_v1(&persisted.run).map_err(core_error)?;
    let source = match ports.materialize_explanation_source(&plan) {
        Ok(source) => source,
    ...
    let input = match build_explanation_provider_input_v1(&plan, &source) {
        Ok(input) => input,
        Err(_) => {
            return reject_and_persist(
                persistence,
    ...
    if persisted.run.state == AiInferenceRunStateV1::Accepted {
        let (executing, _) = begin_explanation_inference_v1(&persisted.run, persisted.run.revision)
            .map_err(core_error)?;
        persisted = persistence
            .persist_explanation_transition(AiExplanationTransitionV1 {
    ...
    let provider_result = match ports.explain(provider_request(&plan, input)) {
        Ok(result) => result,
    ...
    let ready =
        complete_explanation_inference_v1(&persisted.run, persisted.run.revision, provider_result)
            .map_err(core_error)?;
    persistence
        .persist_explanation_transition(AiExplanationTransitionV1 {
            current_revision: persisted.run.revision,
...
fn provider_request(
    ...
    input_utf8: Vec<u8>,
) -> AiProviderExplanationRequestV1 {
    AiProviderExplanationRequestV1 {
        request_id: plan.run_id.to_vec(),
        input_utf8,
        maximum_reasons: plan.maximum_reasons,
        maximum_reason_text_bytes: plan.maximum_reason_text_bytes,
...
async fn reject_and_persist(
    persistence: &AiInferencePersistenceV1,
    persisted: PersistedAiExplanationRunV1,
    status: AiInferenceTerminalStatusV1,
) -> Result<PersistedAiExplanationRunV1, AiInferenceWorkerErrorV1> {
    let rejected = reject_explanation_inference_v1(&persisted.run, persisted.run.revision, status)
        .map_err(core_error)?;
    persistence
        .persist_explanation_transition(AiExplanationTransitionV1 {
            current_revision: persisted.run.revision,
