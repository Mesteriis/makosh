use makosh_ai_contracts::{
    validate_translation_inference_request_v1,
    wire::{
        AiInferenceTerminalStatusV1, AiProviderTranslationRequestV1,
        CommunicationTranslationInferenceRequestV1,
    },
};
use makosh_ai_inference_core::{
    AiInferenceCoreErrorV1, AiInferenceRunStateV1, AiTranslationExecutionPlanV1,
    accept_translation_inference_v1, begin_translation_inference_v1,
    build_translation_provider_input_v1, complete_translation_inference_v1,
    reject_translation_inference_v1, translation_inference_execution_plan_v1,
};
use makosh_ai_inference_persistence::{
    AiInferencePersistenceErrorV1, AiInferencePersistenceV1, AiTranslationTransitionV1,
    PersistedAiTranslationRunV1,
};
use prost::Message;
...
pub(crate) async fn execute_translation_payload_v1(
    persistence: &AiInferencePersistenceV1,
    ports: &mut dyn AiInferenceExecutionPortsV1,
    logical_owner_id: &str,
    payload: &[u8],
) -> Result<Vec<u8>, AiInferenceWorkerErrorV1> {
    let request = CommunicationTranslationInferenceRequestV1::decode(payload)
        .map_err(|_| AiInferenceWorkerErrorV1::InvalidRequest)?;
    if request.logical_owner_id != logical_owner_id
        || validate_translation_inference_request_v1(&request).is_err()
    {
        return Err(AiInferenceWorkerErrorV1::InvalidRequest);
    }
    let accepted = accept_translation_inference_v1(request).map_err(core_error)?;
    let persisted = persistence
        .accept_translation_run(accepted)
        .await
        .map_err(persistence_error)?
        .persisted;
    let terminal = drive_translation_run_v1(persistence, ports, persisted).await?;
    terminal
        .run
        .terminal_result
...
pub(crate) async fn recover_pending_translations_v1(
    persistence: &AiInferencePersistenceV1,
    ports: &mut dyn AiInferenceExecutionPortsV1,
    logical_owner_id: &str,
) -> Result<u32, AiInferenceWorkerErrorV1> {
    let pending = persistence
        .load_recoverable_translation_runs(logical_owner_id, 128)
        .await
        .map_err(persistence_error)?;
    let mut recovered = 0;
    for run in pending {
        match drive_translation_run_v1(persistence, ports, run).await {
            Ok(_) => recovered += 1,
...
async fn drive_translation_run_v1(
    persistence: &AiInferencePersistenceV1,
    ports: &mut dyn AiInferenceExecutionPortsV1,
    mut persisted: PersistedAiTranslationRunV1,
) -> Result<PersistedAiTranslationRunV1, AiInferenceWorkerErrorV1> {
    if matches!(
        persisted.run.state,
    ...
    let plan = translation_inference_execution_plan_v1(&persisted.run).map_err(core_error)?;
    let source = match ports.materialize_translation_source(&plan) {
        Ok(source) => source,
    ...
    let input = match build_translation_provider_input_v1(&plan, &source) {
        Ok(input) => input,
        Err(_) => {
            return reject_and_persist(
                persistence,
    ...
    if persisted.run.state == AiInferenceRunStateV1::Accepted {
        let (executing, _) = begin_translation_inference_v1(&persisted.run, persisted.run.revision)
            .map_err(core_error)?;
        persisted = persistence
            .persist_translation_transition(AiTranslationTransitionV1 {
    ...
    let ready =
        complete_translation_inference_v1(&persisted.run, persisted.run.revision, provider_result)
            .map_err(core_error)?;
    persistence
        .persist_translation_transition(AiTranslationTransitionV1 {
            current_revision: persisted.run.revision,
...
async fn reject_and_persist(
    ...
    let rejected = reject_translation_inference_v1(&persisted.run, persisted.run.revision, status)
        .map_err(core_error)?;
    persistence
        .persist_translation_transition(AiTranslationTransitionV1 {
            current_revision: persisted.run.revision,
