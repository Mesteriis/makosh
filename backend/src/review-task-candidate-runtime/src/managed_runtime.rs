...
use makosh_review_task_candidate_promotion_api::review_task_candidate_promotion_result_contract_reference_v1;
...
use crate::{
    ...
    submission::{
        ...
        consume_review_task_candidate_submission_once_v1,
    },
};

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ReviewTaskCandidateRuntimeAdmissionV1 {
...
pub enum ReviewTaskCandidateManagedRuntimeErrorV1 {
    Admission,
    EventContract,
    EventUnavailable,
    InvalidTransition,
...
pub struct ReviewTaskCandidateManagedRuntimeV1 {
    admission: ReviewTaskCandidateRuntimeAdmissionV1,
...
impl ReviewTaskCandidateManagedRuntimeV1 {
    ...
    pub async fn open(
    ...
    ) -> Result<Self, ReviewTaskCandidateManagedRuntimeErrorV1> {
        ...
        let password = std::str::from_utf8(&password)
            .map_err(|_| ReviewTaskCandidateManagedRuntimeErrorV1::Admission)?;
        let persistence = ReviewTaskCandidatePersistenceV1::connect_runtime(
            &binding,
        ...
        .await
        .map_err(ReviewTaskCandidateManagedRuntimeErrorV1::Persistence)?;
        persistence
        ...
        .map_err(|_| ReviewTaskCandidateManagedRuntimeErrorV1::EventUnavailable)?;
        ...
        .await
        .map_err(|_| ReviewTaskCandidateManagedRuntimeErrorV1::EventUnavailable)?;
        let mut client_realtime = ReviewTaskCandidateClientRealtimePublisherV1::default();
        ...
        control_channel
            ...
            .map_err(|_| ReviewTaskCandidateManagedRuntimeErrorV1::Unavailable)?;
        Ok(Self {
    ...
    pub async fn pump_control_once(
        ...
        if validate_module_client_response_v1(&response).is_err() {
            return Err(ReviewTaskCandidateManagedRuntimeErrorV1::Unavailable);
        }
        self.control_channel
            .write_response(
                correlation_id,
    ...
    pub async fn consume_submission_once(
        ...
        self.control_channel
            ...
            .map_err(|_| ReviewTaskCandidateManagedRuntimeErrorV1::Unavailable)?;
        let mut dispatcher = RejectManagedControlRequestsV2;
        let result = consume_review_task_candidate_submission_once_v1(
            &self.persistence,
            &self.event_connection,
            &self.submission_subscription,
            &mut self.control_channel,
            &mut dispatcher,
            &ReviewTaskCandidateSubmissionRuntimeContextV1 {
    ...
    pub async fn relay_outbox_once(
        ...
        relay_review_task_candidate_outbox_once_v1(
            &self.persistence,
    ...
    fn write_control_error(
        ...
        error_code: &str,
    ) -> Result<(), ReviewTaskCandidateManagedRuntimeErrorV1> {
        self.control_channel
            .write_response(
                correlation_id,
                ManagedRuntimeControlResponseV1 {
...
async fn dispatch_client(
    ...
    let valid_identity = request.protocol_major == 1
        && request.module_id == REVIEW_TASK_CANDIDATE_MODULE_ID_V1
        && request.owner_id == REVIEW_TASK_CANDIDATE_OWNER_V1
        ...
        if request.contract.as_ref() == Some(&command_contract_v1()) {
            (
                decide_payload_v1(
                    ...
                    channel,
                    dispatcher,
                    &ReviewTaskCandidateClientRuntimeContextV1 {
...
fn validate_admission(
    ...
    if admission.logical_owner_id != REVIEW_TASK_CANDIDATE_OWNER_V1
...
) -> Result<zeroize::Zeroizing<Vec<u8>>, ReviewTaskCandidateManagedRuntimeErrorV1> {
    for attempt in 0..20 {
...
fn storage_binding(
    ...
    admission: &ReviewTaskCandidateRuntimeAdmissionV1,
) -> Result<StorageBindingV1, ReviewTaskCandidateManagedRuntimeErrorV1> {
    if configuration.runtime_instance_id != admission.runtime_instance_id
        || configuration.logical_owner_id != REVIEW_TASK_CANDIDATE_OWNER_V1
        || configuration.owner != REVIEW_TASK_CANDIDATE_OWNER_V1
...
fn event_relay_error(
    error: ReviewTaskCandidateEventRelayErrorV1,
) -> ReviewTaskCandidateManagedRuntimeErrorV1 {
    match error {
        ReviewTaskCandidateEventRelayErrorV1::InvalidTimestamp => {
            ReviewTaskCandidateManagedRuntimeErrorV1::EventContract
        }
        ReviewTaskCandidateEventRelayErrorV1::Persistence(error) => {
            ReviewTaskCandidateManagedRuntimeErrorV1::Persistence(error)
        }
        ReviewTaskCandidateEventRelayErrorV1::EventUnavailable => {
...
fn submission_error(
    error: ReviewTaskCandidateSubmissionErrorV1,
) -> ReviewTaskCandidateManagedRuntimeErrorV1 {
    match error {
        ReviewTaskCandidateSubmissionErrorV1::InvalidEnvelope
        | ReviewTaskCandidateSubmissionErrorV1::InvalidPayload
        | ReviewTaskCandidateSubmissionErrorV1::Blob(
            crate::blob_materialization::ReviewTaskCandidateBlobErrorV1::InvalidReceipt,
        ) => ReviewTaskCandidateManagedRuntimeErrorV1::EventContract,
        ReviewTaskCandidateSubmissionErrorV1::Blob(
            crate::blob_materialization::ReviewTaskCandidateBlobErrorV1::Unavailable,
        ) => ReviewTaskCandidateManagedRuntimeErrorV1::Unavailable,
        ReviewTaskCandidateSubmissionErrorV1::Persistence(error) => {
            ReviewTaskCandidateManagedRuntimeErrorV1::Persistence(error)
        }
...
fn client_realtime_error(
    error: ReviewTaskCandidateClientRealtimeErrorV1,
) -> ReviewTaskCandidateManagedRuntimeErrorV1 {
    match error {
        ReviewTaskCandidateClientRealtimeErrorV1::InvalidTransition => {
            ReviewTaskCandidateManagedRuntimeErrorV1::InvalidTransition
        }
        ReviewTaskCandidateClientRealtimeErrorV1::Persistence(error) => {
            ReviewTaskCandidateManagedRuntimeErrorV1::Persistence(error)
        }
        ReviewTaskCandidateClientRealtimeErrorV1::Unavailable => {
            ReviewTaskCandidateManagedRuntimeErrorV1::Unavailable
        }
    }
}
...
mod tests {
    ...
    fn admission() -> ReviewTaskCandidateRuntimeAdmissionV1 {
        ReviewTaskCandidateRuntimeAdmissionV1 {
            logical_owner_id: REVIEW_TASK_CANDIDATE_OWNER_V1.to_owned(),
    ...
    fn admission_separates_review_owner_from_human_owner() {
        ...
        invalid.logical_human_owner_id = REVIEW_TASK_CANDIDATE_OWNER_V1.to_owned();
        assert_eq!(
            validate_admission(&invalid),
            Err(ReviewTaskCandidateManagedRuntimeErrorV1::Admission)
        );
    }
}
