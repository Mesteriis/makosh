use makosh_consistency_persistence::{
    ConsistencyPersistenceErrorV1, consistency_storage_bundle_v1,
};
...
fn main() -> Result<(), String> {
    ...
    match command.as_deref() {
        ...
        Some(v) if v == OsStr::new("export-storage-bundle") => {
            export(&mut args, consistency_storage_bundle_v1().encode_to_vec())
        }
        Some(v) if v == OsStr::new("export-module-descriptor") => {
            let build = args
                ...
                .filter(|v| !v.is_empty())
                .ok_or_else(|| "Consistency descriptor build id is required".to_owned())?;
            export(
                &mut args,
                consistency_module_descriptor_v1(&build).encode_to_vec(),
            )
        }
        Some(v) if v == OsStr::new("export-settings-schema") => {
            export(&mut args, consistency_settings_schema_bytes_v1())
        }
        _ => Err("Consistency runtime command is unavailable".into()),
    }
}
fn serve<I: Iterator<Item = OsString>>(args: &mut std::iter::Peekable<I>) -> Result<(), String> {
    ...
    let schema = decode_settings_schema_v1(&schema_bytes)
        .map_err(|_| "Consistency settings schema is invalid".to_owned())?;
    let snapshot = decode_settings_snapshot_v1(&read(&paths.settings_snapshot)?)
        .map_err(|_| "Consistency settings snapshot is invalid".to_owned())?;
    ...
    let config =
        ManagedEngineRuntimeConfigurationV1::decode(read(&paths.runtime_configuration)?.as_slice())
            .map_err(|_| "Consistency runtime configuration is invalid".to_owned())?;
    ...
    {
        return Err("Consistency runtime configuration is stale".into());
    }
    let storage = config
        .storage
        .clone()
        .ok_or_else(|| "Consistency storage is unavailable".to_owned())?;
    let admission = ConsistencyRuntimeAdmissionV1 {
    ...
    let executor = tokio::runtime::Runtime::new()
        .map_err(|_| "Consistency executor is unavailable".to_owned())?;
    let mut runtime = executor
        .block_on(ConsistencyManagedRuntimeV1::open(
            inherited()?,
            descriptor,
            schema_bytes,
            &admission,
    ...
    loop {
        match executor.block_on(runtime.service_once(now()?)) {
            Ok(_) => failures = 0,
            Err(ConsistencyManagedRuntimeErrorV1::ControlClosed) => return Ok(()),
            Err(ConsistencyManagedRuntimeErrorV1::EventUnavailable)
            | Err(ConsistencyManagedRuntimeErrorV1::Persistence(
...
fn runtime_error(error: ConsistencyManagedRuntimeErrorV1) -> String {
    let code = match error {
        ConsistencyManagedRuntimeErrorV1::Admission => "admission",
        ConsistencyManagedRuntimeErrorV1::EventContract => "event_contract",
        ConsistencyManagedRuntimeErrorV1::EventUnavailable => "event_unavailable",
        ConsistencyManagedRuntimeErrorV1::Persistence(_) => "persistence",
        ConsistencyManagedRuntimeErrorV1::ControlClosed => "control_closed",
        ConsistencyManagedRuntimeErrorV1::Unavailable => "unavailable",
    };
    format!("Consistency runtime failed: {code}")
}
fn now() -> Result<i64, String> {
    std::time::SystemTime::now()
        ...
        .ok_or_else(|| "Consistency clock is invalid".into())
}
fn export<I: Iterator<Item = OsString>>(args: &mut I, bytes: Vec<u8>) -> Result<(), String> {
    if args.next().is_some() {
        return Err("Consistency runtime command is unavailable".into());
    }
    std::io::Write::write_all(&mut std::io::stdout(), &bytes)
        .map_err(|_| "Consistency artifact is unavailable".into())
}
fn parse<I: Iterator<Item = OsString>>(args: &mut std::iter::Peekable<I>) -> Result<Paths, String> {
    ...
    if args.next().is_some() {
        return Err("Consistency runtime arguments are invalid".into());
    }
    Ok(Paths {
        descriptor,
        settings_schema,
...
fn required<I: Iterator<Item = OsString>>(args: &mut I, name: &str) -> Result<String, String> {
    ...
    args.next()
        .and_then(|v| v.into_string().ok())
        .filter(|v| !v.is_empty())
        .ok_or_else(|| "Consistency runtime arguments are invalid".into())
}
fn inherited() -> Result<UnixStream, String> {
    let fd = unsafe { libc::dup(std::io::stdin().as_raw_fd()) };
    if fd < 0 {
        return Err("Consistency control channel is unavailable".into());
    }
    Ok(unsafe { UnixStream::from_raw_fd(fd) })
}
fn read(path: &Path) -> Result<Vec<u8>, String> {
    let metadata = std::fs::symlink_metadata(path)
        .map_err(|_| "Consistency contract is unavailable".to_owned())?;
    if metadata.file_type().is_symlink() || !metadata.is_file() || metadata.len() > 512 * 1024 {
        return Err("Consistency contract is unavailable".into());
    }
    std::fs::read(path).map_err(|_| "Consistency contract is unavailable".into())
}
