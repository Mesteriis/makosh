use makosh_consistency_core::{ConsistencyEdgeV1, ConsistencyNodeV1};
use makosh_consistency_persistence::{
    ApplyConsistencyMutationV1, ConsistencyEnvelopeRecordV1, ConsistencyMutationV1,
    ConsistencyPersistenceErrorV1, ConsistencyPersistenceV1, ConsistencyReplayOutcomeV1,
};
...
pub enum ConsistencySourceV1 {
    Persons,
    Relationships,
}
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ConsistencyExecutionContextV1 {
    pub logical_owner_id: String,
    pub projection_generation: u64,
    pub now_unix_millis: i64,
}
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum ConsistencyExecutionOutcomeV1 {
    Applied,
    Replayed,
    Ignored,
}
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum ConsistencyExecutionErrorV1 {
    InvalidContext,
    InvalidEnvelope,
    InvalidPayload,
...
struct Normalized {
    ...
    mutation: ConsistencyMutationV1,
}
pub async fn process_consistency_source_event_v1(
    persistence: &ConsistencyPersistenceV1,
    record: &OutboxRecordV1,
    source: ConsistencySourceV1,
    context: &ConsistencyExecutionContextV1,
) -> Result<ConsistencyExecutionOutcomeV1, ConsistencyExecutionErrorV1> {
    ...
    let Some(event) = normalize(source, &envelope)? else {
        return Ok(ConsistencyExecutionOutcomeV1::Ignored);
    };
    validate_envelope(record, &envelope, &event, context)?;
    let outcome = persistence
        .apply_once(&ApplyConsistencyMutationV1 {
        ...
        .await
        .map_err(ConsistencyExecutionErrorV1::Persistence)?;
    Ok(match outcome {
        ConsistencyReplayOutcomeV1::Applied => ConsistencyExecutionOutcomeV1::Applied,
        ConsistencyReplayOutcomeV1::Replayed => ConsistencyExecutionOutcomeV1::Replayed,
    })
}
fn normalize(
    source: ConsistencySourceV1,
    envelope: &DurableEnvelopeV1,
) -> Result<Option<Normalized>, ConsistencyExecutionErrorV1> {
    match source {
        ConsistencySourceV1::Relationships => normalize_relationship(envelope),
        ConsistencySourceV1::Persons => normalize_persons(envelope),
    }
}
fn normalize_relationship(
    envelope: &DurableEnvelopeV1,
) -> Result<Option<Normalized>, ConsistencyExecutionErrorV1> {
    let value: RelationshipChangedV1 = exact_decode(
        &envelope.payload,
        ConsistencyExecutionErrorV1::InvalidPayload,
    )?;
    let state = RelationshipStateV1::try_from(value.state)
        .map_err(|_| ConsistencyExecutionErrorV1::InvalidPayload)?;
    let relation = RelationshipTypeV1::try_from(value.relationship_type)
        .map_err(|_| ConsistencyExecutionErrorV1::InvalidPayload)?;
    if state as i32 == 0 || relation as i32 == 0 {
        return Err(ConsistencyExecutionErrorV1::InvalidPayload);
    }
    let time = value
        .occurred_at
        .ok_or(ConsistencyExecutionErrorV1::InvalidPayload)?;
    let revision = positive(value.relationship_revision)?;
    let edge = ConsistencyEdgeV1 {
    ...
    Ok(Some(Normalized {
        ...
        mutation: ConsistencyMutationV1::UpsertEdge(edge),
    }))
}
fn normalize_persons(
    envelope: &DurableEnvelopeV1,
) -> Result<Option<Normalized>, ConsistencyExecutionErrorV1> {
    let value: PersonsOwnerEventV1 = exact_decode(
        &envelope.payload,
        ConsistencyExecutionErrorV1::InvalidPayload,
    )?;
    match value.event {
...
fn participant(
...
) -> Result<ConsistencyNodeV1, ConsistencyExecutionErrorV1> {
    let value = value.ok_or(ConsistencyExecutionErrorV1::InvalidPayload)?;
    let (kind, owner) = match RelationshipParticipantKindV1::try_from(value.kind) {
        ...
        _ => return Err(ConsistencyExecutionErrorV1::InvalidPayload),
    };
    Ok(ConsistencyNodeV1 {
        owner: owner.into(),
        kind: kind.into(),
...
fn validate_envelope(
    ...
    event: &Normalized,
    context: &ConsistencyExecutionContextV1,
) -> Result<(), ConsistencyExecutionErrorV1> {
    let contract = envelope
        .contract
        .as_ref()
        .ok_or(ConsistencyExecutionErrorV1::InvalidEnvelope)?;
    let source = envelope
        .source
        .as_ref()
        .ok_or(ConsistencyExecutionErrorV1::InvalidEnvelope)?;
    ...
    {
        return Err(ConsistencyExecutionErrorV1::InvalidEnvelope);
    }
    Ok(())
}
fn lineage_edge_id(mut first: [u8; 16], mut second: [u8; 16]) -> [u8; 16] {
    ...
    hash.update(b"makosh.consistency.person-lineage.v1");
    hash.update(first);
    hash.update(second);
    hash.finalize()[..16].try_into().expect("fixed")
}
fn validate_context(
    value: &ConsistencyExecutionContextV1,
) -> Result<(), ConsistencyExecutionErrorV1> {
    if value.logical_owner_id.is_empty()
        || value.projection_generation == 0
        || value.now_unix_millis <= 0
    {
        Err(ConsistencyExecutionErrorV1::InvalidContext)
    } else {
        Ok(())
    }
}
fn exact_decode<M: Message + Default>(
    bytes: &[u8],
    error: ConsistencyExecutionErrorV1,
) -> Result<M, ConsistencyExecutionErrorV1> {
    let value = M::decode(bytes).map_err(|_| error)?;
    (value.encode_to_vec() == bytes)
        .then_some(value)
        .ok_or(error)
}
fn id16(value: &[u8]) -> Result<[u8; 16], ConsistencyExecutionErrorV1> {
    value
        .try_into()
        .ok()
...
fn positive(value: u64) -> Result<u64, ConsistencyExecutionErrorV1> {
    (value > 0)
        .then_some(value)
        .ok_or(ConsistencyExecutionErrorV1::InvalidPayload)
}
fn millis(seconds: i64, nanos: i32) -> Result<i64, ConsistencyExecutionErrorV1> {
