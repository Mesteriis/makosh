...
use crate::{
    AttachmentTranslationPersistenceErrorV1,
    model::{ATTACHMENT_TRANSLATION_REALTIME_LIMIT_V1, valid_identity, valid_timestamp},
    repository::state_from_code,
};

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct AttachmentTranslationRealtimeTransitionV1 {
    pub sequence: u64,
    pub run_id: [u8; 16],
...
impl crate::AttachmentTranslationPersistenceV1 {
    pub async fn client_realtime_window(
        ...
        Vec<AttachmentTranslationRealtimeTransitionV1>,
        AttachmentTranslationPersistenceErrorV1,
    > {
        if !valid_identity(logical_owner_id)
            || after_sequence == Some(0)
            || !(1..=ATTACHMENT_TRANSLATION_REALTIME_LIMIT_V1).contains(&limit)
        {
...
pub(crate) async fn insert_realtime_transition(
    ...
    if inserted == 1 {
        ...
        Err(AttachmentTranslationPersistenceErrorV1::InvalidRow)
    }
}

fn transition_from_row(
    row: sqlx::postgres::PgRow,
) -> Result<AttachmentTranslationRealtimeTransitionV1, AttachmentTranslationPersistenceErrorV1> {
    ...
    Ok(AttachmentTranslationRealtimeTransitionV1 {
...
fn rejection_from_code(
    value: i16,
) -> Result<AttachmentTranslationRejectionCodeV1, AttachmentTranslationPersistenceErrorV1> {
    match value {
