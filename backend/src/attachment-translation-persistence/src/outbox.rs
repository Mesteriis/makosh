...
use crate::{
    ...
    model::{
        ATTACHMENT_TRANSLATION_MAX_EVENT_BYTES_V1, ATTACHMENT_TRANSLATION_OUTBOX_LIMIT_V1, nonzero,
...
impl AttachmentTranslationPersistenceV1 {
    pub async fn unpublished_source_prepare_events(
        ...
        limit: u16,
    ) -> Result<Vec<UnpublishedAttachmentTranslationEventV1>, AttachmentTranslationPersistenceErrorV1>
    {
        if !valid_identity(logical_owner_id)
            || !(1..=ATTACHMENT_TRANSLATION_OUTBOX_LIMIT_V1).contains(&limit)
        {
            return Err(AttachmentTranslationPersistenceErrorV1::InvalidInput);
        }
        sqlx::query(
            "SELECT message_id, envelope_sha256, envelope_bytes
             FROM makosh_data.attachment_translation_outbox
        ...
        .map(|row| {
            ...
            let envelope_bytes: Vec<u8> = row
                ...
                .map_err(|_| AttachmentTranslationPersistenceErrorV1::InvalidRow)?;
            let event = UnpublishedAttachmentTranslationEventV1 {
            ...
            if !nonzero(&event.message_id)
                ...
                || event.envelope_bytes.len() > ATTACHMENT_TRANSLATION_MAX_EVENT_BYTES_V1
            {
    ...
    pub async fn mark_source_prepare_published(
        ...
        let updated = sqlx::query(
            "UPDATE makosh_data.attachment_translation_outbox
