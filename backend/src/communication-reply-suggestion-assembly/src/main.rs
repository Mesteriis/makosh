...
use makosh_communication_reply_suggestion_assembly::materialize_reply_suggestion_release_assembly_v1;
...
fn main() -> ExitCode {
    match arguments(std::env::args().skip(1).collect()) {
        Some(arguments) => {
            if materialize_reply_suggestion_release_assembly_v1(
            ...
            println!("communication-reply-suggestion-release-assembly: ok");
            ExitCode::SUCCESS
        }
        None => fail(
            "usage: makosh-communication-reply-suggestion-assembly --build-id <id> \
...
fn fail(message: &str) -> ExitCode {
    eprintln!("communication-reply-suggestion-release-assembly: {message}");
    ExitCode::FAILURE
}
...
mod tests {
    ...
    fn parses_exact_options() {
        let parsed = arguments(vec![
            "--runtime".to_owned(),
            "/tmp/makosh-communication-reply-suggestion-runtime".to_owned(),
            "--build-id".to_owned(),
            "build-1".to_owned(),
            "--output-dir".to_owned(),
            "/tmp/communication-reply-suggestion-assembly".to_owned(),
        ])
        .expect("arguments");
        assert_eq!(parsed.build_id, "build-1");
        assert_eq!(
            parsed.runtime,
            PathBuf::from("/tmp/makosh-communication-reply-suggestion-runtime")
        );
    }
