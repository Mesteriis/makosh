...
use makosh_attachment_translation_persistence::{
    AttachmentTranslationPersistenceErrorV1, attachment_translation_storage_bundle_v1,
};
use makosh_attachment_translation_runtime::{
    ...
    AttachmentTranslationRuntimeAdmissionV1, attachment_translation_module_descriptor_v1,
...
fn main() -> Result<(), String> {
    ...
    let Some(command) = arguments.next() else {
        return Err("Attachment Translation command is required".to_owned());
    };
    match command.to_str() {
        Some("export-storage-bundle") => export_bytes(
            &mut arguments,
            attachment_translation_storage_bundle_v1().encode_to_vec(),
        ),
        Some("export-module-descriptor") => {
            let build_id = required_string(&mut arguments, "build id")?;
            export_bytes(
                &mut arguments,
                attachment_translation_module_descriptor_v1(&build_id).encode_to_vec(),
            )
        }
        ...
        _ => Err("Attachment Translation command is invalid".to_owned()),
    }
}
    ...
    I: Iterator<Item = OsString>,
{
    let output = arguments
        .next()
        .map(PathBuf::from)
        .ok_or_else(|| "Attachment Translation output path is required".to_owned())?;
    require_no_arguments(arguments)?;
    fs::write(output, bytes).map_err(|_| "Attachment Translation export failed".to_owned())
}
    ...
    I: Iterator<Item = OsString>,
{
    ...
    decode_settings_schema_v1(&settings_schema)
        .map_err(|_| "Attachment Translation settings schema is invalid".to_owned())?;
    let configuration = ManagedWorkflowRuntimeConfigurationV1::decode(
        read_contract(&paths.runtime_configuration)?.as_slice(),
    )
    .map_err(|_| "Attachment Translation runtime configuration is invalid".to_owned())?;
    ...
    if configuration.runtime_instance_id != paths.runtime_instance_id {
        return Err("Attachment Translation runtime configuration is stale".to_owned());
    }
    let storage = configuration
        .storage
        .clone()
        .ok_or_else(|| "Attachment Translation storage is unavailable".to_owned())?;
    let admission = AttachmentTranslationRuntimeAdmissionV1 {
    ...
    let executor = tokio::runtime::Runtime::new()
        .map_err(|_| "Attachment Translation executor is unavailable".to_owned())?;
    let mut runtime = executor
        .block_on(AttachmentTranslationManagedRuntimeV1::open(
    ...
    loop {
        let now = std::time::SystemTime::now()
            ...
            .map_err(|_| "Attachment Translation clock is invalid".to_owned())?
            .as_millis()
            .try_into()
            .map_err(|_| "Attachment Translation clock is invalid".to_owned())?;
...
fn retry_runtime(
    result: Result<bool, AttachmentTranslationManagedRuntimeErrorV1>,
) -> Result<(), String> {
    match result {
        Ok(_)
        | Err(AttachmentTranslationManagedRuntimeErrorV1::EventUnavailable)
        | Err(AttachmentTranslationManagedRuntimeErrorV1::Unavailable)
        | Err(AttachmentTranslationManagedRuntimeErrorV1::Persistence(
...
fn runtime_error(error: AttachmentTranslationManagedRuntimeErrorV1) -> String {
    let code = match error {
        AttachmentTranslationManagedRuntimeErrorV1::Admission => "admission",
        AttachmentTranslationManagedRuntimeErrorV1::EventContract => "event_contract",
        AttachmentTranslationManagedRuntimeErrorV1::EventUnavailable => "event_unavailable",
        AttachmentTranslationManagedRuntimeErrorV1::InvalidTransition => "invalid_transition",
        AttachmentTranslationManagedRuntimeErrorV1::Persistence(_) => "persistence",
        AttachmentTranslationManagedRuntimeErrorV1::Unavailable => "unavailable",
    };
    format!("Attachment Translation runtime failed: {code}")
}
    ...
    I: Iterator<Item = OsString>,
{
    if arguments.next().as_deref() != Some(OsStr::new(name)) {
        return Err("Attachment Translation arguments are invalid".to_owned());
    }
    let value = arguments
        .next()
        .and_then(|value| value.into_string().ok())
        .ok_or_else(|| format!("Attachment Translation {name} is required"))?;
    if value.is_empty() {
        return Err(format!("Attachment Translation {name} is invalid"));
    }
    Ok(value)
}
...
fn inherited_control_channel() -> Result<UnixStream, String> {
    ...
    if duplicated < 0 {
        return Err("Attachment Translation control channel is unavailable".to_owned());
    }
...
fn read_contract(path: &Path) -> Result<Vec<u8>, String> {
    if !path.is_absolute() {
        return Err("Attachment Translation contract path is invalid".to_owned());
    }
    let bytes =
        fs::read(path).map_err(|_| "Attachment Translation contract is unavailable".to_owned())?;
    if bytes.is_empty() || bytes.len() > 2 * 1024 * 1024 {
        return Err("Attachment Translation contract is invalid".to_owned());
    }
    Ok(bytes)
}
