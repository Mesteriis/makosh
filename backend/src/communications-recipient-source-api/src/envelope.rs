...
use crate::{
    ...
    COMMUNICATION_RECIPIENT_SOURCE_PREPARE_CONTRACT_NAME_V1,
    COMMUNICATION_RECIPIENT_SOURCE_PREPARED_CONTRACT_NAME_V1,
    COMMUNICATION_RECIPIENT_SOURCE_REJECTED_CONTRACT_NAME_V1,
    COMMUNICATIONS_RECIPIENT_SOURCE_CAPABILITY_ID_V1,
    COMMUNICATIONS_RECIPIENT_SOURCE_CONTRACT_MAJOR_V1,
    COMMUNICATIONS_RECIPIENT_SOURCE_CONTRACT_REVISION_V1, COMMUNICATIONS_RECIPIENT_SOURCE_OWNER_V1,
    COMMUNICATIONS_RECIPIENT_SOURCE_SCHEMA_SHA256,
    wire::{
        CommunicationRecipientBodySourceReceiptV1, CommunicationRecipientSourcePreparedV1,
        CommunicationRecipientSourceRejectedV1, PrepareCommunicationRecipientSourceCommandV1,
    },
};

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct CommunicationRecipientSourceEnvelopeContextV1 {
...
pub enum CommunicationRecipientSourceEnvelopeBuildErrorV1 {
    InvalidContext,
    InvalidPayload,
    InvalidEnvelope,
    OutboxRejected,
...
pub fn build_communication_recipient_source_prepare_outbox_record_v1(
    run_id: [u8; 16],
    source_message_id: [u8; 16],
    expected_source_revision: u64,
    logical_owner_id: &str,
    deadline_unix_seconds: i64,
    context: &CommunicationRecipientSourceEnvelopeContextV1,
) -> Result<OutboxRecordV1, CommunicationRecipientSourceEnvelopeBuildErrorV1> {
    ...
    if deadline_unix_seconds <= context.recorded_at_unix_seconds {
        return Err(CommunicationRecipientSourceEnvelopeBuildErrorV1::InvalidPayload);
    }
    let payload = PrepareCommunicationRecipientSourceCommandV1 {
    ...
    build_envelope(
        ...
        Semantics::Command(CommandMetadataV1 {
            ...
            target_capability: COMMUNICATIONS_RECIPIENT_SOURCE_CAPABILITY_ID_V1.to_owned(),
            idempotency_key: Sha256::digest(
                [
                    b"communications-recipient-source-prepare-v1".as_slice(),
                    &run_id,
                ]
                .concat(),
            )
...
pub fn build_communication_recipient_source_prepared_outbox_record_v1(
    command_message_id: [u8; 16],
    payload: CommunicationRecipientSourcePreparedV1,
    context: &CommunicationRecipientSourceEnvelopeContextV1,
) -> Result<OutboxRecordV1, CommunicationRecipientSourceEnvelopeBuildErrorV1> {
    validate_context(context)?;
...
pub fn build_communication_recipient_source_rejected_outbox_record_v1(
    command_message_id: [u8; 16],
    payload: CommunicationRecipientSourceRejectedV1,
    context: &CommunicationRecipientSourceEnvelopeContextV1,
) -> Result<OutboxRecordV1, CommunicationRecipientSourceEnvelopeBuildErrorV1> {
    validate_context(context)?;
...
fn build_envelope(
    ...
    validate_envelope_v1(&envelope)
        .map_err(|_| CommunicationRecipientSourceEnvelopeBuildErrorV1::InvalidEnvelope)?;
...
fn validate_context(
    context: &CommunicationRecipientSourceEnvelopeContextV1,
) -> Result<(), CommunicationRecipientSourceEnvelopeBuildErrorV1> {
    ...
    {
        return Err(CommunicationRecipientSourceEnvelopeBuildErrorV1::InvalidContext);
    }
    Ok(())
}
...
fn validate_prepared_payload(
    payload: &CommunicationRecipientSourcePreparedV1,
) -> Result<[u8; 16], CommunicationRecipientSourceEnvelopeBuildErrorV1> {
    let run_id = id16(&payload.run_id)?;
...
fn valid_source_receipt(receipt: &CommunicationRecipientBodySourceReceiptV1) -> bool {
    receipt.reference_id.len() == 16
        && receipt.reference_id.iter().any(|byte| *byte != 0)
        && (1..=COMMUNICATION_RECIPIENT_SOURCE_MAX_BYTES_V1).contains(&receipt.declared_bytes)
        ...
        && receipt.custody_transfer_source_proof.len()
            <= COMMUNICATION_RECIPIENT_SOURCE_MAX_PROOF_BYTES_V1
}
...
fn id16(bytes: &[u8]) -> Result<[u8; 16], CommunicationRecipientSourceEnvelopeBuildErrorV1> {
    bytes
        .try_into()
        .ok()
        .filter(valid_id)
        .ok_or(CommunicationRecipientSourceEnvelopeBuildErrorV1::InvalidPayload)
}

fn result_message_id(label: &[u8], run_id: &[u8; 16]) -> [u8; 16] {
    let mut hasher = Sha256::new();
    hasher.update(b"communications-recipient-source-result-v1");
    hasher.update(label);
...
fn outbox_error(_: OutboxRecordError) -> CommunicationRecipientSourceEnvelopeBuildErrorV1 {
    CommunicationRecipientSourceEnvelopeBuildErrorV1::OutboxRejected
}
...
mod tests {
    ...
    fn context() -> CommunicationRecipientSourceEnvelopeContextV1 {
        CommunicationRecipientSourceEnvelopeContextV1 {
            module_id: "makosh-communication-recipient-suggestion-runtime".to_owned(),
            runtime_instance_id: "runtime-recipient-1".to_owned(),
    ...
    fn prepared_payload() -> CommunicationRecipientSourcePreparedV1 {
        CommunicationRecipientSourcePreparedV1 {
            ...
            body_source: Some(CommunicationRecipientBodySourceReceiptV1 {
    ...
    fn command_contains_only_canonical_ids_and_exact_capability() {
        let record = build_communication_recipient_source_prepare_outbox_record_v1(
            [1; 16],
            [2; 16],
            3,
            "owner-1",
        ...
        let payload =
            PrepareCommunicationRecipientSourceCommandV1::decode(envelope.payload.as_slice())
                .expect("payload");
    ...
    fn prepared_result_is_receipt_only_and_hash_bound() {
        let record = build_communication_recipient_source_prepared_outbox_record_v1(
            [9; 16],
            prepared_payload(),
            &context(),
        )
        ...
        let payload = CommunicationRecipientSourcePreparedV1::decode(envelope.payload.as_slice())
            .expect("payload");
        assert_eq!(payload.body_source.expect("receipt").sha256, vec![7; 32]);
    }
    ...
    fn rejects_invalid_revision_and_oversized_proof() {
        assert_eq!(
            ...
            Err(CommunicationRecipientSourceEnvelopeBuildErrorV1::InvalidPayload)
        );
        ...
        payload
            ...
            vec![8; COMMUNICATION_RECIPIENT_SOURCE_MAX_PROOF_BYTES_V1 + 1];
        assert_eq!(
