...
use crate::{
    ...
    model::{
        COMMUNICATION_RECIPIENT_SUGGESTION_REALTIME_LIMIT_V1, valid_identity, valid_timestamp,
    },
    repository::state_from_code,
};

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct CommunicationRecipientSuggestionRealtimeTransitionV1 {
    pub sequence: u64,
    pub run_id: [u8; 16],
...
impl crate::CommunicationRecipientSuggestionPersistenceV1 {
    pub async fn client_realtime_window(
        ...
        Vec<CommunicationRecipientSuggestionRealtimeTransitionV1>,
        CommunicationRecipientSuggestionPersistenceErrorV1,
    > {
        if !valid_identity(logical_owner_id)
            || after_sequence == Some(0)
            || !(1..=COMMUNICATION_RECIPIENT_SUGGESTION_REALTIME_LIMIT_V1).contains(&limit)
        {
        ...
        let rows = if let Some(after) = after_sequence {
            ...
            sqlx::query(
                "SELECT realtime_sequence, run_id, state, state_revision,
                 ...
                 FROM makosh_data.communication_recipient_suggestion_realtime
...
pub(crate) async fn insert_realtime_transition(
    ...
    let inserted = sqlx::query(
        "INSERT INTO makosh_data.communication_recipient_suggestion_realtime (
    ...
    if inserted == 1 {
        ...
        Err(CommunicationRecipientSuggestionPersistenceErrorV1::InvalidRow)
    }
}
...
fn transition_from_row(
    ...
    CommunicationRecipientSuggestionRealtimeTransitionV1,
    CommunicationRecipientSuggestionPersistenceErrorV1,
> {
    ...
    Ok(CommunicationRecipientSuggestionRealtimeTransitionV1 {
...
fn rejection_from_code(
    ...
    CommunicationRecipientSuggestionRejectionCodeV1,
    CommunicationRecipientSuggestionPersistenceErrorV1,
> {
    match value {
