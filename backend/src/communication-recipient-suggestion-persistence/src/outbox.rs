...
use crate::{
    ...
    model::{
        ...
        COMMUNICATION_RECIPIENT_SUGGESTION_OUTBOX_LIMIT_V1, nonzero, valid_identity,
        valid_timestamp,
    },
};
...
impl CommunicationRecipientSuggestionPersistenceV1 {
    pub async fn unpublished_source_prepare_events(
        ...
        Vec<UnpublishedCommunicationRecipientSuggestionEventV1>,
        CommunicationRecipientSuggestionPersistenceErrorV1,
    > {
        if !valid_identity(logical_owner_id)
            || !(1..=COMMUNICATION_RECIPIENT_SUGGESTION_OUTBOX_LIMIT_V1).contains(&limit)
        {
        ...
        .map(|row| {
            ...
            let envelope_bytes: Vec<u8> = row
                ...
                .map_err(|_| CommunicationRecipientSuggestionPersistenceErrorV1::InvalidRow)?;
            let event = UnpublishedCommunicationRecipientSuggestionEventV1 {
