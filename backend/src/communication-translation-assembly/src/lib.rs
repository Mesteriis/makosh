//! Communication Translation workflow release assembly.
//!
...
pub const COMMUNICATION_TRANSLATION_DESCRIPTOR_FILE: &str =
    "communication_translation.runtime.descriptor.pb";
pub const COMMUNICATION_TRANSLATION_SETTINGS_FILE: &str =
    "communication_translation.runtime.settings.pb";
pub const COMMUNICATION_TRANSLATION_STORAGE_BUNDLE_FILE: &str =
    "communication_translation.storage.bundle.pb";
...
#[derive(Clone, Debug, Eq, PartialEq, Deserialize, Serialize)]
#[serde(untagged)]
pub enum CommunicationTranslationReleaseArtifactInputV1 {
    ModuleRuntime(ModuleRuntimeArtifactInputV1),
    StorageBundle(StorageBundleArtifactInputV1),
}

impl CommunicationTranslationReleaseArtifactInputV1 {
    #[must_use]
...
#[derive(Clone, Debug, Eq, PartialEq, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
pub struct CommunicationTranslationReleaseArtifactFragmentV1 {
    pub version: u32,
    pub owner_id: String,
    pub module_id: String,
    pub artifacts: Vec<CommunicationTranslationReleaseArtifactInputV1>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct CommunicationTranslationReleaseAssemblyPathsV1 {
...
pub enum CommunicationTranslationReleaseAssemblyErrorV1 {
    InvalidInput,
...
pub fn materialize_communication_translation_release_assembly_v1(
    output_directory: &Path,
    build_id: &str,
    runtime_source: &Path,
) -> Result<
    CommunicationTranslationReleaseAssemblyPathsV1,
    CommunicationTranslationReleaseAssemblyErrorV1,
> {
    validate_inputs(output_directory, build_id, runtime_source)?;
    let descriptor = communication_translation_module_descriptor_v1(build_id);
    let settings_schema = communication_translation_settings_schema_v1();
    let storage_bundle = communication_translation_storage_bundle_v1();
    ...
    {
        return Err(CommunicationTranslationReleaseAssemblyErrorV1::InvalidCanonicalArtifact);
    }

    let paths = CommunicationTranslationReleaseAssemblyPathsV1 {
        descriptor: output_directory.join(COMMUNICATION_TRANSLATION_DESCRIPTOR_FILE),
        settings_schema: output_directory.join(COMMUNICATION_TRANSLATION_SETTINGS_FILE),
        storage_bundle: output_directory.join(COMMUNICATION_TRANSLATION_STORAGE_BUNDLE_FILE),
        artifact_fragment: output_directory.join(COMMUNICATION_TRANSLATION_ARTIFACT_FRAGMENT_FILE),
    };
    ...
    let writes = [
        ...
        (
            ...
            serde_json::to_vec_pretty(&fragment).map_err(|_| {
                CommunicationTranslationReleaseAssemblyErrorV1::FragmentEncodingFailed
            })?,
        ),
    ];
    ...
    builder
        ...
        .map_err(|_| CommunicationTranslationReleaseAssemblyErrorV1::OutputUnavailable)?;
    for (path, bytes) in &writes {
        if write_new_private_file(path, bytes).is_err() {
            let _ = fs::remove_dir_all(output_directory);
            return Err(CommunicationTranslationReleaseAssemblyErrorV1::ArtifactWriteFailed);
        }
    }
    Ok(paths)
}
...
fn validate_inputs(
    ...
    build_id: &str,
    runtime_source: &Path,
) -> Result<(), CommunicationTranslationReleaseAssemblyErrorV1> {
    ...
    {
        return Err(CommunicationTranslationReleaseAssemblyErrorV1::InvalidInput);
    }
    Ok(())
}
...
fn artifact_fragment(
    ...
    CommunicationTranslationReleaseArtifactFragmentV1,
    CommunicationTranslationReleaseAssemblyErrorV1,
> {
    let artifacts = vec![
        CommunicationTranslationReleaseArtifactInputV1::ModuleRuntime(
            ModuleRuntimeArtifactInputV1 {
                artifact_kind: "module_runtime".to_owned(),
                artifact_id: COMMUNICATION_TRANSLATION_RUNTIME_ARTIFACT_ID.to_owned(),
        ...
        CommunicationTranslationReleaseArtifactInputV1::StorageBundle(
            StorageBundleArtifactInputV1 {
                artifact_kind: "storage_bundle".to_owned(),
                artifact_id: COMMUNICATION_TRANSLATION_STORAGE_ARTIFACT_ID.to_owned(),
    ...
    Ok(CommunicationTranslationReleaseArtifactFragmentV1 {
        version: COMMUNICATION_TRANSLATION_ASSEMBLY_FRAGMENT_VERSION_V1,
        owner_id: COMMUNICATION_TRANSLATION_ASSEMBLY_OWNER_ID.to_owned(),
        module_id: COMMUNICATION_TRANSLATION_ASSEMBLY_MODULE_ID.to_owned(),
        artifacts,
    })
}
...
fn utf8_path(path: &Path) -> Result<String, CommunicationTranslationReleaseAssemblyErrorV1> {
    path.to_str()
        .filter(|value| !value.is_empty())
        .map(str::to_owned)
        .ok_or(CommunicationTranslationReleaseAssemblyErrorV1::InvalidInput)
}
...
mod tests {
    ...
    fn materializes_exact_unsigned_runtime_and_storage_fragment() {
        ...
        let storage =
            StorageBundleV1::decode(fs::read(paths.storage_bundle).expect("storage").as_slice())
                .expect("storage");
        let fragment: CommunicationTranslationReleaseArtifactFragmentV1 =
            serde_json::from_slice(&fs::read(paths.artifact_fragment).expect("fragment"))
                .expect("fragment");
        assert_eq!(
            descriptor.module_id,
            COMMUNICATION_TRANSLATION_ASSEMBLY_MODULE_ID
        );
        assert_eq!(
            storage.owner_id,
            COMMUNICATION_TRANSLATION_ASSEMBLY_OWNER_ID
        );
        assert_eq!(
            fragment
                .artifacts
                .iter()
                .map(CommunicationTranslationReleaseArtifactInputV1::artifact_id)
                .collect::<Vec<_>>(),
            [
                COMMUNICATION_TRANSLATION_RUNTIME_ARTIFACT_ID,
                COMMUNICATION_TRANSLATION_STORAGE_ARTIFACT_ID,
            ]
        );
    ...
    fn rejects_missing_runtime_and_existing_output() {
        ...
        assert_eq!(
            materialize_communication_translation_release_assembly_v1(
                &output,
                "build-1",
                &root.join("missing"),
            ),
            Err(CommunicationTranslationReleaseAssemblyErrorV1::InvalidInput)
        );
        fs::create_dir(&output).expect("existing output");
        let runtime = root.join("runtime");
        fs::write(&runtime, b"runtime").expect("runtime");
        assert_eq!(
            materialize_communication_translation_release_assembly_v1(&output, "build-1", &runtime),
            Err(CommunicationTranslationReleaseAssemblyErrorV1::InvalidInput)
        );
        fs::remove_dir_all(root).expect("cleanup");
    }
    ...
    fn temporary_directory() -> PathBuf {
        let root = std::env::current_dir()
            ...
            .join("target")
            .join(format!(
                "communication-translation-assembly-{}-{}",
                std::process::id(),
