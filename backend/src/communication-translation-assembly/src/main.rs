...
use makosh_communication_translation_assembly::materialize_communication_translation_release_assembly_v1;
...
fn main() -> ExitCode {
    match arguments(std::env::args().skip(1).collect()) {
        Some(arguments) => {
            if materialize_communication_translation_release_assembly_v1(
            ...
            println!("communication-translation-release-assembly: ok");
            ExitCode::SUCCESS
        }
        None => fail(
            "usage: makosh-communication-translation-assembly --build-id <id> \
...
fn fail(message: &str) -> ExitCode {
    eprintln!("communication-translation-release-assembly: {message}");
    ExitCode::FAILURE
}
...
mod tests {
    ...
    fn parses_exact_options() {
        let parsed = arguments(vec![
            "--runtime".to_owned(),
            "/tmp/makosh-communication-translation-runtime".to_owned(),
            "--build-id".to_owned(),
            "build-1".to_owned(),
            "--output-dir".to_owned(),
            "/tmp/communication-translation-assembly".to_owned(),
        ])
        .expect("arguments");
        assert_eq!(parsed.build_id, "build-1");
        assert_eq!(
            parsed.runtime,
            PathBuf::from("/tmp/makosh-communication-translation-runtime")
        );
    }
