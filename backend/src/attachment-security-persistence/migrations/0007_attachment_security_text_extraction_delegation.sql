CREATE TABLE makosh_data.attachment_security_text_extraction_delegation_inbox (
  ...
  extraction_run_id BYTEA NOT NULL CHECK (octet_length(extraction_run_id) = 16),
...
CREATE TABLE makosh_data.attachment_security_text_extraction_delegation_outbox (
  message_id BYTEA PRIMARY KEY CHECK (octet_length(message_id) = 16),
  envelope_sha256 BYTEA NOT NULL CHECK (octet_length(envelope_sha256) = 32),
  exact_envelope_bytes BYTEA NOT NULL CHECK (octet_length(exact_envelope_bytes) > 0),
...
CREATE TABLE makosh_data.attachment_security_text_extraction_delegation_jobs (
  request_message_id BYTEA PRIMARY KEY REFERENCES
    makosh_data.attachment_security_text_extraction_delegation_inbox (message_id),
  request_id BYTEA NOT NULL UNIQUE CHECK (octet_length(request_id) = 16),
  ...
  result_message_id BYTEA UNIQUE REFERENCES
    makosh_data.attachment_security_text_extraction_delegation_outbox (message_id),
...
CREATE INDEX attachment_security_text_extraction_delegation_jobs_pending_idx
  ON makosh_data.attachment_security_text_extraction_delegation_jobs (
    next_attempt_at_unix_seconds, request_message_id
  ) WHERE state = 1;

CREATE INDEX attachment_security_text_extraction_delegation_outbox_pending_idx
  ON makosh_data.attachment_security_text_extraction_delegation_outbox (
