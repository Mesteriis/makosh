CREATE TABLE makosh_data.attachment_security_archive_delegation_inbox (
  ...
  archive_run_id BYTEA NOT NULL CHECK (octet_length(archive_run_id) = 16),
  ...
  candidate_envelope_sha256 BYTEA NOT NULL CHECK (
    octet_length(candidate_envelope_sha256) = 32
  ),
...
CREATE TABLE makosh_data.attachment_security_archive_delegation_outbox (
  message_id BYTEA PRIMARY KEY CHECK (octet_length(message_id) = 16),
  envelope_sha256 BYTEA NOT NULL CHECK (octet_length(envelope_sha256) = 32),
  exact_envelope_bytes BYTEA NOT NULL CHECK (octet_length(exact_envelope_bytes) > 0),
...
CREATE TABLE makosh_data.attachment_security_archive_delegation_jobs (
  request_message_id BYTEA PRIMARY KEY REFERENCES
    makosh_data.attachment_security_archive_delegation_inbox (message_id),
  request_id BYTEA NOT NULL UNIQUE CHECK (octet_length(request_id) = 16),
  current_reference_id BYTEA CHECK (
    current_reference_id IS NULL OR octet_length(current_reference_id) = 16
  ),
  current_receipt_sha256 BYTEA CHECK (
    current_receipt_sha256 IS NULL OR octet_length(current_receipt_sha256) = 32
  ),
  declared_size BIGINT CHECK (
    declared_size IS NULL OR declared_size BETWEEN 1 AND 104857600
  ),
  ...
  result_message_id BYTEA UNIQUE REFERENCES
    makosh_data.attachment_security_archive_delegation_outbox (message_id),
  completed_at_unix_seconds BIGINT,
  CHECK (
    (
      rejection_code IS NULL
      AND current_reference_id IS NOT NULL
      AND current_receipt_sha256 IS NOT NULL
      AND declared_size IS NOT NULL
      AND predecessor_custody_source_proof IS NOT NULL
    )
    OR (
      rejection_code IS NOT NULL
      AND current_reference_id IS NULL
      AND current_receipt_sha256 IS NULL
...
CREATE INDEX attachment_security_archive_delegation_jobs_pending_idx
  ON makosh_data.attachment_security_archive_delegation_jobs (
    next_attempt_at_unix_seconds,
    request_message_id
  )
  WHERE state = 1;

CREATE INDEX attachment_security_archive_delegation_outbox_pending_idx
  ON makosh_data.attachment_security_archive_delegation_outbox (
