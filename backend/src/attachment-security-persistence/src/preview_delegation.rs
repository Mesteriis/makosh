...
pub enum AttachmentSecurityPreviewDelegationWorkV1 {
    Delegate {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ClaimedAttachmentSecurityPreviewDelegationV1 {
    pub command_message_id: [u8; 16],
    pub work: AttachmentSecurityPreviewDelegationWorkV1,
    pub worker_id: String,
    pub attempt_count: u32,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum PersistAttachmentSecurityPreviewDelegationOutcomeV1 {
    Duplicate,
    Ready,
    Rejected(AttachmentPreviewCustodyDelegationRejectCodeV1),
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum RetryAttachmentSecurityPreviewDelegationOutcomeV1 {
    Scheduled,
    Exhausted,
}
...
impl AttachmentSecurityPersistenceV1 {
    pub async fn persist_preview_delegation_request(
        &self,
    ...
    > {
        ...
        if !inserted {
            ...
            transaction
                ...
                .await
                .map_err(|_| AttachmentSecurityPersistenceErrorV1::StorageUnavailable)?;
            return Ok(PersistAttachmentSecurityPreviewDelegationOutcomeV1::Duplicate);
        }
        ...
        let rejection_code = source.as_ref().map(|_| None).unwrap_or(Some(
            AttachmentPreviewCustodyDelegationRejectCodeV1::NotSafe,
        ));
        ...
        Ok(rejection_code.map_or(
            PersistAttachmentSecurityPreviewDelegationOutcomeV1::Ready,
            PersistAttachmentSecurityPreviewDelegationOutcomeV1::Rejected,
        ))
    }

    pub async fn claim_next_preview_delegation(
        &self,
        worker_id: &str,
        claimed_at_unix_seconds: i64,
        lease_expires_at_unix_seconds: i64,
    ) -> Result<
        Option<ClaimedAttachmentSecurityPreviewDelegationV1>,
        AttachmentSecurityPersistenceErrorV1,
    > {
        ...
        let row = sqlx::query(
            "WITH next_job AS (
             ...
             RETURNING job.request_message_id, job.current_reference_id,
                       ...
                       job.attempt_count, inbox.request_id, inbox.preview_run_id,
    ...
    pub async fn retry_preview_delegation(
        &self,
    ...
    > {
        ...
        if claimed.attempt_count >= MAX_ATTEMPTS {
            return Ok(RetryAttachmentSecurityPreviewDelegationOutcomeV1::Exhausted);
        }
        ...
        Ok(RetryAttachmentSecurityPreviewDelegationOutcomeV1::Scheduled)
    }

    pub async fn complete_preview_delegation_with_outbox(
        &self,
        claimed: &ClaimedAttachmentSecurityPreviewDelegationV1,
    ...
    pub async fn pending_preview_delegation_outbox(
        &self,
        limit: u32,
    ...
    pub async fn mark_preview_delegation_outbox_published(
        &self,
        message_id: [u8; 16],
...
async fn insert_delegation_job(
    ...
    rejection_code: Option<AttachmentPreviewCustodyDelegationRejectCodeV1>,
...
> {
    let envelope = decode_envelope_v1(exact_envelope_bytes)
        .map_err(|_| AttachmentSecurityPersistenceErrorV1::InvalidInput)?;
    let request = RequestAttachmentPreviewCustodyDelegationV1::decode(envelope.payload.as_slice())
        .map_err(|_| AttachmentSecurityPersistenceErrorV1::InvalidInput)?;
    ...
    if !exact_contract(
        ...
        ATTACHMENT_PREVIEW_INGRESS_OWNER_V1,
        ATTACHMENT_PREVIEW_CUSTODY_DELEGATION_REQUESTED_CONTRACT_NAME_V1,
        ATTACHMENT_PREVIEW_INGRESS_CONTRACT_MAJOR_V1,
        ATTACHMENT_PREVIEW_INGRESS_CONTRACT_REVISION_V1,
        &ATTACHMENT_PREVIEW_INGRESS_SCHEMA_SHA256,
    ) || metadata.target_capability != ATTACHMENT_SECURITY_PREVIEW_DELEGATION_CAPABILITY_ID_V1
        || metadata.command_id != request.request_id
        || envelope.message_id != request.request_id
        || envelope.partition_key != request.preview_run_id
...
fn claimed_from_row(
    ...
    worker_id: &str,
) -> Result<ClaimedAttachmentSecurityPreviewDelegationV1, AttachmentSecurityPersistenceErrorV1> {
    ...
    let work = if let Some(code) = rejection_code {
        AttachmentSecurityPreviewDelegationWorkV1::Reject {
            request,
            code: AttachmentPreviewCustodyDelegationRejectCodeV1::try_from(i32::from(code))
                .map_err(|_| AttachmentSecurityPersistenceErrorV1::InvalidRow)?,
        }
    } else {
    ...
    let claimed = ClaimedAttachmentSecurityPreviewDelegationV1 {
...
fn request_from_row(
    row: &sqlx::postgres::PgRow,
) -> Result<RequestAttachmentPreviewCustodyDelegationV1, AttachmentSecurityPersistenceErrorV1> {
    let request = RequestAttachmentPreviewCustodyDelegationV1 {
        request_id: required_bytes(row, "request_id")?,
        preview_run_id: required_bytes(row, "preview_run_id")?,
...
fn valid_result_for_claim(
    ...
    match &claimed.work {
        AttachmentSecurityPreviewDelegationWorkV1::Delegate {
            ...
            let payload = AttachmentPreviewCustodyDelegatedV1::decode(envelope.payload.as_slice())
                .map_err(|_| AttachmentSecurityPersistenceErrorV1::InvalidInput)?;
            Ok(exact_contract(
                envelope.contract.as_ref(),
                ATTACHMENT_PREVIEW_INGRESS_OWNER_V1,
                ...
                == &attachment_preview_custody_delegated_message_id_v1(id16(&request.request_id)?)
                && payload.request_id == request.request_id
                && payload.preview_run_id == request.preview_run_id
                && payload.attachment_anchor_id == request.attachment_anchor_id
        ...
        AttachmentSecurityPreviewDelegationWorkV1::Reject { request, code } => {
            let payload =
                AttachmentPreviewCustodyDelegationRejectedV1::decode(envelope.payload.as_slice())
                    .map_err(|_| AttachmentSecurityPersistenceErrorV1::InvalidInput)?;
            Ok(exact_contract(
                envelope.contract.as_ref(),
                ATTACHMENT_PREVIEW_INGRESS_OWNER_V1,
                ...
                == &attachment_preview_custody_delegation_rejected_message_id_v1(id16(
                    &request.request_id,
                )?)
                && payload.request_id == request.request_id
                && payload.preview_run_id == request.preview_run_id
...
fn valid_claim(value: &ClaimedAttachmentSecurityPreviewDelegationV1) -> bool {
        ...
        && match &value.work {
            AttachmentSecurityPreviewDelegationWorkV1::Delegate {
                ...
                valid_request(request)
                    ...
                    && (1..=ATTACHMENT_PREVIEW_MAX_SOURCE_BYTES_V1).contains(declared_size)
                    && (1..=ATTACHMENT_PREVIEW_MAX_PROOF_BYTES_V1)
                        .contains(&predecessor_custody_source_proof.len())
            }
            AttachmentSecurityPreviewDelegationWorkV1::Reject { request, code } => {
                valid_request(request)
                    && *code != AttachmentPreviewCustodyDelegationRejectCodeV1::Unspecified
            }
        }
}

fn valid_request(value: &RequestAttachmentPreviewCustodyDelegationV1) -> bool {
    [
        value.request_id.as_slice(),
        value.preview_run_id.as_slice(),
...
mod tests {
    ...
    fn request() -> RequestAttachmentPreviewCustodyDelegationV1 {
        RequestAttachmentPreviewCustodyDelegationV1 {
            request_id: vec![1; 16],
