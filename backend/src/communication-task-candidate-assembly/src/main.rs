...
use makosh_communication_task_candidate_assembly::materialize_communication_task_candidate_release_assembly_v1;
...
fn main() -> ExitCode {
    match arguments(std::env::args().skip(1).collect()) {
        Some(arguments) => {
            if materialize_communication_task_candidate_release_assembly_v1(
            ...
            println!("communication-task-candidate-release-assembly: ok");
            ExitCode::SUCCESS
        }
        None => fail(
            "usage: makosh-communication-task-candidate-assembly --build-id <id> \
...
fn fail(message: &str) -> ExitCode {
    eprintln!("communication-task-candidate-release-assembly: {message}");
    ExitCode::FAILURE
}
...
mod tests {
    ...
    fn parses_exact_options() {
        let parsed = arguments(vec![
            "--runtime".to_owned(),
            "/tmp/makosh-communication-task-candidate-runtime".to_owned(),
            "--build-id".to_owned(),
            "build-1".to_owned(),
            "--output-dir".to_owned(),
            "/tmp/communication-task-candidate-assembly".to_owned(),
        ])
        .expect("arguments");
        assert_eq!(parsed.build_id, "build-1");
        assert_eq!(
            parsed.runtime,
            PathBuf::from("/tmp/makosh-communication-task-candidate-runtime")
        );
    }
