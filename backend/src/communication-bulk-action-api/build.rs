...
fn main() {
    ...
    let descriptor = output.join("communication-bulk-action-v1.bin");
    ...
    std::fs::write(
        output.join("communication_bulk_action_schema.rs"),
        format!("pub const COMMUNICATION_BULK_ACTION_SCHEMA_SHA256: [u8; 32] = {digest:?};\n"),
    )
    .expect("Communication bulk action schema digest must be written");
}
