...
pub const PACKAGE: &str = "makosh-risk-assembly";
pub const RISK_FRAGMENT_FILE_V1: &str = "risk.release-artifacts.json";
...
pub fn materialize_risk_release_assembly_v1(
    output: &Path,
    build: &str,
    runtime: &Path,
    ...
    let descriptor = risk_module_descriptor_v1(build);
    let settings_bytes = risk_settings_schema_bytes_v1();
    let settings =
        decode_settings_schema_v1(&settings_bytes).map_err(|_| AssemblyErrorV1::InvalidArtifact)?;
    let storage = risk_storage_bundle_v1();
    ...
    let paths = PathsV1 {
        descriptor: output.join("risk.runtime.descriptor.pb"),
        settings_schema: output.join("risk.runtime.settings.pb"),
        storage_bundle: output.join("risk.storage.bundle.pb"),
        artifact_fragment: output.join(RISK_FRAGMENT_FILE_V1),
    };
    let utf8 = |path: &Path| {
        path.to_str()
            .map(str::to_owned)
    ...
    let fragment = FragmentV1 {
        ...
        artifacts: vec![
            ArtifactV1::Runtime(RuntimeInputV1 {
                ...
                artifact_id: "risk.runtime.v1".into(),
                relative_path: "bin/makosh-risk-runtime".into(),
                source_path: utf8(runtime)?,
                required: true,
                descriptor: ContractInputV1 {
                    relative_path: "contracts/risk.runtime.descriptor.pb".into(),
                    source_path: utf8(&paths.descriptor)?,
                },
                settings_schema: ContractInputV1 {
                    relative_path: "contracts/risk.runtime.settings.pb".into(),
                    source_path: utf8(&paths.settings_schema)?,
                },
            }),
            ArtifactV1::Storage(StorageInputV1 {
                artifact_kind: "storage_bundle".into(),
                artifact_id: "risk.storage.v1".into(),
                relative_path: "storage/risk.storage.bundle.pb".into(),
...
mod tests {
    ...
    fn materializes_compiler_shape() {
        let root = std::env::temp_dir().join(format!("makosh-risk-{}", std::process::id()));
        let _ = fs::remove_dir_all(&root);
        fs::create_dir(&root).unwrap();
        let runtime = root.join("runtime");
        fs::write(&runtime, b"runtime").unwrap();
        let paths =
            materialize_risk_release_assembly_v1(&root.join("out"), "build", &runtime).unwrap();
        let fragment: FragmentV1 =
