...
use makosh_zulip_api::ZulipCommandV1;
use makosh_zulip_delivery_intent_contract::ZULIP_DELIVERY_INTENT_TARGET_BLOB_CAPABILITY_ID_V1;
use makosh_zulip_persistence::{
    ClaimedZulipDeliveryIntentJobV1, ZulipDeliveryIntentJobStateV1, ZulipDeliveryIntentJobV1,
    ZulipDurablePersistence,
};
...
pub enum ZulipDeliveryIntentExecutionErrorV1 {
    InvalidJob,
    InvalidBody,
    ControlChannel,
    CustodyDenied,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct ZulipDeliveryIntentTargetBodyReceiptV1 {
    pub reference_id: [u8; 16],
    pub receipt_sha256: [u8; 32],
}

pub fn transfer_zulip_delivery_intent_body_v1(
    control_channel: &mut ManagedControlChannelV2<UnixStream>,
    claimed: &ClaimedZulipDeliveryIntentJobV1,
) -> Result<ZulipDeliveryIntentTargetBodyReceiptV1, ZulipDeliveryIntentExecutionErrorV1> {
    if claimed.state != ZulipDeliveryIntentJobStateV1::PendingCustody
        || claimed.target_body_reference_id.is_some()
        || claimed.target_body_receipt_sha256.is_some()
    {
        return Err(ZulipDeliveryIntentExecutionErrorV1::InvalidJob);
    }
    with_blocking_control_channel(control_channel, |channel| {
        ...
        .map_err(|_| ZulipDeliveryIntentExecutionErrorV1::CustodyDenied)?;
        ...
        BlobDataClient::new(transfer.data_socket_path)
            ...
            .map_err(|_| ZulipDeliveryIntentExecutionErrorV1::BlobUnavailable)?;
        Ok(ZulipDeliveryIntentTargetBodyReceiptV1 {
            reference_id,
            receipt_sha256: claimed.job.body_sha256,
        })
    })
...
pub fn read_zulip_delivery_intent_body_v1(
    control_channel: &mut ManagedControlChannelV2<UnixStream>,
    claimed: &ClaimedZulipDeliveryIntentJobV1,
) -> Result<Vec<u8>, ZulipDeliveryIntentExecutionErrorV1> {
    if claimed.state != ZulipDeliveryIntentJobStateV1::BodyReady {
        return Err(ZulipDeliveryIntentExecutionErrorV1::InvalidJob);
    }
    let reference_id = claimed
        .target_body_reference_id
        .ok_or(ZulipDeliveryIntentExecutionErrorV1::InvalidJob)?;
    let receipt_sha256 = claimed
        .target_body_receipt_sha256
        .ok_or(ZulipDeliveryIntentExecutionErrorV1::InvalidJob)?;
    if receipt_sha256 != claimed.job.body_sha256 {
        return Err(ZulipDeliveryIntentExecutionErrorV1::InvalidBody);
    }
    ...
    materialize_zulip_delivery_intent_v1(&claimed.job, &bytes)?;
    Ok(bytes)
}

pub fn materialize_zulip_delivery_intent_v1(
    job: &ZulipDeliveryIntentJobV1,
    body: &[u8],
) -> Result<ZulipCommandV1, ZulipDeliveryIntentExecutionErrorV1> {
    let body_len =
        u64::try_from(body.len()).map_err(|_| ZulipDeliveryIntentExecutionErrorV1::InvalidBody)?;
    ...
    let text = std::str::from_utf8(body)
        .map_err(|_| ZulipDeliveryIntentExecutionErrorV1::InvalidBody)?
        .to_owned();
    if let Some(route) = job.provider_chat_id.strip_prefix("stream:") {
        let (stream, topic) = route
            .split_once(':')
            .filter(|(stream, topic)| !stream.is_empty() && !topic.is_empty())
            .ok_or(ZulipDeliveryIntentExecutionErrorV1::InvalidJob)?;
        return Ok(ZulipCommandV1::SendStream {
...
pub async fn enqueue_zulip_delivery_intent_v1(
    durable: &ZulipDurablePersistence,
    job: &ZulipDeliveryIntentJobV1,
    body: &[u8],
    requested_at_unix_seconds: i64,
) -> Result<(), ZulipDeliveryIntentExecutionErrorV1> {
    if requested_at_unix_seconds <= 0 {
        return Err(ZulipDeliveryIntentExecutionErrorV1::InvalidJob);
    }
    let command = materialize_zulip_delivery_intent_v1(job, body)?;
    match crate::submit_command(durable, &command, requested_at_unix_seconds).await {
        Ok(operation) if operation.operation_id == job.provider_operation_id => Ok(()),
        Err(ZulipRuntimeErrorV1::OperationAlreadyKnown) => durable
            .command_operation_status(&job.provider_operation_id)
            .await
            .map_err(|_| ZulipDeliveryIntentExecutionErrorV1::QueueUnavailable)?
            .is_some()
            .then_some(())
            .ok_or(ZulipDeliveryIntentExecutionErrorV1::QueueUnavailable),
        _ => Err(ZulipDeliveryIntentExecutionErrorV1::QueueUnavailable),
    }
}
...
fn with_blocking_control_channel<T>(
    ...
    ) -> Result<T, ZulipDeliveryIntentExecutionErrorV1>,
) -> Result<T, ZulipDeliveryIntentExecutionErrorV1> {
...
fn prepare_blocking_control_channel(
    ...
    channel
        ...
        .map_err(|_| ZulipDeliveryIntentExecutionErrorV1::ControlChannel)
}
...
mod tests {
    ...
    fn job(body: &[u8]) -> ZulipDeliveryIntentJobV1 {
        ZulipDeliveryIntentJobV1 {
            intent_id: [1; 16],
            command_message_id: [2; 16],
            command_envelope_sha256: [5; 32],
            logical_owner_id: "owner-1".to_owned(),
            account_id: "zulip-account".to_owned(),
            provider_chat_id: "stream:42:delivery".to_owned(),
    ...
    #[test]
    fn materializes_exact_provider_route_into_existing_zulip_queue_contract() {
        let body = b"Reply body";
        let request = materialize_zulip_delivery_intent_v1(&job(body), body).expect("request");
    ...
    fn materializes_a_non_reply_as_the_existing_send_text_command() {
        ...
        job.provider_chat_id = "direct:7".to_owned();

        assert_eq!(
            materialize_zulip_delivery_intent_v1(&job, body),
            Ok(ZulipCommandV1::SendDirect {
                operation_id: job.provider_operation_id,
                account_id: "zulip-account".to_owned(),
                recipients: vec!["7".to_owned()],
                content: "New message".to_owned(),
            })
        );
    }
    ...
    fn rejects_body_bytes_that_do_not_match_the_admitted_receipt() {
        ...
        assert_eq!(
            materialize_zulip_delivery_intent_v1(&admitted, b"different"),
            Err(ZulipDeliveryIntentExecutionErrorV1::InvalidBody),
        );
    }
    ...
    fn rejects_non_utf8_body_instead_of_reinterpreting_provider_content() {
        ...
        assert_eq!(
            materialize_zulip_delivery_intent_v1(&job(&body), &body),
            Err(ZulipDeliveryIntentExecutionErrorV1::InvalidBody),
        );
    }
}
