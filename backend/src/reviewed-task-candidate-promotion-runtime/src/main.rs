...
fn main() -> Result<(), String> {
    ...
    match command.to_str() {
        Some("export-storage-bundle") => export(
            &mut arguments,
            reviewed_task_candidate_promotion_storage_bundle_v1().encode_to_vec(),
        ),
        Some("export-module-descriptor") => {
            let build_id = required_string(&mut arguments, "build id")?;
            export(
                &mut arguments,
                reviewed_task_candidate_promotion_module_descriptor_v1(&build_id).encode_to_vec(),
            )
        }
    ...
    I: Iterator<Item = OsString>,
{
    ...
    let admission = ReviewedTaskCandidatePromotionRuntimeAdmissionV1 {
    ...
    let mut runtime = executor
        .block_on(ReviewedTaskCandidatePromotionManagedRuntimeV1::open(
    ...
    loop {
        ...
        retry(executor.block_on(runtime.consume_task_created_once(now)))?;
        retry(executor.block_on(runtime.consume_task_rejected_once(now)))?;
...
fn retry(
    result: Result<bool, ReviewedTaskCandidatePromotionManagedRuntimeErrorV1>,
) -> Result<(), String> {
    match result {
        Ok(_)
...
fn runtime_error(error: ReviewedTaskCandidatePromotionManagedRuntimeErrorV1) -> String {
    let code = match error {
        ReviewedTaskCandidatePromotionManagedRuntimeErrorV1::Admission => "admission",
        ReviewedTaskCandidatePromotionManagedRuntimeErrorV1::EventContract => "event_contract",
        ReviewedTaskCandidatePromotionManagedRuntimeErrorV1::EventUnavailable => {
            "event_unavailable"
        }
        ReviewedTaskCandidatePromotionManagedRuntimeErrorV1::Persistence(_) => "persistence",
        ReviewedTaskCandidatePromotionManagedRuntimeErrorV1::Unavailable => "unavailable",
    };
