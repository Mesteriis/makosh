...
use makosh_tasks_command_api::{
    ...
    task_creation_from_reviewed_candidate_rejected_consume_request_v1,
    task_creation_from_reviewed_candidate_rejected_contract_reference_v1,
};
use prost::Message;
use sha2::{Digest, Sha256};

pub const REVIEWED_TASK_CANDIDATE_PROMOTION_STORAGE_CAPABILITY_ID_V1: &str =
    "reviewed_task_candidate_promotion.storage.v1";
const STORAGE_CONNECTION_BUDGET_V1: u32 = 4;

#[must_use]
pub fn reviewed_task_candidate_promotion_settings_schema_v1() -> SettingsSchemaV1 {
    SettingsSchemaV1 {
        major: 1,
        revision: 1,
        definitions: Vec::new(),
...
#[must_use]
pub fn reviewed_task_candidate_promotion_settings_schema_bytes_v1() -> Vec<u8> {
    reviewed_task_candidate_promotion_settings_schema_v1().encode_to_vec()
}

#[must_use]
pub fn reviewed_task_candidate_promotion_module_descriptor_v1(
    build_id: &str,
) -> ModuleDescriptorV1 {
    let settings = reviewed_task_candidate_promotion_settings_schema_bytes_v1();
    ModuleDescriptorV1 {
        descriptor_major: 1,
        descriptor_revision: 1,
        module_id: REVIEWED_TASK_CANDIDATE_PROMOTION_MODULE_ID_V1.to_owned(),
        owner_id: REVIEWED_TASK_CANDIDATE_PROMOTION_OWNER_V1.to_owned(),
        ...
        display_name: "Reviewed Task Candidate Promotion".to_owned(),
    }
}
...
fn storage_capability() -> CapabilityDescriptorV1 {
    CapabilityDescriptorV1 {
        capability_id: REVIEWED_TASK_CANDIDATE_PROMOTION_STORAGE_CAPABILITY_ID_V1.to_owned(),
        capability_revision: 1,
        criticality: CapabilityCriticalityV1::Required as i32,
...
mod tests {
    ...
    fn descriptor_is_event_storage_only_workflow() {
        let descriptor = reviewed_task_candidate_promotion_module_descriptor_v1("build-1");
        validate_descriptor_v1(&descriptor).expect("descriptor");
        validate_settings_schema_v1(&reviewed_task_candidate_promotion_settings_schema_v1())
            .expect("settings");
