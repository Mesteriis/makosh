//! Unsigned Speech-to-Text engine release assembly.
...
pub const SPEECH_TO_TEXT_ASSEMBLY_FRAGMENT_VERSION_V1: u32 = 1;
pub const SPEECH_TO_TEXT_RUNTIME_ARTIFACT_ID_V1: &str = "speech_to_text.runtime.v1";
pub const SPEECH_TO_TEXT_STORAGE_ARTIFACT_ID_V1: &str = "speech_to_text.storage.v1";
pub const SPEECH_TO_TEXT_DESCRIPTOR_FILE_V1: &str = "speech-to-text.runtime.descriptor.pb";
pub const SPEECH_TO_TEXT_SETTINGS_FILE_V1: &str = "speech-to-text.runtime.settings.pb";
...
#[derive(Clone, Debug, Eq, PartialEq, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
pub struct SpeechToTextReleaseArtifactFragmentV1 {
    pub version: u32,
    pub owner_id: String,
    pub module_id: String,
    pub artifacts: Vec<SpeechToTextReleaseArtifactInputV1>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct SpeechToTextReleaseAssemblyPathsV1 {
...
pub enum SpeechToTextReleaseAssemblyErrorV1 {
    InvalidInput,
...
pub fn materialize_speech_to_text_release_assembly_v1(
    output_directory: &Path,
    build_id: &str,
    runtime_source: &Path,
) -> Result<SpeechToTextReleaseAssemblyPathsV1, SpeechToTextReleaseAssemblyErrorV1> {
    validate_inputs(output_directory, build_id, runtime_source)?;
    let descriptor = speech_to_text_module_descriptor_v1(build_id);
    let settings = speech_to_text_settings_schema_v1();
    let storage = speech_to_text_storage_bundle_v1();
    ...
    {
        return Err(SpeechToTextReleaseAssemblyErrorV1::InvalidCanonicalArtifact);
    }
    let paths = SpeechToTextReleaseAssemblyPathsV1 {
        descriptor: output_directory.join(SPEECH_TO_TEXT_DESCRIPTOR_FILE_V1),
        settings_schema: output_directory.join(SPEECH_TO_TEXT_SETTINGS_FILE_V1),
        storage_bundle: output_directory.join(SPEECH_TO_TEXT_STORAGE_BUNDLE_FILE_V1),
        artifact_fragment: output_directory.join(SPEECH_TO_TEXT_ARTIFACT_FRAGMENT_FILE_V1),
    };
    ...
    let fragment_bytes = serde_json::to_vec_pretty(&fragment)
        .map_err(|_| SpeechToTextReleaseAssemblyErrorV1::FragmentEncodingFailed)?;
    let mut directory = DirBuilder::new();
    directory.mode(0o700);
    directory
        .create(output_directory)
        .map_err(|_| SpeechToTextReleaseAssemblyErrorV1::OutputUnavailable)?;
    let writes = [
    ...
    for (path, bytes) in writes {
        if write_new_private_file(path, &bytes).is_err() {
            ...
            return Err(SpeechToTextReleaseAssemblyErrorV1::ArtifactWriteFailed);
        }
    }
    Ok(paths)
}
...
fn validate_inputs(
    ...
    build_id: &str,
    runtime_source: &Path,
) -> Result<(), SpeechToTextReleaseAssemblyErrorV1> {
    ...
    if !output_directory.is_absolute()
        ...
        || !valid_runtime
    {
        return Err(SpeechToTextReleaseAssemblyErrorV1::InvalidInput);
    }
    Ok(())
}
...
fn artifact_fragment(
    ...
    module_id: &str,
    runtime_source: &Path,
    paths: &SpeechToTextReleaseAssemblyPathsV1,
) -> Result<SpeechToTextReleaseArtifactFragmentV1, SpeechToTextReleaseAssemblyErrorV1> {
    let artifacts = vec![
        SpeechToTextReleaseArtifactInputV1::ModuleRuntime(ModuleRuntimeArtifactInputV1 {
            artifact_kind: "module_runtime".to_owned(),
            artifact_id: SPEECH_TO_TEXT_RUNTIME_ARTIFACT_ID_V1.to_owned(),
        ...
        SpeechToTextReleaseArtifactInputV1::StorageBundle(StorageBundleArtifactInputV1 {
            artifact_kind: "storage_bundle".to_owned(),
            artifact_id: SPEECH_TO_TEXT_STORAGE_ARTIFACT_ID_V1.to_owned(),
    ...
    Ok(SpeechToTextReleaseArtifactFragmentV1 {
        version: SPEECH_TO_TEXT_ASSEMBLY_FRAGMENT_VERSION_V1,
...
fn utf8_path(path: &Path) -> Result<String, SpeechToTextReleaseAssemblyErrorV1> {
    path.to_str()
        .filter(|value| !value.is_empty())
        .map(str::to_owned)
        .ok_or(SpeechToTextReleaseAssemblyErrorV1::InvalidInput)
}
...
mod tests {
    ...
    #[test]
    fn materializes_exact_unsigned_speech_engine_artifacts() {
        let root = temporary_directory();
        let runtime = root.join("runtime");
        fs::write(&runtime, b"runtime").expect("runtime fixture");
        let output = root.join("assembly");
        let paths = materialize_speech_to_text_release_assembly_v1(&output, "build-1", &runtime)
            .expect("assembly");
        let descriptor = decode_descriptor_v1(&fs::read(paths.descriptor).expect("descriptor"))
            .expect("descriptor");
        let fragment: SpeechToTextReleaseArtifactFragmentV1 =
