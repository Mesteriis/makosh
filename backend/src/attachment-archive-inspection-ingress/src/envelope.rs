...
pub struct ArchiveInspectionCustodyEnvelopeContextV1 {
...
pub enum ArchiveInspectionCustodyEnvelopeBuildErrorV1 {
    InvalidContext,
    InvalidPayload,
    InvalidEnvelope,
    OutboxRejected,
...
pub fn build_request_archive_inspection_custody_delegation_outbox_record_v1(
    payload: RequestArchiveInspectionCustodyDelegationV1,
    ...
    let archive_run_id = id16(&payload.archive_run_id)?;
    if deadline_unix_seconds <= context.recorded_at_unix_seconds {
        return Err(ArchiveInspectionCustodyEnvelopeBuildErrorV1::InvalidPayload);
    }
    build_envelope(
        request_id,
        &archive_run_id,
        ...
        Semantics::Command(CommandMetadataV1 {
            ...
            target_capability: ATTACHMENT_SECURITY_ARCHIVE_DELEGATION_CAPABILITY_ID_V1.to_owned(),
            idempotency_key: Sha256::digest(
                [
                    b"archive-inspection-custody-delegation-v1".as_slice(),
                    &request_id,
                ]
                .concat(),
            )
...
pub fn build_archive_inspection_custody_delegated_outbox_record_v1(
    command_message_id: [u8; 16],
    ...
    build_result(
        ResultEnvelopeV1 {
            command_message_id,
            request_id,
            archive_run_id: id16(&payload.archive_run_id)?,
            label: b"delegated",
            contract_name: ARCHIVE_INSPECTION_CUSTODY_DELEGATED_CONTRACT_NAME_V1,
...
pub fn build_archive_inspection_custody_delegation_rejected_outbox_record_v1(
    command_message_id: [u8; 16],
    ...
    build_result(
        ResultEnvelopeV1 {
            ...
            archive_run_id,
            label: b"rejected",
            contract_name: ARCHIVE_INSPECTION_CUSTODY_DELEGATION_REJECTED_CONTRACT_NAME_V1,
...
fn build_envelope(
    ...
    validate_envelope_v1(&envelope)
        .map_err(|_| ArchiveInspectionCustodyEnvelopeBuildErrorV1::InvalidEnvelope)?;
    OutboxRecordV1::accept(envelope.encode_to_vec()).map_err(outbox_error)
}

fn validate_request(
    payload: &RequestArchiveInspectionCustodyDelegationV1,
) -> Result<[u8; 16], ArchiveInspectionCustodyEnvelopeBuildErrorV1> {
    let request_id = id16(&payload.request_id)?;
    id16(&payload.archive_run_id)?;
    id16(&payload.attachment_anchor_id)?;
...
fn validate_delegated(
    ...
    if !(1..=ATTACHMENT_ARCHIVE_INSPECTION_MAX_SOURCE_BYTES_V1).contains(&payload.declared_size)
...
fn validate_context(
    context: &ArchiveInspectionCustodyEnvelopeContextV1,
) -> Result<(), ArchiveInspectionCustodyEnvelopeBuildErrorV1> {
    ...
    {
        return Err(ArchiveInspectionCustodyEnvelopeBuildErrorV1::InvalidContext);
    }
    Ok(())
}

fn id16(bytes: &[u8]) -> Result<[u8; 16], ArchiveInspectionCustodyEnvelopeBuildErrorV1> {
    bytes
        .try_into()
        .ok()
        .filter(valid_id)
        .ok_or(ArchiveInspectionCustodyEnvelopeBuildErrorV1::InvalidPayload)
}

fn sha256(bytes: &[u8]) -> Result<[u8; 32], ArchiveInspectionCustodyEnvelopeBuildErrorV1> {
    bytes
        .try_into()
        .ok()
...
fn timestamp(context: &ArchiveInspectionCustodyEnvelopeContextV1) -> Timestamp {
    Timestamp {
...
fn outbox_error(_: OutboxRecordError) -> ArchiveInspectionCustodyEnvelopeBuildErrorV1 {
    ArchiveInspectionCustodyEnvelopeBuildErrorV1::OutboxRejected
}
...
mod tests {
    ...
    fn context(module_id: &str) -> ArchiveInspectionCustodyEnvelopeContextV1 {
    ...
    fn command_is_bounded_and_targets_exact_source_capability() {
        let record = build_request_archive_inspection_custody_delegation_outbox_record_v1(
            request(),
            1_800_000_030,
            &context("makosh-attachment-archive-inspection-runtime"),
        )
    ...
    fn delegated_result_is_command_linked_and_proof_bounded() {
        let payload = ArchiveInspectionCustodyDelegatedV1 {
            request_id: vec![1; 16],
            archive_run_id: vec![2; 16],
            attachment_anchor_id: vec![3; 16],
            candidate_message_id: vec![4; 16],
        ...
        let record = build_archive_inspection_custody_delegated_outbox_record_v1(
            [1; 16],
            payload,
    ...
    fn private_result_rejects_empty_or_oversized_proof() {
        let mut payload = ArchiveInspectionCustodyDelegatedV1 {
            request_id: vec![1; 16],
            archive_run_id: vec![2; 16],
            attachment_anchor_id: vec![3; 16],
            candidate_message_id: vec![4; 16],
        ...
        assert_eq!(
            ...
            Err(ArchiveInspectionCustodyEnvelopeBuildErrorV1::InvalidPayload)
        );
        payload.custody_transfer_source_proof =
            vec![10; ATTACHMENT_ARCHIVE_INSPECTION_MAX_PROOF_BYTES_V1 + 1];
        assert_eq!(
            build_archive_inspection_custody_delegated_outbox_record_v1(
                [1; 16],
                payload,
