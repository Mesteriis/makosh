CREATE TABLE makosh_data.tasks_reviewed_candidate_inbox (
    ...
    CHECK (
        (NOT completed AND NOT rejected AND task_id IS NULL AND completed_at_unix_millis IS NULL)
        OR (completed AND NOT rejected AND length(task_id) = 16
            AND completed_at_unix_millis >= received_at_unix_millis)
        OR (completed AND rejected AND task_id IS NULL
            AND completed_at_unix_millis >= received_at_unix_millis)
    )
);
...
CREATE TABLE makosh_data.tasks_state (
    ...
    PRIMARY KEY (logical_owner_id, task_id),
    UNIQUE (logical_owner_id, approved_candidate_id),
    CHECK (length(logical_owner_id) BETWEEN 1 AND 128),
