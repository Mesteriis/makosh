...
fn main() {
    ...
    let descriptor = output.join("mail-contacts-sync-v1.bin");
    ...
    std::fs::write(
        output.join("mail_contacts_sync_schema.rs"),
        format!("pub const MAIL_CONTACTS_SYNC_SCHEMA_SHA256_V1: [u8; 32] = {digest:?};\n"),
    )
    .expect("Mail Contacts sync schema digest must be written");
}
