//! Communication Recipient Suggestion workflow release assembly.
//!
...
pub const COMMUNICATION_RECIPIENT_SUGGESTION_ASSEMBLY_FRAGMENT_VERSION_V1: u32 = 1;
...
pub const COMMUNICATION_RECIPIENT_SUGGESTION_DESCRIPTOR_FILE: &str =
    "communication_recipient_suggestion.runtime.descriptor.pb";
pub const COMMUNICATION_RECIPIENT_SUGGESTION_SETTINGS_FILE: &str =
    "communication_recipient_suggestion.runtime.settings.pb";
pub const COMMUNICATION_RECIPIENT_SUGGESTION_STORAGE_BUNDLE_FILE: &str =
    "communication_recipient_suggestion.storage.bundle.pb";
...
const SETTINGS_RELATIVE_PATH: &str =
    "contracts/communication_recipient_suggestion.runtime.settings.pb";
const STORAGE_RELATIVE_PATH: &str = "storage/communication_recipient_suggestion.storage.bundle.pb";
...
#[derive(Clone, Debug, Eq, PartialEq, Deserialize, Serialize)]
#[serde(untagged)]
pub enum CommunicationRecipientSuggestionReleaseArtifactInputV1 {
    ModuleRuntime(ModuleRuntimeArtifactInputV1),
    StorageBundle(StorageBundleArtifactInputV1),
}

impl CommunicationRecipientSuggestionReleaseArtifactInputV1 {
    #[must_use]
...
#[derive(Clone, Debug, Eq, PartialEq, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
pub struct CommunicationRecipientSuggestionReleaseArtifactFragmentV1 {
    pub version: u32,
    pub owner_id: String,
    pub module_id: String,
    pub artifacts: Vec<CommunicationRecipientSuggestionReleaseArtifactInputV1>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct CommunicationRecipientSuggestionReleaseAssemblyPathsV1 {
...
pub enum CommunicationRecipientSuggestionReleaseAssemblyErrorV1 {
    InvalidInput,
...
pub fn materialize_communication_recipient_suggestion_release_assembly_v1(
    output_directory: &Path,
    build_id: &str,
    runtime_source: &Path,
) -> Result<
    CommunicationRecipientSuggestionReleaseAssemblyPathsV1,
    CommunicationRecipientSuggestionReleaseAssemblyErrorV1,
> {
    validate_inputs(output_directory, build_id, runtime_source)?;
    let descriptor = communication_recipient_suggestion_module_descriptor_v1(build_id);
    let settings_schema = communication_recipient_suggestion_settings_schema_v1();
    let storage_bundle = communication_recipient_suggestion_storage_bundle_v1();
    ...
    {
        return Err(
            CommunicationRecipientSuggestionReleaseAssemblyErrorV1::InvalidCanonicalArtifact,
        );
    }

    let paths = CommunicationRecipientSuggestionReleaseAssemblyPathsV1 {
        descriptor: output_directory.join(COMMUNICATION_RECIPIENT_SUGGESTION_DESCRIPTOR_FILE),
        settings_schema: output_directory.join(COMMUNICATION_RECIPIENT_SUGGESTION_SETTINGS_FILE),
        storage_bundle: output_directory
            .join(COMMUNICATION_RECIPIENT_SUGGESTION_STORAGE_BUNDLE_FILE),
        artifact_fragment: output_directory
            .join(COMMUNICATION_RECIPIENT_SUGGESTION_ARTIFACT_FRAGMENT_FILE),
    };
    ...
    let writes = [
        ...
        (
            ...
            serde_json::to_vec_pretty(&fragment).map_err(|_| {
                CommunicationRecipientSuggestionReleaseAssemblyErrorV1::FragmentEncodingFailed
            })?,
        ),
    ];
    ...
    builder
        ...
        .map_err(|_| CommunicationRecipientSuggestionReleaseAssemblyErrorV1::OutputUnavailable)?;
    for (path, bytes) in &writes {
        if write_new_private_file(path, bytes).is_err() {
            let _ = fs::remove_dir_all(output_directory);
            return Err(
                CommunicationRecipientSuggestionReleaseAssemblyErrorV1::ArtifactWriteFailed,
            );
        }
    }
    Ok(paths)
...
fn validate_inputs(
    ...
    build_id: &str,
    runtime_source: &Path,
) -> Result<(), CommunicationRecipientSuggestionReleaseAssemblyErrorV1> {
    ...
    {
        return Err(CommunicationRecipientSuggestionReleaseAssemblyErrorV1::InvalidInput);
    }
    Ok(())
}
...
fn artifact_fragment(
    ...
    CommunicationRecipientSuggestionReleaseArtifactFragmentV1,
    CommunicationRecipientSuggestionReleaseAssemblyErrorV1,
> {
    let artifacts = vec![
        CommunicationRecipientSuggestionReleaseArtifactInputV1::ModuleRuntime(
            ModuleRuntimeArtifactInputV1 {
                artifact_kind: "module_runtime".to_owned(),
                artifact_id: COMMUNICATION_RECIPIENT_SUGGESTION_RUNTIME_ARTIFACT_ID.to_owned(),
        ...
        CommunicationRecipientSuggestionReleaseArtifactInputV1::StorageBundle(
            StorageBundleArtifactInputV1 {
                artifact_kind: "storage_bundle".to_owned(),
                artifact_id: COMMUNICATION_RECIPIENT_SUGGESTION_STORAGE_ARTIFACT_ID.to_owned(),
    ...
    Ok(CommunicationRecipientSuggestionReleaseArtifactFragmentV1 {
        version: COMMUNICATION_RECIPIENT_SUGGESTION_ASSEMBLY_FRAGMENT_VERSION_V1,
        owner_id: COMMUNICATION_RECIPIENT_SUGGESTION_ASSEMBLY_OWNER_ID.to_owned(),
        module_id: COMMUNICATION_RECIPIENT_SUGGESTION_ASSEMBLY_MODULE_ID.to_owned(),
        artifacts,
    })
}
...
fn utf8_path(
    path: &Path,
) -> Result<String, CommunicationRecipientSuggestionReleaseAssemblyErrorV1> {
    path.to_str()
        .filter(|value| !value.is_empty())
        .map(str::to_owned)
        .ok_or(CommunicationRecipientSuggestionReleaseAssemblyErrorV1::InvalidInput)
}
...
mod tests {
    ...
    fn materializes_exact_unsigned_runtime_and_storage_fragment() {
        ...
        let storage =
            StorageBundleV1::decode(fs::read(paths.storage_bundle).expect("storage").as_slice())
                .expect("storage");
        let fragment: CommunicationRecipientSuggestionReleaseArtifactFragmentV1 =
            serde_json::from_slice(&fs::read(paths.artifact_fragment).expect("fragment"))
                .expect("fragment");
        assert_eq!(
            descriptor.module_id,
            COMMUNICATION_RECIPIENT_SUGGESTION_ASSEMBLY_MODULE_ID
        );
        assert_eq!(
            storage.owner_id,
            COMMUNICATION_RECIPIENT_SUGGESTION_ASSEMBLY_OWNER_ID
        );
        assert_eq!(
            fragment
                .artifacts
                .iter()
                .map(CommunicationRecipientSuggestionReleaseArtifactInputV1::artifact_id)
                .collect::<Vec<_>>(),
            [
                COMMUNICATION_RECIPIENT_SUGGESTION_RUNTIME_ARTIFACT_ID,
                COMMUNICATION_RECIPIENT_SUGGESTION_STORAGE_ARTIFACT_ID,
            ]
        );
    ...
    fn rejects_missing_runtime_and_existing_output() {
        ...
        assert_eq!(
            materialize_communication_recipient_suggestion_release_assembly_v1(
                &output,
                "build-1",
                &root.join("missing"),
            ),
            Err(CommunicationRecipientSuggestionReleaseAssemblyErrorV1::InvalidInput)
        );
    ...
    fn temporary_directory() -> PathBuf {
        let root = std::env::current_dir()
            ...
            .join("target")
            .join(format!(
                "communication-recipient-suggestion-assembly-{}-{}",
                std::process::id(),
