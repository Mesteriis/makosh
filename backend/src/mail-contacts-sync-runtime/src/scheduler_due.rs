...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct MailContactsSyncDueRuntimeContextV1 {
    pub runtime_instance_id: [u8; 16],
    pub runtime_generation: u64,
    pub grant_epoch: u64,
    pub contract: MailContactsSyncDueContractV1,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct MailContactsSyncDueMessageV1 {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct DecodedMailContactsSyncDueCommandV1 {
    ...
    pub acceptance_receipt: MailContactsSyncDueMessageV1,
}
...
pub fn decode_mail_contacts_sync_due_command_v1(
    exact_bytes: &[u8],
    context: &MailContactsSyncDueRuntimeContextV1,
) -> Result<DecodedMailContactsSyncDueCommandV1, MailContactsSyncDueAdapterErrorV1> {
    validate_context(context)?;
    let envelope = DurableEnvelopeV1::decode(exact_bytes)
        .map_err(|_| MailContactsSyncDueAdapterErrorV1::InvalidEnvelope)?;
    validate_envelope_v1(&envelope)
        .map_err(|_| MailContactsSyncDueAdapterErrorV1::InvalidEnvelope)?;
    if envelope.contract.as_ref() != Some(&job_contract(&context.contract)) {
        return Err(MailContactsSyncDueAdapterErrorV1::WrongContract);
    }
    let source = envelope
        .source
        .as_ref()
        .ok_or(MailContactsSyncDueAdapterErrorV1::WrongSource)?;
    let actor = envelope
        .actor
        .as_ref()
        .ok_or(MailContactsSyncDueAdapterErrorV1::WrongSource)?;
    ...
    {
        return Err(MailContactsSyncDueAdapterErrorV1::WrongSource);
    }
    let metadata = match envelope.semantics.as_ref() {
        ...
        _ => return Err(MailContactsSyncDueAdapterErrorV1::WrongCommand),
    };
    let command = ScheduledJobCommandV1::decode(envelope.payload.as_slice())
        .map_err(|_| MailContactsSyncDueAdapterErrorV1::WrongCommand)?;
    validate_scheduled_job_command_v1(&command)
        .map_err(|_| MailContactsSyncDueAdapterErrorV1::WrongCommand)?;
    let job_kind = command
        .job_kind
        .as_ref()
        .ok_or(MailContactsSyncDueAdapterErrorV1::WrongCommand)?;
    if job_kind.owner != MAIL_CONTACTS_SYNC_OWNER_ID_V1
        || job_kind.name != JOB_NAME_V1
        || job_kind.major != JOB_MAJOR_V1
        || command.trigger_kind != JobTriggerKindV1::Time as i32
        || !valid_configuration_instance_id(&command.scope_id)
    {
        return Err(MailContactsSyncDueAdapterErrorV1::WrongCommand);
    }
    if envelope.partition_key != command.scope_id.as_bytes() {
        return Err(MailContactsSyncDueAdapterErrorV1::WrongCorrelation);
    }
    let schedule_id = id16(&command.schedule_id)?;
    ...
    let expected_attempt =
        u32::try_from(lease.epoch).map_err(|_| MailContactsSyncDueAdapterErrorV1::WrongCommand)?;
    if lease.run_id.as_slice() != run_id
        ...
        || timestamp_millis(
            envelope
                .recorded_at
                .as_ref()
                .ok_or(MailContactsSyncDueAdapterErrorV1::InvalidTime)?,
    ...
    Ok(DecodedMailContactsSyncDueCommandV1 {
        configuration_instance_id: command.scope_id,
        schedule_id,
        command_message_id,
...
pub fn build_mail_contacts_sync_terminal_receipt_v1(
    due: &DecodedMailContactsSyncDueCommandV1,
...
fn receipt_envelope(
    ...
    let receipt = JobRunReceiptV1 {
        ...
        lease: Some(JobLeaseV1 {
            ...
            expires_at_unix_millis: i64::try_from(binding.lease_expires_at_unix_millis)
                .map_err(|_| MailContactsSyncDueAdapterErrorV1::InvalidTime)?,
        }),
    ...
    let (domain, kind, semantics) = match outcome {
        ...
        JobRunOutcomeV1::Succeeded | JobRunOutcomeV1::Failed => (
            ...
            Semantics::Result(ResultMetadataV1 {
                ...
                command_message_id: binding.command_message_id.to_vec(),
                outcome: if outcome == JobRunOutcomeV1::Succeeded {
                    ResultOutcomeV1::Succeeded as i32
                } else {
                    ResultOutcomeV1::Failed as i32
                },
    ...
    let envelope = DurableEnvelopeV1 {
        ...
        actor: Some(ActorRefV1 {
            ...
            actor_id: MAIL_CONTACTS_SYNC_MODULE_ID_V1.as_bytes().to_vec(),
        }),
        trace: None,
        source_fence: Some(SourceFenceV1 {
            kind: FenceKindV1::GrantEpoch as i32,
            scope_id: MAIL_CONTACTS_SYNC_MODULE_ID_V1.as_bytes().to_vec(),
    ...
    Ok(MailContactsSyncDueMessageV1 {
        message_id,
        contract_kind: kind,
...
fn validate_context(
    context: &MailContactsSyncDueRuntimeContextV1,
) -> Result<(), MailContactsSyncDueAdapterErrorV1> {
    ...
    {
        return Err(MailContactsSyncDueAdapterErrorV1::InvalidContext);
    }
    Ok(())
}
...
fn job_contract(contract: &MailContactsSyncDueContractV1) -> ContractRefV1 {
    ContractRefV1 {
...
fn receipt_contract(contract: &MailContactsSyncDueContractV1) -> ContractRefV1 {
    ContractRefV1 {
...
fn id16(value: &[u8]) -> Result<[u8; 16], MailContactsSyncDueAdapterErrorV1> {
    let value: [u8; 16] = value
        .try_into()
        .map_err(|_| MailContactsSyncDueAdapterErrorV1::WrongCorrelation)?;
    value
        .iter()
        .any(|byte| *byte != 0)
        .then_some(value)
        .ok_or(MailContactsSyncDueAdapterErrorV1::WrongCorrelation)
}

fn positive_millis(value: i64) -> Result<u64, MailContactsSyncDueAdapterErrorV1> {
    u64::try_from(value)
        .ok()
...
fn millis_timestamp(value: u64) -> Result<Timestamp, MailContactsSyncDueAdapterErrorV1> {
    Ok(Timestamp {
        seconds: i64::try_from(value / 1_000)
            .map_err(|_| MailContactsSyncDueAdapterErrorV1::InvalidTime)?,
...
fn timestamp_millis(value: &Timestamp) -> Result<u64, MailContactsSyncDueAdapterErrorV1> {
    if value.seconds < 0
...
mod tests {
    ...
    fn due_envelope() -> Vec<u8> {
        ...
        let command = ScheduledJobCommandV1 {
            ...
            schedule_id: vec![7; 16],
            schedule_revision: 1,
            scope_id: "mail-sync-account-1".to_owned(),
        ...
        DurableEnvelopeV1 {
            ...
            partition_key: b"mail-sync-account-1".to_vec(),
    ...
    #[test]
    fn due_scope_is_configuration_instance_and_receipts_are_stable() {
        let due = decode_mail_contacts_sync_due_command_v1(&due_envelope(), &context())
            .expect("valid due");
        assert_eq!(due.configuration_instance_id, "mail-sync-account-1");
        let duplicate = decode_mail_contacts_sync_due_command_v1(&due_envelope(), &context())
            .expect("duplicate");
        assert_eq!(due, duplicate);
        let terminal = build_mail_contacts_sync_terminal_receipt_v1(
            &due,
    ...
    #[test]
    fn rejects_account_or_provider_data_as_scope() {
        let mut envelope = DurableEnvelopeV1::decode(due_envelope().as_slice()).expect("decode");
        let command = ScheduledJobCommandV1::decode(envelope.payload.as_slice()).expect("command");
        let mut command = command;
        command.scope_id = "google/account/secret".to_owned();
        envelope.partition_key = command.scope_id.as_bytes().to_vec();
        envelope.payload = command.encode_to_vec();
        assert_eq!(
            decode_mail_contacts_sync_due_command_v1(&envelope.encode_to_vec(), &context()),
            Err(MailContactsSyncDueAdapterErrorV1::WrongCommand)
        );
    }
}
