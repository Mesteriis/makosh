...
use crate::{
    ...
    UnpublishedCommunicationNoteCandidateEventV1,
    model::{
        COMMUNICATION_NOTE_CANDIDATE_MAX_EVENT_BYTES_V1,
        COMMUNICATION_NOTE_CANDIDATE_OUTBOX_LIMIT_V1, nonzero, valid_identity, valid_timestamp,
    },
};
...
impl CommunicationNoteCandidatePersistenceV1 {
    ...
    > {
        if !valid_identity(logical_owner_id)
            || !(1..=COMMUNICATION_NOTE_CANDIDATE_OUTBOX_LIMIT_V1).contains(&limit)
        {
            return Err(CommunicationNoteCandidatePersistenceErrorV1::InvalidInput);
        }
        sqlx::query(
            "SELECT message_id, envelope_sha256, envelope_bytes
             FROM makosh_data.communication_note_candidate_extraction_outbox
        ...
        .map(|row| {
            ...
            let envelope_bytes: Vec<u8> = row
                ...
                .map_err(|_| CommunicationNoteCandidatePersistenceErrorV1::InvalidRow)?;
            let event = UnpublishedCommunicationNoteCandidateEventV1 {
    ...
    pub async fn mark_event_published(
        ...
        let updated = sqlx::query(
            "UPDATE makosh_data.communication_note_candidate_extraction_outbox
