...
use crate::{
    ...
    model::{COMMUNICATION_NOTE_CANDIDATE_REALTIME_LIMIT_V1, valid_identity, valid_timestamp},
    repository::state_from_code,
};

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct CommunicationNoteCandidateRealtimeTransitionV1 {
    pub sequence: u64,
    pub run_id: [u8; 16],
...
impl crate::CommunicationNoteCandidatePersistenceV1 {
    pub async fn client_realtime_window(
        ...
        Vec<CommunicationNoteCandidateRealtimeTransitionV1>,
        CommunicationNoteCandidatePersistenceErrorV1,
    > {
        if !valid_identity(logical_owner_id)
            || after_sequence == Some(0)
            || !(1..=COMMUNICATION_NOTE_CANDIDATE_REALTIME_LIMIT_V1).contains(&limit)
        {
        ...
        let rows = if let Some(after) = after_sequence {
            ...
            sqlx::query(
                "SELECT realtime_sequence, run_id, state, state_revision,
                 ...
                 FROM makosh_data.communication_note_candidate_extraction_realtime
...
pub(crate) async fn insert_realtime_transition(
    ...
    let inserted = sqlx::query(
        "INSERT INTO makosh_data.communication_note_candidate_extraction_realtime (
...
fn transition_from_row(
    ...
    CommunicationNoteCandidateRealtimeTransitionV1,
    CommunicationNoteCandidatePersistenceErrorV1,
> {
    ...
    Ok(CommunicationNoteCandidateRealtimeTransitionV1 {
...
fn rejection_from_code(
    value: i16,
) -> Result<CommunicationNoteCandidateRejectionCodeV1, CommunicationNoteCandidatePersistenceErrorV1>
{
    match value {
