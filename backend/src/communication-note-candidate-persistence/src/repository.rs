...
use crate::{
    model::{
        COMMUNICATION_NOTE_CANDIDATE_MAX_CUSTODY_PROOF_BYTES_V1,
        COMMUNICATION_NOTE_CANDIDATE_MAX_EVENT_BYTES_V1,
        COMMUNICATION_NOTE_CANDIDATE_MAX_SOURCE_READ_RECEIPT_BYTES_V1,
        COMMUNICATION_NOTE_CANDIDATE_RECOVERY_LIMIT_V1, CommunicationNoteCandidateBlobCleanupV1,
        CommunicationNoteCandidateInboxResultV1, CommunicationNoteCandidatePersistenceErrorV1,
        CommunicationNoteCandidateSourceResultV1, CreateCommunicationNoteCandidateOutcomeV1,
        CreateCommunicationNoteCandidateRunV1, PersistedCommunicationNoteCandidateRunV1,
        UnpublishedCommunicationNoteCandidateEventV1, decode_candidates, encode_candidates,
...
#[derive(Clone)]
pub struct CommunicationNoteCandidatePersistenceV1 {
    pub(crate) pool: PgPool,
}

impl CommunicationNoteCandidatePersistenceV1 {
    pub async fn connect_runtime(
        ...
        pgbouncer_port: u32,
        password: &str,
    ) -> Result<Self, CommunicationNoteCandidatePersistenceErrorV1> {
        ...
        {
            return Err(CommunicationNoteCandidatePersistenceErrorV1::StorageUnavailable);
        }
        let port = u16::try_from(pgbouncer_port)
            .map_err(|_| CommunicationNoteCandidatePersistenceErrorV1::StorageUnavailable)?;
    ...
    pub async fn create_run(
        &self,
        input: CreateCommunicationNoteCandidateRunV1,
    ) -> Result<
        CreateCommunicationNoteCandidateOutcomeV1,
    ...
    > {
        ...
        if persisted.request_fingerprint != fingerprint {
            return Err(CommunicationNoteCandidatePersistenceErrorV1::RequestConflict);
        }
        if inserted == 1 {
            Ok(CreateCommunicationNoteCandidateOutcomeV1::Created(
                persisted,
            ))
        } else {
            Ok(CreateCommunicationNoteCandidateOutcomeV1::Existing(
                persisted,
            ))
        }
    }
    ...
    > {
        ...
        let row = sqlx::query(SELECT_RUN)
            ...
            .await
            .map_err(storage_error)?
            .ok_or(CommunicationNoteCandidatePersistenceErrorV1::NotFound)?;
    ...
    pub async fn load_recoverable_runs(
        ...
        Vec<PersistedCommunicationNoteCandidateRunV1>,
        CommunicationNoteCandidatePersistenceErrorV1,
    > {
        ...
        sqlx::query(SELECT_RECOVERABLE_RUNS)
            .bind(logical_owner_id)
            .bind(i64::from(COMMUNICATION_NOTE_CANDIDATE_RECOVERY_LIMIT_V1))
            .fetch_all(&self.pool)
            .await
            .map_err(storage_error)?
            .into_iter()
    ...
    pub async fn persist_source_result(
        &self,
        input: CommunicationNoteCandidateSourceResultV1,
    ) -> Result<CommunicationNoteCandidateInboxResultV1, CommunicationNoteCandidatePersistenceErrorV1>
    {
        ...
        .map_err(storage_error)?
        {
            ...
            {
                return Err(CommunicationNoteCandidatePersistenceErrorV1::InboxConflict);
            }
            transaction.commit().await.map_err(storage_error)?;
            return self
                .load_run(&input.logical_owner_id, &input.run_id)
                .await
                .map(CommunicationNoteCandidateInboxResultV1::Duplicate);
        }
        let current =
            load_for_update(&mut transaction, &input.logical_owner_id, &input.run_id).await?;
        let next =
            transition_communication_note_candidate_v1(&current.status, input.transition.clone())
                .map_err(|_| CommunicationNoteCandidatePersistenceErrorV1::InvalidTransition)?;
        persist_status(
            &mut transaction,
            &input.logical_owner_id,
            &input.run_id,
        ...
        self.load_run(&input.logical_owner_id, &input.run_id)
            .await
            .map(CommunicationNoteCandidateInboxResultV1::Applied)
    }
    ...
    pub async fn persist_extraction_transition(
        ...
        review_submissions: &[UnpublishedCommunicationNoteCandidateEventV1],
    ...
    > {
        if !valid_identity(logical_owner_id)
            ...
            || !matches!(
                ...
                CommunicationNoteCandidateTransitionV1::Complete { .. }
                    | CommunicationNoteCandidateTransitionV1::Reject(_)
            )
        {
            return Err(CommunicationNoteCandidatePersistenceErrorV1::InvalidInput);
        }
        ...
        let next = transition_communication_note_candidate_v1(&current.status, transition)
            .map_err(|_| CommunicationNoteCandidatePersistenceErrorV1::InvalidTransition)?;
        persist_status(
            &mut transaction,
            logical_owner_id,
    ...
    > {
        ...
        let mut transaction = self.pool.begin().await.map_err(storage_error)?;
        let current = load_for_update(&mut transaction, logical_owner_id, run_id).await?;
        if current.status.state == CommunicationNoteCandidateStateV1::PreparingSource {
            transaction.commit().await.map_err(storage_error)?;
            return Ok(current);
        }
        let next = transition_communication_note_candidate_v1(
            &current.status,
    ...
    > {
        ...
        .rows_affected();
        if updated != 1 {
            return Err(CommunicationNoteCandidatePersistenceErrorV1::RevisionConflict);
        }
        self.load_run(logical_owner_id, run_id).await
    }
...
fn valid_review_submissions(events: &[UnpublishedCommunicationNoteCandidateEventV1]) -> bool {
    ...
    events.iter().all(|event| {
        nonzero(&event.message_id)
            ...
            && event.envelope_bytes.len() <= COMMUNICATION_NOTE_CANDIDATE_MAX_EVENT_BYTES_V1
...
async fn load_for_update(
    ...
    logical_owner_id: &str,
    run_id: &[u8; 16],
) -> Result<PersistedCommunicationNoteCandidateRunV1, CommunicationNoteCandidatePersistenceErrorV1>
{
...
async fn persist_status(
    ...
    next: &CommunicationNoteCandidateStatusV1,
    occurred_at_unix_millis: i64,
    materialization: Option<(&[u8], &CommunicationNoteCandidateBlobCleanupV1)>,
) -> Result<(), CommunicationNoteCandidatePersistenceErrorV1> {
    ...
    if updated == 1 {
        ...
        Err(CommunicationNoteCandidatePersistenceErrorV1::RevisionConflict)
    }
}
...
) -> Result<PersistedCommunicationNoteCandidateRunV1, CommunicationNoteCandidatePersistenceErrorV1>
{
    ...
    let source_cleanup = match (
        ...
        (Some(reference), Some(declared), Some(sha256), Some(custody_proof)) => {
            Some(CommunicationNoteCandidateBlobCleanupV1 {
        ...
        _ => return Err(CommunicationNoteCandidatePersistenceErrorV1::InvalidRow),
    };
    ...
    let completeness = candidates
        .as_ref()
        .map(|_| CommunicationNoteCandidateCompletenessV1::Complete);
    ...
    validate_communication_note_candidate_status_v1(&status)
        .map_err(|_| CommunicationNoteCandidatePersistenceErrorV1::InvalidRow)?;
    let persisted = PersistedCommunicationNoteCandidateRunV1 {
    ...
    if !valid_identity(&persisted.logical_owner_id)
        ...
        || validate_communication_note_candidate_draft_v1(&persisted.draft).is_err()
...
fn validate_create(
    ...
    if !valid_identity(&input.logical_owner_id)
        ...
        || input.source_prepare_envelope_bytes.len()
            > COMMUNICATION_NOTE_CANDIDATE_MAX_EVENT_BYTES_V1
        || validate_communication_note_candidate_draft_v1(&input.draft).is_err()
        || validate_communication_note_candidate_status_v1(
            &accepted_communication_note_candidate_status_v1(),
        )
        .is_err()
    {
        return Err(CommunicationNoteCandidatePersistenceErrorV1::InvalidInput);
        ...
        || !matches!(
            ...
            CommunicationNoteCandidateTransitionV1::SourcePrepared { .. }
                | CommunicationNoteCandidateTransitionV1::Reject(
                    CommunicationNoteCandidateRejectionCodeV1::SourceRejected
                        | CommunicationNoteCandidateRejectionCodeV1::InvalidRequest
                        | CommunicationNoteCandidateRejectionCodeV1::Policy
                )
        )
    {
        return Err(CommunicationNoteCandidatePersistenceErrorV1::InvalidInput);
...
fn materialization(
    ...
    Option<(&[u8], &CommunicationNoteCandidateBlobCleanupV1)>,
    CommunicationNoteCandidatePersistenceErrorV1,
> {
    match (
        &input.transition,
        ...
        (CommunicationNoteCandidateTransitionV1::Reject(_), None, None) => Ok(None),
        _ => Err(CommunicationNoteCandidatePersistenceErrorV1::InvalidInput),
    }
}

fn valid_materialization(
    request: &[u8],
    cleanup: &CommunicationNoteCandidateBlobCleanupV1,
) -> bool {
    !request.is_empty()
        && request.len() <= COMMUNICATION_NOTE_CANDIDATE_MAX_SOURCE_READ_RECEIPT_BYTES_V1
        && valid_cleanup(cleanup)
}

fn valid_cleanup(cleanup: &CommunicationNoteCandidateBlobCleanupV1) -> bool {
    nonzero(&cleanup.reference_id)
        ...
        && cleanup.custody_proof.len() <= COMMUNICATION_NOTE_CANDIDATE_MAX_CUSTODY_PROOF_BYTES_V1
}

fn valid_materialization_state(run: &PersistedCommunicationNoteCandidateRunV1) -> bool {
    ...
    match run.status.state {
        ...
        CommunicationNoteCandidateStateV1::Rejected if run.status.source_evidence_id.is_none() => {
            materialization.is_none() && run.cleanup_completed_at_unix_millis.is_none()
        }
...
fn state_code(value: CommunicationNoteCandidateStateV1) -> i16 {
    match value {
        CommunicationNoteCandidateStateV1::Accepted => 1,
        CommunicationNoteCandidateStateV1::PreparingSource => 2,
        CommunicationNoteCandidateStateV1::Extracting => 3,
        CommunicationNoteCandidateStateV1::Ready => 4,
        CommunicationNoteCandidateStateV1::Rejected => 5,
    }
}

pub(crate) fn state_from_code(
    value: i16,
) -> Result<CommunicationNoteCandidateStateV1, CommunicationNoteCandidatePersistenceErrorV1> {
    match value {
        1 => Ok(CommunicationNoteCandidateStateV1::Accepted),
        2 => Ok(CommunicationNoteCandidateStateV1::PreparingSource),
        3 => Ok(CommunicationNoteCandidateStateV1::Extracting),
        4 => Ok(CommunicationNoteCandidateStateV1::Ready),
        5 => Ok(CommunicationNoteCandidateStateV1::Rejected),
...
fn optional_rejection_from_code(
    ...
    Option<CommunicationNoteCandidateRejectionCodeV1>,
    CommunicationNoteCandidatePersistenceErrorV1,
> {
    value
        .map(|code| match code {
            1 => Ok(CommunicationNoteCandidateRejectionCodeV1::InvalidRequest),
            2 => Ok(CommunicationNoteCandidateRejectionCodeV1::SourceRejected),
            3 => Ok(CommunicationNoteCandidateRejectionCodeV1::ExtractionRejected),
            4 => Ok(CommunicationNoteCandidateRejectionCodeV1::Policy),
            _ => Err(CommunicationNoteCandidatePersistenceErrorV1::InvalidRow),
        })
        .transpose()
}
...
fn column<T>(
    row: &PgRow,
    name: &'static str,
) -> Result<T, CommunicationNoteCandidatePersistenceErrorV1>
where
...
fn array16(value: Vec<u8>) -> Result<[u8; 16], CommunicationNoteCandidatePersistenceErrorV1> {
    value
        .try_into()
        .map_err(|_| CommunicationNoteCandidatePersistenceErrorV1::InvalidRow)
}

fn array32(value: Vec<u8>) -> Result<[u8; 32], CommunicationNoteCandidatePersistenceErrorV1> {
    value
        .try_into()
        .map_err(|_| CommunicationNoteCandidatePersistenceErrorV1::InvalidRow)
}
...
fn optional_array16(
    value: Option<Vec<u8>>,
) -> Result<Option<[u8; 16]>, CommunicationNoteCandidatePersistenceErrorV1> {
    value.map(array16).transpose()
}

fn optional_array32(
    value: Option<Vec<u8>>,
) -> Result<Option<[u8; 32]>, CommunicationNoteCandidatePersistenceErrorV1> {
    value.map(array32).transpose()
}

fn signed(value: u64) -> Result<i64, CommunicationNoteCandidatePersistenceErrorV1> {
    i64::try_from(value).map_err(|_| CommunicationNoteCandidatePersistenceErrorV1::InvalidInput)
}

fn optional_signed(
    value: Option<u64>,
) -> Result<Option<i64>, CommunicationNoteCandidatePersistenceErrorV1> {
    value.map(signed).transpose()
}

fn positive_u64(value: i64) -> Result<u64, CommunicationNoteCandidatePersistenceErrorV1> {
    u64::try_from(value)
        .ok()
...
fn optional_positive_u64(
    value: Option<i64>,
) -> Result<Option<u64>, CommunicationNoteCandidatePersistenceErrorV1> {
    value.map(positive_u64).transpose()
}

fn storage_error(_: sqlx::Error) -> CommunicationNoteCandidatePersistenceErrorV1 {
