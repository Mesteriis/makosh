...
fn main() {
    ...
    let descriptor = output.join("review-attention-client-v1.bin");
    ...
    std::fs::write(
        output.join("review_attention_client_schema.rs"),
        format!("pub const REVIEW_ATTENTION_CLIENT_SCHEMA_SHA256_V1: [u8; 32] = {digest:?};\n"),
    )
    .expect("Review attention client schema digest must be written");
}
