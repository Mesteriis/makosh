//! Unsigned Review note-candidate release assembly.
//!
...
pub const REVIEW_NOTE_CANDIDATE_ASSEMBLY_FRAGMENT_VERSION_V1: u32 = 1;
pub const REVIEW_NOTE_CANDIDATE_RUNTIME_ARTIFACT_ID_V1: &str = "review.note-candidate.runtime.v1";
pub const REVIEW_NOTE_CANDIDATE_STORAGE_ARTIFACT_ID_V1: &str = "review.note-candidate.storage.v1";
pub const REVIEW_NOTE_CANDIDATE_DESCRIPTOR_FILE_V1: &str =
    "review-note-candidate.runtime.descriptor.pb";
pub const REVIEW_NOTE_CANDIDATE_SETTINGS_FILE_V1: &str =
    "review-note-candidate.runtime.settings.pb";
pub const REVIEW_NOTE_CANDIDATE_STORAGE_BUNDLE_FILE_V1: &str =
    "review-note-candidate.storage.bundle.pb";
...
#[derive(Clone, Debug, Eq, PartialEq, Deserialize, Serialize)]
#[serde(untagged)]
pub enum ReviewNoteCandidateReleaseArtifactInputV1 {
    ModuleRuntime(ModuleRuntimeArtifactInputV1),
    StorageBundle(StorageBundleArtifactInputV1),
}

impl ReviewNoteCandidateReleaseArtifactInputV1 {
...
#[derive(Clone, Debug, Eq, PartialEq, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
pub struct ReviewNoteCandidateReleaseArtifactFragmentV1 {
    pub version: u32,
    pub owner_id: String,
    pub module_id: String,
    pub artifacts: Vec<ReviewNoteCandidateReleaseArtifactInputV1>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ReviewNoteCandidateReleaseAssemblyPathsV1 {
...
pub enum ReviewNoteCandidateReleaseAssemblyErrorV1 {
    InvalidInput,
...
pub fn materialize_review_note_candidate_release_assembly_v1(
    output_directory: &Path,
    build_id: &str,
    runtime_source: &Path,
) -> Result<ReviewNoteCandidateReleaseAssemblyPathsV1, ReviewNoteCandidateReleaseAssemblyErrorV1> {
    validate_inputs(output_directory, build_id, runtime_source)?;
    let descriptor = review_note_candidate_module_descriptor_v1(build_id);
    let settings = review_note_candidate_settings_schema_v1();
    let storage = review_note_candidate_storage_bundle_v1();
    ...
    {
        return Err(ReviewNoteCandidateReleaseAssemblyErrorV1::InvalidCanonicalArtifact);
    }

    let paths = ReviewNoteCandidateReleaseAssemblyPathsV1 {
        descriptor: output_directory.join(REVIEW_NOTE_CANDIDATE_DESCRIPTOR_FILE_V1),
        settings_schema: output_directory.join(REVIEW_NOTE_CANDIDATE_SETTINGS_FILE_V1),
        storage_bundle: output_directory.join(REVIEW_NOTE_CANDIDATE_STORAGE_BUNDLE_FILE_V1),
        artifact_fragment: output_directory.join(REVIEW_NOTE_CANDIDATE_ARTIFACT_FRAGMENT_FILE_V1),
    };
    ...
    let fragment_bytes = serde_json::to_vec_pretty(&fragment)
        .map_err(|_| ReviewNoteCandidateReleaseAssemblyErrorV1::FragmentEncodingFailed)?;

    let mut directory = DirBuilder::new();
    directory.mode(0o700);
    directory
        .create(output_directory)
        .map_err(|_| ReviewNoteCandidateReleaseAssemblyErrorV1::OutputUnavailable)?;
    let writes = [
    ...
    for (path, bytes) in writes {
        if write_new_private_file(path, &bytes).is_err() {
            ...
            return Err(ReviewNoteCandidateReleaseAssemblyErrorV1::ArtifactWriteFailed);
        }
    }
    Ok(paths)
}
...
fn validate_inputs(
    ...
    build_id: &str,
    runtime_source: &Path,
) -> Result<(), ReviewNoteCandidateReleaseAssemblyErrorV1> {
    ...
    if !output_directory.is_absolute()
        ...
        || !valid_runtime
    {
        return Err(ReviewNoteCandidateReleaseAssemblyErrorV1::InvalidInput);
    }
    Ok(())
}
...
fn artifact_fragment(
    ...
    module_id: &str,
    runtime_source: &Path,
    paths: &ReviewNoteCandidateReleaseAssemblyPathsV1,
) -> Result<ReviewNoteCandidateReleaseArtifactFragmentV1, ReviewNoteCandidateReleaseAssemblyErrorV1>
{
    let artifacts = vec![
        ReviewNoteCandidateReleaseArtifactInputV1::ModuleRuntime(ModuleRuntimeArtifactInputV1 {
            artifact_kind: "module_runtime".to_owned(),
            artifact_id: REVIEW_NOTE_CANDIDATE_RUNTIME_ARTIFACT_ID_V1.to_owned(),
        ...
        ReviewNoteCandidateReleaseArtifactInputV1::StorageBundle(StorageBundleArtifactInputV1 {
            artifact_kind: "storage_bundle".to_owned(),
            artifact_id: REVIEW_NOTE_CANDIDATE_STORAGE_ARTIFACT_ID_V1.to_owned(),
    ...
    Ok(ReviewNoteCandidateReleaseArtifactFragmentV1 {
        version: REVIEW_NOTE_CANDIDATE_ASSEMBLY_FRAGMENT_VERSION_V1,
...
fn utf8_path(path: &Path) -> Result<String, ReviewNoteCandidateReleaseAssemblyErrorV1> {
    path.to_str()
        .filter(|value| !value.is_empty())
        .map(str::to_owned)
        .ok_or(ReviewNoteCandidateReleaseAssemblyErrorV1::InvalidInput)
}
...
mod tests {
    ...
    #[test]
    fn materializes_exact_unsigned_review_note_candidate_artifacts() {
        let root = temporary_directory();
        let runtime = root.join("runtime");
        fs::write(&runtime, b"runtime").expect("runtime fixture");
        let output = root.join("assembly");
        let paths =
            materialize_review_note_candidate_release_assembly_v1(&output, "build-1", &runtime)
                .expect("assembly");
        ...
        let fragment: ReviewNoteCandidateReleaseArtifactFragmentV1 =
    ...
    fn refuses_existing_output_and_symlink_runtime() {
        ...
        assert_eq!(
            materialize_review_note_candidate_release_assembly_v1(&output, "build-1", &runtime),
            Err(ReviewNoteCandidateReleaseAssemblyErrorV1::InvalidInput)
        );
        let symlink = root.join("runtime-link");
        std::os::unix::fs::symlink(&runtime, &symlink).expect("symlink fixture");
        assert_eq!(
            materialize_review_note_candidate_release_assembly_v1(
                &root.join("other"),
                "build-1",
                &symlink,
            ),
