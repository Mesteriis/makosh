...
fn main() -> Result<(), String> {
    ...
    let Some(command) = arguments.next() else {
        return Err("Contacts command is required".to_owned());
    };
    match command.to_str() {
        Some("export-storage-bundle") => {
            export(&mut arguments, contacts_storage_bundle_v1().encode_to_vec())
        }
        Some("export-module-descriptor") => {
            let build_id = required_string(&mut arguments, "build id")?;
            export(
                &mut arguments,
                contacts_module_descriptor_v1(&build_id).encode_to_vec(),
            )
        }
        Some("export-settings-schema") => {
            export(&mut arguments, contacts_settings_schema_bytes_v1())
        }
        Some("serve-inherited") => serve_inherited(&mut arguments),
        _ => Err("Contacts command is invalid".to_owned()),
    }
}
    ...
    I: Iterator<Item = OsString>,
{
    ...
    decode_settings_schema_v1(&settings_schema)
        .map_err(|_| "Contacts settings schema is invalid".to_owned())?;
    let configuration = ManagedDomainRuntimeConfigurationV1::decode(
        read_contract(&paths.runtime_configuration)?.as_slice(),
    )
    .map_err(|_| "Contacts runtime configuration is invalid".to_owned())?;
    ...
    if configuration.runtime_instance_id != paths.runtime_instance_id {
        return Err("Contacts runtime configuration is stale".to_owned());
    }
    let storage = configuration
        .storage
        .clone()
        .ok_or_else(|| "Contacts storage is unavailable".to_owned())?;
    let admission = ContactsRuntimeAdmissionV1 {
    ...
    let executor = tokio::runtime::Runtime::new()
        .map_err(|_| "Contacts executor is unavailable".to_owned())?;
    let mut runtime = executor
        .block_on(ContactsManagedRuntimeV1::open(
...
fn retry_runtime(result: Result<bool, ContactsManagedRuntimeErrorV1>) -> Result<(), String> {
    match result {
        Ok(_)
        | Err(ContactsManagedRuntimeErrorV1::EventUnavailable)
        | Err(ContactsManagedRuntimeErrorV1::Unavailable)
        | Err(ContactsManagedRuntimeErrorV1::Persistence(
...
fn runtime_error(error: ContactsManagedRuntimeErrorV1) -> String {
    let code = match error {
        ContactsManagedRuntimeErrorV1::Admission => "admission",
        ContactsManagedRuntimeErrorV1::EventContract => "event_contract",
        ContactsManagedRuntimeErrorV1::EventUnavailable => "event_unavailable",
        ContactsManagedRuntimeErrorV1::Persistence(_) => "persistence",
        ContactsManagedRuntimeErrorV1::Unavailable => "unavailable",
    };
    format!("Contacts runtime failed: {code}")
}
    ...
    I: Iterator<Item = OsString>,
{
    let output = arguments
        .next()
        .map(PathBuf::from)
        .ok_or_else(|| "Contacts output path is required".to_owned())?;
    require_no_arguments(arguments)?;
    fs::write(output, bytes).map_err(|_| "Contacts export failed".to_owned())
}
    ...
    I: Iterator<Item = OsString>,
{
    if name.starts_with("--") && arguments.next().as_deref() != Some(OsStr::new(name)) {
        return Err("Contacts arguments are invalid".to_owned());
    }
    arguments
        .next()
        .and_then(|value| value.into_string().ok())
        .filter(|value| !value.is_empty())
        .ok_or_else(|| format!("Contacts {name} is required"))
}
...
fn inherited_control_channel() -> Result<UnixStream, String> {
    ...
    if duplicated < 0 {
        return Err("Contacts control channel is unavailable".to_owned());
    }
...
fn read_contract(path: &Path) -> Result<Vec<u8>, String> {
    if !path.is_absolute() {
        return Err("Contacts contract path is invalid".to_owned());
    }
    let metadata =
        fs::symlink_metadata(path).map_err(|_| "Contacts contract is unavailable".to_owned())?;
    if metadata.file_type().is_symlink() || !metadata.is_file() || metadata.len() > 2 * 1024 * 1024
    {
        return Err("Contacts contract is invalid".to_owned());
    }
    let bytes = fs::read(path).map_err(|_| "Contacts contract is unavailable".to_owned())?;
    if bytes.is_empty() {
        return Err("Contacts contract is invalid".to_owned());
    }
    Ok(bytes)
