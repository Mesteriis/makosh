...
use crate::{
    client_port::{
        CommunicationNoteCandidateClientRuntimeContextV1,
        get_communication_note_candidate_payload_v1, start_communication_note_candidate_payload_v1,
    },
    client_realtime::{
        CommunicationNoteCandidateClientRealtimeErrorV1,
        CommunicationNoteCandidateClientRealtimePublisherV1,
    },
    contracts::{
        communication_note_candidate_command_contract_v1,
        communication_note_candidate_query_contract_v1,
    },
    event_outbox::{CommunicationNoteCandidateEventRelayErrorV1, relay_outbox_once_v1},
    extraction::{
        CommunicationNoteCandidateExtractionErrorV1,
        recover_accepted_communication_note_candidate_once_v1,
    },
    review_submission::CommunicationNoteCandidateReviewSubmissionContextV1,
    source_results::{
        CommunicationNoteCandidateSourcePreparedContextV1,
        CommunicationNoteCandidateSourceResultErrorV1, consume_note_source_prepared_once_v1,
        consume_note_source_rejected_once_v1,
    },
};

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct CommunicationNoteCandidateRuntimeAdmissionV1 {
...
pub enum CommunicationNoteCandidateManagedRuntimeErrorV1 {
    Admission,
    EventContract,
    EventUnavailable,
    InvalidTransition,
...
pub struct CommunicationNoteCandidateManagedRuntimeV1 {
    admission: CommunicationNoteCandidateRuntimeAdmissionV1,
    ...
    client_realtime: CommunicationNoteCandidateClientRealtimePublisherV1,
}

impl CommunicationNoteCandidateManagedRuntimeV1 {
    pub async fn open(
    ...
    ) -> Result<Self, CommunicationNoteCandidateManagedRuntimeErrorV1> {
        validate_admission(admission)?;
        if event_hub_endpoint.trim().is_empty() || event_credential_revision == 0 {
            return Err(CommunicationNoteCandidateManagedRuntimeErrorV1::Admission);
        }
        ...
        let vault_public_key = storage_configuration
            ...
            .as_slice()
            .try_into()
            .map_err(|_| CommunicationNoteCandidateManagedRuntimeErrorV1::Admission)?;
        ...
        let password = std::str::from_utf8(&password)
            .map_err(|_| CommunicationNoteCandidateManagedRuntimeErrorV1::Admission)?;
        let persistence = CommunicationNoteCandidatePersistenceV1::connect_runtime(
            &binding,
        ...
        .await
        .map_err(CommunicationNoteCandidateManagedRuntimeErrorV1::Persistence)?;
        persistence
        ...
        .map_err(|_| CommunicationNoteCandidateManagedRuntimeErrorV1::EventUnavailable)?;
        ...
        .await
        .map_err(|_| CommunicationNoteCandidateManagedRuntimeErrorV1::EventUnavailable)?;
        let mut client_realtime = CommunicationNoteCandidateClientRealtimePublisherV1::default();
        ...
        control_channel
            ...
            .map_err(|_| CommunicationNoteCandidateManagedRuntimeErrorV1::Unavailable)?;
        Ok(Self {
    ...
    pub async fn relay_outbox_once(
        &self,
        now_unix_millis: i64,
    ) -> Result<bool, CommunicationNoteCandidateManagedRuntimeErrorV1> {
    ...
    pub async fn consume_source_prepared_once(
        ...
        let source_context = CommunicationNoteCandidateSourcePreparedContextV1 {
            logical_owner_id: &self.admission.logical_owner_id,
            module_id: COMMUNICATION_NOTE_CANDIDATE_MODULE_ID_V1,
            runtime_instance_id: &runtime_instance_id,
            runtime_generation: self.admission.runtime_generation,
            consumed_at_unix_millis: now_unix_millis,
        };
        let mut dispatcher = RejectManagedControlRequestsV2;
        let result = consume_note_source_prepared_once_v1(
            &self.persistence,
            &self.event_connection,
    ...
    pub async fn process_extraction_once(
        &mut self,
        now_unix_millis: i64,
    ) -> Result<bool, CommunicationNoteCandidateManagedRuntimeErrorV1> {
        self.control_channel
            ...
            .map_err(|_| CommunicationNoteCandidateManagedRuntimeErrorV1::Unavailable)?;
        let runtime_instance_id = self.admission.runtime_instance_id.clone();
        let submission_context = CommunicationNoteCandidateReviewSubmissionContextV1 {
            module_id: COMMUNICATION_NOTE_CANDIDATE_MODULE_ID_V1,
            runtime_instance_id: &runtime_instance_id,
            runtime_generation: self.admission.runtime_generation,
            now_unix_millis,
        ...
        let mut dispatcher = RejectManagedControlRequestsV2;
        let result = recover_accepted_communication_note_candidate_once_v1(
            &self.persistence,
            &mut self.control_channel,
            &mut dispatcher,
            &self.admission.logical_owner_id,
    ...
    pub async fn pump_control_once(
        ...
        if validate_module_client_response_v1(&response).is_err() {
            return Err(CommunicationNoteCandidateManagedRuntimeErrorV1::Unavailable);
        }
        self.control_channel
            .write_response(
                correlation_id,
    ...
    fn write_client_error(
        ...
        error_code: &str,
    ) -> Result<(), CommunicationNoteCandidateManagedRuntimeErrorV1> {
        self.control_channel
            .write_response(
                correlation_id,
                ManagedRuntimeControlResponseV1 {
...
async fn dispatch_client(
    ...
    let valid_identity = request.protocol_major == 1
        && request.module_id == COMMUNICATION_NOTE_CANDIDATE_MODULE_ID_V1
        && request.owner_id == COMMUNICATION_NOTE_CANDIDATE_OWNER_V1
        && request.logical_owner_id == admission.logical_owner_id;
    let (response_payload, accepted_route) = if valid_identity {
        if request.contract.as_ref() == Some(&communication_note_candidate_command_contract_v1()) {
            (
                start_communication_note_candidate_payload_v1(
                    persistence,
                    &admission.logical_owner_id,
                    &CommunicationNoteCandidateClientRuntimeContextV1 {
            ...
            == Some(&communication_note_candidate_query_contract_v1())
        {
            (
                get_communication_note_candidate_payload_v1(
                    persistence,
                    &admission.logical_owner_id,
                    &request.request_payload,
                )
...
fn bind_source_subscriptions(
    ...
    CommunicationNoteCandidateManagedRuntimeErrorV1,
> {
    if permits.len() != 2 {
        return Err(CommunicationNoteCandidateManagedRuntimeErrorV1::Admission);
    }
...
fn exact_permit(
...
) -> Result<RuntimeSubscribePermitV1, CommunicationNoteCandidateManagedRuntimeErrorV1> {
    ...
    let permit = matching
        .next()
        .cloned()
        .ok_or(CommunicationNoteCandidateManagedRuntimeErrorV1::Admission)?;
...
) -> Result<zeroize::Zeroizing<Vec<u8>>, CommunicationNoteCandidateManagedRuntimeErrorV1> {
    for attempt in 0..20 {
...
fn storage_binding(
    ...
    admission: &CommunicationNoteCandidateRuntimeAdmissionV1,
) -> Result<StorageBindingV1, CommunicationNoteCandidateManagedRuntimeErrorV1> {
...
fn storage_owner_is_exact(logical_owner_id: &str, owner: &str) -> bool {
    logical_owner_id == owner && owner == COMMUNICATION_NOTE_CANDIDATE_OWNER_V1
}

fn event_relay_error(
    error: CommunicationNoteCandidateEventRelayErrorV1,
) -> CommunicationNoteCandidateManagedRuntimeErrorV1 {
    match error {
        CommunicationNoteCandidateEventRelayErrorV1::InvalidTimestamp => {
            CommunicationNoteCandidateManagedRuntimeErrorV1::EventContract
        }
        CommunicationNoteCandidateEventRelayErrorV1::Persistence(error) => {
            CommunicationNoteCandidateManagedRuntimeErrorV1::Persistence(error)
        }
        CommunicationNoteCandidateEventRelayErrorV1::EventUnavailable => {
            CommunicationNoteCandidateManagedRuntimeErrorV1::EventUnavailable
        }
    }
}

fn source_result_error(
    error: CommunicationNoteCandidateSourceResultErrorV1,
) -> CommunicationNoteCandidateManagedRuntimeErrorV1 {
    match error {
        CommunicationNoteCandidateSourceResultErrorV1::InvalidEnvelope
        | CommunicationNoteCandidateSourceResultErrorV1::InvalidPayload => {
            CommunicationNoteCandidateManagedRuntimeErrorV1::EventContract
        }
        CommunicationNoteCandidateSourceResultErrorV1::Blob(
            crate::CommunicationNoteCandidateBlobErrorV1::InvalidReceipt,
        ) => CommunicationNoteCandidateManagedRuntimeErrorV1::EventContract,
        CommunicationNoteCandidateSourceResultErrorV1::Blob(
            crate::CommunicationNoteCandidateBlobErrorV1::Unavailable,
        ) => CommunicationNoteCandidateManagedRuntimeErrorV1::Unavailable,
        CommunicationNoteCandidateSourceResultErrorV1::Extraction(error) => extraction_error(error),
        CommunicationNoteCandidateSourceResultErrorV1::Persistence(error) => {
            CommunicationNoteCandidateManagedRuntimeErrorV1::Persistence(error)
        }
...
fn extraction_error(
    error: CommunicationNoteCandidateExtractionErrorV1,
) -> CommunicationNoteCandidateManagedRuntimeErrorV1 {
    match error {
        CommunicationNoteCandidateExtractionErrorV1::InvalidState => {
            CommunicationNoteCandidateManagedRuntimeErrorV1::InvalidTransition
        }
        CommunicationNoteCandidateExtractionErrorV1::Persistence(error) => {
            CommunicationNoteCandidateManagedRuntimeErrorV1::Persistence(error)
        }
        CommunicationNoteCandidateExtractionErrorV1::ReviewSubmission => {
            CommunicationNoteCandidateManagedRuntimeErrorV1::Unavailable
        }
        CommunicationNoteCandidateExtractionErrorV1::Blob(
            crate::CommunicationNoteCandidateBlobErrorV1::InvalidReceipt,
        ) => CommunicationNoteCandidateManagedRuntimeErrorV1::InvalidTransition,
        CommunicationNoteCandidateExtractionErrorV1::Blob(
            crate::CommunicationNoteCandidateBlobErrorV1::Unavailable,
...
fn client_realtime_error(
    error: CommunicationNoteCandidateClientRealtimeErrorV1,
) -> CommunicationNoteCandidateManagedRuntimeErrorV1 {
    match error {
        CommunicationNoteCandidateClientRealtimeErrorV1::InvalidTransition => {
            CommunicationNoteCandidateManagedRuntimeErrorV1::InvalidTransition
        }
        CommunicationNoteCandidateClientRealtimeErrorV1::Persistence(error) => {
            CommunicationNoteCandidateManagedRuntimeErrorV1::Persistence(error)
        }
        CommunicationNoteCandidateClientRealtimeErrorV1::Unavailable => {
            CommunicationNoteCandidateManagedRuntimeErrorV1::Unavailable
        }
    }
}
...
mod tests {
    ...
    fn admission_requires_exact_managed_fences() {
        assert_eq!(
            validate_admission(&CommunicationNoteCandidateRuntimeAdmissionV1 {
