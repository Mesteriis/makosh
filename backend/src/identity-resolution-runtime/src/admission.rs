use makosh_identity_resolution_api::{
    IDENTITY_RESOLUTION_MODULE_ID_V1, IDENTITY_RESOLUTION_OWNER_ID_V1,
    IDENTITY_RESOLUTION_PERSONS_EVIDENCE_CAPABILITY_ID_V1,
    IDENTITY_RESOLUTION_REVIEW_CANDIDATE_CAPABILITY_ID_V1,
    IDENTITY_RESOLUTION_STORAGE_CAPABILITY_ID_V1,
    identity_resolution_person_match_candidate_contract_reference_v1,
    identity_resolution_person_match_candidate_publish_request_v1,
};
use makosh_persons_api::{
    persons_review_candidate_contract_reference_v1, persons_review_candidate_publish_request_v1,
};
use makosh_runtime_protocol::v1::{
    CapabilityCriticalityV1, CapabilityDescriptorV1, CapabilityRequestV1, EventRouteDirectionV1,
    EventSubscriptionRequirementV1, ModuleDescriptorV1, ModuleKindV1, ProtocolRangeV1,
    ProvidedSurfaceKindV1, ProvidedSurfaceV1, RuntimeBudgetRequestV1, SettingsSchemaRefV1,
    SettingsSchemaV1, StorageNamespaceRequestV1, capability_request_v1::Request,
};
use prost::Message;
use sha2::{Digest, Sha256};

#[must_use]
pub fn identity_resolution_settings_schema_bytes_v1() -> Vec<u8> {
    SettingsSchemaV1 {
        major: 1,
        revision: 1,
        definitions: Vec::new(),
    }
    .encode_to_vec()
}

#[must_use]
pub fn identity_resolution_module_descriptor_v1(build_id: &str) -> ModuleDescriptorV1 {
    let settings = identity_resolution_settings_schema_bytes_v1();
    ModuleDescriptorV1 {
        descriptor_major: 1,
        descriptor_revision: 1,
        module_id: IDENTITY_RESOLUTION_MODULE_ID_V1.to_owned(),
        owner_id: IDENTITY_RESOLUTION_OWNER_ID_V1.to_owned(),
        module_kind: ModuleKindV1::Engine as i32,
        module_version: "1".to_owned(),
        build_id: build_id.to_owned(),
        runtime_protocol_range: Some(ProtocolRangeV1 {
            minimum_major: 2,
            maximum_major: 2,
            minimum_revision: 1,
        }),
        capabilities: vec![
            event_capability(
                IDENTITY_RESOLUTION_PERSONS_EVIDENCE_CAPABILITY_ID_V1,
                ProvidedSurfaceKindV1::DurableConsumer,
                persons_review_candidate_contract_reference_v1(),
                consume_request(),
            ),
            event_capability(
                IDENTITY_RESOLUTION_REVIEW_CANDIDATE_CAPABILITY_ID_V1,
                ProvidedSurfaceKindV1::DurablePublisher,
                identity_resolution_person_match_candidate_contract_reference_v1(),
                identity_resolution_person_match_candidate_publish_request_v1(),
            ),
            CapabilityDescriptorV1 {
                capability_id: IDENTITY_RESOLUTION_STORAGE_CAPABILITY_ID_V1.to_owned(),
                capability_revision: 1,
                criticality: CapabilityCriticalityV1::Required as i32,
                requests: vec![CapabilityRequestV1 {
                    request: Some(Request::StorageNamespace(StorageNamespaceRequestV1 {
                        owner_id: IDENTITY_RESOLUTION_OWNER_ID_V1.to_owned(),
                        connection_budget: 4,
                        timeout_millis: 5000,
                    })),
                }],
                ..Default::default()
            },
        ],
        settings_schema_ref: Some(SettingsSchemaRefV1 {
            major: 1,
            revision: 1,
            artifact_size_bytes: settings.len() as u64,
            sha256: Sha256::digest(&settings).to_vec(),
        }),
        runtime_budget_request: Some(RuntimeBudgetRequestV1 {
            max_processes: 1,
            max_connections: 4,
            max_memory_bytes: 64 * 1024 * 1024,
            max_cpu_millis: 500,
        }),
        display_name: "Identity Resolution".to_owned(),
    }
}
fn consume_request() -> CapabilityRequestV1 {
    let mut request = persons_review_candidate_publish_request_v1();
    if let Some(Request::EventRoute(route)) = &mut request.request {
        route.direction = EventRouteDirectionV1::Consume as i32;
        route.subscription_requirement = EventSubscriptionRequirementV1::Required as i32;
        route.max_deliver = 10;
        route.ack_wait_millis = 30_000;
    }
    request
}
fn event_capability(
    id: &str,
    kind: ProvidedSurfaceKindV1,
    contract: makosh_runtime_protocol::v1::ContractReferenceV1,
    request: CapabilityRequestV1,
) -> CapabilityDescriptorV1 {
    CapabilityDescriptorV1 {
        capability_id: id.to_owned(),
        capability_revision: 1,
        criticality: CapabilityCriticalityV1::Required as i32,
        provides: vec![ProvidedSurfaceV1 {
            kind: kind as i32,
            contract: Some(contract),
            client_rpc_route: None,
            client_blob_route: None,
        }],
        requests: vec![request],
        ..Default::default()
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    use makosh_runtime_protocol::validation::descriptor::validate_descriptor_v1;
    #[test]
    fn descriptor_is_exact_engine_without_client_or_persons_command() {
        let d = identity_resolution_module_descriptor_v1("test");
        validate_descriptor_v1(&d).expect("descriptor");
        assert_eq!(d.module_kind, ModuleKindV1::Engine as i32);
        assert_eq!(d.capabilities.len(), 3);
        assert!(
            d.capabilities
                .iter()
                .flat_map(|c| &c.provides)
                .all(|p| p.kind != ProvidedSurfaceKindV1::ClientRpc as i32)
        );
    }
}
