use makosh_review_obligation_candidate_core::{
    ...
    ReviewObligationCandidateStateV1, ReviewObligationCandidateV1,
    create_review_obligation_candidate_v1, decide_review_obligation_candidate_v1,
...
use crate::{
    ...
    PersistedReviewObligationCandidateSubmissionV1, RejectReviewObligationCandidateSubmissionV1,
    ReserveReviewObligationCandidateSubmissionOutcomeV1,
    ReserveReviewObligationCandidateSubmissionV1, ReviewObligationCandidateDecisionOutcomeV1,
    ...
    model::{
        REVIEW_OBLIGATION_CANDIDATE_MAX_PAGE_SIZE_V1, REVIEW_OBLIGATION_CANDIDATE_OUTBOX_LIMIT_V1,
        REVIEW_OBLIGATION_CANDIDATE_REALTIME_LIMIT_V1,
        REVIEW_OBLIGATION_CANDIDATE_RECOVERY_LIMIT_V1, decision_fingerprint,
        decision_replay_fingerprint, nonzero, valid_blob, valid_cleanup, valid_identity,
        valid_outbox,
    },
    row_codec::{
...
#[derive(Clone)]
pub struct ReviewObligationCandidatePersistenceV1 {
    pool: PgPool,
}

impl ReviewObligationCandidatePersistenceV1 {
    pub async fn connect_runtime(
        ...
        pgbouncer_port: u32,
        password: &str,
    ) -> Result<Self, ReviewObligationCandidatePersistenceErrorV1> {
        ...
        {
            return Err(ReviewObligationCandidatePersistenceErrorV1::StorageUnavailable);
        }
        let options = PgConnectOptions::new()
            .host(pgbouncer_host)
            .port(
                u16::try_from(pgbouncer_port)
                    .map_err(|_| ReviewObligationCandidatePersistenceErrorV1::StorageUnavailable)?,
            )
    ...
    pub async fn reserve_submission(
        &self,
        input: ReserveReviewObligationCandidateSubmissionV1,
    ) -> Result<
        ReserveReviewObligationCandidateSubmissionOutcomeV1,
    ...
    > {
        ...
        let inserted = sqlx::query(
            "INSERT INTO makosh_data.review_obligation_candidate_submissions (
        ...
        if !same_submission(&persisted, &input) {
            return Err(ReviewObligationCandidatePersistenceErrorV1::SubmissionConflict);
        }
        let outcome = if inserted == 1 {
            Ok(ReserveReviewObligationCandidateSubmissionOutcomeV1::Reserved(persisted))
        } else {
            Ok(ReserveReviewObligationCandidateSubmissionOutcomeV1::Existing(persisted))
        };
    ...
    pub async fn load_recoverable_submissions(
        ...
        Vec<PersistedReviewObligationCandidateSubmissionV1>,
        ReviewObligationCandidatePersistenceErrorV1,
    > {
        if !valid_identity(logical_owner_id) {
            return Err(ReviewObligationCandidatePersistenceErrorV1::InvalidInput);
        }
        ...
        let submissions = sqlx::query(SELECT_RECOVERABLE_SUBMISSIONS)
            ...
            .bind(i64::from(REVIEW_OBLIGATION_CANDIDATE_RECOVERY_LIMIT_V1))
    ...
    pub async fn persist_materialization(
        &self,
        input: PersistReviewObligationCandidateMaterializationV1,
    ) -> Result<
    ...
    > {
        ...
        let affected = sqlx::query(
            "UPDATE makosh_data.review_obligation_candidate_submissions
    ...
    pub async fn complete_blob_cleanup(
        ...
        materialization: &crate::ReviewObligationCandidateBlobCleanupV1,
            ...
            && current.cleanup_completed_at_unix_millis.is_some()
        {
            ...
            Err(ReviewObligationCandidatePersistenceErrorV1::RevisionConflict)
        }
    }

    pub async fn complete_submission(
        &self,
        input: CompleteReviewObligationCandidateSubmissionV1,
        ...
        let review = create_review_obligation_candidate_v1(input.draft.clone())
            .map_err(|_| ReviewObligationCandidatePersistenceErrorV1::InvalidTransition)?;
        let mut transaction = self
            .begin_owner_transaction(&input.logical_owner_id)
            .await?;
        ...
        if submission.completed {
            let review_id = submission
                .review_id
                .ok_or(ReviewObligationCandidatePersistenceErrorV1::SubmissionConflict)?;
            let review =
    ...
    pub async fn reject_submission(
        &self,
        input: RejectReviewObligationCandidateSubmissionV1,
    ) -> Result<(), ReviewObligationCandidatePersistenceErrorV1> {
        if !valid_identity(&input.logical_owner_id)
            || !nonzero(&input.submission_message_id)
        ...
        if submission.completed {
            ...
            return if submission.rejected {
                ...
                Err(ReviewObligationCandidatePersistenceErrorV1::SubmissionConflict)
            };
        }
        sqlx::query(
            "UPDATE makosh_data.review_obligation_candidate_submissions
    ...
    pub async fn load_review(
        ...
        let review = sqlx::query(SELECT_REVIEW_BY_ID)
            ...
            .await
            .map_err(storage_error)?
            .ok_or(ReviewObligationCandidatePersistenceErrorV1::NotFound)
    ...
    pub async fn list_reviews(
        &self,
        logical_owner_id: &str,
        input: ListReviewObligationCandidatesV1,
    ) -> Result<ReviewObligationCandidatePageV1, ReviewObligationCandidatePersistenceErrorV1> {
        if !valid_identity(logical_owner_id)
            || input.limit == 0
            || input.limit > REVIEW_OBLIGATION_CANDIDATE_MAX_PAGE_SIZE_V1
            || input
                .after_review_id
        ...
        let rows = sqlx::query(
            "SELECT logical_owner_id, review_id, candidate_id, candidate_digest,
                    source_evidence_id, source_evidence_revision, statement,
                    due_at_unix_seconds, due_at_nanos, obligated_party_id,
                    beneficiary_party_id, condition, state, promotion_status, review_revision,
                    ...
                    ARRAY(SELECT evidence_link_id FROM makosh_data.review_obligation_candidate_evidence e
                        WHERE e.logical_owner_id=$1 AND e.review_id=s.review_id ORDER BY evidence_link_id) evidence_link_ids,
                    ARRAY(SELECT evidence_owner_id FROM makosh_data.review_obligation_candidate_evidence e
                        WHERE e.logical_owner_id=$1 AND e.review_id=s.review_id ORDER BY evidence_link_id) evidence_owner_ids,
                    ARRAY(SELECT evidence_record_id FROM makosh_data.review_obligation_candidate_evidence e
                        WHERE e.logical_owner_id=$1 AND e.review_id=s.review_id ORDER BY evidence_link_id) evidence_record_ids,
                    ARRAY(SELECT evidence_revision FROM makosh_data.review_obligation_candidate_evidence e
                        WHERE e.logical_owner_id=$1 AND e.review_id=s.review_id ORDER BY evidence_link_id) evidence_revisions,
                    ARRAY(SELECT evidence_digest FROM makosh_data.review_obligation_candidate_evidence e
                        WHERE e.logical_owner_id=$1 AND e.review_id=s.review_id ORDER BY evidence_link_id) evidence_digests
             FROM makosh_data.review_obligation_candidate_state s
        ...
        let next_after_review_id = has_more
            ...
            .flatten();
        transaction.commit().await.map_err(storage_error)?;
        Ok(ReviewObligationCandidatePageV1 {
            reviews,
    ...
    pub async fn decide(
        &self,
        input: DecideReviewObligationCandidateOperationV1,
    ) -> Result<
        ReviewObligationCandidateDecisionOutcomeV1,
        ReviewObligationCandidatePersistenceErrorV1,
    > {
        ...
        if let Some(row) = sqlx::query(
            "SELECT request_sha256, decision_fingerprint, review_id, result_review_revision
             FROM makosh_data.review_obligation_candidate_operations
        ...
        .map_err(storage_error)?
        {
            ...
            {
                return Err(ReviewObligationCandidatePersistenceErrorV1::OperationConflict);
            }
            ...
            .await?;
            transaction.commit().await.map_err(storage_error)?;
            return Ok(ReviewObligationCandidateDecisionOutcomeV1::Replayed(review));
        }
        let current =
            load_review_for_update(&mut transaction, &input.logical_owner_id, &input.review_id)
                .await?;
        let next = decide_review_obligation_candidate_v1(
            &current,
        ...
        sqlx::query(
            "INSERT INTO makosh_data.review_obligation_candidate_operations (
        ...
        transaction.commit().await.map_err(storage_error)?;
        Ok(ReviewObligationCandidateDecisionOutcomeV1::Applied(next))
    }

    pub async fn load_decision_replay(
        &self,
        input: &CheckReviewObligationCandidateDecisionReplayV1,
    ) -> Result<Option<ReviewObligationCandidateV1>, ReviewObligationCandidatePersistenceErrorV1>
    {
    ...
    pub async fn persist_promotion_result(
        &self,
        input: PersistReviewObligationCandidatePromotionResultV1,
    ) -> Result<ReviewObligationCandidateInboxOutcomeV1, ReviewObligationCandidatePersistenceErrorV1>
    {
        ...
        if current.candidate_id != input.candidate_id {
            return Err(ReviewObligationCandidatePersistenceErrorV1::InboxConflict);
        }
        if let Some(row) = sqlx::query(
            "SELECT result_envelope_sha256, review_id
             FROM makosh_data.review_obligation_candidate_promotion_inbox
        ...
        .map_err(storage_error)?
        {
            ...
            transaction.commit().await.map_err(storage_error)?;
            return Ok(ReviewObligationCandidateInboxOutcomeV1::Duplicate(current));
        }
        let next = record_review_obligation_candidate_promotion_v1(
            &current,
            input.expected_review_revision,
            input.result,
            input.occurred_at,
        ...
        update_review(&mut transaction, current.review_revision, &next).await?;
        sqlx::query(
            "INSERT INTO makosh_data.review_obligation_candidate_promotion_inbox (
        ...
        .await?;
        transaction.commit().await.map_err(storage_error)?;
        Ok(ReviewObligationCandidateInboxOutcomeV1::Applied(next))
    }

    pub async fn load_pending_promotions(
        &self,
        logical_owner_id: &str,
    ) -> Result<Vec<ReviewObligationCandidateV1>, ReviewObligationCandidatePersistenceErrorV1> {
        if !valid_identity(logical_owner_id) {
            return Err(ReviewObligationCandidatePersistenceErrorV1::InvalidInput);
        }
        let mut transaction = self.begin_owner_transaction(logical_owner_id).await?;
    ...
    pub async fn realtime_after(
        ...
        Vec<ReviewObligationCandidateRealtimeTransitionV1>,
        ReviewObligationCandidatePersistenceErrorV1,
    > {
        if !valid_identity(logical_owner_id)
            || limit == 0
            || limit > REVIEW_OBLIGATION_CANDIDATE_REALTIME_LIMIT_V1
        {
            return Err(ReviewObligationCandidatePersistenceErrorV1::InvalidInput);
        }
        let mut transaction = self.begin_owner_transaction(logical_owner_id).await?;
        let transitions = sqlx::query(
            "SELECT realtime_sequence, review_id, candidate_id, state,
             ...
             FROM makosh_data.review_obligation_candidate_realtime
    ...
    pub async fn unpublished_outbox(
        ...
        Vec<ReviewObligationCandidateOutboxRecordV1>,
        ReviewObligationCandidatePersistenceErrorV1,
    > {
        if !valid_identity(logical_owner_id)
            || limit == 0
            || limit > REVIEW_OBLIGATION_CANDIDATE_OUTBOX_LIMIT_V1
        {
            return Err(ReviewObligationCandidatePersistenceErrorV1::InvalidInput);
        }
        let mut transaction = self.begin_owner_transaction(logical_owner_id).await?;
        let records = sqlx::query(
            "SELECT message_id, envelope_sha256, envelope_bytes
             FROM makosh_data.review_obligation_candidate_outbox
    ...
    pub async fn mark_outbox_published(
        ...
        let mut transaction = self.begin_owner_transaction(logical_owner_id).await?;
        let affected = sqlx::query(
            "UPDATE makosh_data.review_obligation_candidate_outbox
        ...
        .rows_affected();
        if affected != 1 {
            return Err(ReviewObligationCandidatePersistenceErrorV1::NotFound);
        }
        transaction.commit().await.map_err(storage_error)?;
        Ok(())
    }
    ...
    async fn begin_owner_transaction(
        &self,
        logical_owner_id: &str,
    ) -> Result<Transaction<'_, Postgres>, ReviewObligationCandidatePersistenceErrorV1> {
        if !valid_identity(logical_owner_id) {
            return Err(ReviewObligationCandidatePersistenceErrorV1::InvalidInput);
        }
...
async fn insert_outbox(
    transaction: &mut sqlx::Transaction<'_, sqlx::Postgres>,
    logical_owner_id: &str,
    record: &ReviewObligationCandidateOutboxRecordV1,
    created_at_unix_millis: i64,
) -> Result<(), ReviewObligationCandidatePersistenceErrorV1> {
    sqlx::query(
        "INSERT INTO makosh_data.review_obligation_candidate_outbox (
...
async fn insert_realtime(
    ...
    sqlx::query(
        "INSERT INTO makosh_data.review_obligation_candidate_realtime (
...
fn validate_decision(
    ...
    let event_valid = match input.decision {
        ...
        ReviewObligationCandidateDecisionV1::Reject => input.approved_event.is_none(),
    };
    if !valid_identity(&input.logical_owner_id)
...
fn same_submission(
    persisted: &PersistedReviewObligationCandidateSubmissionV1,
    input: &ReserveReviewObligationCandidateSubmissionV1,
) -> bool {
...
> {
    let state = match row.try_get::<i16, _>("state").map_err(invalid_row)? {
        1 => ReviewObligationCandidateStateV1::Pending,
        2 => ReviewObligationCandidateStateV1::Approved,
        3 => ReviewObligationCandidateStateV1::Rejected,
    ...
    let promotion_status = match row
        ...
        .map_err(invalid_row)?
    {
        1 => ReviewObligationCandidatePromotionStatusV1::NotRequested,
        2 => ReviewObligationCandidatePromotionStatusV1::Pending,
        3 => ReviewObligationCandidatePromotionStatusV1::Succeeded,
        4 => ReviewObligationCandidatePromotionStatusV1::Failed,
        _ => return Err(ReviewObligationCandidatePersistenceErrorV1::InvalidRow),
    };
    Ok(ReviewObligationCandidateRealtimeTransitionV1 {
...
fn outbox_from_row(
    row: sqlx::postgres::PgRow,
) -> Result<ReviewObligationCandidateOutboxRecordV1, ReviewObligationCandidatePersistenceErrorV1> {
...
fn transition_error(
    error: makosh_review_obligation_candidate_core::ReviewObligationCandidateTransitionErrorV1,
) -> ReviewObligationCandidatePersistenceErrorV1 {
    match error {
        makosh_review_obligation_candidate_core::ReviewObligationCandidateTransitionErrorV1::RevisionConflict => {
            ReviewObligationCandidatePersistenceErrorV1::RevisionConflict
        }
        _ => ReviewObligationCandidatePersistenceErrorV1::InvalidTransition,
    }
}

fn invalid_row(_: sqlx::Error) -> ReviewObligationCandidatePersistenceErrorV1 {
