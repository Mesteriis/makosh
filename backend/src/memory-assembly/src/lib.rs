...
pub const PACKAGE: &str = "makosh-memory-assembly";
pub const MEMORY_FRAGMENT_FILE_V1: &str = "memory.release-artifacts.json";
...
pub fn materialize_memory_release_assembly_v1(
    output: &Path,
    build: &str,
    runtime: &Path,
    ...
    let descriptor = memory_module_descriptor_v1(build);
    let settings_bytes = memory_settings_schema_bytes_v1();
    let settings =
        decode_settings_schema_v1(&settings_bytes).map_err(|_| AssemblyErrorV1::InvalidArtifact)?;
    let storage = memory_storage_bundle_v1();
    ...
    let paths = PathsV1 {
        descriptor: output.join("memory.runtime.descriptor.pb"),
        settings_schema: output.join("memory.runtime.settings.pb"),
        storage_bundle: output.join("memory.storage.bundle.pb"),
        artifact_fragment: output.join(MEMORY_FRAGMENT_FILE_V1),
    };
    let utf8 = |path: &Path| {
        path.to_str()
            .map(str::to_owned)
    ...
    let fragment = FragmentV1 {
        ...
        artifacts: vec![
            ArtifactV1::Runtime(RuntimeInputV1 {
                ...
                artifact_id: "memory.runtime.v1".into(),
                relative_path: "bin/makosh-memory-runtime".into(),
                source_path: utf8(runtime)?,
                required: true,
                descriptor: ContractInputV1 {
                    relative_path: "contracts/memory.runtime.descriptor.pb".into(),
                    source_path: utf8(&paths.descriptor)?,
                },
                settings_schema: ContractInputV1 {
                    relative_path: "contracts/memory.runtime.settings.pb".into(),
                    source_path: utf8(&paths.settings_schema)?,
                },
            }),
            ArtifactV1::Storage(StorageInputV1 {
                artifact_kind: "storage_bundle".into(),
                artifact_id: "memory.storage.v1".into(),
                relative_path: "storage/memory.storage.bundle.pb".into(),
...
mod tests {
    ...
    fn materializes_compiler_shape() {
        let root = std::env::temp_dir().join(format!("makosh-memory-{}", std::process::id()));
        let _ = fs::remove_dir_all(&root);
        fs::create_dir(&root).unwrap();
        let runtime = root.join("runtime");
        fs::write(&runtime, b"runtime").unwrap();
        let paths =
            materialize_memory_release_assembly_v1(&root.join("out"), "build", &runtime).unwrap();
        let fragment: FragmentV1 =
