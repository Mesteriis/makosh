...
use makosh_ai_inference_assembly::materialize_ai_inference_release_assembly_v1;
...
fn main() -> ExitCode {
    let Some(arguments) = arguments(std::env::args().skip(1).collect()) else {
        return fail(
            "usage: makosh-ai-inference-assembly --build-id <id> \
             --output-dir <absolute-path> --runtime <absolute-path>",
        );
    };
    if materialize_ai_inference_release_assembly_v1(
    ...
    println!("ai-inference-release-assembly: ok");
    ExitCode::SUCCESS
}
