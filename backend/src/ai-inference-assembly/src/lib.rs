//! Unsigned AI inference release assembly.
//!
//! This unit materializes canonical engine contracts and Storage migration
//! input. It never launches the runtime, selects a provider or signs artifacts.
...
pub const AI_INFERENCE_ASSEMBLY_FRAGMENT_VERSION_V1: u32 = 1;
pub const AI_INFERENCE_RUNTIME_ARTIFACT_ID_V1: &str = "ai_inference.runtime.v1";
pub const AI_INFERENCE_STORAGE_ARTIFACT_ID_V1: &str = "ai_inference.storage.v1";
pub const AI_INFERENCE_DESCRIPTOR_FILE_V1: &str = "ai-inference.runtime.descriptor.pb";
pub const AI_INFERENCE_SETTINGS_FILE_V1: &str = "ai-inference.runtime.settings.pb";
...
#[derive(Clone, Debug, Eq, PartialEq, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
pub struct AiInferenceReleaseArtifactFragmentV1 {
    pub version: u32,
    pub owner_id: String,
    pub module_id: String,
    pub artifacts: Vec<AiInferenceReleaseArtifactInputV1>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct AiInferenceReleaseAssemblyPathsV1 {
...
pub enum AiInferenceReleaseAssemblyErrorV1 {
    InvalidInput,
...
pub fn materialize_ai_inference_release_assembly_v1(
    output_directory: &Path,
    build_id: &str,
    runtime_source: &Path,
) -> Result<AiInferenceReleaseAssemblyPathsV1, AiInferenceReleaseAssemblyErrorV1> {
    validate_inputs(output_directory, build_id, runtime_source)?;
    let descriptor = ai_inference_module_descriptor_v1(build_id);
    let settings = ai_inference_settings_schema_v1();
    let storage = ai_inference_storage_bundle_v1();
    ...
    {
        return Err(AiInferenceReleaseAssemblyErrorV1::InvalidCanonicalArtifact);
    }

    let paths = AiInferenceReleaseAssemblyPathsV1 {
        descriptor: output_directory.join(AI_INFERENCE_DESCRIPTOR_FILE_V1),
        settings_schema: output_directory.join(AI_INFERENCE_SETTINGS_FILE_V1),
        storage_bundle: output_directory.join(AI_INFERENCE_STORAGE_BUNDLE_FILE_V1),
        artifact_fragment: output_directory.join(AI_INFERENCE_ARTIFACT_FRAGMENT_FILE_V1),
    };
    ...
    let fragment_bytes = serde_json::to_vec_pretty(&fragment)
        .map_err(|_| AiInferenceReleaseAssemblyErrorV1::FragmentEncodingFailed)?;

    let mut directory = DirBuilder::new();
    directory.mode(0o700);
    directory
        .create(output_directory)
        .map_err(|_| AiInferenceReleaseAssemblyErrorV1::OutputUnavailable)?;
    let writes = [
    ...
    for (path, bytes) in writes {
        if write_new_private_file(path, &bytes).is_err() {
            ...
            return Err(AiInferenceReleaseAssemblyErrorV1::ArtifactWriteFailed);
        }
    }
    Ok(paths)
}
...
fn validate_inputs(
    ...
    build_id: &str,
    runtime_source: &Path,
) -> Result<(), AiInferenceReleaseAssemblyErrorV1> {
    ...
    if !output_directory.is_absolute()
        ...
        || !valid_runtime
    {
        return Err(AiInferenceReleaseAssemblyErrorV1::InvalidInput);
    }
    Ok(())
}
...
fn artifact_fragment(
    ...
    module_id: &str,
    runtime_source: &Path,
    paths: &AiInferenceReleaseAssemblyPathsV1,
) -> Result<AiInferenceReleaseArtifactFragmentV1, AiInferenceReleaseAssemblyErrorV1> {
    let artifacts = vec![
        AiInferenceReleaseArtifactInputV1::ModuleRuntime(ModuleRuntimeArtifactInputV1 {
            artifact_kind: "module_runtime".to_owned(),
            artifact_id: AI_INFERENCE_RUNTIME_ARTIFACT_ID_V1.to_owned(),
        ...
        AiInferenceReleaseArtifactInputV1::StorageBundle(StorageBundleArtifactInputV1 {
            artifact_kind: "storage_bundle".to_owned(),
            artifact_id: AI_INFERENCE_STORAGE_ARTIFACT_ID_V1.to_owned(),
    ...
    Ok(AiInferenceReleaseArtifactFragmentV1 {
        version: AI_INFERENCE_ASSEMBLY_FRAGMENT_VERSION_V1,
...
fn utf8_path(path: &Path) -> Result<String, AiInferenceReleaseAssemblyErrorV1> {
    path.to_str()
        .filter(|value| !value.is_empty())
        .map(str::to_owned)
        .ok_or(AiInferenceReleaseAssemblyErrorV1::InvalidInput)
}
...
mod tests {
    ...
    #[test]
    fn materializes_exact_unsigned_ai_engine_artifacts() {
        let root = temporary_directory();
        let runtime = root.join("runtime");
        fs::write(&runtime, b"runtime").expect("runtime fixture");
        let output = root.join("assembly");
        let paths = materialize_ai_inference_release_assembly_v1(&output, "build-1", &runtime)
            .expect("assembly");
        ...
        StorageBundleV1::decode(fs::read(paths.storage_bundle).expect("storage").as_slice())
            .expect("storage");
        let fragment: AiInferenceReleaseArtifactFragmentV1 =
    ...
    fn refuses_existing_output_and_symlink_runtime() {
        ...
        assert_eq!(
            materialize_ai_inference_release_assembly_v1(&output, "build-1", &runtime),
            Err(AiInferenceReleaseAssemblyErrorV1::InvalidInput)
        );
        let symlink = root.join("runtime-link");
        std::os::unix::fs::symlink(&runtime, &symlink).expect("symlink fixture");
        assert_eq!(
            materialize_ai_inference_release_assembly_v1(&root.join("other"), "build-1", &symlink,),
            Err(AiInferenceReleaseAssemblyErrorV1::InvalidInput)
        );
        fs::remove_dir_all(root).expect("cleanup");
    }
