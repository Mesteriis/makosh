...
fn main() -> Result<(), String> {
    ...
    let Some(command) = arguments.next() else {
        return Err("Tasks command is required".to_owned());
    };
    match command.to_str() {
        Some("export-storage-bundle") => {
            export(&mut arguments, tasks_storage_bundle_v1().encode_to_vec())
        }
        Some("export-module-descriptor") => {
            let build_id = required_string(&mut arguments, "build id")?;
            export(
                &mut arguments,
                tasks_module_descriptor_v1(&build_id).encode_to_vec(),
            )
        }
        Some("export-settings-schema") => export(&mut arguments, tasks_settings_schema_bytes_v1()),
        Some("serve-inherited") => serve_inherited(&mut arguments),
        _ => Err("Tasks command is invalid".to_owned()),
    }
}
    ...
    I: Iterator<Item = OsString>,
{
    ...
    if configuration.runtime_instance_id != paths.runtime_instance_id {
        return Err("Tasks runtime configuration is stale".to_owned());
    }
    let storage = configuration
        .storage
        .clone()
        .ok_or_else(|| "Tasks storage is unavailable".to_owned())?;
    let admission = TasksRuntimeAdmissionV1 {
    ...
    let executor =
        tokio::runtime::Runtime::new().map_err(|_| "Tasks executor is unavailable".to_owned())?;
    let mut runtime = executor
        .block_on(TasksManagedRuntimeV1::open(
    ...
    loop {
        let now = std::time::SystemTime::now()
            ...
            .map_err(|_| "Tasks clock is invalid".to_owned())?
            .as_millis()
            .try_into()
            .map_err(|_| "Tasks clock is invalid".to_owned())?;
...
fn retry_runtime(result: Result<bool, TasksManagedRuntimeErrorV1>) -> Result<(), String> {
    match result {
        Ok(_)
        | Err(TasksManagedRuntimeErrorV1::EventUnavailable)
        | Err(TasksManagedRuntimeErrorV1::Unavailable)
        | Err(TasksManagedRuntimeErrorV1::Persistence(
...
fn runtime_error(error: TasksManagedRuntimeErrorV1) -> String {
    let code = match error {
        TasksManagedRuntimeErrorV1::Admission => "admission",
        TasksManagedRuntimeErrorV1::EventContract => "event_contract",
        TasksManagedRuntimeErrorV1::EventUnavailable => "event_unavailable",
        TasksManagedRuntimeErrorV1::Persistence(_) => "persistence",
        TasksManagedRuntimeErrorV1::Unavailable => "unavailable",
    };
    format!("Tasks runtime failed: {code}")
}
    ...
    I: Iterator<Item = OsString>,
{
    let output = arguments
        .next()
        .map(PathBuf::from)
        .ok_or_else(|| "Tasks output path is required".to_owned())?;
    require_no_arguments(arguments)?;
    fs::write(output, bytes).map_err(|_| "Tasks export failed".to_owned())
}
    ...
    I: Iterator<Item = OsString>,
{
    if name.starts_with("--") && arguments.next().as_deref() != Some(OsStr::new(name)) {
        return Err("Tasks arguments are invalid".to_owned());
    }
    arguments
        .next()
        .and_then(|value| value.into_string().ok())
        .filter(|value| !value.is_empty())
        .ok_or_else(|| format!("Tasks {name} is required"))
}
...
fn inherited_control_channel() -> Result<UnixStream, String> {
    ...
    if duplicated < 0 {
        return Err("Tasks control channel is unavailable".to_owned());
    }
...
fn read_contract(path: &Path) -> Result<Vec<u8>, String> {
    if !path.is_absolute() {
        return Err("Tasks contract path is invalid".to_owned());
    }
    let bytes = fs::read(path).map_err(|_| "Tasks contract is unavailable".to_owned())?;
    if bytes.is_empty() || bytes.len() > 2 * 1024 * 1024 {
        return Err("Tasks contract is invalid".to_owned());
    }
    Ok(bytes)
}
