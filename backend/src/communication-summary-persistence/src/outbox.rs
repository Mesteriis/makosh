...
use crate::{
    ...
    UnpublishedCommunicationSummaryEventV1,
    model::{
        COMMUNICATION_SUMMARY_MAX_EVENT_BYTES_V1, COMMUNICATION_SUMMARY_OUTBOX_LIMIT_V1, nonzero,
...
impl CommunicationSummaryPersistenceV1 {
    pub async fn unpublished_source_prepare_events(
        ...
        limit: u16,
    ) -> Result<Vec<UnpublishedCommunicationSummaryEventV1>, CommunicationSummaryPersistenceErrorV1>
    {
        if !valid_identity(logical_owner_id)
            || !(1..=COMMUNICATION_SUMMARY_OUTBOX_LIMIT_V1).contains(&limit)
        {
            return Err(CommunicationSummaryPersistenceErrorV1::InvalidInput);
        }
        sqlx::query(
            "SELECT message_id, envelope_sha256, envelope_bytes
             FROM makosh_data.communication_summary_outbox
        ...
        .map(|row| {
            ...
            let envelope_bytes: Vec<u8> = row
                ...
                .map_err(|_| CommunicationSummaryPersistenceErrorV1::InvalidRow)?;
            let event = UnpublishedCommunicationSummaryEventV1 {
            ...
            if !nonzero(&event.message_id)
                ...
                || event.envelope_bytes.len() > COMMUNICATION_SUMMARY_MAX_EVENT_BYTES_V1
            {
    ...
    pub async fn mark_source_prepare_published(
        ...
        let updated = sqlx::query(
            "UPDATE makosh_data.communication_summary_outbox
