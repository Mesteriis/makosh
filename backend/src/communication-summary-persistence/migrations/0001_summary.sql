CREATE TABLE makosh_data.communication_summary_runs (
    ...
    candidate_summary_utf8 BYTEA,
    candidate_resolved_length SMALLINT,
    candidate_resolved_language SMALLINT,
    ...
    CHECK (
        (state = 4 AND candidate_summary_utf8 IS NOT NULL
          AND length(candidate_summary_utf8) BETWEEN 1 AND 65536
          AND candidate_resolved_length BETWEEN 1 AND 3
          AND candidate_resolved_language BETWEEN 1 AND 4
          AND candidate_completeness IN (1, 2)
          AND candidate_confidence_basis_points BETWEEN 0 AND 10000
          AND rejection_code IS NULL)
        OR
        (state = 5 AND candidate_summary_utf8 IS NULL AND rejection_code BETWEEN 1 AND 4)
        OR
        (state IN (1, 2, 3) AND candidate_summary_utf8 IS NULL AND rejection_code IS NULL)
    )
);

CREATE INDEX communication_summary_recoverable_idx
ON makosh_data.communication_summary_runs (
    logical_owner_id,
    state,
    state_revision
);

CREATE TABLE makosh_data.communication_summary_inbox (
    logical_owner_id TEXT NOT NULL,
...
CREATE TABLE makosh_data.communication_summary_outbox (
    logical_owner_id TEXT NOT NULL,
...
CREATE INDEX communication_summary_outbox_pending_idx
ON makosh_data.communication_summary_outbox (
    logical_owner_id,
...
CREATE TABLE makosh_data.communication_summary_realtime (
...
CREATE INDEX communication_summary_realtime_owner_idx
ON makosh_data.communication_summary_realtime (
    logical_owner_id,
    realtime_sequence
);
