...
use makosh_obligations_api::{
    ...
    OBLIGATIONS_REVIEWED_CANDIDATE_MAX_BLOB_BYTES_V1, wire::ReviewedObligationCandidateContentV1,
};
use makosh_obligations_persistence::{ObligationsBlobCleanupV1, ObligationsBlobReceiptV1};
...
pub(crate) enum ObligationsBlobErrorV1 {
    InvalidReceipt,
    Unavailable,
}
...
pub(crate) fn transfer_candidate_v1(
    ...
    source: &ObligationsBlobReceiptV1,
) -> Result<ObligationsBlobCleanupV1, ObligationsBlobErrorV1> {
    validate_receipt(source)?;
    let transfer = request_managed_blob_custody_transfer_v2(
        channel,
        dispatcher,
        ManagedBlobCustodyTransferRequestV1 {
            capability_id: OBLIGATIONS_REVIEWED_CANDIDATE_BLOB_CAPABILITY_ID_V1,
    ...
    BlobDataClient::new(&transfer.data_socket_path)
        ...
        .map_err(|_| ObligationsBlobErrorV1::Unavailable)?;
...
pub(crate) fn read_candidate_v1(
    channel: &mut ManagedControlChannelV2<UnixStream>,
    dispatcher: &mut dyn ManagedControlRequestDispatcherV2<UnixStream>,
    receipt: &ObligationsBlobCleanupV1,
) -> Result<Zeroizing<Vec<u8>>, ObligationsBlobErrorV1> {
    ...
    {
        return Err(ObligationsBlobErrorV1::InvalidReceipt);
    }
    Ok(bytes)
}
...
pub(crate) fn decode_candidate_content_v1(
    bytes: &[u8],
) -> Result<ReviewedObligationCandidateContentV1, ObligationsBlobErrorV1> {
    let content = ReviewedObligationCandidateContentV1::decode(bytes)
        .map_err(|_| ObligationsBlobErrorV1::InvalidReceipt)?;
    ...
    if !valid(&content.statement, 240)
        || content
            .condition
            .as_deref()
            .is_some_and(|value| !valid(value, 4_000))
        || content.due_at.as_ref().is_some_and(|value| {
            value.unix_seconds <= 0 || !(0..1_000_000_000).contains(&value.nanos)
        })
        || id16(&content.obligated_party_id).is_err()
        || content
            .beneficiary_party_id
            .as_deref()
            .is_some_and(|value| id16(value).is_err())
        || content.evidence_links.iter().any(|value| {
            id16(&value.evidence_link_id).is_err()
                || value.evidence_owner_id.trim().is_empty()
                || value.evidence_owner_id.len() > 128
                || value.evidence_owner_id.chars().any(char::is_control)
                || id16(&value.evidence_record_id).is_err()
...
pub(crate) fn release_candidate_v1(
    ...
    dispatcher: &mut dyn ManagedControlRequestDispatcherV2<UnixStream>,
    command_id: [u8; 16],
    cleanup: &ObligationsBlobCleanupV1,
    accepted: bool,
) -> Result<(), ObligationsBlobErrorV1> {
...
fn validate_receipt(receipt: &ObligationsBlobReceiptV1) -> Result<(), ObligationsBlobErrorV1> {
    if receipt.reference_id.iter().all(|byte| *byte == 0)
        || !(1..=OBLIGATIONS_REVIEWED_CANDIDATE_MAX_BLOB_BYTES_V1).contains(&receipt.declared_bytes)
        ...
        || receipt.custody_transfer_source_proof.len()
            > makosh_obligations_api::OBLIGATIONS_REVIEWED_CANDIDATE_MAX_PROOF_BYTES_V1
    {
        return Err(ObligationsBlobErrorV1::InvalidReceipt);
    }
    Ok(())
...
fn validate_cleanup(receipt: &ObligationsBlobCleanupV1) -> Result<(), ObligationsBlobErrorV1> {
    if receipt.reference_id.iter().all(|byte| *byte == 0)
        || !(1..=OBLIGATIONS_REVIEWED_CANDIDATE_MAX_BLOB_BYTES_V1).contains(&receipt.declared_bytes)
        || receipt.sha256.iter().all(|byte| *byte == 0)
...
fn id16(value: &[u8]) -> Result<[u8; 16], ObligationsBlobErrorV1> {
    value
        .try_into()
        .ok()
        .filter(|value: &[u8; 16]| value.iter().any(|byte| *byte != 0))
        .ok_or(ObligationsBlobErrorV1::InvalidReceipt)
}

fn classify_blob_client_error_v1(error: BlobClientError) -> ObligationsBlobErrorV1 {
    match error {
        BlobClientError::Rejected(_) | BlobClientError::InvalidSessionRequest => {
            ObligationsBlobErrorV1::InvalidReceipt
        }
        _ => ObligationsBlobErrorV1::Unavailable,
    }
}

fn release_operation_id(command_id: [u8; 16]) -> [u8; 16] {
    let mut digest = Sha256::new();
    digest.update(b"makosh.obligations.reviewed-candidate.release.v1\0");
...
mod tests {
    ...
    fn denied_custody_is_terminal_while_transport_outage_is_retryable() {
        ...
        assert_eq!(
            ...
            ObligationsBlobErrorV1::Unavailable
        );
    }
}
