...
pub const PACKAGE: &str = "makosh-identity-resolution-assembly";
pub const IDENTITY_RESOLUTION_FRAGMENT_FILE_V1: &str = "identity-resolution.release-artifacts.json";
...
pub fn materialize_identity_resolution_release_assembly_v1(
    output: &Path,
    build_id: &str,
    runtime: &Path,
    ...
    let descriptor = identity_resolution_module_descriptor_v1(build_id);
    let settings_bytes = identity_resolution_settings_schema_bytes_v1();
    let settings =
        decode_settings_schema_v1(&settings_bytes).map_err(|_| AssemblyErrorV1::InvalidArtifact)?;
    let storage = identity_resolution_storage_bundle_v1();
    ...
    let paths = PathsV1 {
        descriptor: output.join("identity-resolution.runtime.descriptor.pb"),
        settings_schema: output.join("identity-resolution.runtime.settings.pb"),
        storage_bundle: output.join("identity-resolution.storage.bundle.pb"),
        artifact_fragment: output.join(IDENTITY_RESOLUTION_FRAGMENT_FILE_V1),
    };
    let utf8 = |p: &Path| {
        p.to_str()
            .map(str::to_owned)
    ...
    let fragment = FragmentV1 {
        ...
        artifacts: vec![
            ArtifactV1::Runtime(RuntimeInputV1 {
                ...
                artifact_id: "identity_resolution.runtime.v1".to_owned(),
                relative_path: "bin/makosh-identity-resolution-runtime".to_owned(),
                source_path: utf8(runtime)?,
                required: true,
                descriptor: ContractInputV1 {
                    relative_path: "contracts/identity-resolution.runtime.descriptor.pb".to_owned(),
                    source_path: utf8(&paths.descriptor)?,
                },
                settings_schema: ContractInputV1 {
                    relative_path: "contracts/identity-resolution.runtime.settings.pb".to_owned(),
                    source_path: utf8(&paths.settings_schema)?,
                },
            }),
            ArtifactV1::Storage(StorageInputV1 {
                artifact_kind: "storage_bundle".to_owned(),
                artifact_id: "identity_resolution.storage.v1".to_owned(),
                relative_path: "storage/identity-resolution.storage.bundle.pb".to_owned(),
...
mod tests {
    ...
    fn materializes_compiler_shape() {
        ...
        let paths = materialize_identity_resolution_release_assembly_v1(
            &root.join("out"),
            "build-1",
            &runtime,
        )
