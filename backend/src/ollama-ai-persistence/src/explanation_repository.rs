...
impl OllamaAiPersistenceV1 {
    pub async fn accept_explanation_run(
        ...
        .rows_affected()
            == 1;
        let persisted =
            load_explanation_run_in_transaction(&mut transaction, logical_owner_id, run.request_id)
                .await?
    ...
    pub async fn load_explanation_run(
        ...
        let mut transaction = self.begin_owner_transaction(logical_owner_id).await?;
        let persisted =
            load_explanation_run_in_transaction(&mut transaction, logical_owner_id, request_id)
                .await?;
        transaction.commit().await.map_err(storage_error)?;
        Ok(persisted)
    }
