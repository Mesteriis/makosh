use makosh_documents_assembly::materialize_documents_release_assembly_v1;
...
fn main() -> ExitCode {
    let Some(arguments) = arguments(std::env::args().skip(1).collect()) else {
        return fail(
            "usage: makosh-documents-assembly --build-id <id> --output-dir <absolute-path> --runtime <absolute-path>",
        );
    };
    if materialize_documents_release_assembly_v1(
    ...
    println!("documents-release-assembly: ok");
    ExitCode::SUCCESS
}
