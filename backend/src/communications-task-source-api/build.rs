...
fn main() {
    ...
    let descriptor = output.join("communications-task-source-v1.bin");
    ...
    std::fs::write(
        output.join("communications_task_source_schema.rs"),
        format!("pub const COMMUNICATIONS_TASK_SOURCE_SCHEMA_SHA256: [u8; 32] = {digest:?};\n"),
    )
    .expect("Communications task source schema digest must be written");
}
