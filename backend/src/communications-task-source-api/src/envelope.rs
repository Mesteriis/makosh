...
use crate::{
    COMMUNICATION_TASK_SOURCE_MAX_BYTES_V1, COMMUNICATION_TASK_SOURCE_MAX_PROOF_BYTES_V1,
    COMMUNICATION_TASK_SOURCE_PREPARE_CONTRACT_NAME_V1,
    COMMUNICATION_TASK_SOURCE_PREPARED_CONTRACT_NAME_V1,
    COMMUNICATION_TASK_SOURCE_REJECTED_CONTRACT_NAME_V1,
    COMMUNICATIONS_TASK_SOURCE_CAPABILITY_ID_V1, COMMUNICATIONS_TASK_SOURCE_CONTRACT_MAJOR_V1,
    COMMUNICATIONS_TASK_SOURCE_CONTRACT_REVISION_V1, COMMUNICATIONS_TASK_SOURCE_OWNER_V1,
    COMMUNICATIONS_TASK_SOURCE_SCHEMA_SHA256,
    wire::{
...
pub struct CommunicationTaskSourceEnvelopeContextV1 {
...
pub enum CommunicationTaskSourceEnvelopeBuildErrorV1 {
    InvalidContext,
    InvalidPayload,
    InvalidEnvelope,
    OutboxRejected,
...
pub fn build_communication_task_source_prepare_outbox_record_v1(
    run_id: [u8; 16],
    source_message_id: [u8; 16],
    expected_source_revision: u64,
    logical_owner_id: &str,
    deadline_unix_seconds: i64,
    context: &CommunicationTaskSourceEnvelopeContextV1,
) -> Result<OutboxRecordV1, CommunicationTaskSourceEnvelopeBuildErrorV1> {
    ...
    if deadline_unix_seconds <= context.recorded_at_unix_seconds {
        return Err(CommunicationTaskSourceEnvelopeBuildErrorV1::InvalidPayload);
    }
    let payload = PrepareCommunicationTaskSourceCommandV1 {
    ...
    build_envelope(
        ...
        Semantics::Command(CommandMetadataV1 {
            ...
            target_capability: COMMUNICATIONS_TASK_SOURCE_CAPABILITY_ID_V1.to_owned(),
            idempotency_key: Sha256::digest(
                [b"communications-task-source-prepare-v1".as_slice(), &run_id].concat(),
            )
            .to_vec(),
...
pub fn build_communication_task_source_prepared_outbox_record_v1(
    command_message_id: [u8; 16],
    payload: CommunicationTaskSourcePreparedV1,
    context: &CommunicationTaskSourceEnvelopeContextV1,
) -> Result<OutboxRecordV1, CommunicationTaskSourceEnvelopeBuildErrorV1> {
    validate_context(context)?;
...
pub fn build_communication_task_source_rejected_outbox_record_v1(
    command_message_id: [u8; 16],
    payload: CommunicationTaskSourceRejectedV1,
    context: &CommunicationTaskSourceEnvelopeContextV1,
) -> Result<OutboxRecordV1, CommunicationTaskSourceEnvelopeBuildErrorV1> {
    validate_context(context)?;
...
fn build_envelope(
    ...
    let envelope = DurableEnvelopeV1 {
        ...
        contract: Some(ContractRefV1 {
            owner: COMMUNICATIONS_TASK_SOURCE_OWNER_V1.to_owned(),
            name: contract_name.to_owned(),
            major: COMMUNICATIONS_TASK_SOURCE_CONTRACT_MAJOR_V1,
            revision: COMMUNICATIONS_TASK_SOURCE_CONTRACT_REVISION_V1,
            schema_sha256: COMMUNICATIONS_TASK_SOURCE_SCHEMA_SHA256.to_vec(),
        }),
    ...
    validate_envelope_v1(&envelope)
        .map_err(|_| CommunicationTaskSourceEnvelopeBuildErrorV1::InvalidEnvelope)?;
...
fn validate_context(
    context: &CommunicationTaskSourceEnvelopeContextV1,
) -> Result<(), CommunicationTaskSourceEnvelopeBuildErrorV1> {
    ...
    {
        return Err(CommunicationTaskSourceEnvelopeBuildErrorV1::InvalidContext);
    }
    Ok(())
}
...
fn validate_prepared_payload(
    payload: &CommunicationTaskSourcePreparedV1,
) -> Result<[u8; 16], CommunicationTaskSourceEnvelopeBuildErrorV1> {
    let run_id = id16(&payload.run_id)?;
...
fn valid_source_receipt(receipt: &CommunicationTaskSourceContentReceiptV1) -> bool {
    receipt.reference_id.len() == 16
        && receipt.reference_id.iter().any(|byte| *byte != 0)
        && (1..=COMMUNICATION_TASK_SOURCE_MAX_BYTES_V1).contains(&receipt.declared_bytes)
        ...
        && receipt.custody_transfer_source_proof.len()
            <= COMMUNICATION_TASK_SOURCE_MAX_PROOF_BYTES_V1
}
...
fn id16(bytes: &[u8]) -> Result<[u8; 16], CommunicationTaskSourceEnvelopeBuildErrorV1> {
    bytes
        .try_into()
        .ok()
        .filter(valid_id)
        .ok_or(CommunicationTaskSourceEnvelopeBuildErrorV1::InvalidPayload)
}

fn result_message_id(label: &[u8], run_id: &[u8; 16]) -> [u8; 16] {
    let mut hasher = Sha256::new();
    hasher.update(b"communications-task-source-result-v1");
    hasher.update(label);
...
fn outbox_error(_: OutboxRecordError) -> CommunicationTaskSourceEnvelopeBuildErrorV1 {
    CommunicationTaskSourceEnvelopeBuildErrorV1::OutboxRejected
}
...
mod tests {
    ...
    fn prepared_payload() -> CommunicationTaskSourcePreparedV1 {
        CommunicationTaskSourcePreparedV1 {
            ...
            source_content: Some(CommunicationTaskSourceContentReceiptV1 {
    ...
    fn command_contains_only_canonical_ids_and_exact_capability() {
        let record = build_communication_task_source_prepare_outbox_record_v1(
            [1; 16],
            [2; 16],
            3,
            "owner-1",
        ...
        assert_eq!(command.target_capability, "communications.task-source.v1");
    ...
    fn prepared_result_is_receipt_only_and_hash_bound() {
        let record = build_communication_task_source_prepared_outbox_record_v1(
            [9; 16],
            prepared_payload(),
            &context(),
        )
        ...
        assert_eq!(
            ...
            "communication_task_source_prepared"
        );
    ...
    fn rejects_invalid_revision_and_oversized_proof() {
        assert_eq!(
            ...
            Err(CommunicationTaskSourceEnvelopeBuildErrorV1::InvalidPayload)
        );
        ...
        payload
            ...
            vec![8; COMMUNICATION_TASK_SOURCE_MAX_PROOF_BYTES_V1 + 1];
        assert_eq!(
            build_communication_task_source_prepared_outbox_record_v1([9; 16], payload, &context(),),
            Err(CommunicationTaskSourceEnvelopeBuildErrorV1::InvalidPayload)
        );
    }
}
