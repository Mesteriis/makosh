...
pub(crate) async fn verify_exact_outbox(
    ...
    {
        return Err(ReviewedTaskCandidatePromotionPersistenceErrorV1::OutboxConflict);
    }
    Ok(())
}
...
impl ReviewedTaskCandidatePromotionPersistenceV1 {
    pub async fn unpublished_events(
        ...
        logical_owner_id: &str,
        limit: u16,
    ) -> Result<Vec<UnpublishedPromotionEventV1>, ReviewedTaskCandidatePromotionPersistenceErrorV1>
    {
...
fn event_from_row(
...
) -> Result<UnpublishedPromotionEventV1, ReviewedTaskCandidatePromotionPersistenceErrorV1> {
    ...
    {
        return Err(ReviewedTaskCandidatePromotionPersistenceErrorV1::InvalidRow);
    }
    Ok(event)
}
