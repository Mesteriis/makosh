...
pub struct DelayedDeliveryIntentRequestV1 {
...
pub enum DelayedDeliveryIntentResponseV1 {
...
impl DelayedDeliveryIntentRequestV1 {
    #[must_use]
...
pub fn decode_delivery_intent_response_v1(
    ...
    payload: &[u8],
) -> Result<DelayedDeliveryIntentResponseV1, DeliveryIntentRequestErrorV1> {
    ...
    if error == DeliveryIntentErrorCodeV1::DeliveryIntentErrorCodeUnavailable {
        return Ok(DelayedDeliveryIntentResponseV1::Retryable);
    }
    if error != DeliveryIntentErrorCodeV1::DeliveryIntentErrorCodeUnspecified {
        return Ok(DelayedDeliveryIntentResponseV1::Rejected);
    }
    ...
    Ok(DelayedDeliveryIntentResponseV1::Accepted { intent_id })
}
...
mod tests {
    ...
    #[test]
    fn accepts_only_the_stable_delivery_operation_receipt() {
        let payload = SubmitDeliveryIntentResponseV1 {
            intent_id: vec![7; 16],
        ...
        .encode_to_vec();
        assert_eq!(
            decode_delivery_intent_response_v1([7; 16], &payload),
            Ok(DelayedDeliveryIntentResponseV1::Accepted { intent_id: [7; 16] })
        );
        assert_eq!(
            decode_delivery_intent_response_v1([8; 16], &payload),
