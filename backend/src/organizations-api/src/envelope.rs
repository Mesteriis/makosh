...
use crate::{
    ORGANIZATIONS_MODULE_ID_V1, client_wire::OrganizationChangedV1,
    organizations_lifecycle_event_contract_reference_v1,
};

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct OrganizationsEnvelopeContextV1 {
...
pub enum OrganizationsEnvelopeBuildErrorV1 {
    InvalidContext,
    InvalidPayload,
    InvalidEnvelope,
    OutboxRejected,
...
pub fn build_organization_changed_outbox_record_v1(
    operation_id: [u8; 16],
    payload: OrganizationChangedV1,
    context: &OrganizationsEnvelopeContextV1,
) -> Result<OutboxRecordV1, OrganizationsEnvelopeBuildErrorV1> {
    validate_context(context)?;
    let event_id = id16(&payload.event_id)?;
    let organization_id = id16(&payload.organization_id)?;
    let occurred_at = payload
        .occurred_at
        .as_ref()
        .filter(|value| value.unix_seconds > 0 && (0..1_000_000_000).contains(&value.nanos))
        .ok_or(OrganizationsEnvelopeBuildErrorV1::InvalidPayload)?;
    if !nonzero(&operation_id)
        || payload.organization_revision == 0
        || payload.state == 0
        || !valid_owner(&payload.logical_owner_id)
    {
        return Err(OrganizationsEnvelopeBuildErrorV1::InvalidPayload);
    }
    let contract = organizations_lifecycle_event_contract_reference_v1();
    let envelope = DurableEnvelopeV1 {
        ...
        source: Some(SourceRefV1 {
            ...
            runtime_instance_id: digest(
                b"organizations-runtime-instance-v1",
        ...
        partition_key: organization_id.to_vec(),
        causation_message_id: operation_id.to_vec(),
        correlation_id: organization_id.to_vec(),
    ...
    validate_envelope_v1(&envelope)
        .map_err(|_| OrganizationsEnvelopeBuildErrorV1::InvalidEnvelope)?;
    OutboxRecordV1::accept(envelope.encode_to_vec()).map_err(outbox_error)
}

fn validate_context(
    context: &OrganizationsEnvelopeContextV1,
) -> Result<(), OrganizationsEnvelopeBuildErrorV1> {
    if context.module_id != ORGANIZATIONS_MODULE_ID_V1
    ...
    {
        return Err(OrganizationsEnvelopeBuildErrorV1::InvalidContext);
    }
    Ok(())
}

fn id16(bytes: &[u8]) -> Result<[u8; 16], OrganizationsEnvelopeBuildErrorV1> {
    let value: [u8; 16] = bytes
        .try_into()
        .map_err(|_| OrganizationsEnvelopeBuildErrorV1::InvalidPayload)?;
    nonzero(&value)
        .then_some(value)
        .ok_or(OrganizationsEnvelopeBuildErrorV1::InvalidPayload)
}
...
fn outbox_error(_: OutboxRecordError) -> OrganizationsEnvelopeBuildErrorV1 {
    OrganizationsEnvelopeBuildErrorV1::OutboxRejected
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::client_wire::{OrganizationChangedV1, OrganizationStateV1, TimestampV1};

    #[test]
    fn lifecycle_event_excludes_private_profile_and_source_content() {
        let record = build_organization_changed_outbox_record_v1(
            ...
            &OrganizationsEnvelopeContextV1 {
                ...
                runtime_instance_id: "organizations-runtime-1".to_owned(),
        ...
        let decoded = OrganizationChangedV1::decode(envelope.payload.as_slice()).expect("payload");
        assert_eq!(decoded.organization_id, vec![3; 16]);
        for private in [
            b"Private legal name".as_slice(),
            b"private-source-record".as_slice(),
        ] {
            assert!(
                !record
                    .exact_bytes()
