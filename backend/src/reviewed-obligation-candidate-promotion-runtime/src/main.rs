...
fn main() -> Result<(), String> {
    ...
    match command.to_str() {
        Some("export-storage-bundle") => export(
            &mut arguments,
            reviewed_obligation_candidate_promotion_storage_bundle_v1().encode_to_vec(),
        ),
        Some("export-module-descriptor") => {
            let build_id = required_string(&mut arguments, "build id")?;
            export(
                &mut arguments,
                reviewed_obligation_candidate_promotion_module_descriptor_v1(&build_id)
                    .encode_to_vec(),
            )
        }
    ...
    I: Iterator<Item = OsString>,
{
    ...
    let admission = ReviewedObligationCandidatePromotionRuntimeAdmissionV1 {
    ...
    let mut runtime = executor
        .block_on(ReviewedObligationCandidatePromotionManagedRuntimeV1::open(
    ...
    loop {
        ...
        retry(executor.block_on(runtime.consume_obligation_created_once(now)))?;
        retry(executor.block_on(runtime.consume_obligation_rejected_once(now)))?;
...
fn retry(
    result: Result<bool, ReviewedObligationCandidatePromotionManagedRuntimeErrorV1>,
) -> Result<(), String> {
    match result {
        Ok(_)
        | Err(ReviewedObligationCandidatePromotionManagedRuntimeErrorV1::EventUnavailable)
        | Err(ReviewedObligationCandidatePromotionManagedRuntimeErrorV1::Unavailable)
        | Err(ReviewedObligationCandidatePromotionManagedRuntimeErrorV1::Persistence(
            makosh_reviewed_obligation_candidate_promotion_persistence::ReviewedObligationCandidatePromotionPersistenceErrorV1::StorageUnavailable,
        )) => Ok(()),
        Err(error) => Err(runtime_error(error)),
    }
}

fn runtime_error(error: ReviewedObligationCandidatePromotionManagedRuntimeErrorV1) -> String {
    let code = match error {
        ReviewedObligationCandidatePromotionManagedRuntimeErrorV1::Admission => "admission",
        ...
        ReviewedObligationCandidatePromotionManagedRuntimeErrorV1::Persistence(_) => "persistence",
        ReviewedObligationCandidatePromotionManagedRuntimeErrorV1::Unavailable => "unavailable",
    };
