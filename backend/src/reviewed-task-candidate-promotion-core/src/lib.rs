...
pub enum ReviewedTaskCandidatePromotionIdentityErrorV1 {
    ...
    InvalidTasksResultMessageId,
    InvalidReviewId,
    InvalidCandidateId,
    InvalidDecisionRevision,
}

pub fn derive_reviewed_task_candidate_command_id_v1(
    approval_message_id: [u8; 16],
    review_id: [u8; 16],
    candidate_id: [u8; 16],
    decision_revision: u64,
) -> Result<[u8; 16], ReviewedTaskCandidatePromotionIdentityErrorV1> {
    if !nonzero(&approval_message_id) {
        return Err(ReviewedTaskCandidatePromotionIdentityErrorV1::InvalidApprovalMessageId);
    }
    if !nonzero(&review_id) {
        return Err(ReviewedTaskCandidatePromotionIdentityErrorV1::InvalidReviewId);
    }
    if !nonzero(&candidate_id) {
        return Err(ReviewedTaskCandidatePromotionIdentityErrorV1::InvalidCandidateId);
    }
...
pub fn derive_reviewed_task_candidate_result_id_v1(
    tasks_result_message_id: [u8; 16],
    command_id: [u8; 16],
    review_id: [u8; 16],
) -> Result<[u8; 16], ReviewedTaskCandidatePromotionIdentityErrorV1> {
    if !nonzero(&tasks_result_message_id) {
        return Err(ReviewedTaskCandidatePromotionIdentityErrorV1::InvalidTasksResultMessageId);
    }
...
mod tests {
    ...
    fn command_identity_is_stable_and_binds_decision_revision() {
        let first = derive_reviewed_task_candidate_command_id_v1([1; 16], [2; 16], [3; 16], 4)
            .expect("command ID");
        assert_eq!(
            first,
            derive_reviewed_task_candidate_command_id_v1([1; 16], [2; 16], [3; 16], 4)
                .expect("replayed command ID")
        );
        assert_ne!(
            first,
            derive_reviewed_task_candidate_command_id_v1([1; 16], [2; 16], [3; 16], 5)
                .expect("next decision command ID")
        );
    }

    #[test]
    fn result_identity_is_tasks_result_and_review_bound() {
        let first = derive_reviewed_task_candidate_result_id_v1([4; 16], [5; 16], [6; 16])
            .expect("result ID");
        assert_ne!(
            first,
            derive_reviewed_task_candidate_result_id_v1([7; 16], [5; 16], [6; 16])
