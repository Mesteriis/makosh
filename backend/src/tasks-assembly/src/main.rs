...
use makosh_tasks_assembly::materialize_tasks_release_assembly_v1;
...
fn main() -> ExitCode {
    let Some(arguments) = arguments(std::env::args().skip(1).collect()) else {
        return fail(
            "usage: makosh-tasks-assembly --build-id <id> \
             --output-dir <absolute-path> --runtime <absolute-path>",
        );
    };
    if materialize_tasks_release_assembly_v1(
    ...
    println!("tasks-release-assembly: ok");
    ExitCode::SUCCESS
}
