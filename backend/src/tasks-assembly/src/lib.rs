//! Unsigned Tasks release assembly.
//!
...
pub const TASKS_ASSEMBLY_FRAGMENT_VERSION_V1: u32 = 1;
pub const TASKS_RUNTIME_ARTIFACT_ID_V1: &str = "tasks.runtime.v1";
pub const TASKS_STORAGE_ARTIFACT_ID_V1: &str = "tasks.storage.v1";
pub const TASKS_DESCRIPTOR_FILE_V1: &str = "tasks.runtime.descriptor.pb";
pub const TASKS_SETTINGS_FILE_V1: &str = "tasks.runtime.settings.pb";
...
#[derive(Clone, Debug, Eq, PartialEq, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
pub struct TasksReleaseArtifactFragmentV1 {
    pub version: u32,
    pub owner_id: String,
    pub module_id: String,
    pub artifacts: Vec<TasksReleaseArtifactInputV1>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct TasksReleaseAssemblyPathsV1 {
...
pub enum TasksReleaseAssemblyErrorV1 {
    InvalidInput,
...
pub fn materialize_tasks_release_assembly_v1(
    output_directory: &Path,
    build_id: &str,
    runtime_source: &Path,
) -> Result<TasksReleaseAssemblyPathsV1, TasksReleaseAssemblyErrorV1> {
    validate_inputs(output_directory, build_id, runtime_source)?;
    let descriptor = tasks_module_descriptor_v1(build_id);
    let settings = tasks_settings_schema_v1();
    let storage = tasks_storage_bundle_v1();
    ...
    {
        return Err(TasksReleaseAssemblyErrorV1::InvalidCanonicalArtifact);
    }

    let paths = TasksReleaseAssemblyPathsV1 {
        descriptor: output_directory.join(TASKS_DESCRIPTOR_FILE_V1),
        settings_schema: output_directory.join(TASKS_SETTINGS_FILE_V1),
        storage_bundle: output_directory.join(TASKS_STORAGE_BUNDLE_FILE_V1),
        artifact_fragment: output_directory.join(TASKS_ARTIFACT_FRAGMENT_FILE_V1),
    };
    ...
    let fragment_bytes = serde_json::to_vec_pretty(&fragment)
        .map_err(|_| TasksReleaseAssemblyErrorV1::FragmentEncodingFailed)?;

    let mut directory = DirBuilder::new();
    directory.mode(0o700);
    directory
        .create(output_directory)
        .map_err(|_| TasksReleaseAssemblyErrorV1::OutputUnavailable)?;
    let writes = [
    ...
    for (path, bytes) in writes {
        if write_new_private_file(path, &bytes).is_err() {
            ...
            return Err(TasksReleaseAssemblyErrorV1::ArtifactWriteFailed);
        }
    }
    Ok(paths)
}
...
fn validate_inputs(
    ...
    build_id: &str,
    runtime_source: &Path,
) -> Result<(), TasksReleaseAssemblyErrorV1> {
    ...
    if !output_directory.is_absolute()
        ...
        || !valid_runtime
    {
        return Err(TasksReleaseAssemblyErrorV1::InvalidInput);
    }
    Ok(())
}
...
fn artifact_fragment(
    ...
    module_id: &str,
    runtime_source: &Path,
    paths: &TasksReleaseAssemblyPathsV1,
) -> Result<TasksReleaseArtifactFragmentV1, TasksReleaseAssemblyErrorV1> {
    let artifacts = vec![
        TasksReleaseArtifactInputV1::ModuleRuntime(ModuleRuntimeArtifactInputV1 {
            artifact_kind: "module_runtime".to_owned(),
            artifact_id: TASKS_RUNTIME_ARTIFACT_ID_V1.to_owned(),
        ...
        TasksReleaseArtifactInputV1::StorageBundle(StorageBundleArtifactInputV1 {
            artifact_kind: "storage_bundle".to_owned(),
            artifact_id: TASKS_STORAGE_ARTIFACT_ID_V1.to_owned(),
    ...
    Ok(TasksReleaseArtifactFragmentV1 {
        version: TASKS_ASSEMBLY_FRAGMENT_VERSION_V1,
...
fn utf8_path(path: &Path) -> Result<String, TasksReleaseAssemblyErrorV1> {
    path.to_str()
        .filter(|value| !value.is_empty())
        .map(str::to_owned)
        .ok_or(TasksReleaseAssemblyErrorV1::InvalidInput)
}
...
mod tests {
    ...
    #[test]
    fn materializes_exact_unsigned_tasks_artifacts() {
        let root = temporary_directory();
        let runtime = root.join("runtime");
        fs::write(&runtime, b"runtime").expect("runtime fixture");
        let output = root.join("assembly");
        let paths =
            materialize_tasks_release_assembly_v1(&output, "build-1", &runtime).expect("assembly");
        ...
        let fragment: TasksReleaseArtifactFragmentV1 =
    ...
    fn refuses_existing_output_and_symlink_runtime() {
        ...
        assert_eq!(
            materialize_tasks_release_assembly_v1(&output, "build-1", &runtime),
            Err(TasksReleaseAssemblyErrorV1::InvalidInput)
        );
        let symlink = root.join("runtime-link");
        std::os::unix::fs::symlink(&runtime, &symlink).expect("symlink fixture");
        assert_eq!(
            materialize_tasks_release_assembly_v1(&root.join("other"), "build-1", &symlink,),
            Err(TasksReleaseAssemblyErrorV1::InvalidInput)
        );
        fs::remove_dir_all(root).expect("cleanup");
    }
