use makosh_relationships_assembly::materialize_relationships_release_assembly_v1;
...
fn main() -> ExitCode {
    let Some(arguments) = arguments(std::env::args().skip(1).collect()) else {
        return fail(
            "usage: makosh-relationships-assembly --build-id <id> --output-dir <absolute-path> --runtime <absolute-path>",
        );
    };
    if materialize_relationships_release_assembly_v1(
    ...
    println!("relationships-release-assembly: ok");
    ExitCode::SUCCESS
}
