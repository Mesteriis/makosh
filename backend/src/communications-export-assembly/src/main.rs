...
use makosh_communications_export_assembly::materialize_communications_export_release_assembly_v1;
...
fn main() -> ExitCode {
    match arguments(std::env::args().skip(1).collect()) {
        Some(arguments) => {
            if materialize_communications_export_release_assembly_v1(
            ...
            println!("communications-export-release-assembly: ok");
            ExitCode::SUCCESS
        }
        None => fail(
            "usage: makosh-communications-export-assembly --build-id <id> \
...
fn fail(message: &str) -> ExitCode {
    eprintln!("communications-export-release-assembly: {message}");
    ExitCode::FAILURE
}
...
mod tests {
    ...
    fn parses_exact_options() {
        let parsed = arguments(vec![
            "--runtime".to_owned(),
            "/tmp/makosh-communications-export-runtime".to_owned(),
            "--build-id".to_owned(),
            "build-1".to_owned(),
            "--output-dir".to_owned(),
            "/tmp/communications-export-assembly".to_owned(),
        ])
        .expect("arguments");
        assert_eq!(parsed.build_id, "build-1");
        assert_eq!(
            parsed.runtime,
            PathBuf::from("/tmp/makosh-communications-export-runtime")
        );
    }

    #[test]
    fn rejects_missing_or_duplicate_options() {
