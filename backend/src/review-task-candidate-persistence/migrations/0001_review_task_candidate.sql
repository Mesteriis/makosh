CREATE TABLE makosh_data.review_task_candidate_submissions (
...
CREATE INDEX review_task_candidate_submission_recovery_idx
ON makosh_data.review_task_candidate_submissions (
    logical_owner_id,
    completed,
    received_at_unix_millis
);

CREATE TABLE makosh_data.review_task_candidate_state (
    logical_owner_id TEXT NOT NULL,
    ...
    CHECK (assignee_label_hint IS NULL OR char_length(assignee_label_hint) BETWEEN 1 AND 120),
    ...
    CHECK (
        ...
        OR (state = 2 AND promotion_status BETWEEN 2 AND 4
            ...
            AND ((promotion_status = 3 AND length(promoted_task_id) = 16)
                OR (promotion_status IN (2, 4) AND promoted_task_id IS NULL)))
...
CREATE INDEX review_task_candidate_promotion_recovery_idx
ON makosh_data.review_task_candidate_state (
    logical_owner_id,
    state,
    promotion_status,
    review_revision
...
CREATE TABLE makosh_data.review_task_candidate_operations (
    logical_owner_id TEXT NOT NULL,
...
CREATE TABLE makosh_data.review_task_candidate_promotion_inbox (
    logical_owner_id TEXT NOT NULL,
...
CREATE TABLE makosh_data.review_task_candidate_outbox (
    logical_owner_id TEXT NOT NULL,
...
CREATE INDEX review_task_candidate_outbox_pending_idx
ON makosh_data.review_task_candidate_outbox (
    logical_owner_id,
...
CREATE TABLE makosh_data.review_task_candidate_realtime (
...
CREATE INDEX review_task_candidate_realtime_owner_idx
ON makosh_data.review_task_candidate_realtime (
    logical_owner_id,
    realtime_sequence
);
