...
use crate::{
    ...
    ReviewTaskCandidateOutboxRecordV1, ReviewTaskCandidatePageV1,
    ReviewTaskCandidatePersistenceErrorV1, ReviewTaskCandidateRealtimeTransitionV1,
    model::{
        REVIEW_TASK_CANDIDATE_MAX_PAGE_SIZE_V1, REVIEW_TASK_CANDIDATE_OUTBOX_LIMIT_V1,
        REVIEW_TASK_CANDIDATE_REALTIME_LIMIT_V1, REVIEW_TASK_CANDIDATE_RECOVERY_LIMIT_V1,
...
#[derive(Clone)]
pub struct ReviewTaskCandidatePersistenceV1 {
    pool: PgPool,
}

impl ReviewTaskCandidatePersistenceV1 {
    pub async fn connect_runtime(
        ...
        pgbouncer_port: u32,
        password: &str,
    ) -> Result<Self, ReviewTaskCandidatePersistenceErrorV1> {
        ...
        {
            return Err(ReviewTaskCandidatePersistenceErrorV1::StorageUnavailable);
        }
        let options = PgConnectOptions::new()
            .host(pgbouncer_host)
            .port(
                u16::try_from(pgbouncer_port)
                    .map_err(|_| ReviewTaskCandidatePersistenceErrorV1::StorageUnavailable)?,
            )
    ...
    pub async fn verify_storage_ready(&self) -> Result<(), ReviewTaskCandidatePersistenceErrorV1> {
    ...
    pub async fn reserve_submission(
        &self,
        input: ReserveReviewTaskCandidateSubmissionV1,
    ) -> Result<ReserveReviewTaskCandidateSubmissionOutcomeV1, ReviewTaskCandidatePersistenceErrorV1>
    {
        ...
        let outcome = if inserted == 1 {
            Ok(ReserveReviewTaskCandidateSubmissionOutcomeV1::Reserved(
                persisted,
            ))
        } else {
            Ok(ReserveReviewTaskCandidateSubmissionOutcomeV1::Existing(
                persisted,
            ))
        };
    ...
    ) -> Result<Vec<PersistedReviewTaskCandidateSubmissionV1>, ReviewTaskCandidatePersistenceErrorV1>
    {
        ...
        let submissions = sqlx::query(SELECT_RECOVERABLE_SUBMISSIONS)
            ...
            .bind(i64::from(REVIEW_TASK_CANDIDATE_RECOVERY_LIMIT_V1))
    ...
    pub async fn complete_submission(
        &self,
        input: CompleteReviewTaskCandidateSubmissionV1,
        ...
        let review = create_review_task_candidate_v1(input.draft.clone())
            .map_err(|_| ReviewTaskCandidatePersistenceErrorV1::InvalidTransition)?;
        let mut transaction = self
            .begin_owner_transaction(&input.logical_owner_id)
            .await?;
        ...
        if submission.completed {
            let review_id = submission
                .review_id
                .ok_or(ReviewTaskCandidatePersistenceErrorV1::SubmissionConflict)?;
            let review =
    ...
    pub async fn reject_submission(
        &self,
        input: RejectReviewTaskCandidateSubmissionV1,
    ) -> Result<(), ReviewTaskCandidatePersistenceErrorV1> {
        if !valid_identity(&input.logical_owner_id)
            || !nonzero(&input.submission_message_id)
        ...
        if submission.completed {
            ...
            return if submission.rejected {
                ...
                Err(ReviewTaskCandidatePersistenceErrorV1::SubmissionConflict)
            };
        }
        sqlx::query(
            "UPDATE makosh_data.review_task_candidate_submissions
    ...
    pub async fn load_review(
        ...
        let review = sqlx::query(SELECT_REVIEW_BY_ID)
            ...
            .await
            .map_err(storage_error)?
            .ok_or(ReviewTaskCandidatePersistenceErrorV1::NotFound)
    ...
    pub async fn list_reviews(
        &self,
        logical_owner_id: &str,
        input: ListReviewTaskCandidatesV1,
    ) -> Result<ReviewTaskCandidatePageV1, ReviewTaskCandidatePersistenceErrorV1> {
        if !valid_identity(logical_owner_id)
            || input.limit == 0
            || input.limit > REVIEW_TASK_CANDIDATE_MAX_PAGE_SIZE_V1
            || input
                .after_review_id
    ...
    pub async fn decide(
        &self,
        input: DecideReviewTaskCandidateOperationV1,
    ) -> Result<ReviewTaskCandidateDecisionOutcomeV1, ReviewTaskCandidatePersistenceErrorV1> {
        ...
        .map_err(storage_error)?
        {
            ...
            .await?;
            transaction.commit().await.map_err(storage_error)?;
            return Ok(ReviewTaskCandidateDecisionOutcomeV1::Replayed(review));
        }
        let current =
            load_review_for_update(&mut transaction, &input.logical_owner_id, &input.review_id)
                .await?;
        let next = decide_review_task_candidate_v1(
            &current,
    ...
    pub async fn persist_promotion_result(
        ...
        .await?;
        transaction.commit().await.map_err(storage_error)?;
        Ok(ReviewTaskCandidateInboxOutcomeV1::Applied(next))
    }

    pub async fn load_pending_promotions(
        &self,
        logical_owner_id: &str,
    ) -> Result<Vec<ReviewTaskCandidateV1>, ReviewTaskCandidatePersistenceErrorV1> {
        if !valid_identity(logical_owner_id) {
            return Err(ReviewTaskCandidatePersistenceErrorV1::InvalidInput);
        }
        let mut transaction = self.begin_owner_transaction(logical_owner_id).await?;
    ...
    pub async fn realtime_after(
        ...
        after_sequence: u64,
        limit: u16,
    ) -> Result<Vec<ReviewTaskCandidateRealtimeTransitionV1>, ReviewTaskCandidatePersistenceErrorV1>
    {
        if !valid_identity(logical_owner_id)
            || limit == 0
            || limit > REVIEW_TASK_CANDIDATE_REALTIME_LIMIT_V1
        {
            return Err(ReviewTaskCandidatePersistenceErrorV1::InvalidInput);
        }
        let mut transaction = self.begin_owner_transaction(logical_owner_id).await?;
    ...
    pub async fn unpublished_outbox(
        ...
        logical_owner_id: &str,
        limit: u16,
    ) -> Result<Vec<ReviewTaskCandidateOutboxRecordV1>, ReviewTaskCandidatePersistenceErrorV1> {
        if !valid_identity(logical_owner_id)
            || limit == 0
            || limit > REVIEW_TASK_CANDIDATE_OUTBOX_LIMIT_V1
        {
            return Err(ReviewTaskCandidatePersistenceErrorV1::InvalidInput);
        }
        let mut transaction = self.begin_owner_transaction(logical_owner_id).await?;
    ...
    pub async fn mark_outbox_published(
        ...
        .rows_affected();
        if affected != 1 {
            return Err(ReviewTaskCandidatePersistenceErrorV1::NotFound);
        }
        transaction.commit().await.map_err(storage_error)?;
        Ok(())
    }
    ...
    async fn begin_owner_transaction(
        &self,
        logical_owner_id: &str,
    ) -> Result<Transaction<'_, Postgres>, ReviewTaskCandidatePersistenceErrorV1> {
        if !valid_identity(logical_owner_id) {
            return Err(ReviewTaskCandidatePersistenceErrorV1::InvalidInput);
        }
...
async fn insert_outbox(
    transaction: &mut sqlx::Transaction<'_, sqlx::Postgres>,
    logical_owner_id: &str,
    record: &ReviewTaskCandidateOutboxRecordV1,
...
fn validate_decision(
    ...
    let event_valid = match input.decision {
        ...
        ReviewTaskCandidateDecisionV1::Reject => input.approved_event.is_none(),
    };
    if !valid_identity(&input.logical_owner_id)
...
fn same_submission(
    persisted: &PersistedReviewTaskCandidateSubmissionV1,
    input: &ReserveReviewTaskCandidateSubmissionV1,
) -> bool {
...
fn realtime_from_row(
...
) -> Result<ReviewTaskCandidateRealtimeTransitionV1, ReviewTaskCandidatePersistenceErrorV1> {
    let state = match row.try_get::<i16, _>("state").map_err(invalid_row)? {
        1 => ReviewTaskCandidateStateV1::Pending,
        2 => ReviewTaskCandidateStateV1::Approved,
        3 => ReviewTaskCandidateStateV1::Rejected,
    ...
    let promotion_status = match row
        ...
        .map_err(invalid_row)?
    {
        1 => ReviewTaskCandidatePromotionStatusV1::NotRequested,
        2 => ReviewTaskCandidatePromotionStatusV1::Pending,
        3 => ReviewTaskCandidatePromotionStatusV1::Succeeded,
        4 => ReviewTaskCandidatePromotionStatusV1::Failed,
        _ => return Err(ReviewTaskCandidatePersistenceErrorV1::InvalidRow),
    };
    Ok(ReviewTaskCandidateRealtimeTransitionV1 {
...
fn outbox_from_row(
    row: sqlx::postgres::PgRow,
) -> Result<ReviewTaskCandidateOutboxRecordV1, ReviewTaskCandidatePersistenceErrorV1> {
    ...
    if !valid_outbox(&record) {
        return Err(ReviewTaskCandidatePersistenceErrorV1::InvalidRow);
    }
    Ok(record)
}
...
fn invalid_row(_: sqlx::Error) -> ReviewTaskCandidatePersistenceErrorV1 {
