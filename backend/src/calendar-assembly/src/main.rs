...
use makosh_calendar_assembly::materialize_calendar_release_assembly_v1;
...
fn main() -> ExitCode {
    let Some(arguments) = arguments(std::env::args().skip(1).collect()) else {
        return fail(
            "usage: makosh-calendar-assembly --build-id <id> \
             --output-dir <absolute-path> --runtime <absolute-path>",
        );
    };
    if materialize_calendar_release_assembly_v1(
    ...
    println!("calendar-release-assembly: ok");
    ExitCode::SUCCESS
}
