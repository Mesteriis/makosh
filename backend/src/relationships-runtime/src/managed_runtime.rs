...
use makosh_relationships_api::RELATIONSHIPS_OWNER_ID_V1;
...
use crate::{
    RelationshipsClientRuntimeContextV1, dispatch_relationships_client_request_v1,
    event_outbox::{RelationshipsEventRelayErrorV1, relay_relationships_outbox_once_v1},
};

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct RelationshipsRuntimeAdmissionV1 {
...
pub enum RelationshipsManagedRuntimeErrorV1 {
    Admission,
    EventUnavailable,
...
pub struct RelationshipsManagedRuntimeV1 {
    admission: RelationshipsRuntimeAdmissionV1,
...
impl RelationshipsManagedRuntimeV1 {
    ...
    pub async fn open(
        ...
        let vault_public_key = storage_configuration
            ...
            .as_slice()
            .try_into()
            .map_err(|_| RelationshipsManagedRuntimeErrorV1::Admission)?;
        ...
        let password = std::str::from_utf8(&password)
            .map_err(|_| RelationshipsManagedRuntimeErrorV1::Admission)?;
        let persistence = RelationshipsPersistenceV1::connect_runtime(
            &binding,
        ...
        .await
        .map_err(RelationshipsManagedRuntimeErrorV1::Persistence)?;
        persistence
        ...
        .map_err(|_| RelationshipsManagedRuntimeErrorV1::EventUnavailable)?;
        ...
        control_channel
            ...
            .map_err(|_| RelationshipsManagedRuntimeErrorV1::Unavailable)?;
        Ok(Self {
    ...
    pub async fn pump_control_once(
        &mut self,
        now_unix_millis: i64,
    ) -> Result<bool, RelationshipsManagedRuntimeErrorV1> {
        ...
        let response = dispatch_relationships_client_request_v1(
            &self.persistence,
            &self.admission.logical_human_owner_id,
            request,
            RelationshipsClientRuntimeContextV1 {
        ...
        if validate_module_client_response_v1(&response).is_err() {
            return Err(RelationshipsManagedRuntimeErrorV1::Unavailable);
        }
    ...
    pub async fn relay_outbox_once(
        ...
        relay_relationships_outbox_once_v1(
            &self.persistence,
            &self.admission.logical_human_owner_id,
    ...
    fn write_control_error(
        ...
        error_code: &str,
    ) -> Result<(), RelationshipsManagedRuntimeErrorV1> {
        self.control_channel
            .write_response(
                correlation_id,
                ManagedRuntimeControlResponseV1 {
...
fn validate_admission(
    admission: &RelationshipsRuntimeAdmissionV1,
) -> Result<[u8; 16], RelationshipsManagedRuntimeErrorV1> {
    if admission.logical_owner_id != RELATIONSHIPS_OWNER_ID_V1
    ...
    runtime_source_reference(&admission.runtime_instance_id)
        .ok_or(RelationshipsManagedRuntimeErrorV1::Admission)
}
...
async fn resolve_storage_credential(
...
) -> Result<zeroize::Zeroizing<Vec<u8>>, RelationshipsManagedRuntimeErrorV1> {
    for attempt in 0..20 {
    ...
    Err(RelationshipsManagedRuntimeErrorV1::Unavailable)
}

fn storage_binding(
    configuration: &ManagedStorageRuntimeConfigurationV1,
    admission: &RelationshipsRuntimeAdmissionV1,
) -> Result<StorageBindingV1, RelationshipsManagedRuntimeErrorV1> {
    if configuration.runtime_instance_id != admission.runtime_instance_id
        || configuration.logical_owner_id != RELATIONSHIPS_OWNER_ID_V1
        || configuration.owner != RELATIONSHIPS_OWNER_ID_V1
...
fn relay_error(error: RelationshipsEventRelayErrorV1) -> RelationshipsManagedRuntimeErrorV1 {
    match error {
        RelationshipsEventRelayErrorV1::InvalidTimestamp => {
            RelationshipsManagedRuntimeErrorV1::Admission
        }
        RelationshipsEventRelayErrorV1::Persistence(error) => {
            RelationshipsManagedRuntimeErrorV1::Persistence(error)
        }
        RelationshipsEventRelayErrorV1::EventUnavailable => {
            RelationshipsManagedRuntimeErrorV1::EventUnavailable
        }
    }
}
...
mod tests {
    ...
    fn admission_requires_exact_owner_and_runtime_source() {
        let valid = RelationshipsRuntimeAdmissionV1 {
        ...
        invalid.logical_owner_id = "persons".to_owned();
        assert_eq!(
            validate_admission(&invalid),
            Err(RelationshipsManagedRuntimeErrorV1::Admission)
        );
    }
}
