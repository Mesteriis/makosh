...
use makosh_relationships_persistence::{
    RelationshipsPersistenceErrorV1, relationships_storage_bundle_v1,
};
use makosh_relationships_runtime::{
    RelationshipsManagedRuntimeErrorV1, RelationshipsManagedRuntimeV1,
    RelationshipsRuntimeAdmissionV1, relationships_module_descriptor_v1,
    relationships_settings_schema_bytes_v1,
};
...
fn main() -> Result<(), String> {
    ...
    let Some(command) = arguments.next() else {
        return Err("Relationships command is required".to_owned());
    };
    match command.to_str() {
        Some("export-storage-bundle") => export(
            &mut arguments,
            relationships_storage_bundle_v1().encode_to_vec(),
        ),
        Some("export-module-descriptor") => {
            let build_id = required_string(&mut arguments, "build id")?;
            export(
                &mut arguments,
                relationships_module_descriptor_v1(&build_id).encode_to_vec(),
            )
        }
        Some("export-settings-schema") => {
            export(&mut arguments, relationships_settings_schema_bytes_v1())
        }
        Some("serve-inherited") => serve_inherited(&mut arguments),
        _ => Err("Relationships command is invalid".to_owned()),
    }
}
    ...
    I: Iterator<Item = OsString>,
{
    ...
    decode_settings_schema_v1(&settings_schema)
        .map_err(|_| "Relationships settings schema is invalid".to_owned())?;
    let configuration = ManagedDomainRuntimeConfigurationV1::decode(
        read_contract(&paths.runtime_configuration)?.as_slice(),
    )
    .map_err(|_| "Relationships runtime configuration is invalid".to_owned())?;
    ...
    if configuration.runtime_instance_id != paths.runtime_instance_id {
        return Err("Relationships runtime configuration is stale".to_owned());
    }
    let storage = configuration
        .storage
        .clone()
        .ok_or_else(|| "Relationships storage is unavailable".to_owned())?;
    let admission = RelationshipsRuntimeAdmissionV1 {
    ...
    let executor = tokio::runtime::Runtime::new()
        .map_err(|_| "Relationships executor is unavailable".to_owned())?;
    let mut runtime = executor
        .block_on(RelationshipsManagedRuntimeV1::open(
    ...
    let mut poll_backoff =
        ManagedRuntimePollBackoffV1::new(Duration::from_millis(25), Duration::from_millis(100))
            .map_err(|_| "Relationships runtime polling bounds are invalid".to_owned())?;
    loop {
        let now = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .map_err(|_| "Relationships clock is invalid".to_owned())?
            .as_millis()
            .try_into()
            .map_err(|_| "Relationships clock is invalid".to_owned())?;
...
fn retry_runtime(result: Result<bool, RelationshipsManagedRuntimeErrorV1>) -> Result<bool, String> {
    match result {
        Ok(progressed) => Ok(progressed),
        Err(RelationshipsManagedRuntimeErrorV1::EventUnavailable)
        | Err(RelationshipsManagedRuntimeErrorV1::Persistence(
            RelationshipsPersistenceErrorV1::StorageUnavailable,
        )) => Ok(false),
        Err(error) => Err(runtime_error(error)),
    }
}
fn runtime_error(error: RelationshipsManagedRuntimeErrorV1) -> String {
    let code = match error {
        RelationshipsManagedRuntimeErrorV1::Admission => "admission",
        RelationshipsManagedRuntimeErrorV1::EventUnavailable => "event_unavailable",
        RelationshipsManagedRuntimeErrorV1::Persistence(_) => "persistence",
        RelationshipsManagedRuntimeErrorV1::Unavailable => "unavailable",
    };
    format!("Relationships runtime failed: {code}")
}
fn export<I: Iterator<Item = OsString>>(arguments: &mut I, bytes: Vec<u8>) -> Result<(), String> {
    let output = arguments
        .next()
        .map(PathBuf::from)
        .ok_or_else(|| "Relationships output path is required".to_owned())?;
    require_no_arguments(arguments)?;
    fs::write(output, bytes).map_err(|_| "Relationships export failed".to_owned())
}
...
fn required_string<I: Iterator<Item = OsString>>(
    ...
    if name.starts_with("--") && arguments.next().as_deref() != Some(OsStr::new(name)) {
        return Err("Relationships arguments are invalid".to_owned());
    }
    arguments
        .next()
        .and_then(|value| value.into_string().ok())
        .filter(|value| !value.is_empty())
        .ok_or_else(|| format!("Relationships {name} is required"))
}
...
fn inherited_control_channel() -> Result<UnixStream, String> {
    ...
    if duplicated < 0 {
        return Err("Relationships control channel is unavailable".to_owned());
    }
    Ok(unsafe { UnixStream::from_raw_fd(duplicated) })
}
fn read_contract(path: &Path) -> Result<Vec<u8>, String> {
    if !path.is_absolute() {
        return Err("Relationships contract path is invalid".to_owned());
    }
    let bytes = fs::read(path).map_err(|_| "Relationships contract is unavailable".to_owned())?;
    if bytes.is_empty() || bytes.len() > 2 * 1024 * 1024 {
        return Err("Relationships contract is invalid".to_owned());
    }
    Ok(bytes)
}
