...
use makosh_ai_contracts::{
    ...
    AI_OWNER_V1, encode_summary_source_content_v1,
    wire::{AiPrivateSourceReceiptV1, AiSummarySourceContentV1},
};
...
use makosh_communication_summary_persistence::CommunicationSummaryBlobCleanupV1;
use makosh_communications_ai_source_api::decode_communication_summary_source_content_v1;
...
use crate::COMMUNICATION_SUMMARY_BLOB_CAPABILITY_ID_V1;

const MAX_PROOF_BYTES_V1: usize = 2_048;

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct CommunicationSummarySourceBlobReceiptV1 {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) struct CommunicationSummaryBlobMaterializationV1 {
    pub ai_source: AiPrivateSourceReceiptV1,
    pub source_cleanup: CommunicationSummaryBlobCleanupV1,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum CommunicationSummaryBlobErrorV1 {
    InvalidReceipt,
    Unavailable,
}

pub(crate) fn materialize_summary_source_for_ai_v1(
    channel: &mut ManagedControlChannelV2<UnixStream>,
    dispatcher: &mut dyn ManagedControlRequestDispatcherV2<UnixStream>,
    run_id: [u8; 16],
    source: &CommunicationSummarySourceBlobReceiptV1,
) -> Result<CommunicationSummaryBlobMaterializationV1, CommunicationSummaryBlobErrorV1> {
    validate_source(source)?;
    let transfer = request_managed_blob_custody_transfer_v2(
        channel,
        dispatcher,
        ManagedBlobCustodyTransferRequestV1 {
            capability_id: COMMUNICATION_SUMMARY_BLOB_CAPABILITY_ID_V1,
    ...
    .map_err(|_| CommunicationSummaryBlobErrorV1::Unavailable)?;
    ...
    let source_content = decode_communication_summary_source_content_v1(&raw_source)
        .map_err(|_| CommunicationSummaryBlobErrorV1::InvalidReceipt)?;
    let encoded = Zeroizing::new(
        encode_summary_source_content_v1(&AiSummarySourceContentV1 {
    ...
    if !(1..=AI_MAX_PRIVATE_SOURCE_BYTES_V1).contains(&declared_bytes) {
        return Err(CommunicationSummaryBlobErrorV1::InvalidReceipt);
    }
    ...
    Ok(CommunicationSummaryBlobMaterializationV1 {
...
pub(crate) fn release_summary_source_blobs_v1(
    channel: &mut ManagedControlChannelV2<UnixStream>,
    dispatcher: &mut dyn ManagedControlRequestDispatcherV2<UnixStream>,
    run_id: [u8; 16],
    ai_source: &AiPrivateSourceReceiptV1,
    source_cleanup: &CommunicationSummaryBlobCleanupV1,
    accepted: bool,
) -> Result<(), CommunicationSummaryBlobErrorV1> {
...
fn read_exact(
    ...
    declared_bytes: u64,
    sha256: &[u8; 32],
) -> Result<Zeroizing<Vec<u8>>, CommunicationSummaryBlobErrorV1> {
...
fn ai_reference_id(run_id: [u8; 16], local_reference: [u8; 16], sha256: [u8; 32]) -> [u8; 16] {
    ...
    digest.update(b"makosh.communication_summary.ai-source.v1\0");
...
fn release_operation_id(run_id: [u8; 16], label: &[u8]) -> [u8; 16] {
    let mut digest = Sha256::new();
    digest.update(b"makosh.communication_summary.blob-release.v1\0");
    digest.update(label);
    digest.update(run_id);
    digest.finalize()[..16].try_into().expect("digest prefix")
}

fn id16(value: &[u8]) -> Result<[u8; 16], CommunicationSummaryBlobErrorV1> {
    value
        .try_into()
        .ok()
        .filter(|value: &[u8; 16]| value.iter().any(|byte| *byte != 0))
        .ok_or(CommunicationSummaryBlobErrorV1::InvalidReceipt)
}

fn id32(value: &[u8]) -> Result<[u8; 32], CommunicationSummaryBlobErrorV1> {
    value
        .try_into()
        .ok()
