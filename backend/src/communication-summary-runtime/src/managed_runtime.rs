...
use crate::{
    client_port::{
        CommunicationSummaryClientRuntimeContextV1, get_communication_summary_payload_v1,
        start_communication_summary_payload_v1,
    },
    client_realtime::{
        CommunicationSummaryClientRealtimeErrorV1, CommunicationSummaryClientRealtimePublisherV1,
    },
    consume_summary_source_prepared_once_v1, consume_summary_source_rejected_once_v1,
    contracts::{
        communication_summary_command_contract_v1, communication_summary_query_contract_v1,
    },
    event_outbox::CommunicationSummaryEventRelayErrorV1,
    inference::CommunicationSummaryInferenceErrorV1,
    recover_accepted_communication_summary_once_v1, relay_source_prepare_outbox_once_v1,
    source_results::CommunicationSummarySourceResultErrorV1,
};

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct CommunicationSummaryRuntimeAdmissionV1 {
...
pub enum CommunicationSummaryManagedRuntimeErrorV1 {
    Admission,
    EventContract,
    EventUnavailable,
    InvalidTransition,
...
pub struct CommunicationSummaryManagedRuntimeV1 {
    admission: CommunicationSummaryRuntimeAdmissionV1,
    ...
    client_realtime: CommunicationSummaryClientRealtimePublisherV1,
}

impl CommunicationSummaryManagedRuntimeV1 {
    pub async fn open(
    ...
    ) -> Result<Self, CommunicationSummaryManagedRuntimeErrorV1> {
        validate_admission(admission)?;
        if event_hub_endpoint.trim().is_empty() || event_credential_revision == 0 {
            return Err(CommunicationSummaryManagedRuntimeErrorV1::Admission);
        }
        ...
        let vault_public_key = storage_configuration
            ...
            .as_slice()
            .try_into()
            .map_err(|_| CommunicationSummaryManagedRuntimeErrorV1::Admission)?;
        ...
        let password = std::str::from_utf8(&password)
            .map_err(|_| CommunicationSummaryManagedRuntimeErrorV1::Admission)?;
        let persistence = CommunicationSummaryPersistenceV1::connect_runtime(
            &binding,
        ...
        .await
        .map_err(CommunicationSummaryManagedRuntimeErrorV1::Persistence)?;
        persistence
        ...
        .map_err(|_| CommunicationSummaryManagedRuntimeErrorV1::EventUnavailable)?;
        ...
        .await
        .map_err(|_| CommunicationSummaryManagedRuntimeErrorV1::EventUnavailable)?;
        let mut client_realtime = CommunicationSummaryClientRealtimePublisherV1::default();
        ...
        control_channel
            ...
            .map_err(|_| CommunicationSummaryManagedRuntimeErrorV1::Unavailable)?;
        Ok(Self {
    ...
    pub async fn relay_source_prepare_outbox_once(
        &self,
        now_unix_millis: i64,
    ) -> Result<bool, CommunicationSummaryManagedRuntimeErrorV1> {
    ...
    pub async fn consume_source_prepared_once(
        ...
        self.control_channel
            ...
            .map_err(|_| CommunicationSummaryManagedRuntimeErrorV1::Unavailable)?;
        let mut dispatcher = RejectManagedControlRequestsV2;
        let result = consume_summary_source_prepared_once_v1(
            &self.persistence,
            &self.event_connection,
    ...
    pub async fn pump_control_once(
        ...
        if validate_module_client_response_v1(&response).is_err() {
            return Err(CommunicationSummaryManagedRuntimeErrorV1::Unavailable);
        }
        self.control_channel
            .write_response(
                correlation_id,
    ...
    fn write_client_error(
        ...
        error_code: &str,
    ) -> Result<(), CommunicationSummaryManagedRuntimeErrorV1> {
        self.control_channel
            .write_response(
                correlation_id,
                ManagedRuntimeControlResponseV1 {
...
async fn dispatch_client(
    ...
    let valid_identity = request.protocol_major == 1
        && request.module_id == COMMUNICATION_SUMMARY_MODULE_ID_V1
        && request.owner_id == COMMUNICATION_SUMMARY_OWNER_V1
        && request.logical_owner_id == admission.logical_owner_id;
    let (response_payload, accepted_route) = if valid_identity {
        if request.contract.as_ref() == Some(&communication_summary_command_contract_v1()) {
            (
                start_communication_summary_payload_v1(
                    persistence,
                    &admission.logical_owner_id,
                    &CommunicationSummaryClientRuntimeContextV1 {
        ...
        } else if request.contract.as_ref() == Some(&communication_summary_query_contract_v1()) {
            (
                get_communication_summary_payload_v1(
                    persistence,
                    &admission.logical_owner_id,
                    &request.request_payload,
                )
...
fn bind_source_subscriptions(
    ...
    CommunicationSummaryManagedRuntimeErrorV1,
> {
    if permits.len() != 2 {
        return Err(CommunicationSummaryManagedRuntimeErrorV1::Admission);
    }
...
fn exact_permit(
...
) -> Result<RuntimeSubscribePermitV1, CommunicationSummaryManagedRuntimeErrorV1> {
    ...
    let permit = matching
        .next()
        .cloned()
        .ok_or(CommunicationSummaryManagedRuntimeErrorV1::Admission)?;
...
) -> Result<zeroize::Zeroizing<Vec<u8>>, CommunicationSummaryManagedRuntimeErrorV1> {
    for attempt in 0..20 {
...
fn storage_binding(
    ...
    admission: &CommunicationSummaryRuntimeAdmissionV1,
) -> Result<StorageBindingV1, CommunicationSummaryManagedRuntimeErrorV1> {
...
fn storage_owner_is_exact(logical_owner_id: &str, owner: &str) -> bool {
    logical_owner_id == owner && owner == COMMUNICATION_SUMMARY_OWNER_V1
}

fn event_relay_error(
    error: CommunicationSummaryEventRelayErrorV1,
) -> CommunicationSummaryManagedRuntimeErrorV1 {
    match error {
        CommunicationSummaryEventRelayErrorV1::InvalidTimestamp => {
            CommunicationSummaryManagedRuntimeErrorV1::EventContract
        }
        CommunicationSummaryEventRelayErrorV1::Persistence(error) => {
            CommunicationSummaryManagedRuntimeErrorV1::Persistence(error)
        }
        CommunicationSummaryEventRelayErrorV1::EventUnavailable => {
            CommunicationSummaryManagedRuntimeErrorV1::EventUnavailable
        }
    }
}

fn source_result_error(
    error: CommunicationSummarySourceResultErrorV1,
) -> CommunicationSummaryManagedRuntimeErrorV1 {
    match error {
        CommunicationSummarySourceResultErrorV1::InvalidEnvelope
        | CommunicationSummarySourceResultErrorV1::InvalidPayload => {
            CommunicationSummaryManagedRuntimeErrorV1::EventContract
        }
        CommunicationSummarySourceResultErrorV1::Blob(
            crate::CommunicationSummaryBlobErrorV1::InvalidReceipt,
        ) => CommunicationSummaryManagedRuntimeErrorV1::EventContract,
        CommunicationSummarySourceResultErrorV1::Blob(
            crate::CommunicationSummaryBlobErrorV1::Unavailable,
        ) => CommunicationSummaryManagedRuntimeErrorV1::Unavailable,
        CommunicationSummarySourceResultErrorV1::Inference(error) => inference_error(error),
        CommunicationSummarySourceResultErrorV1::Persistence(error) => {
            CommunicationSummaryManagedRuntimeErrorV1::Persistence(error)
        }
...
fn inference_error(
    error: CommunicationSummaryInferenceErrorV1,
) -> CommunicationSummaryManagedRuntimeErrorV1 {
    match error {
        CommunicationSummaryInferenceErrorV1::InvalidRequest
        | CommunicationSummaryInferenceErrorV1::InvalidResult => {
            CommunicationSummaryManagedRuntimeErrorV1::InvalidTransition
        }
        CommunicationSummaryInferenceErrorV1::Persistence(error) => {
            CommunicationSummaryManagedRuntimeErrorV1::Persistence(error)
        }
        CommunicationSummaryInferenceErrorV1::Unavailable => {
            CommunicationSummaryManagedRuntimeErrorV1::Unavailable
        }
    }
}
...
fn client_realtime_error(
    error: CommunicationSummaryClientRealtimeErrorV1,
) -> CommunicationSummaryManagedRuntimeErrorV1 {
    match error {
        CommunicationSummaryClientRealtimeErrorV1::InvalidTransition => {
            CommunicationSummaryManagedRuntimeErrorV1::InvalidTransition
        }
        CommunicationSummaryClientRealtimeErrorV1::Persistence(error) => {
            CommunicationSummaryManagedRuntimeErrorV1::Persistence(error)
        }
        CommunicationSummaryClientRealtimeErrorV1::Unavailable => {
            CommunicationSummaryManagedRuntimeErrorV1::Unavailable
        }
    }
}
...
mod tests {
    ...
    fn admission_requires_exact_managed_fences() {
        assert_eq!(
            validate_admission(&CommunicationSummaryRuntimeAdmissionV1 {
    ...
    fn module_identity_is_exact_workflow_unit() {
        assert_eq!(
            COMMUNICATION_SUMMARY_MODULE_ID_V1,
