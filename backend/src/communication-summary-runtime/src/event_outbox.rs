...
pub enum CommunicationSummaryEventRelayErrorV1 {
    InvalidTimestamp,
...
pub async fn relay_source_prepare_outbox_once_v1(
...
) -> Result<bool, CommunicationSummaryEventRelayErrorV1> {
    if published_at_unix_millis <= 0 {
        return Err(CommunicationSummaryEventRelayErrorV1::InvalidTimestamp);
    }
    let Some(record) = persistence
        .unpublished_source_prepare_events(logical_owner_id, 1)
        .await
        .map_err(CommunicationSummaryEventRelayErrorV1::Persistence)?
        .into_iter()
        .next()
    else {
        return Ok(false);
    ...
    connection
        ...
        .await
        .map_err(|_| CommunicationSummaryEventRelayErrorV1::EventUnavailable)?;
    persistence
