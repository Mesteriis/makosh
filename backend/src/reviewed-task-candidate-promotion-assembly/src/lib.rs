//! Unsigned Reviewed Task Candidate Promotion workflow release assembly.
//!
...
pub const RUNTIME_ARTIFACT_ID_V1: &str = "reviewed_task_candidate_promotion.runtime.v1";
pub const STORAGE_ARTIFACT_ID_V1: &str = "reviewed_task_candidate_promotion.storage.v1";
pub const DESCRIPTOR_FILE_V1: &str = "reviewed_task_candidate_promotion.runtime.descriptor.pb";
pub const SETTINGS_FILE_V1: &str = "reviewed_task_candidate_promotion.runtime.settings.pb";
pub const STORAGE_BUNDLE_FILE_V1: &str = "reviewed_task_candidate_promotion.storage.bundle.pb";
pub const ARTIFACT_FRAGMENT_FILE_V1: &str =
    "reviewed_task_candidate_promotion.release-artifacts.json";

const RUNTIME_RELATIVE_PATH_V1: &str = "bin/makosh-reviewed-task-candidate-promotion-runtime";
const DESCRIPTOR_RELATIVE_PATH_V1: &str =
    "contracts/reviewed_task_candidate_promotion.runtime.descriptor.pb";
const SETTINGS_RELATIVE_PATH_V1: &str =
    "contracts/reviewed_task_candidate_promotion.runtime.settings.pb";
const STORAGE_RELATIVE_PATH_V1: &str =
    "storage/reviewed_task_candidate_promotion.storage.bundle.pb";
...
pub fn materialize_reviewed_task_candidate_promotion_release_assembly_v1(
    output_directory: &Path,
    build_id: &str,
    runtime_source: &Path,
) -> Result<ReleaseAssemblyPathsV1, ReleaseAssemblyErrorV1> {
    validate_inputs(output_directory, build_id, runtime_source)?;
    let descriptor = reviewed_task_candidate_promotion_module_descriptor_v1(build_id);
    let settings = reviewed_task_candidate_promotion_settings_schema_v1();
    let storage = reviewed_task_candidate_promotion_storage_bundle_v1();
...
mod tests {
    ...
    fn materializes_exact_unsigned_workflow_artifacts() {
        ...
        let paths = materialize_reviewed_task_candidate_promotion_release_assembly_v1(
        ...
        assert_eq!(descriptor.owner_id, "reviewed_task_candidate_promotion");
        assert_eq!(
            descriptor.module_kind,
            makosh_runtime_protocol::v1::ModuleKindV1::Workflow as i32
        );
        assert_eq!(storage.owner_id, "reviewed_task_candidate_promotion");
        assert_eq!(fragment.owner_id, "reviewed_task_candidate_promotion");
    ...
    fn temporary_directory() -> PathBuf {
        ...
        let path = std::env::temp_dir().join(format!(
            "makosh-reviewed-task-candidate-promotion-assembly-{}-{id}",
            std::process::id()
        ));
