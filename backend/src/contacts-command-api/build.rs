...
fn main() {
    ...
    let descriptor = output.join("contacts-command-v1.bin");
    ...
    std::fs::write(
        output.join("contacts_command_schema.rs"),
        format!("pub const CONTACTS_COMMAND_SCHEMA_SHA256_V1: [u8; 32] = {digest:?};\n"),
    )
    .expect("Contacts command schema digest must be written");
}
