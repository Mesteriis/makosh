...
fn main() {
    ...
    let descriptor = out.join("review-person-match-candidate-v1.bin");
    prost_build::Config::new()
        ...
        .expect("Review person-match candidate protocol must compile");
    let digest: [u8; 32] = Sha256::digest(std::fs::read(&descriptor).expect("descriptor")).into();
    std::fs::write(
        out.join("review_person_match_candidate_schema.rs"),
        format!(
            "pub const REVIEW_PERSON_MATCH_CANDIDATE_SCHEMA_SHA256_V1: [u8; 32] = {digest:?};\n"
        ),
    )
