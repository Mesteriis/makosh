...
use makosh_decisions_api::DECISIONS_OWNER_ID_V1;
...
use crate::{
    DecisionsClientRuntimeContextV1, dispatch_decisions_client_request_v1,
    event_outbox::{DecisionsEventRelayErrorV1, relay_decisions_outbox_once_v1},
};

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct DecisionsRuntimeAdmissionV1 {
...
pub enum DecisionsManagedRuntimeErrorV1 {
    Admission,
    EventUnavailable,
...
pub struct DecisionsManagedRuntimeV1 {
    admission: DecisionsRuntimeAdmissionV1,
...
impl DecisionsManagedRuntimeV1 {
    ...
    pub async fn open(
        ...
        let vault_public_key = storage_configuration
            ...
            .as_slice()
            .try_into()
            .map_err(|_| DecisionsManagedRuntimeErrorV1::Admission)?;
        ...
        let password = std::str::from_utf8(&password)
            .map_err(|_| DecisionsManagedRuntimeErrorV1::Admission)?;
        let persistence = DecisionsPersistenceV1::connect_runtime(
            &binding,
        ...
        .await
        .map_err(DecisionsManagedRuntimeErrorV1::Persistence)?;
        persistence
        ...
        .map_err(|_| DecisionsManagedRuntimeErrorV1::EventUnavailable)?;
        ...
        control_channel
            ...
            .map_err(|_| DecisionsManagedRuntimeErrorV1::Unavailable)?;
        Ok(Self {
    ...
    pub async fn pump_control_once(
        &mut self,
        now_unix_millis: i64,
    ) -> Result<bool, DecisionsManagedRuntimeErrorV1> {
        ...
        let response = dispatch_decisions_client_request_v1(
            &self.persistence,
            &self.admission.logical_human_owner_id,
            request,
            DecisionsClientRuntimeContextV1 {
        ...
        if validate_module_client_response_v1(&response).is_err() {
            return Err(DecisionsManagedRuntimeErrorV1::Unavailable);
        }
    ...
    pub async fn relay_outbox_once(
        ...
        relay_decisions_outbox_once_v1(
            &self.persistence,
            &self.admission.logical_human_owner_id,
    ...
    fn write_control_error(
        ...
        error_code: &str,
    ) -> Result<(), DecisionsManagedRuntimeErrorV1> {
        self.control_channel
            .write_response(
                correlation_id,
                ManagedRuntimeControlResponseV1 {
...
fn validate_admission(
    admission: &DecisionsRuntimeAdmissionV1,
) -> Result<[u8; 16], DecisionsManagedRuntimeErrorV1> {
    if admission.logical_owner_id != DECISIONS_OWNER_ID_V1
    ...
    runtime_source_reference(&admission.runtime_instance_id)
        .ok_or(DecisionsManagedRuntimeErrorV1::Admission)
}
...
async fn resolve_storage_credential(
...
) -> Result<zeroize::Zeroizing<Vec<u8>>, DecisionsManagedRuntimeErrorV1> {
    for attempt in 0..20 {
    ...
    Err(DecisionsManagedRuntimeErrorV1::Unavailable)
}

fn storage_binding(
    configuration: &ManagedStorageRuntimeConfigurationV1,
    admission: &DecisionsRuntimeAdmissionV1,
) -> Result<StorageBindingV1, DecisionsManagedRuntimeErrorV1> {
    if configuration.runtime_instance_id != admission.runtime_instance_id
        || configuration.logical_owner_id != DECISIONS_OWNER_ID_V1
        || configuration.owner != DECISIONS_OWNER_ID_V1
...
fn relay_error(error: DecisionsEventRelayErrorV1) -> DecisionsManagedRuntimeErrorV1 {
    match error {
        DecisionsEventRelayErrorV1::InvalidTimestamp => DecisionsManagedRuntimeErrorV1::Admission,
        DecisionsEventRelayErrorV1::Persistence(error) => {
            DecisionsManagedRuntimeErrorV1::Persistence(error)
        }
        DecisionsEventRelayErrorV1::EventUnavailable => {
            DecisionsManagedRuntimeErrorV1::EventUnavailable
        }
    }
}
...
mod tests {
    ...
    fn admission_requires_exact_owner_and_runtime_source() {
        let valid = DecisionsRuntimeAdmissionV1 {
        ...
        assert_eq!(
            ...
            Err(DecisionsManagedRuntimeErrorV1::Admission)
        );
    }
}
