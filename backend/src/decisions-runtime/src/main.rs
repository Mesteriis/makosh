...
use makosh_decisions_persistence::{DecisionsPersistenceErrorV1, decisions_storage_bundle_v1};
...
fn main() -> Result<(), String> {
    ...
    let Some(command) = arguments.next() else {
        return Err("Decisions command is required".to_owned());
    };
    match command.to_str() {
        Some("export-storage-bundle") => export(
            &mut arguments,
            decisions_storage_bundle_v1().encode_to_vec(),
        ),
        Some("export-module-descriptor") => {
            let build_id = required_string(&mut arguments, "build id")?;
            export(
                &mut arguments,
                decisions_module_descriptor_v1(&build_id).encode_to_vec(),
            )
        }
        Some("export-settings-schema") => {
            export(&mut arguments, decisions_settings_schema_bytes_v1())
        }
        Some("serve-inherited") => serve_inherited(&mut arguments),
        _ => Err("Decisions command is invalid".to_owned()),
    }
}
    ...
    I: Iterator<Item = OsString>,
{
    ...
    decode_settings_schema_v1(&settings_schema)
        .map_err(|_| "Decisions settings schema is invalid".to_owned())?;
    let configuration = ManagedDomainRuntimeConfigurationV1::decode(
        read_contract(&paths.runtime_configuration)?.as_slice(),
    )
    .map_err(|_| "Decisions runtime configuration is invalid".to_owned())?;
    ...
    if configuration.runtime_instance_id != paths.runtime_instance_id {
        return Err("Decisions runtime configuration is stale".to_owned());
    }
    let storage = configuration
        .storage
        .clone()
        .ok_or_else(|| "Decisions storage is unavailable".to_owned())?;
    let admission = DecisionsRuntimeAdmissionV1 {
    ...
    let executor = tokio::runtime::Runtime::new()
        .map_err(|_| "Decisions executor is unavailable".to_owned())?;
    let mut runtime = executor
        .block_on(DecisionsManagedRuntimeV1::open(
    ...
    let mut poll_backoff =
        ManagedRuntimePollBackoffV1::new(Duration::from_millis(25), Duration::from_millis(100))
            .map_err(|_| "Decisions runtime polling bounds are invalid".to_owned())?;
    loop {
        let now = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .map_err(|_| "Decisions clock is invalid".to_owned())?
            .as_millis()
            .try_into()
            .map_err(|_| "Decisions clock is invalid".to_owned())?;
...
fn retry_runtime(result: Result<bool, DecisionsManagedRuntimeErrorV1>) -> Result<bool, String> {
    match result {
        Ok(progressed) => Ok(progressed),
        Err(DecisionsManagedRuntimeErrorV1::EventUnavailable)
        | Err(DecisionsManagedRuntimeErrorV1::Persistence(
...
fn runtime_error(error: DecisionsManagedRuntimeErrorV1) -> String {
    let code = match error {
        DecisionsManagedRuntimeErrorV1::Admission => "admission",
        DecisionsManagedRuntimeErrorV1::EventUnavailable => "event_unavailable",
        DecisionsManagedRuntimeErrorV1::Persistence(_) => "persistence",
        DecisionsManagedRuntimeErrorV1::Unavailable => "unavailable",
    };
    format!("Decisions runtime failed: {code}")
}
...
fn export<I: Iterator<Item = OsString>>(arguments: &mut I, bytes: Vec<u8>) -> Result<(), String> {
    let output = arguments
        .next()
        .map(PathBuf::from)
        .ok_or_else(|| "Decisions output path is required".to_owned())?;
    require_no_arguments(arguments)?;
    fs::write(output, bytes).map_err(|_| "Decisions export failed".to_owned())
}
...
fn required_string<I: Iterator<Item = OsString>>(
    ...
    if name.starts_with("--") && arguments.next().as_deref() != Some(OsStr::new(name)) {
        return Err("Decisions arguments are invalid".to_owned());
    }
    arguments
        .next()
        .and_then(|value| value.into_string().ok())
        .filter(|value| !value.is_empty())
        .ok_or_else(|| format!("Decisions {name} is required"))
}
...
fn inherited_control_channel() -> Result<UnixStream, String> {
    ...
    if duplicated < 0 {
        return Err("Decisions control channel is unavailable".to_owned());
    }
...
fn read_contract(path: &Path) -> Result<Vec<u8>, String> {
    if !path.is_absolute() {
        return Err("Decisions contract path is invalid".to_owned());
    }
    let bytes = fs::read(path).map_err(|_| "Decisions contract is unavailable".to_owned())?;
    if bytes.is_empty() || bytes.len() > 2 * 1024 * 1024 {
        return Err("Decisions contract is invalid".to_owned());
    }
    Ok(bytes)
}
