...
fn main() {
    ...
    let descriptor = output.join("attachment-archive-inspection-v1.bin");
    ...
    std::fs::write(
        output.join("attachment_archive_inspection_schema.rs"),
        format!("pub const ATTACHMENT_ARCHIVE_INSPECTION_SCHEMA_SHA256: [u8; 32] = {digest:?};\n"),
    )
    .expect("Attachment archive inspection schema digest must be written");
}
