...
use makosh_reviewed_note_candidate_promotion_assembly::materialize_reviewed_note_candidate_promotion_release_assembly_v1;
...
fn main() -> ExitCode {
    let Some(arguments) = arguments(std::env::args().skip(1).collect()) else {
        return fail(
            "usage: makosh-reviewed-note-candidate-promotion-assembly --build-id <id> \
             --output-dir <absolute-path> --runtime <absolute-path>",
        );
    };
    if materialize_reviewed_note_candidate_promotion_release_assembly_v1(
    ...
    println!("reviewed-note-candidate-promotion-release-assembly: ok");
    ExitCode::SUCCESS
}
