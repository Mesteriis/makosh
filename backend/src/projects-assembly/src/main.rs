use makosh_projects_assembly::materialize_projects_release_assembly_v1;
...
fn main() -> ExitCode {
    let Some(arguments) = arguments(std::env::args().skip(1).collect()) else {
        return fail(
            "usage: makosh-projects-assembly --build-id <id> --output-dir <absolute-path> --runtime <absolute-path>",
        );
    };
    if materialize_projects_release_assembly_v1(
    ...
    println!("projects-release-assembly: ok");
    ExitCode::SUCCESS
}
