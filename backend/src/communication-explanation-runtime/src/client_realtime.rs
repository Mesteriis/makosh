...
use crate::{
    ...
    contracts::communication_explanation_realtime_contract_v1,
};

const REPLAY_WINDOW_V1: u16 = 1_024;

#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub(crate) struct CommunicationExplanationClientRealtimePublisherV1 {
    last_sequence: Option<u64>,
}

impl CommunicationExplanationClientRealtimePublisherV1 {
    ...
    ) -> Result<bool, CommunicationExplanationClientRealtimeErrorV1> {
        let transitions = persistence
            .client_realtime_window(logical_owner_id, self.last_sequence, REPLAY_WINDOW_V1)
            .await
            .map_err(CommunicationExplanationClientRealtimeErrorV1::Persistence)?;
        let published = !transitions.is_empty();
        for transition in transitions {
            let occurred_at_unix_millis = u64::try_from(transition.occurred_at_unix_millis)
                .map_err(|_| CommunicationExplanationClientRealtimeErrorV1::InvalidTransition)?;
            let payload = CommunicationExplanationStatusChangedV1 {
            ...
            let request = ManagedRuntimeClientRealtimePublishRequestV1 {
                ...
                cursor: format!("communication-explanation/{}", transition.sequence),
                event_kind: COMMUNICATION_EXPLANATION_REALTIME_EVENT_KIND_V1.to_owned(),
                occurred_at_unix_millis,
            ...
            let response = channel
                ...
                .map_err(|_| CommunicationExplanationClientRealtimeErrorV1::Unavailable)?;
            if !response.error_code.is_empty() {
                return Err(CommunicationExplanationClientRealtimeErrorV1::Unavailable);
            }
...
pub(crate) enum CommunicationExplanationClientRealtimeErrorV1 {
    InvalidTransition,
