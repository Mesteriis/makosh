...
use crate::{
    client_port::{
        CommunicationExplanationClientRuntimeContextV1, get_communication_explanation_payload_v1,
        start_communication_explanation_payload_v1,
    },
    client_realtime::{
        CommunicationExplanationClientRealtimeErrorV1,
        CommunicationExplanationClientRealtimePublisherV1,
    },
    consume_explanation_source_prepared_once_v1, consume_explanation_source_rejected_once_v1,
    contracts::{
        communication_explanation_command_contract_v1, communication_explanation_query_contract_v1,
    },
    event_outbox::CommunicationExplanationEventRelayErrorV1,
    inference::CommunicationExplanationInferenceErrorV1,
    recover_accepted_communication_explanation_once_v1, relay_source_prepare_outbox_once_v1,
    source_results::CommunicationExplanationSourceResultErrorV1,
};

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct CommunicationExplanationRuntimeAdmissionV1 {
...
pub enum CommunicationExplanationManagedRuntimeErrorV1 {
    Admission,
    EventContract,
    EventUnavailable,
    InvalidTransition,
...
pub struct CommunicationExplanationManagedRuntimeV1 {
    admission: CommunicationExplanationRuntimeAdmissionV1,
    ...
    client_realtime: CommunicationExplanationClientRealtimePublisherV1,
}

impl CommunicationExplanationManagedRuntimeV1 {
    pub async fn open(
    ...
    ) -> Result<Self, CommunicationExplanationManagedRuntimeErrorV1> {
        validate_admission(admission)?;
        if event_hub_endpoint.trim().is_empty() || event_credential_revision == 0 {
            return Err(CommunicationExplanationManagedRuntimeErrorV1::Admission);
        }
        ...
        let vault_public_key = storage_configuration
            ...
            .as_slice()
            .try_into()
            .map_err(|_| CommunicationExplanationManagedRuntimeErrorV1::Admission)?;
        ...
        let password = std::str::from_utf8(&password)
            .map_err(|_| CommunicationExplanationManagedRuntimeErrorV1::Admission)?;
        let persistence = CommunicationExplanationPersistenceV1::connect_runtime(
            &binding,
        ...
        .await
        .map_err(CommunicationExplanationManagedRuntimeErrorV1::Persistence)?;
        persistence
        ...
        .map_err(|_| CommunicationExplanationManagedRuntimeErrorV1::EventUnavailable)?;
        ...
        .await
        .map_err(|_| CommunicationExplanationManagedRuntimeErrorV1::EventUnavailable)?;
        let mut client_realtime = CommunicationExplanationClientRealtimePublisherV1::default();
        ...
        control_channel
            ...
            .map_err(|_| CommunicationExplanationManagedRuntimeErrorV1::Unavailable)?;
        Ok(Self {
    ...
    pub async fn relay_source_prepare_outbox_once(
        &self,
        now_unix_millis: i64,
    ) -> Result<bool, CommunicationExplanationManagedRuntimeErrorV1> {
    ...
    pub async fn consume_source_prepared_once(
        ...
        self.control_channel
            ...
            .map_err(|_| CommunicationExplanationManagedRuntimeErrorV1::Unavailable)?;
        let mut dispatcher = RejectManagedControlRequestsV2;
        let result = consume_explanation_source_prepared_once_v1(
            &self.persistence,
            &self.event_connection,
    ...
    pub async fn pump_control_once(
        ...
        if validate_module_client_response_v1(&response).is_err() {
            return Err(CommunicationExplanationManagedRuntimeErrorV1::Unavailable);
        }
        self.control_channel
            .write_response(
                correlation_id,
    ...
    fn write_client_error(
        ...
        error_code: &str,
    ) -> Result<(), CommunicationExplanationManagedRuntimeErrorV1> {
        self.control_channel
            .write_response(
                correlation_id,
                ManagedRuntimeControlResponseV1 {
...
async fn dispatch_client(
    ...
    let valid_identity = request.protocol_major == 1
        && request.module_id == COMMUNICATION_EXPLANATION_MODULE_ID_V1
        && request.owner_id == COMMUNICATION_EXPLANATION_OWNER_V1
        && request.logical_owner_id == admission.logical_owner_id;
    let (response_payload, accepted_route) = if valid_identity {
        if request.contract.as_ref() == Some(&communication_explanation_command_contract_v1()) {
            (
                start_communication_explanation_payload_v1(
                    persistence,
                    &admission.logical_owner_id,
                    &CommunicationExplanationClientRuntimeContextV1 {
        ...
        } else if request.contract.as_ref() == Some(&communication_explanation_query_contract_v1())
        {
            (
                get_communication_explanation_payload_v1(
                    persistence,
                    &admission.logical_owner_id,
                    &request.request_payload,
                )
...
fn bind_source_subscriptions(
    ...
    CommunicationExplanationManagedRuntimeErrorV1,
> {
    if permits.len() != 2 {
        return Err(CommunicationExplanationManagedRuntimeErrorV1::Admission);
    }
...
fn exact_permit(
...
) -> Result<RuntimeSubscribePermitV1, CommunicationExplanationManagedRuntimeErrorV1> {
    ...
    let permit = matching
        .next()
        .cloned()
        .ok_or(CommunicationExplanationManagedRuntimeErrorV1::Admission)?;
...
async fn resolve_storage_credential(
...
) -> Result<zeroize::Zeroizing<Vec<u8>>, CommunicationExplanationManagedRuntimeErrorV1> {
    for attempt in 0..20 {
    ...
    Err(CommunicationExplanationManagedRuntimeErrorV1::Unavailable)
}

fn storage_binding(
    configuration: &ManagedStorageRuntimeConfigurationV1,
    admission: &CommunicationExplanationRuntimeAdmissionV1,
) -> Result<StorageBindingV1, CommunicationExplanationManagedRuntimeErrorV1> {
...
fn storage_owner_is_exact(logical_owner_id: &str, owner: &str) -> bool {
    logical_owner_id == owner && owner == COMMUNICATION_EXPLANATION_OWNER_V1
}

fn event_relay_error(
    error: CommunicationExplanationEventRelayErrorV1,
) -> CommunicationExplanationManagedRuntimeErrorV1 {
    match error {
        CommunicationExplanationEventRelayErrorV1::InvalidTimestamp => {
            CommunicationExplanationManagedRuntimeErrorV1::EventContract
        }
        CommunicationExplanationEventRelayErrorV1::Persistence(error) => {
            CommunicationExplanationManagedRuntimeErrorV1::Persistence(error)
        }
        CommunicationExplanationEventRelayErrorV1::EventUnavailable => {
            CommunicationExplanationManagedRuntimeErrorV1::EventUnavailable
        }
    }
}

fn source_result_error(
    error: CommunicationExplanationSourceResultErrorV1,
) -> CommunicationExplanationManagedRuntimeErrorV1 {
    match error {
        CommunicationExplanationSourceResultErrorV1::InvalidEnvelope
        | CommunicationExplanationSourceResultErrorV1::InvalidPayload => {
            CommunicationExplanationManagedRuntimeErrorV1::EventContract
        }
        CommunicationExplanationSourceResultErrorV1::Blob(
            crate::CommunicationExplanationBlobErrorV1::InvalidReceipt,
        ) => CommunicationExplanationManagedRuntimeErrorV1::EventContract,
        CommunicationExplanationSourceResultErrorV1::Blob(
            crate::CommunicationExplanationBlobErrorV1::Unavailable,
        ) => CommunicationExplanationManagedRuntimeErrorV1::Unavailable,
        CommunicationExplanationSourceResultErrorV1::Inference(error) => inference_error(error),
        CommunicationExplanationSourceResultErrorV1::Persistence(error) => {
            CommunicationExplanationManagedRuntimeErrorV1::Persistence(error)
        }
...
fn inference_error(
    error: CommunicationExplanationInferenceErrorV1,
) -> CommunicationExplanationManagedRuntimeErrorV1 {
    match error {
        CommunicationExplanationInferenceErrorV1::InvalidRequest
        | CommunicationExplanationInferenceErrorV1::InvalidResult => {
            CommunicationExplanationManagedRuntimeErrorV1::InvalidTransition
        }
        CommunicationExplanationInferenceErrorV1::Persistence(error) => {
            CommunicationExplanationManagedRuntimeErrorV1::Persistence(error)
        }
        CommunicationExplanationInferenceErrorV1::Unavailable => {
            CommunicationExplanationManagedRuntimeErrorV1::Unavailable
        }
    }
}
...
fn client_realtime_error(
    error: CommunicationExplanationClientRealtimeErrorV1,
) -> CommunicationExplanationManagedRuntimeErrorV1 {
    match error {
        CommunicationExplanationClientRealtimeErrorV1::InvalidTransition => {
            CommunicationExplanationManagedRuntimeErrorV1::InvalidTransition
        }
        CommunicationExplanationClientRealtimeErrorV1::Persistence(error) => {
            CommunicationExplanationManagedRuntimeErrorV1::Persistence(error)
        }
        CommunicationExplanationClientRealtimeErrorV1::Unavailable => {
            CommunicationExplanationManagedRuntimeErrorV1::Unavailable
        }
    }
}
...
mod tests {
    ...
    fn admission_requires_exact_managed_fences() {
        assert_eq!(
            validate_admission(&CommunicationExplanationRuntimeAdmissionV1 {
    ...
    fn module_identity_is_exact_workflow_unit() {
        assert_eq!(
            COMMUNICATION_EXPLANATION_MODULE_ID_V1,
