...
use makosh_communication_explanation_persistence::{
    CommunicationExplanationPersistenceErrorV1, communication_explanation_storage_bundle_v1,
};
use makosh_communication_explanation_runtime::{
    ...
    CommunicationExplanationRuntimeAdmissionV1, communication_explanation_module_descriptor_v1,
...
fn main() -> Result<(), String> {
    ...
    let Some(command) = arguments.next() else {
        return Err("Communication Explanation command is required".to_owned());
    };
    match command.to_str() {
        Some("export-storage-bundle") => export_bytes(
            &mut arguments,
            communication_explanation_storage_bundle_v1().encode_to_vec(),
        ),
        Some("export-module-descriptor") => {
            let build_id = required_string(&mut arguments, "build id")?;
            export_bytes(
                &mut arguments,
                communication_explanation_module_descriptor_v1(&build_id).encode_to_vec(),
            )
        }
        ...
        _ => Err("Communication Explanation command is invalid".to_owned()),
    }
}
    ...
    I: Iterator<Item = OsString>,
{
    let output = arguments
        .next()
        .map(PathBuf::from)
        .ok_or_else(|| "Communication Explanation output path is required".to_owned())?;
    require_no_arguments(arguments)?;
    fs::write(output, bytes).map_err(|_| "Communication Explanation export failed".to_owned())
}
    ...
    I: Iterator<Item = OsString>,
{
    ...
    decode_settings_schema_v1(&settings_schema)
        .map_err(|_| "Communication Explanation settings schema is invalid".to_owned())?;
    let configuration = ManagedWorkflowRuntimeConfigurationV1::decode(
        read_contract(&paths.runtime_configuration)?.as_slice(),
    )
    .map_err(|_| "Communication Explanation runtime configuration is invalid".to_owned())?;
    ...
    if configuration.runtime_instance_id != paths.runtime_instance_id {
        return Err("Communication Explanation runtime configuration is stale".to_owned());
    }
    let storage = configuration
        .storage
        .clone()
        .ok_or_else(|| "Communication Explanation storage is unavailable".to_owned())?;
    let admission = CommunicationExplanationRuntimeAdmissionV1 {
    ...
    let executor = tokio::runtime::Runtime::new()
        .map_err(|_| "Communication Explanation executor is unavailable".to_owned())?;
    let mut runtime = executor
        .block_on(CommunicationExplanationManagedRuntimeV1::open(
    ...
    loop {
        let now = std::time::SystemTime::now()
            ...
            .map_err(|_| "Communication Explanation clock is invalid".to_owned())?
            .as_millis()
            .try_into()
            .map_err(|_| "Communication Explanation clock is invalid".to_owned())?;
...
fn retry_runtime(
    result: Result<bool, CommunicationExplanationManagedRuntimeErrorV1>,
) -> Result<(), String> {
    match result {
        Ok(_)
        | Err(CommunicationExplanationManagedRuntimeErrorV1::EventUnavailable)
        | Err(CommunicationExplanationManagedRuntimeErrorV1::Unavailable)
        | Err(CommunicationExplanationManagedRuntimeErrorV1::Persistence(
...
fn runtime_error(error: CommunicationExplanationManagedRuntimeErrorV1) -> String {
    let code = match error {
        CommunicationExplanationManagedRuntimeErrorV1::Admission => "admission",
        CommunicationExplanationManagedRuntimeErrorV1::EventContract => "event_contract",
        CommunicationExplanationManagedRuntimeErrorV1::EventUnavailable => "event_unavailable",
        CommunicationExplanationManagedRuntimeErrorV1::InvalidTransition => "invalid_transition",
        CommunicationExplanationManagedRuntimeErrorV1::Persistence(_) => "persistence",
        CommunicationExplanationManagedRuntimeErrorV1::Unavailable => "unavailable",
    };
    format!("Communication Explanation runtime failed: {code}")
}
    ...
    I: Iterator<Item = OsString>,
{
    if arguments.next().as_deref() != Some(OsStr::new(name)) {
        return Err("Communication Explanation arguments are invalid".to_owned());
    }
    let value = arguments
        .next()
        .and_then(|value| value.into_string().ok())
        .ok_or_else(|| format!("Communication Explanation {name} is required"))?;
    if value.is_empty() {
        return Err(format!("Communication Explanation {name} is invalid"));
    }
    Ok(value)
}
...
fn inherited_control_channel() -> Result<UnixStream, String> {
    ...
    if duplicated < 0 {
        return Err("Communication Explanation control channel is unavailable".to_owned());
    }
...
fn read_contract(path: &Path) -> Result<Vec<u8>, String> {
    if !path.is_absolute() {
        return Err("Communication Explanation contract path is invalid".to_owned());
    }
    let bytes = fs::read(path)
        .map_err(|_| "Communication Explanation contract is unavailable".to_owned())?;
    if bytes.is_empty() || bytes.len() > 2 * 1024 * 1024 {
        return Err("Communication Explanation contract is invalid".to_owned());
    }
    Ok(bytes)
}
