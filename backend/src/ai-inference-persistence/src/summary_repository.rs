use makosh_ai_contracts::wire::{
    ...
    CommunicationSummaryInferenceRequestV1, CommunicationSummaryInferenceResultV1,
};
use makosh_ai_inference_core::{AiInferenceRunStateV1, AiSummaryRunV1};
use sqlx::{Postgres, Row, Transaction, postgres::PgRow};

use crate::{
    AI_INFERENCE_RECOVERY_LIMIT_V1, AiInferencePersistenceErrorV1, AiInferencePersistenceV1,
    AiSummaryPersistenceOutcomeV1, AiSummaryTransitionV1, PersistedAiSummaryRunV1,
    summary_model::{
        validate_summary_accepted, validate_summary_persisted, validate_summary_run,
        validate_summary_transition,
    },
};
...
impl AiInferencePersistenceV1 {
    pub async fn accept_summary_run(
        &self,
        run: AiSummaryRunV1,
    ) -> Result<AiSummaryPersistenceOutcomeV1, AiInferencePersistenceErrorV1> {
        validate_summary_accepted(&run)?;
        ...
        let inserted = sqlx::query(
            "INSERT INTO makosh_data.ai_summary_runs (
               ...
               source_sha256, source_custody_proof, requested_language, requested_length,
        ...
        .bind(i16::try_from(request.length).map_err(|_| invalid_input())?)
        ...
        .rows_affected()
            == 1;
        let mut persisted = load_summary_run_in_transaction(
            &mut transaction,
        ...
        if !inserted && persisted.run.request != run.request {
            ...
            if matches!(
                ...
                let updated = sqlx::query(
                    "UPDATE makosh_data.ai_summary_runs SET source_custody_proof=$1
                     WHERE logical_owner_id=$2 AND run_id=$3 AND request_digest=$4 AND run_state IN (1,2)",
                ).bind(&source.custody_transfer_source_proof).bind(&request.logical_owner_id)
                .bind(&request.run_id).bind(&context.request_digest).execute(&mut *transaction).await
                .map_err(storage_error)?.rows_affected();
                if updated != 1 {
                    return Err(AiInferencePersistenceErrorV1::RevisionConflict);
                }
                persisted = load_summary_run_in_transaction(
                    &mut transaction,
                    &request.logical_owner_id,
                    id16(&request.run_id)?,
                )
        ...
        Ok(AiSummaryPersistenceOutcomeV1 {
            persisted,
            replayed: !inserted,
        })
    }
    ...
    pub async fn load_summary_run(
        ...
        run_id: [u8; 16],
    ) -> Result<Option<PersistedAiSummaryRunV1>, AiInferencePersistenceErrorV1> {
        if !valid_owner(logical_owner_id) || run_id == [0; 16] {
            return Err(invalid_input());
        }
        let mut transaction = self.begin_owner_transaction(logical_owner_id).await?;
        let persisted =
            load_summary_run_in_transaction(&mut transaction, logical_owner_id, run_id).await?;
        transaction.commit().await.map_err(storage_error)?;
        Ok(persisted)
    }

    pub async fn load_recoverable_summary_runs(
        &self,
        logical_owner_id: &str,
        limit: u32,
    ) -> Result<Vec<PersistedAiSummaryRunV1>, AiInferencePersistenceErrorV1> {
        if !valid_owner(logical_owner_id) || !(1..=AI_INFERENCE_RECOVERY_LIMIT_V1).contains(&limit)
        {
            return Err(invalid_input());
        }
        let mut transaction = self.begin_owner_transaction(logical_owner_id).await?;
        let rows = sqlx::query(SELECT_RECOVERABLE_SUMMARY_RUNS)
    ...
    pub async fn persist_summary_transition(
        &self,
        transition: AiSummaryTransitionV1,
    ) -> Result<PersistedAiSummaryRunV1, AiInferencePersistenceErrorV1> {
        validate_summary_run(&transition.next_run)?;
        let request = &transition.next_run.request;
        let mut transaction = self
            .begin_owner_transaction(&request.logical_owner_id)
            .await?;
        let current_row = sqlx::query(SELECT_SUMMARY_RUN_FOR_UPDATE)
            .bind(&request.logical_owner_id)
            .bind(&request.run_id)
        ...
        let selected = validate_summary_transition(&current, &transition)?;
        let result = transition.next_run.terminal_result.as_ref();
        let receipt = result.and_then(|result| result.inference_receipt.as_ref());
        let updated = sqlx::query(
            "UPDATE makosh_data.ai_summary_runs SET state_revision=$4, run_state=$5,
             selected_provider_settings_revision=$6, result_summary_utf8=$7,
             result_resolved_language=$8, result_resolved_length=$9,
...
async fn load_summary_run_in_transaction(
    transaction: &mut Transaction<'_, Postgres>,
    logical_owner_id: &str,
    run_id: [u8; 16],
) -> Result<Option<PersistedAiSummaryRunV1>, AiInferencePersistenceErrorV1> {
    sqlx::query(SELECT_SUMMARY_RUN)
        .bind(logical_owner_id)
...
const SELECT_SUMMARY_RUN: &str = "SELECT logical_owner_id, run_id, request_digest, context_id,
 ...
 source_custody_proof, requested_language, requested_length, maximum_output_bytes,
 maximum_output_tokens, egress_policy, egress_policy_revision, state_revision, run_state,
 selected_provider_settings_revision, result_summary_utf8, result_resolved_language,
 result_resolved_length, result_model_revision_sha256, result_prompt_policy_sha256,
 result_provider_policy_revision, result_completeness, result_confidence_basis_points,
 result_terminal_status FROM makosh_data.ai_summary_runs WHERE logical_owner_id=$1 AND run_id=$2";
const SELECT_SUMMARY_RUN_FOR_UPDATE: &str =
 ...
 result_resolved_length, result_model_revision_sha256, result_prompt_policy_sha256,
 result_provider_policy_revision, result_completeness, result_confidence_basis_points,
 result_terminal_status FROM makosh_data.ai_summary_runs
 WHERE logical_owner_id=$1 AND run_id=$2 FOR UPDATE";
const SELECT_RECOVERABLE_SUMMARY_RUNS: &str =
    "SELECT logical_owner_id, run_id, request_digest, context_id,
 source_evidence_id, source_evidence_revision, contract_major, contract_revision,
 contract_schema_sha256, source_reference_id, source_declared_bytes, source_sha256,
 source_custody_proof, requested_language, requested_length, maximum_output_bytes,
...
fn persisted_from_row(
    ...
    let result = status
        .map(|status| {
            ...
            Ok(CommunicationSummaryInferenceResultV1 {
        ...
        .transpose()?;
    let request = CommunicationSummaryInferenceRequestV1 {
        run_id: row.try_get("run_id").map_err(row_error)?,
        ...
        length: i32::from(
            row.try_get::<i16, _>("requested_length")
                .map_err(row_error)?,
        ),
    ...
    let persisted = PersistedAiSummaryRunV1 {
        run: AiSummaryRunV1 {
            request,
    ...
    validate_summary_run(&persisted.run)?;
    validate_summary_persisted(&persisted)?;
    Ok(persisted)
}

fn same_semantic_request(
    left: &CommunicationSummaryInferenceRequestV1,
    right: &CommunicationSummaryInferenceRequestV1,
) -> bool {
...
fn optional_i16(value: Option<i32>) -> Result<Option<i16>, AiInferencePersistenceErrorV1> {
    value
        .map(|v| i16::try_from(v).map_err(|_| invalid_input()))
        .transpose()
}
fn optional_i32(value: Option<u32>) -> Result<Option<i32>, AiInferencePersistenceErrorV1> {
    value
        .map(|v| i32::try_from(v).map_err(|_| invalid_input()))
        .transpose()
}
