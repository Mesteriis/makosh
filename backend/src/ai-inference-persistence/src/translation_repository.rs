...
use crate::{
    ...
    translation_model::{
        validate_translation_accepted, validate_translation_persisted, validate_translation_run,
...
impl AiInferencePersistenceV1 {
    pub async fn accept_translation_run(
    ...
    ) -> Result<AiTranslationPersistenceOutcomeV1, AiInferencePersistenceErrorV1> {
        validate_translation_accepted(&run)?;
        ...
        .rows_affected()
            == 1;
        let mut persisted = load_translation_run_in_transaction(
            &mut transaction,
        ...
        if !inserted && persisted.run.request != run.request {
            ...
            if matches!(
                ...
                persisted = load_translation_run_in_transaction(
                    &mut transaction,
                    &request.logical_owner_id,
                    id16(&request.run_id)?,
                )
        ...
        Ok(AiTranslationPersistenceOutcomeV1 {
            persisted,
            replayed: !inserted,
        })
    }
    ...
    pub async fn load_translation_run(
        ...
        run_id: [u8; 16],
    ) -> Result<Option<PersistedAiTranslationRunV1>, AiInferencePersistenceErrorV1> {
        if !valid_owner(logical_owner_id) || run_id == [0; 16] {
            return Err(invalid_input());
        }
        let mut transaction = self.begin_owner_transaction(logical_owner_id).await?;
        let persisted =
            load_translation_run_in_transaction(&mut transaction, logical_owner_id, run_id).await?;
        transaction.commit().await.map_err(storage_error)?;
        Ok(persisted)
    }

    pub async fn load_recoverable_translation_runs(
        &self,
        logical_owner_id: &str,
        limit: u32,
    ) -> Result<Vec<PersistedAiTranslationRunV1>, AiInferencePersistenceErrorV1> {
    ...
    pub async fn persist_translation_transition(
    ...
    ) -> Result<PersistedAiTranslationRunV1, AiInferencePersistenceErrorV1> {
        ...
        let selected = validate_translation_transition(&current, &transition)?;
        ...
        transaction.commit().await.map_err(storage_error)?;
        Ok(PersistedAiTranslationRunV1 {
...
fn persisted_from_row(
    ...
    let result = status
        .map(|status| {
            ...
            Ok(CommunicationTranslationInferenceResultV1 {
    ...
    let persisted = PersistedAiTranslationRunV1 {
        run: AiTranslationRunV1 {
            request,
    ...
    validate_translation_run(&persisted.run)?;
    validate_translation_persisted(&persisted)?;
    Ok(persisted)
}

fn same_semantic_request(
    left: &CommunicationTranslationInferenceRequestV1,
    right: &CommunicationTranslationInferenceRequestV1,
) -> bool {
