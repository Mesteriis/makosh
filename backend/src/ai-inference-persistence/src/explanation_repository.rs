...
use crate::{
    AI_INFERENCE_RECOVERY_LIMIT_V1, AiExplanationPersistenceOutcomeV1, AiExplanationTransitionV1,
    AiInferencePersistenceErrorV1, AiInferencePersistenceV1, PersistedAiExplanationRunV1,
    explanation_model::{
        validate_explanation_accepted, validate_explanation_persisted, validate_explanation_run,
...
impl AiInferencePersistenceV1 {
    pub async fn accept_explanation_run(
    ...
    ) -> Result<AiExplanationPersistenceOutcomeV1, AiInferencePersistenceErrorV1> {
        validate_explanation_accepted(&run)?;
        ...
        let inserted = sqlx::query(
            "INSERT INTO makosh_data.ai_explanation_runs (
               ...
               source_sha256, source_custody_proof, maximum_reasons,
               maximum_reason_text_bytes, maximum_output_tokens, egress_policy,
        ...
        .bind(i32::try_from(request.maximum_reasons).map_err(|_| invalid_input())?)
        .bind(i32::try_from(request.maximum_reason_text_bytes).map_err(|_| invalid_input())?)
        ...
        .rows_affected()
            == 1;
        let mut persisted = load_explanation_run_in_transaction(
            &mut transaction,
        ...
        if !inserted && persisted.run.request != run.request {
            ...
            if matches!(
                ...
                persisted = load_explanation_run_in_transaction(
                    &mut transaction,
                    &request.logical_owner_id,
                    id16(&request.run_id)?,
                )
        ...
        Ok(AiExplanationPersistenceOutcomeV1 {
            persisted,
            replayed: !inserted,
        })
    }
    ...
    pub async fn load_explanation_run(
        ...
        run_id: [u8; 16],
    ) -> Result<Option<PersistedAiExplanationRunV1>, AiInferencePersistenceErrorV1> {
        if !valid_owner(logical_owner_id) || run_id == [0; 16] {
            return Err(invalid_input());
        }
        let mut transaction = self.begin_owner_transaction(logical_owner_id).await?;
        let persisted =
            load_explanation_run_in_transaction(&mut transaction, logical_owner_id, run_id).await?;
        transaction.commit().await.map_err(storage_error)?;
        Ok(persisted)
    }

    pub async fn load_recoverable_explanation_runs(
        &self,
        logical_owner_id: &str,
        limit: u32,
    ) -> Result<Vec<PersistedAiExplanationRunV1>, AiInferencePersistenceErrorV1> {
        if !valid_owner(logical_owner_id) || !(1..=AI_INFERENCE_RECOVERY_LIMIT_V1).contains(&limit)
        {
            return Err(invalid_input());
        }
        let mut transaction = self.begin_owner_transaction(logical_owner_id).await?;
        let rows = sqlx::query(SELECT_RECOVERABLE_EXPLANATION_RUNS)
    ...
    pub async fn persist_explanation_transition(
        &self,
        transition: AiExplanationTransitionV1,
    ) -> Result<PersistedAiExplanationRunV1, AiInferencePersistenceErrorV1> {
        validate_explanation_run(&transition.next_run)?;
        let request = &transition.next_run.request;
        let mut transaction = self
            .begin_owner_transaction(&request.logical_owner_id)
            .await?;
        let current_row = sqlx::query(SELECT_EXPLANATION_RUN_FOR_UPDATE)
            .bind(&request.logical_owner_id)
            .bind(&request.run_id)
        ...
        let selected = validate_explanation_transition(&current, &transition)?;
        let result = transition.next_run.terminal_result.as_ref();
        let result_exact_bytes = result
            .map(encode_explanation_inference_result_v1)
            .transpose()
        ...
        transaction.commit().await.map_err(storage_error)?;
        Ok(PersistedAiExplanationRunV1 {
...
async fn load_explanation_run_in_transaction(
    transaction: &mut Transaction<'_, Postgres>,
    logical_owner_id: &str,
    run_id: [u8; 16],
) -> Result<Option<PersistedAiExplanationRunV1>, AiInferencePersistenceErrorV1> {
    sqlx::query(SELECT_EXPLANATION_RUN)
        .bind(logical_owner_id)
...
const SELECT_EXPLANATION_RUN: &str = "SELECT logical_owner_id, run_id, request_digest, context_id,
...
fn persisted_from_row(
    ...
    let result = match (status, result_bytes) {
        (Some(status), Some(bytes)) => {
            let result = decode_explanation_inference_result_v1(bytes.as_slice())
                .map_err(|_| AiInferencePersistenceErrorV1::InvalidRow)?;
            if result.terminal_status != i32::from(status) {
                return Err(AiInferencePersistenceErrorV1::InvalidRow);
            }
            Some(result)
        }
        (None, None) => None,
    ...
    let request = CommunicationExplanationInferenceRequestV1 {
        ...
        maximum_reasons: positive_u32(row.try_get("maximum_reasons").map_err(row_error)?)?,
        maximum_reason_text_bytes: positive_u32(
            row.try_get("maximum_reason_text_bytes")
                .map_err(row_error)?,
        )?,
    ...
    let persisted = PersistedAiExplanationRunV1 {
        run: AiExplanationRunV1 {
            request,
    ...
    validate_explanation_run(&persisted.run)?;
    validate_explanation_persisted(&persisted)?;
    Ok(persisted)
}

fn same_semantic_request(
    left: &CommunicationExplanationInferenceRequestV1,
    right: &CommunicationExplanationInferenceRequestV1,
) -> bool {
