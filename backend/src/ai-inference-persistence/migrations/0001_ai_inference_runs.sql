CREATE TABLE makosh_data.ai_inference_runs (
    ...
    subject_policy SMALLINT NOT NULL,
    ...
    CHECK (subject_policy BETWEEN 1 AND 3),
    ...
    CHECK (
        selected_provider_settings_revision IS NULL
    ...
    CHECK (
        (
            ...
            AND result_prompt_policy_sha256 IS NULL
            AND result_provider_policy_revision IS NULL
            AND result_completeness IS NULL
            AND result_confidence_basis_points IS NULL
        )
        OR (
            run_state = 3
            AND selected_provider_settings_revision IS NOT NULL
