use makosh_graph_persistence::{GraphPersistenceErrorV1, graph_storage_bundle_v1};
use makosh_graph_runtime::{
    GraphManagedRuntimeErrorV1, GraphManagedRuntimeV1, GraphRuntimeAdmissionV1,
    graph_module_descriptor_v1, graph_settings_schema_bytes_v1,
};
...
fn main() -> Result<(), String> {
    ...
    match command.as_deref() {
        ...
        Some(v) if v == OsStr::new("export-storage-bundle") => {
            export(&mut args, graph_storage_bundle_v1().encode_to_vec())
        }
        Some(v) if v == OsStr::new("export-module-descriptor") => {
            let build = args
                ...
                .filter(|v| !v.is_empty())
                .ok_or_else(|| "Graph descriptor build id is required".to_owned())?;
            export(
                &mut args,
                graph_module_descriptor_v1(&build).encode_to_vec(),
            )
        }
        Some(v) if v == OsStr::new("export-settings-schema") => {
            export(&mut args, graph_settings_schema_bytes_v1())
        }
        _ => Err("Graph runtime command is unavailable".into()),
    }
}
fn serve<I: Iterator<Item = OsString>>(args: &mut std::iter::Peekable<I>) -> Result<(), String> {
    ...
    let schema = decode_settings_schema_v1(&schema_bytes)
        .map_err(|_| "Graph settings schema is invalid".to_owned())?;
    let snapshot = decode_settings_snapshot_v1(&read(&paths.settings_snapshot)?)
        .map_err(|_| "Graph settings snapshot is invalid".to_owned())?;
    ...
    let config =
        ManagedEngineRuntimeConfigurationV1::decode(read(&paths.runtime_configuration)?.as_slice())
            .map_err(|_| "Graph runtime configuration is invalid".to_owned())?;
    ...
    {
        return Err("Graph runtime configuration is stale".into());
    }
    let storage = config
        .storage
        .clone()
        .ok_or_else(|| "Graph storage is unavailable".to_owned())?;
    let admission = GraphRuntimeAdmissionV1 {
    ...
    let executor =
        tokio::runtime::Runtime::new().map_err(|_| "Graph executor is unavailable".to_owned())?;
    let mut runtime = executor
        .block_on(GraphManagedRuntimeV1::open(
            inherited()?,
            descriptor,
            schema_bytes,
            &admission,
    ...
    loop {
        match executor.block_on(runtime.service_once(now()?)) {
            Ok(_) => failures = 0,
            Err(GraphManagedRuntimeErrorV1::ControlClosed) => return Ok(()),
            Err(GraphManagedRuntimeErrorV1::EventUnavailable)
            | Err(GraphManagedRuntimeErrorV1::Persistence(
...
fn runtime_error(error: GraphManagedRuntimeErrorV1) -> String {
    let code = match error {
        GraphManagedRuntimeErrorV1::Admission => "admission",
        GraphManagedRuntimeErrorV1::EventContract => "event_contract",
        GraphManagedRuntimeErrorV1::EventUnavailable => "event_unavailable",
        GraphManagedRuntimeErrorV1::Persistence(_) => "persistence",
        GraphManagedRuntimeErrorV1::ControlClosed => "control_closed",
        GraphManagedRuntimeErrorV1::Unavailable => "unavailable",
    };
    format!("Graph runtime failed: {code}")
}
fn now() -> Result<i64, String> {
    std::time::SystemTime::now()
        ...
        .ok_or_else(|| "Graph clock is invalid".into())
}
fn export<I: Iterator<Item = OsString>>(args: &mut I, bytes: Vec<u8>) -> Result<(), String> {
    if args.next().is_some() {
        return Err("Graph runtime command is unavailable".into());
    }
    std::io::Write::write_all(&mut std::io::stdout(), &bytes)
        .map_err(|_| "Graph artifact is unavailable".into())
}
fn parse<I: Iterator<Item = OsString>>(args: &mut std::iter::Peekable<I>) -> Result<Paths, String> {
    ...
    if args.next().is_some() {
        return Err("Graph runtime arguments are invalid".into());
    }
    Ok(Paths {
        descriptor,
        settings_schema,
...
fn required<I: Iterator<Item = OsString>>(args: &mut I, name: &str) -> Result<String, String> {
    ...
    args.next()
        .and_then(|v| v.into_string().ok())
        .filter(|v| !v.is_empty())
        .ok_or_else(|| "Graph runtime arguments are invalid".into())
}
fn inherited() -> Result<UnixStream, String> {
    let fd = unsafe { libc::dup(std::io::stdin().as_raw_fd()) };
    if fd < 0 {
        return Err("Graph control channel is unavailable".into());
    }
    Ok(unsafe { UnixStream::from_raw_fd(fd) })
}
fn read(path: &Path) -> Result<Vec<u8>, String> {
    let metadata =
        std::fs::symlink_metadata(path).map_err(|_| "Graph contract is unavailable".to_owned())?;
    if metadata.file_type().is_symlink() || !metadata.is_file() || metadata.len() > 512 * 1024 {
        return Err("Graph contract is unavailable".into());
    }
    std::fs::read(path).map_err(|_| "Graph contract is unavailable".into())
}
