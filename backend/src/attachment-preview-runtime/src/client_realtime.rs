...
use crate::contracts::realtime_contract_v1;
    ...
    pub(crate) async fn publish_pending(
        ...
        let transitions = persistence
            .realtime_after(
                logical_owner_id,
                self.last_sequence,
                ATTACHMENT_PREVIEW_REALTIME_LIMIT_V1,
            )
            .await
        ...
        for transition in transitions {
            ...
            let request = ManagedRuntimeClientRealtimePublishRequestV1 {
                ...
                event_id: event_id(transition.run_id, transition.status.state_revision).to_vec(),
                cursor: format!("attachment-preview/{}", transition.sequence),
                event_kind: ATTACHMENT_PREVIEW_REALTIME_EVENT_KIND_V1.to_owned(),
                occurred_at_unix_millis,
                causation_id: String::new(),
                correlation_id: String::new(),
                trace_id: String::new(),
                payload: AttachmentPreviewStatusChangedV1 {
                    run_id: transition.run_id.to_vec(),
                    state: transition.status.state as i32,
                    state_revision: transition.status.state_revision,
                    ...
                    content_type: transition
                        ...
                        .content_type
                        .map_or(0, |value| value as i32),
                    preview_size_bytes: transition.status.preview_size_bytes,
                    truncated: transition.status.truncated,
                    occurred_at_unix_millis,
                    error: transition.status.error.map_or(0, |value| value as i32),
                }
                .encode_to_vec(),
            };
