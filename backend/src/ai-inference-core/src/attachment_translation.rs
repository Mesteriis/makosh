...
pub struct AiAttachmentTranslationRunV1 {
    pub request: AttachmentTranslationInferenceRequestV1,
    pub revision: u64,
    pub state: AiInferenceRunStateV1,
    pub terminal_result: Option<AttachmentTranslationInferenceResultV1>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct AiAttachmentTranslationExecutionPlanV1 {
    pub run_id: [u8; 16],
...
pub fn begin_attachment_translation_inference_v1(
    run: &AiAttachmentTranslationRunV1,
    expected_revision: u64,
) -> Result<
    (
        AiAttachmentTranslationRunV1,
        AiAttachmentTranslationExecutionPlanV1,
    ),
    AiInferenceCoreErrorV1,
> {
    ...
    let plan = attachment_translation_inference_execution_plan_v1(run)?;
    Ok((
        AiAttachmentTranslationRunV1 {
...
pub fn attachment_translation_inference_execution_plan_v1(
    run: &AiAttachmentTranslationRunV1,
) -> Result<AiAttachmentTranslationExecutionPlanV1, AiInferenceCoreErrorV1> {
    if !matches!(
        run.state,
    ...
    Ok(AiAttachmentTranslationExecutionPlanV1 {
...
pub fn build_attachment_translation_provider_input_v1(
    plan: &AiAttachmentTranslationExecutionPlanV1,
    source_content_bytes: &[u8],
) -> Result<Vec<u8>, AiInferenceCoreErrorV1> {
    validate_attachment_translation_source_text_v1(&plan.source, source_content_bytes)
        .map_err(|_| AiInferenceCoreErrorV1::InvalidRequest)?;
    Ok(source_content_bytes.to_vec())
}

pub fn complete_attachment_translation_inference_v1(
    run: &AiAttachmentTranslationRunV1,
    expected_revision: u64,
    ...
    let result = AttachmentTranslationInferenceResultV1 {
        ...
        inference_receipt: ready.then(|| AiInferenceReceiptV1 {
            ...
            prompt_policy_sha256: attachment_translation_prompt_policy_sha256_v1().to_vec(),
...
pub fn reject_attachment_translation_inference_v1(
    run: &AiAttachmentTranslationRunV1,
    expected_revision: u64,
...
#[must_use]
pub fn attachment_translation_prompt_policy_sha256_v1() -> [u8; 32] {
    Sha256::digest(AI_ATTACHMENT_TRANSLATION_PROMPT_POLICY_V1).into()
}
...
mod tests {
    use makosh_ai_contracts::{
        ...
        wire::{
            ...
            AiProviderTranslationResultV1, AiUseCaseV1,
        },
    };

    use super::*;

    fn accepted() -> AiAttachmentTranslationRunV1 {
        accept_attachment_translation_inference_v1(
            seal_attachment_translation_inference_request_v1(
                AttachmentTranslationInferenceRequestV1 {
                    ...
                    source: Some(AiPrivateSourceReceiptV1 {
                        ...
                        declared_bytes: 12,
                        sha256: Sha256::digest(b"Message body").to_vec(),
    ...
    #[test]
    fn attachment_translation_lifecycle_is_distinct_raw_text_and_target_fenced() {
        let accepted = accepted();
        let (executing, plan) =
            begin_attachment_translation_inference_v1(&accepted, 1).expect("executing");
        assert_eq!(
            build_attachment_translation_provider_input_v1(&plan, b"Message body").expect("input"),
            b"Message body"
        );
