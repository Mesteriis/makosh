...
pub struct CommunicationsReplayCommandAdmissionV1 {
...
pub enum CommunicationsReplayCommandInboxOutcomeV1 {
    Accepted,
    DuplicateAccepted,
    DuplicateCompleted,
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum CommunicationsReplayResultStoreOutcomeV1 {
    Stored,
    AlreadyStored,
}
...
impl CommunicationsRetainedEvidenceReplayPersistenceV1 {
    pub async fn accept_replay_command(
        &self,
        admission: &CommunicationsReplayCommandAdmissionV1,
        accepted_at_unix_seconds: i64,
    ) -> Result<CommunicationsReplayCommandInboxOutcomeV1, RetainedCommunicationsReplayErrorV1>
    {
        ...
        if inserted.rows_affected() == 1 {
            ...
            return Ok(CommunicationsReplayCommandInboxOutcomeV1::Accepted);
        }
        let row = sqlx::query(
        ...
        transaction.commit().await.map_err(storage_error)?;
        match state {
            0 => Ok(CommunicationsReplayCommandInboxOutcomeV1::DuplicateAccepted),
            1 => Ok(CommunicationsReplayCommandInboxOutcomeV1::DuplicateCompleted),
            _ => Err(RetainedCommunicationsReplayErrorV1::InvalidRow),
        }
    }
    ...
    pub async fn complete_replay_command(
    ...
    ) -> Result<CommunicationsReplayResultStoreOutcomeV1, RetainedCommunicationsReplayErrorV1> {
        ...
        let outcome = if inserted.rows_affected() == 1 {
            CommunicationsReplayResultStoreOutcomeV1::Stored
        } else {
        ...
        if updated.rows_affected() > 1 {
            return Err(RetainedCommunicationsReplayErrorV1::InvalidRow);
        }
        transaction.commit().await.map_err(storage_error)?;
        Ok(outcome)
    }
    ...
    pub async fn pending_replay_results(
        &self,
        limit: i64,
    ) -> Result<Vec<OutboxRecordV1>, RetainedCommunicationsReplayErrorV1> {
...
fn verify_command_row(
    ...
    {
        return Err(RetainedCommunicationsReplayErrorV1::Conflict);
    }
    Ok(())
}
...
fn verify_result_record(
    ...
    let actual = envelope
        .contract
        .as_ref()
        .ok_or(RetainedCommunicationsReplayErrorV1::WrongContract)?;
    ...
    let payload = ReplayCommunicationsEvidenceResultV1::decode(envelope.payload.as_slice())
        .map_err(row_error)?;
    validate_communications_replay_result_v1(&payload)
...
fn result_from_row(
    row: sqlx::postgres::PgRow,
) -> Result<OutboxRecordV1, RetainedCommunicationsReplayErrorV1> {
    let message_id = row_id::<16>(&row, "message_id")?;
    let envelope_sha256 = row_id::<32>(&row, "envelope_sha256")?;
...
fn row_id<const WIDTH: usize>(
    row: &sqlx::postgres::PgRow,
    column: &str,
) -> Result<[u8; WIDTH], RetainedCommunicationsReplayErrorV1> {
...
fn storage_error<T>(_: T) -> RetainedCommunicationsReplayErrorV1 {
