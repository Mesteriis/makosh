...
fn main() -> ExitCode {
    match arguments(std::env::args().skip(1).collect()) {
        Some(arguments) => {
            if materialize_communications_release_assembly_v1(
            ...
            println!("communications-release-assembly: ok");
            ExitCode::SUCCESS
        }
        None => fail(
...
fn fail(message: &str) -> ExitCode {
    eprintln!("communications-release-assembly: {message}");
    ExitCode::FAILURE
}
