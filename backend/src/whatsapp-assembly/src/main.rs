...
use makosh_whatsapp_assembly::materialize_whatsapp_release_assembly_v1;
...
fn main() -> ExitCode {
    match arguments(std::env::args().skip(1).collect()) {
        Some(arguments) => {
            if materialize_whatsapp_release_assembly_v1(
            ...
            println!("whatsapp-release-assembly: ok");
            ExitCode::SUCCESS
        }
        None => fail(
            "usage: makosh-whatsapp-assembly --build-id <id> --output-dir <absolute-path> \
...
fn fail(message: &str) -> ExitCode {
    eprintln!("whatsapp-release-assembly: {message}");
    ExitCode::FAILURE
}
...
mod tests {
    ...
    fn parses_each_exact_option_once_in_any_order() {
        let parsed = arguments(vec![
            "--runtime".to_owned(),
            "/tmp/makosh-whatsapp-runtime".to_owned(),
            "--build-id".to_owned(),
            "build-1".to_owned(),
            "--output-dir".to_owned(),
            "/tmp/whatsapp-assembly".to_owned(),
        ])
        ...
        assert_eq!(
            ...
            PathBuf::from("/tmp/whatsapp-assembly")
        );
        assert_eq!(
            parsed.runtime,
            PathBuf::from("/tmp/makosh-whatsapp-runtime")
        );
    }
