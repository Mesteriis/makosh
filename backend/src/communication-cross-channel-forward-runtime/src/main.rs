...
use makosh_communication_cross_channel_forward_runtime::{
    CrossChannelForwardBlobTransferErrorV1, CrossChannelForwardManagedRuntimeErrorV1,
    CrossChannelForwardManagedRuntimeV1, CrossChannelForwardRuntimeAdmissionV1,
    ...
    I: Iterator<Item = OsString>,
{
    ...
    decode_settings_schema_v1(&settings_schema)
        .map_err(|_| "Communication Cross-channel Forward settings schema is invalid".to_owned())?;
    let configuration = ManagedWorkflowRuntimeConfigurationV1::decode(
        read_contract(&paths.runtime_configuration)?.as_slice(),
    )
    .map_err(|_| {
        "Communication Cross-channel Forward runtime configuration is invalid".to_owned()
    })?;
    ...
    if configuration.runtime_instance_id != paths.runtime_instance_id {
        return Err(
            "Communication Cross-channel Forward runtime configuration is stale".to_owned(),
        );
    }
    let storage = configuration
        .storage
        .clone()
        .ok_or_else(|| "Communication Cross-channel Forward storage is unavailable".to_owned())?;
    let admission = CrossChannelForwardRuntimeAdmissionV1 {
    ...
    let executor = tokio::runtime::Runtime::new()
        .map_err(|_| "Communication Cross-channel Forward executor is unavailable".to_owned())?;
    let mut runtime = executor
        .block_on(CrossChannelForwardManagedRuntimeV1::open(
...
fn retry_runtime(
    result: Result<bool, CrossChannelForwardManagedRuntimeErrorV1>,
) -> Result<(), String> {
    match result {
        Ok(_)
        | Err(CrossChannelForwardManagedRuntimeErrorV1::EventUnavailable)
        | Err(CrossChannelForwardManagedRuntimeErrorV1::Unavailable)
        | Err(CrossChannelForwardManagedRuntimeErrorV1::Persistence(
            CrossChannelForwardPersistenceErrorV1::StorageUnavailable,
        ))
        | Err(CrossChannelForwardManagedRuntimeErrorV1::Blob(
            CrossChannelForwardBlobTransferErrorV1::Unavailable,
        )) => Ok(()),
        Err(error) => Err(runtime_error(error)),
    }
}

fn runtime_error(error: CrossChannelForwardManagedRuntimeErrorV1) -> String {
    let code = match error {
        CrossChannelForwardManagedRuntimeErrorV1::Admission => "admission",
    ...
    format!("Communication Cross-channel Forward runtime failed: {code}")
}
    ...
    I: Iterator<Item = OsString>,
{
    ...
    if value.is_empty() {
        return Err(format!(
            "Communication Cross-channel Forward {name} is invalid"
        ));
    }
    Ok(value)
}
...
fn inherited_control_channel() -> Result<UnixStream, String> {
    ...
    if duplicated < 0 {
        return Err(
            "Communication Cross-channel Forward control channel is unavailable".to_owned(),
        );
    }
...
fn read_contract(path: &Path) -> Result<Vec<u8>, String> {
    if !path.is_absolute() {
        return Err("Communication Cross-channel Forward contract path is invalid".to_owned());
    }
    let bytes = fs::read(path)
        .map_err(|_| "Communication Cross-channel Forward contract is unavailable".to_owned())?;
    if bytes.is_empty() || bytes.len() > 2 * 1024 * 1024 {
        return Err("Communication Cross-channel Forward contract is invalid".to_owned());
    }
    Ok(bytes)
}
