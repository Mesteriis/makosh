...
use makosh_obligations_assembly::materialize_obligations_release_assembly_v1;
...
fn main() -> ExitCode {
    let Some(arguments) = arguments(std::env::args().skip(1).collect()) else {
        return fail(
            "usage: makosh-obligations-assembly --build-id <id> \
             --output-dir <absolute-path> --runtime <absolute-path>",
        );
    };
    if materialize_obligations_release_assembly_v1(
    ...
    println!("obligations-release-assembly: ok");
    ExitCode::SUCCESS
}
