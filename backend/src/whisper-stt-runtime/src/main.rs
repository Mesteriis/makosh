...
fn main() -> Result<(), String> {
    ...
    match command.as_deref() {
        ...
        Some(command) if command == OsStr::new("export-storage-bundle") => export_bytes(
            &mut arguments,
            whisper_stt_storage_bundle_v1().encode_to_vec(),
        ),
        Some(command) if command == OsStr::new("export-module-descriptor") => {
            let build_id = arguments
                ...
                .ok_or_else(|| "Whisper STT descriptor build id is required".to_owned())?;
            export_bytes(
                &mut arguments,
                whisper_stt_module_descriptor_v1(&build_id).encode_to_vec(),
            )
        }
        Some(command) if command == OsStr::new("export-settings-schema") => {
            export_bytes(&mut arguments, whisper_stt_settings_schema_bytes_v1())
        }
        _ => Err("Whisper STT runtime command is unavailable".to_owned()),
    }
}
    ...
    I: Iterator<Item = OsString>,
{
    ...
    let schema = decode_settings_schema_v1(&settings_schema)
        .map_err(|_| "Whisper STT settings schema is invalid".to_owned())?;
    let snapshot = decode_settings_snapshot_v1(&read_contract(&paths.settings_snapshot)?)
        .map_err(|_| "Whisper STT settings snapshot is invalid".to_owned())?;
    ...
    .map_err(|_| "Whisper STT runtime configuration is invalid".to_owned())?;
    ...
    if configuration.runtime_instance_id != paths.runtime_instance_id {
        return Err("Whisper STT runtime configuration is stale".to_owned());
    }
    let settings =
        decode_whisper_stt_settings_v1(&snapshot, &configuration.configuration_instance_id)
            .map_err(|_| "Whisper STT effective settings are invalid".to_owned())?;
    let resources = prepare_whisper_stt_resources_v1(&configuration.runtime_artifacts)
        .map_err(|_| "Whisper STT native resources are invalid".to_owned())?;
    let storage = configuration
        .storage
        .clone()
        .ok_or_else(|| "Whisper STT storage is unavailable".to_owned())?;
    let admission = WhisperSttRuntimeAdmissionV1 {
    ...
    let executor = tokio::runtime::Runtime::new()
        .map_err(|_| "Whisper STT executor is unavailable".to_owned())?;
    let mut runtime = executor
        .block_on(WhisperSttManagedRuntimeV1::open(
...
fn runtime_error(error: WhisperSttManagedRuntimeErrorV1) -> String {
    let reason = match error {
        WhisperSttManagedRuntimeErrorV1::Admission => "admission_rejected",
        WhisperSttManagedRuntimeErrorV1::Persistence(_) => "storage_unavailable",
        WhisperSttManagedRuntimeErrorV1::Unavailable => "runtime_unavailable",
    };
    format!("Whisper STT runtime failed: {reason}")
}
    ...
    I: Iterator<Item = OsString>,
{
    ...
    std::io::Write::write_all(&mut std::io::stdout(), &bytes)
        .map_err(|_| "Whisper STT runtime artifact is unavailable".to_owned())
}
    ...
    I: Iterator<Item = OsString>,
{
    if arguments.next().as_deref() != Some(OsStr::new(name)) {
        return Err("Whisper STT runtime arguments are invalid".to_owned());
    }
    arguments
        .next()
        .and_then(|value| value.into_string().ok())
        .filter(|value| !value.is_empty())
        .ok_or_else(|| "Whisper STT runtime arguments are invalid".to_owned())
}
    ...
    I: Iterator<Item = OsString>,
{
    if arguments.next().is_some() {
        return Err("Whisper STT runtime command is unavailable".to_owned());
    }
    Ok(())
}
...
fn inherited_control_channel() -> Result<UnixStream, String> {
    ...
    if duplicated < 0 {
        return Err("Whisper STT control channel is unavailable".to_owned());
    }
...
fn read_contract(path: &Path) -> Result<Vec<u8>, String> {
    ...
    let metadata = std::fs::symlink_metadata(path)
        .map_err(|_| "Whisper STT contract is unavailable".to_owned())?;
    if metadata.file_type().is_symlink()
        || !metadata.is_file()
        || metadata.len() > MAX_CONTRACT_BYTES
    {
        return Err("Whisper STT contract is unavailable".to_owned());
    }
    std::fs::read(path).map_err(|_| "Whisper STT contract is unavailable".to_owned())
}
