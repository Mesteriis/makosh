//! Exact Mail delivery-intent durable command decoder.
//!
//! This module owns only the Mail integration boundary. It deliberately has no
...
use makosh_mail_delivery_intent_contract::{
    MAIL_DELIVERY_INTENT_SOURCE_MODULE_ID_V1, MAIL_DELIVERY_INTENT_TARGET_CAPABILITY_ID_V1,
    mail_delivery_intent_execute_contract_reference_v1,
    mail_delivery_intent_rejected_contract_reference_v1, validate_mail_delivery_intent_execute_v1,
    validate_mail_delivery_intent_rejected_v1,
    wire::{
        ExecuteMailDeliveryIntentCommandV1, MailDeliveryIntentRejectCodeV1,
        MailDeliveryIntentRejectedV1,
    },
};
use makosh_mail_persistence::{
    MailDeliveryIntentAdmissionV1, MailDeliveryIntentInboxOutcomeV1, MailDeliveryIntentStoreV1,
...
const MESSAGE_DOMAIN: &[u8] = b"makosh.mail.delivery-intent.execute.v1";
...
pub struct DecodedMailDeliveryIntentV1 {
...
pub enum MailDeliveryIntentDecodeErrorV1 {
    InvalidEnvelope,
    WrongContract,
    WrongSource,
    WrongAudience,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum MailDeliveryIntentConsumeErrorV1 {
    Unavailable,
    Decode(MailDeliveryIntentDecodeErrorV1),
    InvalidResultContext,
    InvalidResultEnvelope,
    Persistence,
}

pub async fn consume_next_mail_delivery_intent_v1(
    store: &MailDeliveryIntentStoreV1,
    connection: &RuntimeJetStreamConnection,
    permit: &RuntimeSubscribePermitV1,
    expected_logical_owner_id: &str,
    result_context: &MailDeliveryIntentResultContextV1,
) -> Result<MailDeliveryIntentInboxOutcomeV1, MailDeliveryIntentConsumeErrorV1> {
    let delivery = receive_runtime_pull_delivery(connection, permit)
        .await
        .map_err(|_| MailDeliveryIntentConsumeErrorV1::Unavailable)?;
    let record = OutboxRecordV1::accept(delivery.exact_bytes().to_vec()).map_err(|_| {
        MailDeliveryIntentConsumeErrorV1::Decode(MailDeliveryIntentDecodeErrorV1::InvalidEnvelope)
    })?;
    let outcome =
        accept_mail_delivery_intent_v1(store, &record, expected_logical_owner_id, result_context)
            .await?;
    delivery
        .acknowledge()
        .await
...
pub async fn accept_mail_delivery_intent_v1(
    store: &MailDeliveryIntentStoreV1,
    record: &OutboxRecordV1,
    expected_logical_owner_id: &str,
    result_context: &MailDeliveryIntentResultContextV1,
) -> Result<MailDeliveryIntentInboxOutcomeV1, MailDeliveryIntentConsumeErrorV1> {
    let decoded = decode_mail_delivery_intent_v1(record, expected_logical_owner_id)
        .map_err(MailDeliveryIntentConsumeErrorV1::Decode)?;
    let route_not_found = build_rejected_outbox_v1(
        &decoded,
        MailDeliveryIntentRejectCodeV1::MailDeliveryIntentRejectCodeRouteNotFound,
        result_context,
    )?;
    store
        .accept_command(
            &MailDeliveryIntentAdmissionV1 {
...
pub fn decode_mail_delivery_intent_v1(
    record: &OutboxRecordV1,
    expected_logical_owner_id: &str,
) -> Result<DecodedMailDeliveryIntentV1, MailDeliveryIntentDecodeErrorV1> {
    let envelope = decode_envelope_v1(record.exact_bytes())
        .map_err(|_| MailDeliveryIntentDecodeErrorV1::InvalidEnvelope)?;
    let expected_contract = mail_delivery_intent_execute_contract_reference_v1();
    if !exact_contract(envelope.contract.as_ref(), &expected_contract) {
        return Err(MailDeliveryIntentDecodeErrorV1::WrongContract);
    }
    let Some(Semantics::Command(metadata)) = envelope.semantics.as_ref() else {
        return Err(MailDeliveryIntentDecodeErrorV1::WrongContract);
    };
    if metadata.target_capability != MAIL_DELIVERY_INTENT_TARGET_CAPABILITY_ID_V1 {
        return Err(MailDeliveryIntentDecodeErrorV1::WrongAudience);
    }
    let source = envelope
        .source
        .as_ref()
        .ok_or(MailDeliveryIntentDecodeErrorV1::WrongSource)?;
    if source.module_id != MAIL_DELIVERY_INTENT_SOURCE_MODULE_ID_V1
        || source.runtime_generation == 0
        || envelope.source_fence.as_ref().is_none_or(|fence| {
            fence.scope_id != MAIL_DELIVERY_INTENT_SOURCE_MODULE_ID_V1.as_bytes()
                || fence.epoch != source.runtime_generation
        })
    {
        return Err(MailDeliveryIntentDecodeErrorV1::WrongSource);
    }

    let command = ExecuteMailDeliveryIntentCommandV1::decode(envelope.payload.as_slice())
        .map_err(|_| MailDeliveryIntentDecodeErrorV1::InvalidPayload)?;
    validate_mail_delivery_intent_execute_v1(&command)
        .map_err(|_| MailDeliveryIntentDecodeErrorV1::InvalidPayload)?;
    if command.logical_owner_id != expected_logical_owner_id {
        return Err(MailDeliveryIntentDecodeErrorV1::OwnerMismatch);
    }
    ...
    let body = command
        .body_source
        .ok_or(MailDeliveryIntentDecodeErrorV1::InvalidPayload)?;
    Ok(DecodedMailDeliveryIntentV1 {
        command_message_id,
...
fn build_rejected_outbox_v1(
    decoded: &DecodedMailDeliveryIntentV1,
    code: MailDeliveryIntentRejectCodeV1,
    context: &MailDeliveryIntentResultContextV1,
) -> Result<OutboxRecordV1, MailDeliveryIntentConsumeErrorV1> {
    ...
    {
        return Err(MailDeliveryIntentConsumeErrorV1::InvalidResultContext);
    }
    ...
    validate_mail_delivery_intent_rejected_v1(&payload)
        .map_err(|_| MailDeliveryIntentConsumeErrorV1::InvalidResultEnvelope)?;
...
fn persistence_error(_: MailDurablePersistenceError) -> MailDeliveryIntentConsumeErrorV1 {
    MailDeliveryIntentConsumeErrorV1::Persistence
}

fn outbox_error(_: OutboxRecordError) -> MailDeliveryIntentConsumeErrorV1 {
    MailDeliveryIntentConsumeErrorV1::InvalidResultEnvelope
}

fn id16(value: &[u8]) -> Result<[u8; 16], MailDeliveryIntentDecodeErrorV1> {
    value
        .try_into()
        .map_err(|_| MailDeliveryIntentDecodeErrorV1::InvalidPayload)
}

fn id32(value: &[u8]) -> Result<[u8; 32], MailDeliveryIntentDecodeErrorV1> {
    value
        .try_into()
        .map_err(|_| MailDeliveryIntentDecodeErrorV1::InvalidPayload)
}
...
mod tests {
    ...
    use makosh_mail_delivery_intent_contract::{
        ...
        wire::MailDeliveryIntentBodySourceReceiptV1,
    };
    use makosh_runtime_protocol::v1::ContractReferenceV1;
    use prost_types::Timestamp;
    ...
    #[test]
    fn decoder_accepts_only_exact_mail_contract_source_audience_and_owner() {
        let record = command_record("owner-1");
        let decoded =
            decode_mail_delivery_intent_v1(&record, "owner-1").expect("exact Mail command");
        ...
        assert_eq!(
            decode_mail_delivery_intent_v1(&record, "other-owner"),
            Err(MailDeliveryIntentDecodeErrorV1::OwnerMismatch)
        );
    }

    #[test]
    fn decoder_rejects_a_valid_envelope_with_the_wrong_target_capability() {
        let record = command_record_with("owner-1", "telegram.delivery-intent.execute.v1");
        assert_eq!(
            decode_mail_delivery_intent_v1(&record, "owner-1"),
            Err(MailDeliveryIntentDecodeErrorV1::WrongAudience)
        );
    }

    #[test]
    fn route_failure_result_is_causally_bound_and_contains_no_free_text() {
        let decoded = decode_mail_delivery_intent_v1(&command_record("owner-1"), "owner-1")
            .expect("exact Mail command");
        ...
        let payload = MailDeliveryIntentRejectedV1::decode(envelope.payload.as_slice())
            .expect("typed rejection");
        assert_eq!(
            payload.code,
            MailDeliveryIntentRejectCodeV1::MailDeliveryIntentRejectCodeRouteNotFound as i32
        );
        assert!(
            !record
                .exact_bytes()
    ...
    fn command_record(owner: &str) -> OutboxRecordV1 {
        command_record_with(owner, MAIL_DELIVERY_INTENT_TARGET_CAPABILITY_ID_V1)
    }

    fn command_record_with(owner: &str, target_capability: &str) -> OutboxRecordV1 {
        let intent_id = [1; 16];
        let contract = mail_delivery_intent_execute_contract_reference_v1();
        let payload = ExecuteMailDeliveryIntentCommandV1 {
            ...
            body_source: Some(MailDeliveryIntentBodySourceReceiptV1 {
        ...
        let envelope = DurableEnvelopeV1 {
            ...
            source: Some(SourceRefV1 {
                module_id: MAIL_DELIVERY_INTENT_SOURCE_MODULE_ID_V1.to_owned(),
            ...
            actor: Some(ActorRefV1 {
                kind: ActorKindV1::Module as i32,
                actor_id: MAIL_DELIVERY_INTENT_SOURCE_MODULE_ID_V1.as_bytes().to_vec(),
            }),
            trace: None,
            source_fence: Some(SourceFenceV1 {
                kind: FenceKindV1::RuntimeLease as i32,
                scope_id: MAIL_DELIVERY_INTENT_SOURCE_MODULE_ID_V1.as_bytes().to_vec(),
                epoch: 1,
            }),
