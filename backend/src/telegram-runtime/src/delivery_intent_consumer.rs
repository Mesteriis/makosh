//! Exact Telegram delivery-intent durable command decoder.
//!
//! This module owns only the Telegram integration boundary. It deliberately has no
...
use makosh_telegram_delivery_intent_contract::{
    ...
    wire::{
        ExecuteTelegramDeliveryIntentCommandV1, TelegramDeliveryIntentRejectCodeV1,
...
pub enum TelegramDeliveryIntentDecodeErrorV1 {
    InvalidEnvelope,
    WrongContract,
    WrongSource,
    WrongAudience,
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct TelegramDeliveryIntentResultContextV1 {
...
pub async fn accept_telegram_delivery_intent_v1(
    ...
    let decoded = decode_telegram_delivery_intent_v1(record, expected_logical_owner_id)
        .map_err(TelegramDeliveryIntentConsumeErrorV1::Decode)?;
    let route_not_found = build_rejected_outbox_v1(
        &decoded,
        TelegramDeliveryIntentRejectCodeV1::TelegramDeliveryIntentRejectCodeRouteNotFound,
        result_context,
    )?;
    store
        .accept_command(
            &TelegramDeliveryIntentAdmissionV1 {
...
pub fn decode_telegram_delivery_intent_v1(
    record: &OutboxRecordV1,
    expected_logical_owner_id: &str,
) -> Result<DecodedTelegramDeliveryIntentV1, TelegramDeliveryIntentDecodeErrorV1> {
    let envelope = decode_envelope_v1(record.exact_bytes())
        .map_err(|_| TelegramDeliveryIntentDecodeErrorV1::InvalidEnvelope)?;
    let expected_contract = telegram_delivery_intent_execute_contract_reference_v1();
    if !exact_contract(envelope.contract.as_ref(), &expected_contract) {
        return Err(TelegramDeliveryIntentDecodeErrorV1::WrongContract);
    }
    let Some(Semantics::Command(metadata)) = envelope.semantics.as_ref() else {
        return Err(TelegramDeliveryIntentDecodeErrorV1::WrongContract);
    };
    if metadata.target_capability != TELEGRAM_DELIVERY_INTENT_TARGET_CAPABILITY_ID_V1 {
        return Err(TelegramDeliveryIntentDecodeErrorV1::WrongAudience);
    }
    let source = envelope
        .source
        .as_ref()
        .ok_or(TelegramDeliveryIntentDecodeErrorV1::WrongSource)?;
    if source.module_id != TELEGRAM_DELIVERY_INTENT_SOURCE_MODULE_ID_V1
        || source.runtime_generation == 0
        || envelope.source_fence.as_ref().is_none_or(|fence| {
            fence.scope_id != TELEGRAM_DELIVERY_INTENT_SOURCE_MODULE_ID_V1.as_bytes()
                || fence.epoch != source.runtime_generation
        })
    {
        return Err(TelegramDeliveryIntentDecodeErrorV1::WrongSource);
    }

    let command = ExecuteTelegramDeliveryIntentCommandV1::decode(envelope.payload.as_slice())
        .map_err(|_| TelegramDeliveryIntentDecodeErrorV1::InvalidPayload)?;
    validate_telegram_delivery_intent_execute_v1(&command)
        .map_err(|_| TelegramDeliveryIntentDecodeErrorV1::InvalidPayload)?;
    if command.logical_owner_id != expected_logical_owner_id {
        return Err(TelegramDeliveryIntentDecodeErrorV1::OwnerMismatch);
    }
    ...
    let body = command
        .body_source
        .ok_or(TelegramDeliveryIntentDecodeErrorV1::InvalidPayload)?;
    Ok(DecodedTelegramDeliveryIntentV1 {
        command_message_id,
...
fn build_rejected_outbox_v1(
    decoded: &DecodedTelegramDeliveryIntentV1,
    code: TelegramDeliveryIntentRejectCodeV1,
    context: &TelegramDeliveryIntentResultContextV1,
) -> Result<OutboxRecordV1, TelegramDeliveryIntentConsumeErrorV1> {
    ...
    {
        return Err(TelegramDeliveryIntentConsumeErrorV1::InvalidResultContext);
    }
    let payload = TelegramDeliveryIntentRejectedV1 {
    ...
    validate_telegram_delivery_intent_rejected_v1(&payload)
        .map_err(|_| TelegramDeliveryIntentConsumeErrorV1::InvalidResultEnvelope)?;
...
fn persistence_error(_: TelegramDurablePersistenceError) -> TelegramDeliveryIntentConsumeErrorV1 {
    TelegramDeliveryIntentConsumeErrorV1::Persistence
}

fn outbox_error(_: OutboxRecordError) -> TelegramDeliveryIntentConsumeErrorV1 {
    TelegramDeliveryIntentConsumeErrorV1::InvalidResultEnvelope
}

fn id16(value: &[u8]) -> Result<[u8; 16], TelegramDeliveryIntentDecodeErrorV1> {
    value
        .try_into()
        .map_err(|_| TelegramDeliveryIntentDecodeErrorV1::InvalidPayload)
}

fn id32(value: &[u8]) -> Result<[u8; 32], TelegramDeliveryIntentDecodeErrorV1> {
    value
        .try_into()
        .map_err(|_| TelegramDeliveryIntentDecodeErrorV1::InvalidPayload)
}
...
mod tests {
    ...
    fn decoder_accepts_only_exact_telegram_contract_source_audience_and_owner() {
        ...
        let decoded =
            decode_telegram_delivery_intent_v1(&record, "owner-1").expect("exact Telegram command");
        ...
        assert_eq!(
            decode_telegram_delivery_intent_v1(&record, "other-owner"),
            Err(TelegramDeliveryIntentDecodeErrorV1::OwnerMismatch)
        );
    }
    ...
    fn decoder_rejects_a_valid_envelope_with_the_wrong_target_capability() {
        ...
        assert_eq!(
            decode_telegram_delivery_intent_v1(&record, "owner-1"),
            Err(TelegramDeliveryIntentDecodeErrorV1::WrongAudience)
        );
    }

    #[test]
    fn route_failure_result_is_causally_bound_and_contains_no_free_text() {
        let decoded = decode_telegram_delivery_intent_v1(&command_record("owner-1"), "owner-1")
            .expect("exact Telegram command");
    ...
    fn command_record(owner: &str) -> OutboxRecordV1 {
        command_record_with(owner, TELEGRAM_DELIVERY_INTENT_TARGET_CAPABILITY_ID_V1)
    }

    fn command_record_with(owner: &str, target_capability: &str) -> OutboxRecordV1 {
        let intent_id = [1; 16];
        let contract = telegram_delivery_intent_execute_contract_reference_v1();
        ...
        let envelope = DurableEnvelopeV1 {
            ...
            source: Some(SourceRefV1 {
                module_id: TELEGRAM_DELIVERY_INTENT_SOURCE_MODULE_ID_V1.to_owned(),
            ...
            actor: Some(ActorRefV1 {
                kind: ActorKindV1::Module as i32,
                actor_id: TELEGRAM_DELIVERY_INTENT_SOURCE_MODULE_ID_V1
                    .as_bytes()
                    .to_vec(),
            }),
            trace: None,
            source_fence: Some(SourceFenceV1 {
                kind: FenceKindV1::RuntimeLease as i32,
                scope_id: TELEGRAM_DELIVERY_INTENT_SOURCE_MODULE_ID_V1
                    .as_bytes()
                    .to_vec(),
                epoch: 1,
            }),
