//! Telegram-owned terminal delivery-intent result envelopes.
...
use makosh_telegram_delivery_intent_contract::{
    ...
    validate_telegram_delivery_intent_rejected_v1, validate_telegram_delivery_intent_succeeded_v1,
    wire::{
        TelegramDeliveryIntentRejectCodeV1, TelegramDeliveryIntentRejectedV1,
        TelegramDeliveryIntentSucceededV1,
    },
};
use makosh_telegram_persistence::TelegramDeliveryIntentJobV1;
use prost::Message;
use prost_types::Timestamp;
use sha2::{Digest, Sha256};

const SUCCEEDED_MESSAGE_DOMAIN: &[u8] = b"makosh.telegram.delivery-intent.succeeded.v1";
...
pub enum TelegramDeliveryIntentResultErrorV1 {
    InvalidContext,
    InvalidEnvelope,
}

pub fn build_telegram_delivery_intent_succeeded_outbox_v1(
    job: &TelegramDeliveryIntentJobV1,
    execution_attempt: u32,
    context: &TelegramDeliveryIntentResultContextV1,
) -> Result<OutboxRecordV1, TelegramDeliveryIntentResultErrorV1> {
    let payload = TelegramDeliveryIntentSucceededV1 {
    ...
    validate_telegram_delivery_intent_succeeded_v1(&payload)
        .map_err(|_| TelegramDeliveryIntentResultErrorV1::InvalidEnvelope)?;
...
pub fn build_telegram_delivery_intent_rejected_outbox_v1(
    job: &TelegramDeliveryIntentJobV1,
...
fn build_result_outbox_v1(
    ...
    if context.runtime_instance_id.is_empty()
        ...
        || execution_attempt == 0
    {
        return Err(TelegramDeliveryIntentResultErrorV1::InvalidContext);
    }
...
fn outbox_error(_: OutboxRecordError) -> TelegramDeliveryIntentResultErrorV1 {
    TelegramDeliveryIntentResultErrorV1::InvalidEnvelope
}
...
mod tests {
    ...
    fn terminal_results_are_causally_bound_and_provider_neutral() {
        let job = job();
        let context = context();
        let succeeded = build_telegram_delivery_intent_succeeded_outbox_v1(&job, 2, &context)
        ...
        let rejected = build_telegram_delivery_intent_rejected_outbox_v1(
            &job,
    ...
    fn context() -> TelegramDeliveryIntentResultContextV1 {
    ...
    fn job() -> TelegramDeliveryIntentJobV1 {
        TelegramDeliveryIntentJobV1 {
            ...
            provider_operation_id: "telegram-delivery-intent-01010101010101010101010101010101"
                .to_owned(),
        }
    }
}
