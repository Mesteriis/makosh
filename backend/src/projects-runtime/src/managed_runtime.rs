...
use makosh_projects_api::PROJECTS_OWNER_ID_V1;
...
use crate::{
    ProjectsClientRuntimeContextV1, dispatch_projects_client_request_v1,
    event_outbox::{ProjectsEventRelayErrorV1, relay_projects_outbox_once_v1},
};

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ProjectsRuntimeAdmissionV1 {
...
pub enum ProjectsManagedRuntimeErrorV1 {
    Admission,
    EventUnavailable,
...
pub struct ProjectsManagedRuntimeV1 {
    admission: ProjectsRuntimeAdmissionV1,
...
impl ProjectsManagedRuntimeV1 {
    ...
    pub async fn open(
        ...
        let vault_public_key = storage_configuration
            ...
            .as_slice()
            .try_into()
            .map_err(|_| ProjectsManagedRuntimeErrorV1::Admission)?;
        ...
        let password =
            std::str::from_utf8(&password).map_err(|_| ProjectsManagedRuntimeErrorV1::Admission)?;
        let persistence = ProjectsPersistenceV1::connect_runtime(
            &binding,
        ...
        .await
        .map_err(ProjectsManagedRuntimeErrorV1::Persistence)?;
        persistence
        ...
        .map_err(|_| ProjectsManagedRuntimeErrorV1::EventUnavailable)?;
        ...
        control_channel
            ...
            .map_err(|_| ProjectsManagedRuntimeErrorV1::Unavailable)?;
        Ok(Self {
    ...
    pub async fn pump_control_once(
        &mut self,
        now_unix_millis: i64,
    ) -> Result<bool, ProjectsManagedRuntimeErrorV1> {
        ...
        let response = dispatch_projects_client_request_v1(
            &self.persistence,
            &self.admission.logical_human_owner_id,
            request,
            ProjectsClientRuntimeContextV1 {
        ...
        if validate_module_client_response_v1(&response).is_err() {
            return Err(ProjectsManagedRuntimeErrorV1::Unavailable);
        }
    ...
    fn write_control_error(
        ...
        error_code: &str,
    ) -> Result<(), ProjectsManagedRuntimeErrorV1> {
        self.control_channel
            .write_response(
                correlation_id,
                ManagedRuntimeControlResponseV1 {
...
fn validate_admission(
    admission: &ProjectsRuntimeAdmissionV1,
) -> Result<[u8; 16], ProjectsManagedRuntimeErrorV1> {
    if admission.logical_owner_id != PROJECTS_OWNER_ID_V1
    ...
    runtime_reference_reference(&admission.runtime_instance_id)
        .ok_or(ProjectsManagedRuntimeErrorV1::Admission)
}
...
async fn resolve_storage_credential(
...
) -> Result<zeroize::Zeroizing<Vec<u8>>, ProjectsManagedRuntimeErrorV1> {
    for attempt in 0..20 {
    ...
    Err(ProjectsManagedRuntimeErrorV1::Unavailable)
}

fn storage_binding(
    configuration: &ManagedStorageRuntimeConfigurationV1,
    admission: &ProjectsRuntimeAdmissionV1,
) -> Result<StorageBindingV1, ProjectsManagedRuntimeErrorV1> {
    if configuration.runtime_instance_id != admission.runtime_instance_id
        || configuration.logical_owner_id != PROJECTS_OWNER_ID_V1
        || configuration.owner != PROJECTS_OWNER_ID_V1
...
fn runtime_reference_reference(value: &str) -> Option<[u8; 16]> {
    if value.len() != 32 {
        return None;
    }
    let mut bytes = [0; 16];
...
fn relay_error(error: ProjectsEventRelayErrorV1) -> ProjectsManagedRuntimeErrorV1 {
    match error {
        ProjectsEventRelayErrorV1::InvalidTimestamp => ProjectsManagedRuntimeErrorV1::Admission,
        ProjectsEventRelayErrorV1::Persistence(error) => {
            ProjectsManagedRuntimeErrorV1::Persistence(error)
        }
        ProjectsEventRelayErrorV1::EventUnavailable => {
            ProjectsManagedRuntimeErrorV1::EventUnavailable
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn admission_requires_exact_owner_and_runtime_reference() {
        let valid = ProjectsRuntimeAdmissionV1 {
        ...
        assert_eq!(
            ...
            Err(ProjectsManagedRuntimeErrorV1::Admission)
        );
    }
}
