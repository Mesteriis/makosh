//! Zulip-owned release assembly artifact materialization.
//!
...
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ZulipReleaseArtifactFragmentV1 {
    pub version: u32,
    pub owner_id: String,
    pub module_id: String,
    pub artifacts: Vec<ZulipReleaseArtifactInputV1>,
}

#[derive(Clone, Debug, PartialEq, Eq)]
pub struct ZulipReleaseAssemblyPathsV1 {
...
pub enum ZulipReleaseAssemblyErrorV1 {
    InvalidInput,
...
pub fn materialize_zulip_release_assembly_v1(
    output_directory: &Path,
    build_id: &str,
    runtime_source: &Path,
) -> Result<ZulipReleaseAssemblyPathsV1, ZulipReleaseAssemblyErrorV1> {
    ...
    {
        return Err(ZulipReleaseAssemblyErrorV1::InvalidCanonicalArtifact);
    }

    let descriptor_bytes = descriptor.encode_to_vec();
    let settings_bytes = settings_schema.encode_to_vec();
    let storage_bytes = storage_bundle.encode_to_vec();
    let paths = ZulipReleaseAssemblyPathsV1 {
        descriptor: output_directory.join(ZULIP_DESCRIPTOR_FILE),
        settings_schema: output_directory.join(ZULIP_SETTINGS_FILE),
        storage_bundle: output_directory.join(ZULIP_STORAGE_BUNDLE_FILE),
        artifact_fragment: output_directory.join(ZULIP_ARTIFACT_FRAGMENT_FILE),
    };
    ...
    let fragment_bytes = serde_json::to_vec_pretty(&fragment)
        .map_err(|_| ZulipReleaseAssemblyErrorV1::FragmentEncodingFailed)?;

    let mut builder = DirBuilder::new();
    builder.mode(0o700);
    builder
        .create(output_directory)
        .map_err(|_| ZulipReleaseAssemblyErrorV1::OutputUnavailable)?;
    ...
    for (path, bytes) in writes {
        if write_new_private_file(path, bytes).is_err() {
            ...
            return Err(ZulipReleaseAssemblyErrorV1::ArtifactWriteFailed);
        }
    }
    Ok(paths)
}
...
fn validate_inputs(
    ...
    build_id: &str,
    runtime_source: &Path,
) -> Result<(), ZulipReleaseAssemblyErrorV1> {
    ...
    {
        return Err(ZulipReleaseAssemblyErrorV1::InvalidInput);
    }
    Ok(())
}
...
fn artifact_fragment(
    ...
    storage_bundle: &Path,
) -> Result<ZulipReleaseArtifactFragmentV1, ZulipReleaseAssemblyErrorV1> {
    let artifacts = vec![
        ZulipReleaseArtifactInputV1::ModuleRuntime(ModuleRuntimeArtifactInputV1 {
            artifact_kind: "module_runtime".to_owned(),
            artifact_id: ZULIP_RUNTIME_ARTIFACT_ID.to_owned(),
            relative_path: ZULIP_RUNTIME_RELATIVE_PATH.to_owned(),
            source_path: utf8_path(runtime_source)?,
            required: true,
            descriptor: ReleaseContractInputV1 {
                relative_path: ZULIP_DESCRIPTOR_RELATIVE_PATH.to_owned(),
                source_path: utf8_path(descriptor)?,
            },
            settings_schema: ReleaseContractInputV1 {
                relative_path: ZULIP_SETTINGS_RELATIVE_PATH.to_owned(),
                source_path: utf8_path(settings_schema)?,
            },
        }),
        ZulipReleaseArtifactInputV1::StorageBundle(StorageBundleArtifactInputV1 {
            artifact_kind: "storage_bundle".to_owned(),
            artifact_id: ZULIP_STORAGE_ARTIFACT_ID.to_owned(),
            relative_path: ZULIP_STORAGE_RELATIVE_PATH.to_owned(),
    ...
    Ok(ZulipReleaseArtifactFragmentV1 {
        version: ZULIP_ASSEMBLY_FRAGMENT_VERSION_V1,
        owner_id: ZULIP_ASSEMBLY_OWNER_ID.to_owned(),
        module_id: ZULIP_ASSEMBLY_MODULE_ID.to_owned(),
        artifacts,
    })
}

fn utf8_path(path: &Path) -> Result<String, ZulipReleaseAssemblyErrorV1> {
    path.to_str()
        .filter(|value| !value.is_empty())
        .map(str::to_owned)
        .ok_or(ZulipReleaseAssemblyErrorV1::InvalidInput)
}
...
mod tests {
    ...
    fn materializes_exact_canonical_artifacts_and_sorted_unsigned_fragment() {
        ...
        let paths = materialize_zulip_release_assembly_v1(&output, "build-1", &runtime)
            .expect("materialize Zulip assembly");
        ...
        let fragment: ZulipReleaseArtifactFragmentV1 =
        ...
        assert_eq!(
            fragment
                .artifacts
                .iter()
                .map(ZulipReleaseArtifactInputV1::artifact_id)
                .collect::<Vec<_>>(),
            vec![ZULIP_RUNTIME_ARTIFACT_ID, ZULIP_STORAGE_ARTIFACT_ID]
        );
        assert_eq!(fragment.version, ZULIP_ASSEMBLY_FRAGMENT_VERSION_V1);
        assert_eq!(fragment.owner_id, ZULIP_ASSEMBLY_OWNER_ID);
        assert_eq!(fragment.module_id, ZULIP_ASSEMBLY_MODULE_ID);
    ...
    fn canonical_contract_bytes_are_deterministic_and_output_never_overwrites() {
        ...
        let runtime = root.join("runtime");
        fs::write(&runtime, b"runtime").expect("runtime fixture");
        let first = materialize_zulip_release_assembly_v1(&root.join("first"), "build-1", &runtime)
            .expect("first assembly");
        let second =
            materialize_zulip_release_assembly_v1(&root.join("second"), "build-1", &runtime)
        ...
        assert_eq!(
            materialize_zulip_release_assembly_v1(&root.join("first"), "build-1", &runtime),
            Err(ZulipReleaseAssemblyErrorV1::InvalidInput)
        );
    ...
    fn rejects_relative_missing_empty_and_symlinked_release_inputs() {
        ...
        for (output, build_id, source) in [
            ...
            assert_eq!(
                materialize_zulip_release_assembly_v1(&output, build_id, &source),
                Err(ZulipReleaseAssemblyErrorV1::InvalidInput)
            );
        }
        ...
        assert_eq!(
            materialize_zulip_release_assembly_v1(&existing_file, "build-1", &runtime),
            Err(ZulipReleaseAssemblyErrorV1::InvalidInput)
        );
