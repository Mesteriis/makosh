//! WhatsApp-owned terminal delivery-intent result envelopes.
...
use makosh_whatsapp_delivery_intent_contract::{
    validate_whatsapp_delivery_intent_rejected_v1, validate_whatsapp_delivery_intent_succeeded_v1,
    whatsapp_delivery_intent_rejected_contract_reference_v1,
    whatsapp_delivery_intent_succeeded_contract_reference_v1,
    wire::{
        WhatsAppDeliveryIntentRejectCodeV1, WhatsAppDeliveryIntentRejectedV1,
        WhatsAppDeliveryIntentSucceededV1,
    },
};
use makosh_whatsapp_persistence::WhatsAppDeliveryIntentJobV1;
use prost::Message;
use prost_types::Timestamp;
use sha2::{Digest, Sha256};

const SUCCEEDED_MESSAGE_DOMAIN: &[u8] = b"makosh.whatsapp.delivery-intent.succeeded.v1";
...
pub enum WhatsAppDeliveryIntentResultErrorV1 {
    InvalidContext,
    InvalidEnvelope,
}

pub fn build_whatsapp_delivery_intent_succeeded_outbox_v1(
    job: &WhatsAppDeliveryIntentJobV1,
    execution_attempt: u32,
    context: &WhatsAppDeliveryIntentResultContextV1,
) -> Result<OutboxRecordV1, WhatsAppDeliveryIntentResultErrorV1> {
    let payload = WhatsAppDeliveryIntentSucceededV1 {
    ...
    validate_whatsapp_delivery_intent_succeeded_v1(&payload)
        .map_err(|_| WhatsAppDeliveryIntentResultErrorV1::InvalidEnvelope)?;
...
pub fn build_whatsapp_delivery_intent_rejected_outbox_v1(
    job: &WhatsAppDeliveryIntentJobV1,
...
fn build_result_outbox_v1(
    ...
    if context.runtime_instance_id.is_empty()
        ...
        || execution_attempt == 0
    {
        return Err(WhatsAppDeliveryIntentResultErrorV1::InvalidContext);
    }
...
fn outbox_error(_: OutboxRecordError) -> WhatsAppDeliveryIntentResultErrorV1 {
    WhatsAppDeliveryIntentResultErrorV1::InvalidEnvelope
}
...
mod tests {
    ...
    fn terminal_results_are_causally_bound_and_provider_neutral() {
        let job = job();
        let context = context();
        let succeeded = build_whatsapp_delivery_intent_succeeded_outbox_v1(&job, 2, &context)
        ...
        let rejected = build_whatsapp_delivery_intent_rejected_outbox_v1(
            &job,
    ...
    fn context() -> WhatsAppDeliveryIntentResultContextV1 {
    ...
    fn job() -> WhatsAppDeliveryIntentJobV1 {
        WhatsAppDeliveryIntentJobV1 {
            ...
            provider_operation_id: "whatsapp-delivery-intent-01010101010101010101010101010101"
                .to_owned(),
        }
    }
}
