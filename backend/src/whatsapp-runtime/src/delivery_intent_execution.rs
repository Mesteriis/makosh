...
use crate::enqueue_provider_command;

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum WhatsAppDeliveryIntentExecutionErrorV1 {
    InvalidJob,
    InvalidBody,
    ControlChannel,
    CustodyDenied,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct WhatsAppDeliveryIntentTargetBodyReceiptV1 {
    pub reference_id: [u8; 16],
    pub receipt_sha256: [u8; 32],
}

pub fn transfer_whatsapp_delivery_intent_body_v1(
    control_channel: &mut ManagedControlChannelV2<UnixStream>,
    claimed: &ClaimedWhatsAppDeliveryIntentJobV1,
) -> Result<WhatsAppDeliveryIntentTargetBodyReceiptV1, WhatsAppDeliveryIntentExecutionErrorV1> {
    if claimed.state != WhatsAppDeliveryIntentJobStateV1::PendingCustody
        || claimed.target_body_reference_id.is_some()
        || claimed.target_body_receipt_sha256.is_some()
    {
        return Err(WhatsAppDeliveryIntentExecutionErrorV1::InvalidJob);
    }
    with_blocking_control_channel(control_channel, |channel| {
        ...
        .map_err(|_| WhatsAppDeliveryIntentExecutionErrorV1::CustodyDenied)?;
        ...
        BlobDataClient::new(transfer.data_socket_path)
            ...
            .map_err(|_| WhatsAppDeliveryIntentExecutionErrorV1::BlobUnavailable)?;
        Ok(WhatsAppDeliveryIntentTargetBodyReceiptV1 {
            reference_id,
            receipt_sha256: claimed.job.body_sha256,
        })
    })
...
pub fn read_whatsapp_delivery_intent_body_v1(
    control_channel: &mut ManagedControlChannelV2<UnixStream>,
    claimed: &ClaimedWhatsAppDeliveryIntentJobV1,
) -> Result<Vec<u8>, WhatsAppDeliveryIntentExecutionErrorV1> {
    if claimed.state != WhatsAppDeliveryIntentJobStateV1::BodyReady {
        return Err(WhatsAppDeliveryIntentExecutionErrorV1::InvalidJob);
    }
    let reference_id = claimed
        .target_body_reference_id
        .ok_or(WhatsAppDeliveryIntentExecutionErrorV1::InvalidJob)?;
    let receipt_sha256 = claimed
        .target_body_receipt_sha256
        .ok_or(WhatsAppDeliveryIntentExecutionErrorV1::InvalidJob)?;
    if receipt_sha256 != claimed.job.body_sha256 {
        return Err(WhatsAppDeliveryIntentExecutionErrorV1::InvalidBody);
    }
    ...
    materialize_whatsapp_delivery_intent_v1(&claimed.job, &bytes)?;
    Ok(bytes)
}

pub fn materialize_whatsapp_delivery_intent_v1(
    job: &WhatsAppDeliveryIntentJobV1,
    body: &[u8],
) -> Result<WhatsAppProviderCommand, WhatsAppDeliveryIntentExecutionErrorV1> {
    let body_len = u64::try_from(body.len())
        .map_err(|_| WhatsAppDeliveryIntentExecutionErrorV1::InvalidBody)?;
...
pub async fn enqueue_whatsapp_delivery_intent_v1(
    durable: &WhatsAppDurablePersistence,
    job: &WhatsAppDeliveryIntentJobV1,
    body: &[u8],
    requested_at_unix_seconds: i64,
) -> Result<(), WhatsAppDeliveryIntentExecutionErrorV1> {
    if requested_at_unix_seconds <= 0 {
        return Err(WhatsAppDeliveryIntentExecutionErrorV1::InvalidJob);
    }
    let command = materialize_whatsapp_delivery_intent_v1(job, body)?;
    enqueue_provider_command(durable, &command, requested_at_unix_seconds)
        .await
        .map_err(|_| WhatsAppDeliveryIntentExecutionErrorV1::QueueUnavailable)?;
    Ok(())
}
...
fn with_blocking_control_channel<T>(
    ...
    ) -> Result<T, WhatsAppDeliveryIntentExecutionErrorV1>,
) -> Result<T, WhatsAppDeliveryIntentExecutionErrorV1> {
...
fn prepare_blocking_control_channel(
    ...
    channel
        ...
        .map_err(|_| WhatsAppDeliveryIntentExecutionErrorV1::ControlChannel)
}
...
mod tests {
    ...
    fn job(body: &[u8]) -> WhatsAppDeliveryIntentJobV1 {
        WhatsAppDeliveryIntentJobV1 {
            intent_id: [1; 16],
            command_message_id: [2; 16],
            command_envelope_sha256: [5; 32],
            logical_owner_id: "owner-1".to_owned(),
            account_id: "whatsapp-account".to_owned(),
    ...
    #[test]
    fn materializes_exact_provider_route_into_existing_whatsapp_queue_contract() {
        let body = b"Reply body";
        let request = materialize_whatsapp_delivery_intent_v1(&job(body), body).expect("request");
    ...
    fn materializes_a_non_reply_as_the_existing_send_text_command() {
        ...
        assert_eq!(
            materialize_whatsapp_delivery_intent_v1(&job, body),
            Ok(WhatsAppProviderCommand::SendText {
    ...
    fn rejects_body_bytes_that_do_not_match_the_admitted_receipt() {
        ...
        assert_eq!(
            materialize_whatsapp_delivery_intent_v1(&admitted, b"different"),
            Err(WhatsAppDeliveryIntentExecutionErrorV1::InvalidBody),
        );
    }
    ...
    fn rejects_non_utf8_body_instead_of_reinterpreting_provider_content() {
        ...
        assert_eq!(
            materialize_whatsapp_delivery_intent_v1(&job(&body), &body),
            Err(WhatsAppDeliveryIntentExecutionErrorV1::InvalidBody),
        );
    }
}
