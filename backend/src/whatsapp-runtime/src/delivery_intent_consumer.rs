//! Exact WhatsApp delivery-intent durable command decoder.
//!
//! This module owns only the WhatsApp integration boundary. It deliberately has no
...
use makosh_whatsapp_delivery_intent_contract::{
    WHATSAPP_DELIVERY_INTENT_SOURCE_MODULE_ID_V1, WHATSAPP_DELIVERY_INTENT_TARGET_CAPABILITY_ID_V1,
    ...
    wire::{
        ExecuteWhatsAppDeliveryIntentCommandV1, WhatsAppDeliveryIntentRejectCodeV1,
        WhatsAppDeliveryIntentRejectedV1,
    },
};
use makosh_whatsapp_persistence::{
    WhatsAppDeliveryIntentAdmissionV1, WhatsAppDeliveryIntentInboxOutcomeV1,
    WhatsAppDeliveryIntentStoreV1, WhatsAppDurablePersistenceError,
};
use prost::Message;
...
const REJECTED_MESSAGE_DOMAIN: &[u8] = b"makosh.whatsapp.delivery-intent.rejected.v1";
const WHATSAPP_RUNTIME_MODULE_ID: &str = "makosh-whatsapp-runtime";

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct DecodedWhatsAppDeliveryIntentV1 {
...
pub enum WhatsAppDeliveryIntentDecodeErrorV1 {
    InvalidEnvelope,
    WrongContract,
    WrongSource,
    WrongAudience,
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct WhatsAppDeliveryIntentResultContextV1 {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum WhatsAppDeliveryIntentConsumeErrorV1 {
    Unavailable,
    Decode(WhatsAppDeliveryIntentDecodeErrorV1),
    InvalidResultContext,
    InvalidResultEnvelope,
    Persistence,
}

pub async fn consume_next_whatsapp_delivery_intent_v1(
    store: &WhatsAppDeliveryIntentStoreV1,
    connection: &RuntimeJetStreamConnection,
    permit: &RuntimeSubscribePermitV1,
    expected_logical_owner_id: &str,
    result_context: &WhatsAppDeliveryIntentResultContextV1,
) -> Result<WhatsAppDeliveryIntentInboxOutcomeV1, WhatsAppDeliveryIntentConsumeErrorV1> {
    let delivery = receive_runtime_pull_delivery(connection, permit)
        .await
        .map_err(|_| WhatsAppDeliveryIntentConsumeErrorV1::Unavailable)?;
    let record = OutboxRecordV1::accept(delivery.exact_bytes().to_vec()).map_err(|_| {
        WhatsAppDeliveryIntentConsumeErrorV1::Decode(
            WhatsAppDeliveryIntentDecodeErrorV1::InvalidEnvelope,
        )
    })?;
    let outcome = accept_whatsapp_delivery_intent_v1(
        store,
        &record,
...
pub async fn accept_whatsapp_delivery_intent_v1(
    store: &WhatsAppDeliveryIntentStoreV1,
    record: &OutboxRecordV1,
    expected_logical_owner_id: &str,
    result_context: &WhatsAppDeliveryIntentResultContextV1,
) -> Result<WhatsAppDeliveryIntentInboxOutcomeV1, WhatsAppDeliveryIntentConsumeErrorV1> {
    let decoded = decode_whatsapp_delivery_intent_v1(record, expected_logical_owner_id)
        .map_err(WhatsAppDeliveryIntentConsumeErrorV1::Decode)?;
    let route_not_found = build_rejected_outbox_v1(
        &decoded,
        WhatsAppDeliveryIntentRejectCodeV1::WhatsappDeliveryIntentRejectCodeRouteNotFound,
        result_context,
    )?;
    store
        .accept_command(
            &WhatsAppDeliveryIntentAdmissionV1 {
...
pub fn decode_whatsapp_delivery_intent_v1(
    record: &OutboxRecordV1,
    expected_logical_owner_id: &str,
) -> Result<DecodedWhatsAppDeliveryIntentV1, WhatsAppDeliveryIntentDecodeErrorV1> {
    let envelope = decode_envelope_v1(record.exact_bytes())
        .map_err(|_| WhatsAppDeliveryIntentDecodeErrorV1::InvalidEnvelope)?;
    let expected_contract = whatsapp_delivery_intent_execute_contract_reference_v1();
    if !exact_contract(envelope.contract.as_ref(), &expected_contract) {
        return Err(WhatsAppDeliveryIntentDecodeErrorV1::WrongContract);
    }
    let Some(Semantics::Command(metadata)) = envelope.semantics.as_ref() else {
        return Err(WhatsAppDeliveryIntentDecodeErrorV1::WrongContract);
    };
    if metadata.target_capability != WHATSAPP_DELIVERY_INTENT_TARGET_CAPABILITY_ID_V1 {
        return Err(WhatsAppDeliveryIntentDecodeErrorV1::WrongAudience);
    }
    let source = envelope
        .source
        .as_ref()
        .ok_or(WhatsAppDeliveryIntentDecodeErrorV1::WrongSource)?;
    if source.module_id != WHATSAPP_DELIVERY_INTENT_SOURCE_MODULE_ID_V1
        || source.runtime_generation == 0
        || envelope.source_fence.as_ref().is_none_or(|fence| {
            fence.scope_id != WHATSAPP_DELIVERY_INTENT_SOURCE_MODULE_ID_V1.as_bytes()
                || fence.epoch != source.runtime_generation
        })
    {
        return Err(WhatsAppDeliveryIntentDecodeErrorV1::WrongSource);
    }

    let command = ExecuteWhatsAppDeliveryIntentCommandV1::decode(envelope.payload.as_slice())
        .map_err(|_| WhatsAppDeliveryIntentDecodeErrorV1::InvalidPayload)?;
    validate_whatsapp_delivery_intent_execute_v1(&command)
        .map_err(|_| WhatsAppDeliveryIntentDecodeErrorV1::InvalidPayload)?;
    if command.logical_owner_id != expected_logical_owner_id {
        return Err(WhatsAppDeliveryIntentDecodeErrorV1::OwnerMismatch);
    }
    ...
    let body = command
        .body_source
        .ok_or(WhatsAppDeliveryIntentDecodeErrorV1::InvalidPayload)?;
    Ok(DecodedWhatsAppDeliveryIntentV1 {
        command_message_id,
...
fn build_rejected_outbox_v1(
    decoded: &DecodedWhatsAppDeliveryIntentV1,
    code: WhatsAppDeliveryIntentRejectCodeV1,
    context: &WhatsAppDeliveryIntentResultContextV1,
) -> Result<OutboxRecordV1, WhatsAppDeliveryIntentConsumeErrorV1> {
    ...
    {
        return Err(WhatsAppDeliveryIntentConsumeErrorV1::InvalidResultContext);
    }
    let payload = WhatsAppDeliveryIntentRejectedV1 {
        intent_id: decoded.intent_id.to_vec(),
        logical_owner_id: decoded.logical_owner_id.clone(),
        code: code as i32,
    };
    validate_whatsapp_delivery_intent_rejected_v1(&payload)
        .map_err(|_| WhatsAppDeliveryIntentConsumeErrorV1::InvalidResultEnvelope)?;
    ...
    let envelope = DurableEnvelopeV1 {
        ...
        source: Some(SourceRefV1 {
            module_id: WHATSAPP_RUNTIME_MODULE_ID.to_owned(),
        ...
        actor: Some(ActorRefV1 {
            ...
            actor_id: WHATSAPP_RUNTIME_MODULE_ID.as_bytes().to_vec(),
        }),
        trace: None,
        source_fence: Some(SourceFenceV1 {
            kind: FenceKindV1::RuntimeLease as i32,
            scope_id: WHATSAPP_RUNTIME_MODULE_ID.as_bytes().to_vec(),
...
fn persistence_error(_: WhatsAppDurablePersistenceError) -> WhatsAppDeliveryIntentConsumeErrorV1 {
    WhatsAppDeliveryIntentConsumeErrorV1::Persistence
}

fn outbox_error(_: OutboxRecordError) -> WhatsAppDeliveryIntentConsumeErrorV1 {
    WhatsAppDeliveryIntentConsumeErrorV1::InvalidResultEnvelope
}

fn id16(value: &[u8]) -> Result<[u8; 16], WhatsAppDeliveryIntentDecodeErrorV1> {
    value
        .try_into()
        .map_err(|_| WhatsAppDeliveryIntentDecodeErrorV1::InvalidPayload)
}

fn id32(value: &[u8]) -> Result<[u8; 32], WhatsAppDeliveryIntentDecodeErrorV1> {
    value
        .try_into()
        .map_err(|_| WhatsAppDeliveryIntentDecodeErrorV1::InvalidPayload)
}
...
mod tests {
    ...
    use makosh_whatsapp_delivery_intent_contract::{
        ...
        wire::WhatsAppDeliveryIntentBodySourceReceiptV1,
    };
    use prost_types::Timestamp;
    ...
    #[test]
    fn decoder_accepts_only_exact_whatsapp_contract_source_audience_and_owner() {
        let record = command_record("owner-1");
        let decoded =
            decode_whatsapp_delivery_intent_v1(&record, "owner-1").expect("exact WhatsApp command");
        ...
        assert_eq!(
            decode_whatsapp_delivery_intent_v1(&record, "other-owner"),
            Err(WhatsAppDeliveryIntentDecodeErrorV1::OwnerMismatch)
        );
    }

    #[test]
    fn decoder_rejects_a_valid_envelope_with_the_wrong_target_capability() {
        let record = command_record_with("owner-1", "whatsapp.delivery-intent.execute.v1");
        assert_eq!(
            decode_whatsapp_delivery_intent_v1(&record, "owner-1"),
            Err(WhatsAppDeliveryIntentDecodeErrorV1::WrongAudience)
        );
    }

    #[test]
    fn route_failure_result_is_causally_bound_and_contains_no_free_text() {
        let decoded = decode_whatsapp_delivery_intent_v1(&command_record("owner-1"), "owner-1")
            .expect("exact WhatsApp command");
        let record = build_rejected_outbox_v1(
            &decoded,
            WhatsAppDeliveryIntentRejectCodeV1::WhatsappDeliveryIntentRejectCodeRouteNotFound,
            &WhatsAppDeliveryIntentResultContextV1 {
                runtime_instance_id: "whatsapp-runtime-1".to_owned(),
        ...
        let payload = WhatsAppDeliveryIntentRejectedV1::decode(envelope.payload.as_slice())
    ...
    fn command_record(owner: &str) -> OutboxRecordV1 {
        command_record_with(owner, WHATSAPP_DELIVERY_INTENT_TARGET_CAPABILITY_ID_V1)
    }

    fn command_record_with(owner: &str, target_capability: &str) -> OutboxRecordV1 {
        let intent_id = [1; 16];
        let contract = whatsapp_delivery_intent_execute_contract_reference_v1();
        let payload = ExecuteWhatsAppDeliveryIntentCommandV1 {
            ...
            body_source: Some(WhatsAppDeliveryIntentBodySourceReceiptV1 {
        ...
        let envelope = DurableEnvelopeV1 {
            ...
            source: Some(SourceRefV1 {
                module_id: WHATSAPP_DELIVERY_INTENT_SOURCE_MODULE_ID_V1.to_owned(),
            ...
            actor: Some(ActorRefV1 {
                kind: ActorKindV1::Module as i32,
                actor_id: WHATSAPP_DELIVERY_INTENT_SOURCE_MODULE_ID_V1
                    .as_bytes()
                    .to_vec(),
            }),
            trace: None,
            source_fence: Some(SourceFenceV1 {
                kind: FenceKindV1::RuntimeLease as i32,
                scope_id: WHATSAPP_DELIVERY_INTENT_SOURCE_MODULE_ID_V1
                    .as_bytes()
                    .to_vec(),
                epoch: 1,
            }),
