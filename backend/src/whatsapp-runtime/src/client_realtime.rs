...
use makosh_whatsapp_api::{
    ...
    wire::WhatsAppOperationalProjectionChangedV1,
};
use prost::Message;
use sha2::{Digest, Sha256};

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum WhatsAppClientRealtimeErrorV1 {
    InvalidEvent,
    Unavailable,
}
...
) -> Result<(), WhatsAppClientRealtimeErrorV1>
where
    D: ManagedControlRequestDispatcherV2<UnixStream>,
{
    ...
    {
        return Err(WhatsAppClientRealtimeErrorV1::InvalidEvent);
    }
    let account_digest = Sha256::digest(account_id.as_bytes())[..16]
        .iter()
        .map(|byte| format!("{byte:02x}"))
        .collect::<String>();
    let cursor = format!("whatsapp-operational/{account_digest}/{runtime_generation}/{revision}");
    let request = ManagedRuntimeClientRealtimePublishRequestV1 {
        ...
        cursor: cursor.clone(),
        event_kind: WHATSAPP_OPERATIONAL_PROJECTION_CHANGED_CONTRACT_NAME_V1.to_owned(),
        occurred_at_unix_millis,
        causation_id: String::new(),
        correlation_id: String::new(),
        trace_id: String::new(),
        payload: WhatsAppOperationalProjectionChangedV1 {
    ...
    validate_managed_client_realtime_publish_request_v1(&request)
        .map_err(|_| WhatsAppClientRealtimeErrorV1::InvalidEvent)?;
    if set_blocking(channel).is_err() {
        let _ = restore_nonblocking(channel);
        return Err(WhatsAppClientRealtimeErrorV1::Unavailable);
    }
    ...
    let response = response.map_err(|_| WhatsAppClientRealtimeErrorV1::Unavailable)?;
    restored.map_err(|_| WhatsAppClientRealtimeErrorV1::Unavailable)?;
...
fn event_id(owner: &str, generation: u64, account_id: &str, revision: u64) -> [u8; 16] {
    ...
    hash.update(b"makosh.whatsapp.operational.client-realtime.v1\0");
...
mod tests {
    ...
    #[test]
    fn projection_change_exposes_no_whatsapp_content_or_provider_identity() {
        ...
        let server = thread::spawn(move || {
            ...
            let payload =
                WhatsAppOperationalProjectionChangedV1::decode(request.payload.as_slice())
