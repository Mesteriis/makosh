...
use makosh_communication_recipient_suggestion_api::{
    ...
    wire::CommunicationRecipientSuggestionStatusChangedV1,
};
...
use crate::{
    ...
    contracts::communication_recipient_suggestion_realtime_contract_v1,
};

const REPLAY_WINDOW_V1: u16 = 1_024;

#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub(crate) struct CommunicationRecipientSuggestionClientRealtimePublisherV1 {
    last_sequence: Option<u64>,
}

impl CommunicationRecipientSuggestionClientRealtimePublisherV1 {
    pub(crate) async fn publish_pending(
    ...
    ) -> Result<bool, CommunicationRecipientSuggestionClientRealtimeErrorV1> {
        let transitions = persistence
            .client_realtime_window(logical_owner_id, self.last_sequence, REPLAY_WINDOW_V1)
            .await
            .map_err(CommunicationRecipientSuggestionClientRealtimeErrorV1::Persistence)?;
        ...
        for transition in transitions {
            ...
            let payload = CommunicationRecipientSuggestionStatusChangedV1 {
            ...
            let request = ManagedRuntimeClientRealtimePublishRequestV1 {
                ...
                cursor: format!("communication-recipient-suggestion/{}", transition.sequence),
                event_kind: COMMUNICATION_RECIPIENT_SUGGESTION_REALTIME_EVENT_KIND_V1.to_owned(),
                occurred_at_unix_millis,
            ...
            let response = channel
                ...
                .map_err(|_| CommunicationRecipientSuggestionClientRealtimeErrorV1::Unavailable)?;
            if !response.error_code.is_empty() {
                return Err(CommunicationRecipientSuggestionClientRealtimeErrorV1::Unavailable);
            }
...
pub(crate) enum CommunicationRecipientSuggestionClientRealtimeErrorV1 {
    InvalidTransition,
