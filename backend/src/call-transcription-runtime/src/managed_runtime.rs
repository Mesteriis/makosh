...
use crate::{
    client_port::{CallTranscriptionClientRuntimeContextV1, dispatch_client_request_v1},
    client_realtime::{
        CallTranscriptionClientRealtimeErrorV1, CallTranscriptionClientRealtimePublisherV1,
    },
    event_consumer::{
        CallTranscriptionEventConsumerErrorV1, consume_recording_ready_once_v1,
        consume_recording_rejected_once_v1,
    },
    recovery::{CallTranscriptionRecoveryErrorV1, recover_call_transcription_once_v1},
};

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct CallTranscriptionRuntimeAdmissionV1 {
...
pub enum CallTranscriptionManagedRuntimeErrorV1 {
    Admission,
    EventContract,
    EventUnavailable,
    InvalidTransition,
...
pub struct CallTranscriptionManagedRuntimeV1 {
    admission: CallTranscriptionRuntimeAdmissionV1,
    ...
    recording_ready_subscription: RuntimeSubscribePermitV1,
    recording_rejected_subscription: RuntimeSubscribePermitV1,
    client_realtime: CallTranscriptionClientRealtimePublisherV1,
}

impl CallTranscriptionManagedRuntimeV1 {
    ...
    pub async fn open(
    ...
    ) -> Result<Self, CallTranscriptionManagedRuntimeErrorV1> {
        validate_admission(admission)?;
        if event_hub_endpoint.trim().is_empty() || event_credential_revision == 0 {
            return Err(CallTranscriptionManagedRuntimeErrorV1::Admission);
        }
        ...
        let vault_public_key = storage_configuration
            ...
            .as_slice()
            .try_into()
            .map_err(|_| CallTranscriptionManagedRuntimeErrorV1::Admission)?;
        ...
        let password = std::str::from_utf8(&password)
            .map_err(|_| CallTranscriptionManagedRuntimeErrorV1::Admission)?;
        let persistence = CallTranscriptionPersistenceV1::connect_runtime(
            &binding,
        ...
        .await
        .map_err(CallTranscriptionManagedRuntimeErrorV1::Persistence)?;
        persistence
        ...
        .map_err(|_| CallTranscriptionManagedRuntimeErrorV1::EventUnavailable)?;
        ...
        .map_err(|_| CallTranscriptionManagedRuntimeErrorV1::Admission)?;
        let (recording_ready_subscription, recording_rejected_subscription) =
            bind_recording_subscriptions(
                event_access
                    .subscribe_permits(
                        &admission.registration_id,
                        &admission.runtime_instance_id,
        ...
        .await
        .map_err(|_| CallTranscriptionManagedRuntimeErrorV1::EventUnavailable)?;
        let mut client_realtime = CallTranscriptionClientRealtimePublisherV1::default();
        ...
        control_channel
            ...
            .map_err(|_| CallTranscriptionManagedRuntimeErrorV1::Unavailable)?;
        Ok(Self {
            admission: admission.clone(),
            control_channel,
            persistence,
            event_connection,
            recording_ready_subscription,
            recording_rejected_subscription,
            client_realtime,
        })
    }

    pub async fn consume_recording_ready_once(
        &mut self,
        now_unix_millis: i64,
    ) -> Result<bool, CallTranscriptionManagedRuntimeErrorV1> {
        self.control_channel
            .inner_mut()
            .set_nonblocking(false)
            .map_err(|_| CallTranscriptionManagedRuntimeErrorV1::Unavailable)?;
        let mut dispatcher = RejectManagedControlRequestsV2;
        let result = consume_recording_ready_once_v1(
            &self.persistence,
            &self.event_connection,
            &self.recording_ready_subscription,
    ...
    pub async fn consume_recording_rejected_once(
        &self,
        now_unix_millis: i64,
    ) -> Result<bool, CallTranscriptionManagedRuntimeErrorV1> {
        consume_recording_rejected_once_v1(
            &self.persistence,
            &self.event_connection,
            &self.recording_rejected_subscription,
            &self.admission.logical_owner_id,
            now_unix_millis,
        )
        .await
    ...
    pub async fn recover_once(
        ...
        self.control_channel
            ...
            .map_err(|_| CallTranscriptionManagedRuntimeErrorV1::Unavailable)?;
        let mut dispatcher = RejectManagedControlRequestsV2;
        let result = recover_call_transcription_once_v1(
            &self.persistence,
            &mut self.control_channel,
            &mut dispatcher,
            &self.admission.logical_owner_id,
    ...
    pub async fn pump_control_once(
        ...
        if validate_module_client_response_v1(&response).is_err() {
            return Err(CallTranscriptionManagedRuntimeErrorV1::Unavailable);
        }
        self.control_channel
            .write_response(
                correlation_id,
    ...
    fn write_client_error(
        ...
        error_code: &str,
    ) -> Result<(), CallTranscriptionManagedRuntimeErrorV1> {
        self.control_channel
            .write_response(
                correlation_id,
                ManagedRuntimeControlResponseV1 {
...
async fn dispatch_client(
    ...
    if request.logical_owner_id != admission.logical_owner_id {
        return ModuleClientResponseV1 {
            protocol_major: 1,
    ...
    dispatch_client_request_v1(
        persistence,
        &CallTranscriptionClientRuntimeContextV1 {
...
) -> Result<RuntimeSubscribePermitV1, CallTranscriptionManagedRuntimeErrorV1> {
    let mut matching = permits
        .iter()
        .filter(|permit| permit.contract().is_some_and(|actual| actual == contract));
    let permit = matching
        .next()
        .cloned()
        .ok_or(CallTranscriptionManagedRuntimeErrorV1::Admission)?;
...
fn validate_admission(
    ...
    {
        Err(CallTranscriptionManagedRuntimeErrorV1::Admission)
    } else {
        Ok(())
    }
}
...
) -> Result<zeroize::Zeroizing<Vec<u8>>, CallTranscriptionManagedRuntimeErrorV1> {
    for attempt in 0..20 {
...
fn storage_binding(
    ...
    admission: &CallTranscriptionRuntimeAdmissionV1,
) -> Result<StorageBindingV1, CallTranscriptionManagedRuntimeErrorV1> {
...
fn event_consumer_error(
    error: CallTranscriptionEventConsumerErrorV1,
) -> CallTranscriptionManagedRuntimeErrorV1 {
    match error {
        CallTranscriptionEventConsumerErrorV1::InvalidEnvelope => {
            CallTranscriptionManagedRuntimeErrorV1::EventContract
        }
        CallTranscriptionEventConsumerErrorV1::Ingress(error) => match error {
            crate::ingress::CallTranscriptionIngressErrorV1::InvalidEnvelope
            | crate::ingress::CallTranscriptionIngressErrorV1::InvalidPayload => {
                CallTranscriptionManagedRuntimeErrorV1::EventContract
            }
            crate::ingress::CallTranscriptionIngressErrorV1::Blob(_) => {
                CallTranscriptionManagedRuntimeErrorV1::Unavailable
            }
            crate::ingress::CallTranscriptionIngressErrorV1::Persistence(error) => {
                CallTranscriptionManagedRuntimeErrorV1::Persistence(error)
            }
        },
        CallTranscriptionEventConsumerErrorV1::EventUnavailable => {
            CallTranscriptionManagedRuntimeErrorV1::EventUnavailable
        }
    }
}

fn recovery_error(
    error: CallTranscriptionRecoveryErrorV1,
) -> CallTranscriptionManagedRuntimeErrorV1 {
    match error {
        CallTranscriptionRecoveryErrorV1::InvalidAuthority
        | CallTranscriptionRecoveryErrorV1::InvalidDurableState => {
            CallTranscriptionManagedRuntimeErrorV1::InvalidTransition
        }
        CallTranscriptionRecoveryErrorV1::Blob(_) | CallTranscriptionRecoveryErrorV1::Stt(_) => {
            CallTranscriptionManagedRuntimeErrorV1::Unavailable
        }
        CallTranscriptionRecoveryErrorV1::Persistence(error) => {
            CallTranscriptionManagedRuntimeErrorV1::Persistence(error)
        }
    }
}
...
fn client_realtime_error(
    error: CallTranscriptionClientRealtimeErrorV1,
) -> CallTranscriptionManagedRuntimeErrorV1 {
    match error {
        CallTranscriptionClientRealtimeErrorV1::InvalidTransition => {
            CallTranscriptionManagedRuntimeErrorV1::InvalidTransition
        }
        CallTranscriptionClientRealtimeErrorV1::Persistence(error) => {
            CallTranscriptionManagedRuntimeErrorV1::Persistence(error)
        }
        CallTranscriptionClientRealtimeErrorV1::Unavailable => {
            CallTranscriptionManagedRuntimeErrorV1::Unavailable
        }
    }
}
...
mod tests {
    ...
    fn admission_requires_exact_managed_fences() {
        assert_eq!(
            validate_admission(&CallTranscriptionRuntimeAdmissionV1 {
