use makosh_organizations_assembly::materialize_organizations_release_assembly_v1;
...
fn main() -> ExitCode {
    let Some(arguments) = arguments(std::env::args().skip(1).collect()) else {
        return fail(
            "usage: makosh-organizations-assembly --build-id <id> --output-dir <absolute-path> --runtime <absolute-path>",
        );
    };
    if materialize_organizations_release_assembly_v1(
    ...
    println!("organizations-release-assembly: ok");
    ExitCode::SUCCESS
}
