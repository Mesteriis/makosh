...
use makosh_attachment_preview_assembly::materialize_attachment_preview_release_assembly_v1;
...
fn main() -> ExitCode {
    let Some(arguments) = arguments(std::env::args().skip(1).collect()) else {
        return fail(
            "usage: makosh-attachment-preview-assembly --build-id <id> \
             --output-dir <absolute-path> --runtime <absolute-path>",
        );
    };
    if materialize_attachment_preview_release_assembly_v1(
    ...
    println!("attachment-preview-release-assembly: ok");
    ExitCode::SUCCESS
}
...
fn fail(message: &str) -> ExitCode {
    eprintln!("attachment-preview-release-assembly: {message}");
    ExitCode::FAILURE
}
...
mod tests {
    ...
    fn parses_each_exact_option_once() {
        ...
        assert_eq!(parsed.output_directory, PathBuf::from("/tmp/output"));
        assert_eq!(parsed.runtime, PathBuf::from("/tmp/runtime"));
