//! Event-only Communications source preparation for task candidate extraction.
...
use makosh_communications_task_source_api::{
    COMMUNICATION_TASK_SOURCE_BLOB_TARGET_CAPABILITY_ID_V1,
    COMMUNICATION_TASK_SOURCE_BLOB_TARGET_MODULE_ID_V1,
    COMMUNICATION_TASK_SOURCE_BLOB_TARGET_OWNER_ID_V1, COMMUNICATION_TASK_SOURCE_MAX_BYTES_V1,
    COMMUNICATION_TASK_SOURCE_MAX_PROOF_BYTES_V1, COMMUNICATIONS_TASK_SOURCE_CAPABILITY_ID_V1,
    ...
    wire::{
        CommunicationTaskSourceContentReceiptV1, CommunicationTaskSourceContentV1,
        CommunicationTaskSourcePreparedV1, CommunicationTaskSourceRejectCodeV1,
        CommunicationTaskSourceRejectedV1, PrepareCommunicationTaskSourceCommandV1,
    },
};
...
pub enum CommunicationsTaskSourceDeliveryErrorV1 {
    Unavailable,
    InvalidEnvelope,
    InvalidPayload,
    Persistence,
...
pub async fn consume_next_task_source_prepare_v1(
...
) -> Result<CommunicationsConsumeOutcomeV1, CommunicationsTaskSourceDeliveryErrorV1> {
    let delivery = receive_runtime_pull_delivery(connection, permit)
        .await
        .map_err(delivery_error)?;
    let record = OutboxRecordV1::accept(delivery.exact_bytes().to_vec())
        .map_err(|_| CommunicationsTaskSourceDeliveryErrorV1::InvalidEnvelope)?;
    ...
    let plaintext = match read_and_validate_body(control_channel, dispatcher, &snapshot.body) {
        ...
        Err(SourceMaterializationErrorV1::Unavailable) => {
            return Err(CommunicationsTaskSourceDeliveryErrorV1::Unavailable);
        }
    };
    ...
    let outbox = build_communication_task_source_prepared_outbox_record_v1(
        *record.message_id(),
        CommunicationTaskSourcePreparedV1 {
        ...
        &task_source_envelope_context(context),
    )
    .map_err(|_| CommunicationsTaskSourceDeliveryErrorV1::InvalidPayload)?;
        ...
        .await
    {
        ...
        Err(CommunicationsSourceErrorV1::StaleRevision) => {
            persist_rejection(
                ...
                &record,
                &decoded,
                CommunicationTaskSourceRejectCodeV1::CommunicationTaskSourceRejectCodeStaleSourceRevision,
                context,
            )
            .await?
        }
...
fn decode_prepare_command(
    ...
    logical_human_owner_id: &str,
) -> Result<DecodedPrepareCommandV1, CommunicationsTaskSourceDeliveryErrorV1> {
    ...
    if !exact_contract(
        ...
        source.module_id != COMMUNICATION_TASK_SOURCE_BLOB_TARGET_MODULE_ID_V1
            || source.runtime_generation == 0
    }) {
        return Err(CommunicationsTaskSourceDeliveryErrorV1::InvalidEnvelope);
    }
    let Some(Semantics::Command(command)) = envelope.semantics else {
        return Err(CommunicationsTaskSourceDeliveryErrorV1::InvalidEnvelope);
    };
    if command.target_capability != COMMUNICATIONS_TASK_SOURCE_CAPABILITY_ID_V1 {
        return Err(CommunicationsTaskSourceDeliveryErrorV1::InvalidEnvelope);
    }
    let payload = PrepareCommunicationTaskSourceCommandV1::decode(envelope.payload.as_slice())
        .map_err(|_| CommunicationsTaskSourceDeliveryErrorV1::InvalidPayload)?;
    ...
    {
        return Err(CommunicationsTaskSourceDeliveryErrorV1::InvalidPayload);
    }
...
fn write_target_bound_source(
    ...
    snapshot: &CommunicationsSourceSnapshotV1,
    bytes: Vec<u8>,
) -> Result<CommunicationTaskSourceContentReceiptV1, SourceMaterializationErrorV1> {
    let declared_bytes = u64::try_from(bytes.len()).map_err(|_| content_limit_error())?;
    if !(1..=COMMUNICATION_TASK_SOURCE_MAX_BYTES_V1).contains(&declared_bytes) {
        return Err(content_limit_error());
    }
    ...
    let session = request_managed_blob_session_v2(
        ...
        dispatcher,
        ManagedBlobSessionRequestV1 {
            capability_id: COMMUNICATIONS_TASK_SOURCE_BLOB_CAPABILITY_ID,
            ...
            custody_target: Some(ManagedBlobCustodyTargetV1 {
                owner_id: COMMUNICATION_TASK_SOURCE_BLOB_TARGET_OWNER_ID_V1,
                module_id: COMMUNICATION_TASK_SOURCE_BLOB_TARGET_MODULE_ID_V1,
                capability_id: COMMUNICATION_TASK_SOURCE_BLOB_TARGET_CAPABILITY_ID_V1,
            }),
        },
    )
    .map_err(|_| SourceMaterializationErrorV1::Unavailable)?;
    let source_proof = session.custody_transfer_source_proof;
    if source_proof.is_empty() || source_proof.len() > COMMUNICATION_TASK_SOURCE_MAX_PROOF_BYTES_V1
    {
        return Err(policy_error());
    }
    BlobDataClient::new(session.data_socket_path)
        ...
        .map_err(|_| SourceMaterializationErrorV1::Unavailable)?;
    Ok(CommunicationTaskSourceContentReceiptV1 {
...
fn source_reference_id(
    ...
    hasher.update(b"communications-task-source-copy-v1");
...
async fn persist_rejection(
    ...
    code: CommunicationTaskSourceRejectCodeV1,
    context: &CanonicalEventContextV1,
) -> Result<CommunicationsConsumeOutcomeV1, CommunicationsTaskSourceDeliveryErrorV1> {
    let outbox = build_communication_task_source_rejected_outbox_record_v1(
        *record.message_id(),
...
fn task_source_envelope_context(
    context: &CanonicalEventContextV1,
) -> CommunicationTaskSourceEnvelopeContextV1 {
    CommunicationTaskSourceEnvelopeContextV1 {
...
) -> Result<CommunicationTaskSourceRejectCodeV1, CommunicationsTaskSourceDeliveryErrorV1> {
    match error {
        CommunicationsSourceErrorV1::InvalidRequest => Ok(
            CommunicationTaskSourceRejectCodeV1::CommunicationTaskSourceRejectCodeInvalidRequest,
        ),
        CommunicationsSourceErrorV1::SourceMissingOrInactive => Ok(
            CommunicationTaskSourceRejectCodeV1::CommunicationTaskSourceRejectCodeSourceMissingOrInactive,
        ),
        CommunicationsSourceErrorV1::ContentUnavailable => Ok(
            CommunicationTaskSourceRejectCodeV1::CommunicationTaskSourceRejectCodeContentUnavailable,
        ),
        CommunicationsSourceErrorV1::ContentLimit => Ok(
            CommunicationTaskSourceRejectCodeV1::CommunicationTaskSourceRejectCodeContentLimit,
        ),
        ...
        | CommunicationsSourceErrorV1::OutboxConflict => {
            Err(CommunicationsTaskSourceDeliveryErrorV1::Persistence)
        }
    }
}
...
fn policy_error() -> SourceMaterializationErrorV1 {
    SourceMaterializationErrorV1::Policy(
        CommunicationTaskSourceRejectCodeV1::CommunicationTaskSourceRejectCodePolicy,
    )
}
...
fn invalid_utf8_error() -> SourceMaterializationErrorV1 {
    SourceMaterializationErrorV1::Policy(
        CommunicationTaskSourceRejectCodeV1::CommunicationTaskSourceRejectCodeInvalidUtf8,
    )
}

fn persistence_error(_: CommunicationsSourceErrorV1) -> CommunicationsTaskSourceDeliveryErrorV1 {
    CommunicationsTaskSourceDeliveryErrorV1::Persistence
}

fn delivery_error(_: RuntimePullDeliveryErrorV1) -> CommunicationsTaskSourceDeliveryErrorV1 {
    CommunicationsTaskSourceDeliveryErrorV1::Unavailable
}

fn id16(value: &[u8]) -> Result<[u8; 16], CommunicationsTaskSourceDeliveryErrorV1> {
    value
        .try_into()
        .ok()
        .filter(|id: &[u8; 16]| id.iter().any(|byte| *byte != 0))
        .ok_or(CommunicationsTaskSourceDeliveryErrorV1::InvalidPayload)
}
...
enum SourceMaterializationErrorV1 {
    Policy(CommunicationTaskSourceRejectCodeV1),
    Unavailable,
}
...
mod tests {
    ...
    fn typed_source_contains_subject_and_body_without_sender() {
        ...
        let decoded = CommunicationTaskSourceContentV1::decode(encoded.as_slice()).expect("typed");
