//! Event-only Communications source preparation for communication translations.
...
use makosh_communications_ai_source_api::{
    ...
    CommunicationTranslationSourceContentErrorV1, CommunicationTranslationSourceEnvelopeContextV1,
    ...
    wire::{
        CommunicationTranslationSourceContentReceiptV1, CommunicationTranslationSourceContentV1,
        CommunicationTranslationSourcePreparedV1, CommunicationTranslationSourceRejectCodeV1,
...
pub enum CommunicationsTranslationSourceDeliveryErrorV1 {
    Unavailable,
    InvalidEnvelope,
    InvalidPayload,
    Persistence,
...
pub async fn consume_next_translation_source_prepare_v1(
...
) -> Result<CommunicationsConsumeOutcomeV1, CommunicationsTranslationSourceDeliveryErrorV1> {
    let delivery = receive_runtime_pull_delivery(connection, permit)
        .await
        .map_err(delivery_error)?;
    let record = OutboxRecordV1::accept(delivery.exact_bytes().to_vec())
        .map_err(|_| CommunicationsTranslationSourceDeliveryErrorV1::InvalidEnvelope)?;
    ...
    let plaintext = match read_and_validate_body(control_channel, dispatcher, &snapshot.body) {
        ...
        Err(BodyMaterializationErrorV1::Unavailable) => {
            return Err(CommunicationsTranslationSourceDeliveryErrorV1::Unavailable);
        }
    };
    ...
    let outbox = build_communication_translation_source_prepared_outbox_record_v1(
        *record.message_id(),
        CommunicationTranslationSourcePreparedV1 {
        ...
        &translation_source_envelope_context(context),
    )
    .map_err(|_| CommunicationsTranslationSourceDeliveryErrorV1::InvalidPayload)?;
        ...
        .await
    {
        ...
        Err(CommunicationsSourceErrorV1::StaleRevision) => persist_rejection(
            ...
            &record,
            &decoded,
            CommunicationTranslationSourceRejectCodeV1::CommunicationTranslationSourceRejectCodeStaleRevision,
            context,
        )
        .await?,
...
fn decode_prepare_command(
    ...
    logical_human_owner_id: &str,
) -> Result<DecodedPrepareCommandV1, CommunicationsTranslationSourceDeliveryErrorV1> {
    ...
    if !exact_contract(
        ...
        source.module_id != COMMUNICATION_TRANSLATION_SOURCE_BLOB_TARGET_MODULE_ID_V1
            || source.runtime_generation == 0
    }) {
        return Err(CommunicationsTranslationSourceDeliveryErrorV1::InvalidEnvelope);
    }
    let Some(Semantics::Command(command)) = envelope.semantics else {
        return Err(CommunicationsTranslationSourceDeliveryErrorV1::InvalidEnvelope);
    };
    if command.target_capability != COMMUNICATIONS_TRANSLATION_SOURCE_CAPABILITY_ID_V1 {
        return Err(CommunicationsTranslationSourceDeliveryErrorV1::InvalidEnvelope);
    }
    let payload =
        PrepareCommunicationTranslationSourceCommandV1::decode(envelope.payload.as_slice())
            .map_err(|_| CommunicationsTranslationSourceDeliveryErrorV1::InvalidPayload)?;
    ...
    {
        return Err(CommunicationsTranslationSourceDeliveryErrorV1::InvalidPayload);
    }
...
fn read_and_validate_body(
    ...
    if std::str::from_utf8(&bytes).is_err() {
        return Err(BodyMaterializationErrorV1::Policy(
            CommunicationTranslationSourceRejectCodeV1::CommunicationTranslationSourceRejectCodeInvalidUtf8,
        ));
    }
    Ok(bytes)
}
...
fn write_target_bound_source(
    ...
    bytes: Vec<u8>,
) -> Result<CommunicationTranslationSourceContentReceiptV1, BodyMaterializationErrorV1> {
    let bytes = encode_communication_translation_source_content_v1(
    ...
    .map_err(|error| match error {
        CommunicationTranslationSourceContentErrorV1::Invalid => policy_error(),
        CommunicationTranslationSourceContentErrorV1::Limit => content_limit_error(),
    })?;
    let declared_bytes = u64::try_from(bytes.len()).map_err(|_| content_limit_error())?;
    if declared_bytes == 0 || declared_bytes > COMMUNICATION_TRANSLATION_SOURCE_MAX_BYTES_V1 {
    ...
    let session = request_managed_blob_session_v2(
        ...
        dispatcher,
        ManagedBlobSessionRequestV1 {
            capability_id: COMMUNICATIONS_TRANSLATION_SOURCE_BLOB_CAPABILITY_ID,
            ...
            custody_target: Some(ManagedBlobCustodyTargetV1 {
                owner_id: COMMUNICATION_TRANSLATION_SOURCE_BLOB_TARGET_OWNER_ID_V1,
                module_id: COMMUNICATION_TRANSLATION_SOURCE_BLOB_TARGET_MODULE_ID_V1,
                capability_id: COMMUNICATION_TRANSLATION_SOURCE_BLOB_TARGET_CAPABILITY_ID_V1,
            }),
        },
    )
    .map_err(|_| BodyMaterializationErrorV1::Unavailable)?;
    ...
    if source_proof.is_empty()
        || source_proof.len() > COMMUNICATION_TRANSLATION_SOURCE_MAX_PROOF_BYTES_V1
    {
        return Err(policy_error());
    }
    BlobDataClient::new(session.data_socket_path)
        ...
        .map_err(|_| BodyMaterializationErrorV1::Unavailable)?;
    Ok(CommunicationTranslationSourceContentReceiptV1 {
...
fn source_reference_id(
    ...
    hasher.update(b"communications-ai-translation-source-copy-v1");
...
async fn persist_rejection(
    ...
    code: CommunicationTranslationSourceRejectCodeV1,
    context: &CanonicalEventContextV1,
) -> Result<CommunicationsConsumeOutcomeV1, CommunicationsTranslationSourceDeliveryErrorV1> {
    let outbox = build_communication_translation_source_rejected_outbox_record_v1(
        *record.message_id(),
        CommunicationTranslationSourceRejectedV1 {
            run_id: decoded.run_id.to_vec(),
            code: code as i32,
            logical_owner_id: decoded.logical_owner_id.clone(),
        },
...
fn translation_source_envelope_context(
    context: &CanonicalEventContextV1,
...
fn snapshot_rejection_code(
    ...
    CommunicationTranslationSourceRejectCodeV1,
    CommunicationsTranslationSourceDeliveryErrorV1,
> {
    match error {
        CommunicationsSourceErrorV1::InvalidRequest => Ok(
            CommunicationTranslationSourceRejectCodeV1::CommunicationTranslationSourceRejectCodeInvalidRequest,
        ),
        CommunicationsSourceErrorV1::SourceMissingOrInactive => Ok(
            CommunicationTranslationSourceRejectCodeV1::CommunicationTranslationSourceRejectCodeSourceMissingOrInactive,
        ),
        CommunicationsSourceErrorV1::ContentUnavailable => Ok(
            CommunicationTranslationSourceRejectCodeV1::CommunicationTranslationSourceRejectCodeContentUnavailable,
        ),
        CommunicationsSourceErrorV1::ContentLimit => Ok(
            CommunicationTranslationSourceRejectCodeV1::CommunicationTranslationSourceRejectCodeContentLimit,
        ),
        ...
        | CommunicationsSourceErrorV1::OutboxConflict => {
            Err(CommunicationsTranslationSourceDeliveryErrorV1::Persistence)
        }
    }
}
...
fn policy_error() -> BodyMaterializationErrorV1 {
    BodyMaterializationErrorV1::Policy(
        CommunicationTranslationSourceRejectCodeV1::CommunicationTranslationSourceRejectCodePolicy,
    )
}
...
fn delivery_error(_: RuntimePullDeliveryErrorV1) -> CommunicationsTranslationSourceDeliveryErrorV1 {
    CommunicationsTranslationSourceDeliveryErrorV1::Unavailable
}

fn id16(value: &[u8]) -> Result<[u8; 16], CommunicationsTranslationSourceDeliveryErrorV1> {
    value
        .try_into()
        .ok()
        .filter(|id: &[u8; 16]| id.iter().any(|byte| *byte != 0))
        .ok_or(CommunicationsTranslationSourceDeliveryErrorV1::InvalidPayload)
}
...
enum BodyMaterializationErrorV1 {
    Policy(CommunicationTranslationSourceRejectCodeV1),
    Unavailable,
}
...
mod tests {
    ...
    #[test]
    fn translation_reference_is_deterministic_revision_bound_and_distinct_from_reply() {
    ...
    fn wrong_human_owner_is_rejected_before_source_read() {
        let record =
            makosh_communications_ai_source_api::build_communication_translation_source_prepare_outbox_record_v1(
                ...
                1_800_000_030,
                &CommunicationTranslationSourceEnvelopeContextV1 {
                    module_id: COMMUNICATION_TRANSLATION_SOURCE_BLOB_TARGET_MODULE_ID_V1.to_owned(),
                    runtime_instance_id: "translation-source-runtime-1".to_owned(),
        ...
        assert_eq!(
            ...
            Err(CommunicationsTranslationSourceDeliveryErrorV1::InvalidPayload)
        );
    }
}
