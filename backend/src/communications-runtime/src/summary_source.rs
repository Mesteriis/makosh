//! Event-only Communications source preparation for communication summaries.
...
use makosh_communications_ai_source_api::{
    ...
    COMMUNICATIONS_SUMMARY_SOURCE_CAPABILITY_ID_V1, CommunicationSummarySourceContentErrorV1,
    ...
    encode_communication_summary_source_content_v1,
    wire::{
        CommunicationSummarySourceContentReceiptV1, CommunicationSummarySourceContentV1,
        CommunicationSummarySourcePreparedV1, CommunicationSummarySourceRejectCodeV1,
        CommunicationSummarySourceRejectedV1, PrepareCommunicationSummarySourceCommandV1,
    },
};
...
pub enum CommunicationsSummarySourceDeliveryErrorV1 {
    Unavailable,
    InvalidEnvelope,
    InvalidPayload,
    Persistence,
...
pub async fn consume_next_summary_source_prepare_v1(
...
) -> Result<CommunicationsConsumeOutcomeV1, CommunicationsSummarySourceDeliveryErrorV1> {
    ...
    let plaintext = match read_and_validate_body(control_channel, dispatcher, &snapshot.body) {
        ...
        Err(BodyMaterializationErrorV1::Unavailable) => {
            return Err(CommunicationsSummarySourceDeliveryErrorV1::Unavailable);
        }
    };
    ...
    let outbox = build_communication_summary_source_prepared_outbox_record_v1(
        *record.message_id(),
        CommunicationSummarySourcePreparedV1 {
        ...
        &summary_source_envelope_context(context),
    )
...
fn decode_prepare_command(
    ...
    logical_human_owner_id: &str,
) -> Result<DecodedPrepareCommandV1, CommunicationsSummarySourceDeliveryErrorV1> {
    ...
    if !exact_contract(
        ...
        source.module_id != COMMUNICATION_SUMMARY_SOURCE_BLOB_TARGET_MODULE_ID_V1
            || source.runtime_generation == 0
    }) {
        return Err(CommunicationsSummarySourceDeliveryErrorV1::InvalidEnvelope);
    }
    let Some(Semantics::Command(command)) = envelope.semantics else {
        return Err(CommunicationsSummarySourceDeliveryErrorV1::InvalidEnvelope);
    };
    if command.target_capability != COMMUNICATIONS_SUMMARY_SOURCE_CAPABILITY_ID_V1 {
        return Err(CommunicationsSummarySourceDeliveryErrorV1::InvalidEnvelope);
    }
    let payload = PrepareCommunicationSummarySourceCommandV1::decode(envelope.payload.as_slice())
        .map_err(|_| CommunicationsSummarySourceDeliveryErrorV1::InvalidPayload)?;
    ...
    {
        return Err(CommunicationsSummarySourceDeliveryErrorV1::InvalidPayload);
    }
...
fn read_and_validate_body(
    ...
    if std::str::from_utf8(&bytes).is_err() {
        return Err(BodyMaterializationErrorV1::Policy(
            CommunicationSummarySourceRejectCodeV1::CommunicationSummarySourceRejectCodeInvalidUtf8,
        ));
    }
    Ok(bytes)
}
...
fn write_target_bound_source(
    ...
    let bytes =
        encode_communication_summary_source_content_v1(&CommunicationSummarySourceContentV1 {
            sender_utf8: snapshot.sender_utf8.clone(),
            subject_utf8: snapshot.subject_utf8.clone(),
            body_utf8: bytes,
        })
        .map_err(|error| match error {
            CommunicationSummarySourceContentErrorV1::Invalid => policy_error(),
            CommunicationSummarySourceContentErrorV1::Limit => content_limit_error(),
        })?;
    let declared_bytes = u64::try_from(bytes.len()).map_err(|_| content_limit_error())?;
    if declared_bytes == 0 || declared_bytes > COMMUNICATION_SUMMARY_SOURCE_MAX_BYTES_V1 {
    ...
    let session = request_managed_blob_session_v2(
        ...
        dispatcher,
        ManagedBlobSessionRequestV1 {
            capability_id: COMMUNICATIONS_SUMMARY_SOURCE_BLOB_CAPABILITY_ID,
            ...
            custody_target: Some(ManagedBlobCustodyTargetV1 {
                owner_id: COMMUNICATION_SUMMARY_SOURCE_BLOB_TARGET_OWNER_ID_V1,
                module_id: COMMUNICATION_SUMMARY_SOURCE_BLOB_TARGET_MODULE_ID_V1,
                capability_id: COMMUNICATION_SUMMARY_SOURCE_BLOB_TARGET_CAPABILITY_ID_V1,
            }),
        },
    )
    .map_err(|_| BodyMaterializationErrorV1::Unavailable)?;
    ...
    if source_proof.is_empty()
        || source_proof.len() > COMMUNICATION_SUMMARY_SOURCE_MAX_PROOF_BYTES_V1
    {
        return Err(policy_error());
    }
    BlobDataClient::new(session.data_socket_path)
        ...
        .map_err(|_| BodyMaterializationErrorV1::Unavailable)?;
    Ok(CommunicationSummarySourceContentReceiptV1 {
...
fn source_reference_id(
    ...
    hasher.update(b"communications-ai-summary-source-copy-v1");
...
async fn persist_rejection(
    ...
    code: CommunicationSummarySourceRejectCodeV1,
    context: &CanonicalEventContextV1,
) -> Result<CommunicationsConsumeOutcomeV1, CommunicationsSummarySourceDeliveryErrorV1> {
    let outbox = build_communication_summary_source_rejected_outbox_record_v1(
        *record.message_id(),
        CommunicationSummarySourceRejectedV1 {
            run_id: decoded.run_id.to_vec(),
            code: code as i32,
            logical_owner_id: decoded.logical_owner_id.clone(),
        },
...
fn summary_source_envelope_context(
    context: &CanonicalEventContextV1,
) -> CommunicationSummarySourceEnvelopeContextV1 {
    CommunicationSummarySourceEnvelopeContextV1 {
...
fn policy_error() -> BodyMaterializationErrorV1 {
    BodyMaterializationErrorV1::Policy(
        CommunicationSummarySourceRejectCodeV1::CommunicationSummarySourceRejectCodePolicy,
    )
}
...
fn delivery_error(_: RuntimePullDeliveryErrorV1) -> CommunicationsSummarySourceDeliveryErrorV1 {
    CommunicationsSummarySourceDeliveryErrorV1::Unavailable
}

fn id16(value: &[u8]) -> Result<[u8; 16], CommunicationsSummarySourceDeliveryErrorV1> {
    value
        .try_into()
        .ok()
        .filter(|id: &[u8; 16]| id.iter().any(|byte| *byte != 0))
        .ok_or(CommunicationsSummarySourceDeliveryErrorV1::InvalidPayload)
}
...
enum BodyMaterializationErrorV1 {
    Policy(CommunicationSummarySourceRejectCodeV1),
    Unavailable,
}
...
mod tests {
    ...
    #[test]
    fn summary_reference_is_deterministic_revision_bound_and_distinct_from_reply() {
    ...
    fn wrong_human_owner_is_rejected_before_source_read() {
        let record =
            makosh_communications_ai_source_api::build_communication_summary_source_prepare_outbox_record_v1(
                [7; 16],
                [8; 16],
                3,
                "owner-2",
                1_800_000_030,
                &CommunicationSummarySourceEnvelopeContextV1 {
                    module_id: COMMUNICATION_SUMMARY_SOURCE_BLOB_TARGET_MODULE_ID_V1.to_owned(),
                    runtime_instance_id: "summary-source-runtime-1".to_owned(),
        ...
        assert_eq!(
            ...
            Err(CommunicationsSummarySourceDeliveryErrorV1::InvalidPayload)
        );
    }
}
