...
pub const PACKAGE: &str = "makosh-graph-assembly";
pub const GRAPH_FRAGMENT_FILE_V1: &str = "graph.release-artifacts.json";
...
pub fn materialize_graph_release_assembly_v1(
    output: &Path,
    build: &str,
    runtime: &Path,
    ...
    let descriptor = graph_module_descriptor_v1(build);
    let settings_bytes = graph_settings_schema_bytes_v1();
    let settings =
        decode_settings_schema_v1(&settings_bytes).map_err(|_| AssemblyErrorV1::InvalidArtifact)?;
    let storage = graph_storage_bundle_v1();
    ...
    let paths = PathsV1 {
        descriptor: output.join("graph.runtime.descriptor.pb"),
        settings_schema: output.join("graph.runtime.settings.pb"),
        storage_bundle: output.join("graph.storage.bundle.pb"),
        artifact_fragment: output.join(GRAPH_FRAGMENT_FILE_V1),
    };
    let utf8 = |path: &Path| {
        path.to_str()
            .map(str::to_owned)
    ...
    let fragment = FragmentV1 {
        ...
        artifacts: vec![
            ArtifactV1::Runtime(RuntimeInputV1 {
                ...
                artifact_id: "graph.runtime.v1".into(),
                relative_path: "bin/makosh-graph-runtime".into(),
                source_path: utf8(runtime)?,
                required: true,
                descriptor: ContractInputV1 {
                    relative_path: "contracts/graph.runtime.descriptor.pb".into(),
                    source_path: utf8(&paths.descriptor)?,
                },
                settings_schema: ContractInputV1 {
                    relative_path: "contracts/graph.runtime.settings.pb".into(),
                    source_path: utf8(&paths.settings_schema)?,
                },
            }),
            ArtifactV1::Storage(StorageInputV1 {
                artifact_kind: "storage_bundle".into(),
                artifact_id: "graph.storage.v1".into(),
                relative_path: "storage/graph.storage.bundle.pb".into(),
...
mod tests {
    ...
    fn materializes_compiler_shape() {
        let root = std::env::temp_dir().join(format!("makosh-graph-{}", std::process::id()));
        let _ = fs::remove_dir_all(&root);
        fs::create_dir(&root).unwrap();
        let runtime = root.join("runtime");
        fs::write(&runtime, b"runtime").unwrap();
        let paths =
            materialize_graph_release_assembly_v1(&root.join("out"), "build", &runtime).unwrap();
        let fragment: FragmentV1 =
