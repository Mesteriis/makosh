...
use makosh_review_obligation_candidate_persistence::{
    ReviewObligationCandidatePersistenceErrorV1, review_obligation_candidate_storage_bundle_v1,
};
use makosh_review_obligation_candidate_runtime::{
    ...
    ReviewObligationCandidateRuntimeAdmissionV1, review_obligation_candidate_module_descriptor_v1,
...
fn main() -> Result<(), String> {
    ...
    let Some(command) = arguments.next() else {
        return Err("Review Obligation Candidate command is required".to_owned());
    };
    match command.to_str() {
        Some("export-storage-bundle") => export(
            &mut arguments,
            review_obligation_candidate_storage_bundle_v1().encode_to_vec(),
        ),
        Some("export-module-descriptor") => {
            let build_id = required_string(&mut arguments, "build id")?;
            export(
                &mut arguments,
                review_obligation_candidate_module_descriptor_v1(&build_id).encode_to_vec(),
            )
        }
        ...
        _ => Err("Review Obligation Candidate command is invalid".to_owned()),
    }
}
    ...
    I: Iterator<Item = OsString>,
{
    ...
    decode_settings_schema_v1(&settings_schema)
        .map_err(|_| "Review Obligation Candidate settings schema is invalid".to_owned())?;
    let configuration = ManagedDomainRuntimeConfigurationV1::decode(
        read_contract(&paths.runtime_configuration)?.as_slice(),
    )
    .map_err(|_| "Review Obligation Candidate runtime configuration is invalid".to_owned())?;
    ...
    if configuration.runtime_instance_id != paths.runtime_instance_id {
        return Err("Review Obligation Candidate runtime configuration is stale".to_owned());
    }
    let storage = configuration
        .storage
        .clone()
        .ok_or_else(|| "Review Obligation Candidate storage is unavailable".to_owned())?;
    let admission = ReviewObligationCandidateRuntimeAdmissionV1 {
    ...
    let executor = tokio::runtime::Runtime::new()
        .map_err(|_| "Review Obligation Candidate executor is unavailable".to_owned())?;
    let mut runtime = executor
        .block_on(ReviewObligationCandidateManagedRuntimeV1::open(
    ...
    loop {
        let now = std::time::SystemTime::now()
            ...
            .map_err(|_| "Review Obligation Candidate clock is invalid".to_owned())?
            .as_millis()
            .try_into()
            .map_err(|_| "Review Obligation Candidate clock is invalid".to_owned())?;
...
fn retry_runtime(
    result: Result<bool, ReviewObligationCandidateManagedRuntimeErrorV1>,
) -> Result<(), String> {
    match result {
        Ok(_)
        | Err(ReviewObligationCandidateManagedRuntimeErrorV1::EventUnavailable)
        | Err(ReviewObligationCandidateManagedRuntimeErrorV1::Unavailable)
        | Err(ReviewObligationCandidateManagedRuntimeErrorV1::Persistence(
...
fn runtime_error(error: ReviewObligationCandidateManagedRuntimeErrorV1) -> String {
    let code = match error {
        ReviewObligationCandidateManagedRuntimeErrorV1::Admission => "admission",
        ReviewObligationCandidateManagedRuntimeErrorV1::EventContract => "event_contract",
        ReviewObligationCandidateManagedRuntimeErrorV1::EventUnavailable => "event_unavailable",
        ReviewObligationCandidateManagedRuntimeErrorV1::InvalidTransition => "invalid_transition",
        ReviewObligationCandidateManagedRuntimeErrorV1::Persistence(_) => "persistence",
        ReviewObligationCandidateManagedRuntimeErrorV1::Unavailable => "unavailable",
    };
    format!("Review Obligation Candidate runtime failed: {code}")
}
    ...
    I: Iterator<Item = OsString>,
{
    let output = arguments
        .next()
        .map(PathBuf::from)
        .ok_or_else(|| "Review Obligation Candidate output path is required".to_owned())?;
    require_no_arguments(arguments)?;
    fs::write(output, bytes).map_err(|_| "Review Obligation Candidate export failed".to_owned())
}
    ...
    I: Iterator<Item = OsString>,
{
    if name.starts_with("--") && arguments.next().as_deref() != Some(OsStr::new(name)) {
        return Err("Review Obligation Candidate arguments are invalid".to_owned());
    }
    arguments
        .next()
        .and_then(|value| value.into_string().ok())
        .filter(|value| !value.is_empty())
        .ok_or_else(|| format!("Review Obligation Candidate {name} is required"))
}
...
fn inherited_control_channel() -> Result<UnixStream, String> {
    ...
    if duplicated < 0 {
        return Err("Review Obligation Candidate control channel is unavailable".to_owned());
    }
...
fn read_contract(path: &Path) -> Result<Vec<u8>, String> {
    if !path.is_absolute() {
        return Err("Review Obligation Candidate contract path is invalid".to_owned());
    }
    let bytes = fs::read(path)
        .map_err(|_| "Review Obligation Candidate contract is unavailable".to_owned())?;
    if bytes.is_empty() || bytes.len() > 2 * 1024 * 1024 {
        return Err("Review Obligation Candidate contract is invalid".to_owned());
    }
    Ok(bytes)
}
