//! Unsigned Mail Contacts Sync workflow release assembly.
//!
//! This build unit materializes canonical workflow runtime contracts and
//! workflow-owned Storage input. It does not execute workflow, integration, or
//! domain behavior and never receives distribution signing authority.
...
pub const MAIL_CONTACTS_SYNC_ASSEMBLY_FRAGMENT_VERSION_V1: u32 = 1;
pub const MAIL_CONTACTS_SYNC_RUNTIME_ARTIFACT_ID_V1: &str = "mail_contacts_sync.runtime.v1";
pub const MAIL_CONTACTS_SYNC_STORAGE_ARTIFACT_ID_V1: &str = "mail_contacts_sync.storage.v1";
pub const MAIL_CONTACTS_SYNC_DESCRIPTOR_FILE_V1: &str = "mail_contacts_sync.runtime.descriptor.pb";
pub const MAIL_CONTACTS_SYNC_SETTINGS_FILE_V1: &str = "mail_contacts_sync.runtime.settings.pb";
pub const MAIL_CONTACTS_SYNC_STORAGE_BUNDLE_FILE_V1: &str = "mail_contacts_sync.storage.bundle.pb";
...
#[derive(Clone, Debug, Eq, PartialEq, Deserialize, Serialize)]
#[serde(untagged)]
pub enum MailContactsSyncReleaseArtifactInputV1 {
    ModuleRuntime(ModuleRuntimeArtifactInputV1),
    StorageBundle(StorageBundleArtifactInputV1),
}

impl MailContactsSyncReleaseArtifactInputV1 {
...
#[derive(Clone, Debug, Eq, PartialEq, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
pub struct MailContactsSyncReleaseArtifactFragmentV1 {
    pub version: u32,
    pub owner_id: String,
    pub module_id: String,
    pub artifacts: Vec<MailContactsSyncReleaseArtifactInputV1>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct MailContactsSyncReleaseAssemblyPathsV1 {
...
pub enum MailContactsSyncReleaseAssemblyErrorV1 {
    InvalidInput,
...
pub fn materialize_mail_contacts_sync_release_assembly_v1(
    output_directory: &Path,
    build_id: &str,
    runtime_source: &Path,
) -> Result<MailContactsSyncReleaseAssemblyPathsV1, MailContactsSyncReleaseAssemblyErrorV1> {
    validate_inputs(output_directory, build_id, runtime_source)?;
    let descriptor = mail_contacts_sync_module_descriptor_v1(build_id);
    let settings = mail_contacts_sync_settings_schema_v1();
    let storage = mail_contacts_sync_storage_bundle_v1();
    ...
    {
        return Err(MailContactsSyncReleaseAssemblyErrorV1::InvalidCanonicalArtifact);
    }
    let paths = MailContactsSyncReleaseAssemblyPathsV1 {
        descriptor: output_directory.join(MAIL_CONTACTS_SYNC_DESCRIPTOR_FILE_V1),
        settings_schema: output_directory.join(MAIL_CONTACTS_SYNC_SETTINGS_FILE_V1),
        storage_bundle: output_directory.join(MAIL_CONTACTS_SYNC_STORAGE_BUNDLE_FILE_V1),
        artifact_fragment: output_directory.join(MAIL_CONTACTS_SYNC_ARTIFACT_FRAGMENT_FILE_V1),
    };
    ...
    let fragment_bytes = serde_json::to_vec_pretty(&fragment)
        .map_err(|_| MailContactsSyncReleaseAssemblyErrorV1::FragmentEncodingFailed)?;

    let mut directory = DirBuilder::new();
    directory.mode(0o700);
    directory
        .create(output_directory)
        .map_err(|_| MailContactsSyncReleaseAssemblyErrorV1::OutputUnavailable)?;
    let writes = [
    ...
    for (path, bytes) in writes {
        if write_new_private_file(path, &bytes).is_err() {
            ...
            return Err(MailContactsSyncReleaseAssemblyErrorV1::ArtifactWriteFailed);
        }
    }
    Ok(paths)
}
...
fn validate_inputs(
    ...
    build_id: &str,
    runtime_source: &Path,
) -> Result<(), MailContactsSyncReleaseAssemblyErrorV1> {
    ...
    if !output_directory.is_absolute()
        ...
        || !valid_runtime
    {
        return Err(MailContactsSyncReleaseAssemblyErrorV1::InvalidInput);
    }
    Ok(())
}
...
fn artifact_fragment(
    ...
    module_id: &str,
    runtime_source: &Path,
    paths: &MailContactsSyncReleaseAssemblyPathsV1,
) -> Result<MailContactsSyncReleaseArtifactFragmentV1, MailContactsSyncReleaseAssemblyErrorV1> {
    let artifacts = vec![
        MailContactsSyncReleaseArtifactInputV1::ModuleRuntime(ModuleRuntimeArtifactInputV1 {
            artifact_kind: "module_runtime".to_owned(),
            artifact_id: MAIL_CONTACTS_SYNC_RUNTIME_ARTIFACT_ID_V1.to_owned(),
        ...
        MailContactsSyncReleaseArtifactInputV1::StorageBundle(StorageBundleArtifactInputV1 {
            artifact_kind: "storage_bundle".to_owned(),
            artifact_id: MAIL_CONTACTS_SYNC_STORAGE_ARTIFACT_ID_V1.to_owned(),
    ...
    Ok(MailContactsSyncReleaseArtifactFragmentV1 {
        version: MAIL_CONTACTS_SYNC_ASSEMBLY_FRAGMENT_VERSION_V1,
...
fn utf8_path(path: &Path) -> Result<String, MailContactsSyncReleaseAssemblyErrorV1> {
    path.to_str()
        .filter(|value| !value.is_empty())
        .map(str::to_owned)
        .ok_or(MailContactsSyncReleaseAssemblyErrorV1::InvalidInput)
}
...
mod tests {
    ...
    fn materializes_exact_unsigned_workflow_artifacts() {
        ...
        let paths =
            materialize_mail_contacts_sync_release_assembly_v1(&output, "build-1", &runtime)
                .expect("assembly");
        ...
        StorageBundleV1::decode(fs::read(paths.storage_bundle).expect("storage").as_slice())
            .expect("storage");
        let fragment: MailContactsSyncReleaseArtifactFragmentV1 =
            serde_json::from_slice(&fs::read(paths.artifact_fragment).expect("fragment"))
                .expect("fragment");
        assert_eq!(fragment.owner_id, "mail_contacts_sync");
    ...
    fn refuses_existing_output_and_symlink_runtime() {
        ...
        assert_eq!(
            materialize_mail_contacts_sync_release_assembly_v1(&output, "build-1", &runtime),
            Err(MailContactsSyncReleaseAssemblyErrorV1::InvalidInput)
        );
        let symlink = root.join("runtime-link");
        std::os::unix::fs::symlink(&runtime, &symlink).expect("symlink fixture");
        assert_eq!(
            materialize_mail_contacts_sync_release_assembly_v1(
                &root.join("other"),
                "build-1",
                &symlink,
            ),
