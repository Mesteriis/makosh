...
use makosh_mail_contacts_sync_assembly::materialize_mail_contacts_sync_release_assembly_v1;
...
fn main() -> ExitCode {
    let Some(arguments) = arguments(std::env::args().skip(1).collect()) else {
        return fail(
            "usage: makosh-mail-contacts-sync-assembly --build-id <id> \
             --output-dir <absolute-path> --runtime <absolute-path>",
        );
    };
    if materialize_mail_contacts_sync_release_assembly_v1(
    ...
    println!("mail-contacts-sync-release-assembly: ok");
    ExitCode::SUCCESS
}
...
fn fail(message: &str) -> ExitCode {
    eprintln!("mail-contacts-sync-release-assembly: {message}");
    ExitCode::FAILURE
}
