...
use crate::{
    REVIEW_NOTE_CANDIDATE_PROMOTION_CONTRACT_MAJOR_V1,
    REVIEW_NOTE_CANDIDATE_PROMOTION_CONTRACT_REVISION_V1, REVIEW_NOTE_CANDIDATE_PROMOTION_OWNER_V1,
    REVIEW_NOTE_CANDIDATE_PROMOTION_RESULT_CONTRACT_NAME_V1,
    REVIEW_NOTE_CANDIDATE_PROMOTION_SCHEMA_SHA256_V1,
    wire::{ReviewNoteCandidatePromotionOutcomeV1, ReviewNoteCandidatePromotionResultV1},
};

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ReviewNoteCandidatePromotionEnvelopeContextV1 {
...
pub enum ReviewNoteCandidatePromotionEnvelopeBuildErrorV1 {
    InvalidContext,
    InvalidPayload,
    InvalidEnvelope,
    OutboxRejected,
...
pub fn build_review_note_candidate_promotion_result_outbox_record_v1(
    causation_message_id: [u8; 16],
    payload: ReviewNoteCandidatePromotionResultV1,
    context: &ReviewNoteCandidatePromotionEnvelopeContextV1,
) -> Result<OutboxRecordV1, ReviewNoteCandidatePromotionEnvelopeBuildErrorV1> {
    ...
    let outcome = ReviewNoteCandidatePromotionOutcomeV1::try_from(payload.outcome)
        .map_err(|_| ReviewNoteCandidatePromotionEnvelopeBuildErrorV1::InvalidPayload)?;
    let outcome_valid = match outcome {
        ...
        ReviewNoteCandidatePromotionOutcomeV1::ReviewNoteCandidatePromotionOutcomeFailed => {
            payload.note_id.is_none() && payload.failure_code > 0
        }
        ReviewNoteCandidatePromotionOutcomeV1::ReviewNoteCandidatePromotionOutcomeUnspecified => {
            false
        }
    };
    if causation_message_id.iter().all(|byte| *byte == 0)
        ...
        || !outcome_valid
    {
        return Err(ReviewNoteCandidatePromotionEnvelopeBuildErrorV1::InvalidPayload);
    }
    let envelope = DurableEnvelopeV1 {
        ...
        source: Some(SourceRefV1 {
            ...
            runtime_instance_id: digest(
                b"reviewed-note-promotion-runtime-v1",
    ...
    validate_envelope_v1(&envelope)
        .map_err(|_| ReviewNoteCandidatePromotionEnvelopeBuildErrorV1::InvalidEnvelope)?;
    OutboxRecordV1::accept(envelope.encode_to_vec()).map_err(outbox_error)
}

fn validate_context(
    context: &ReviewNoteCandidatePromotionEnvelopeContextV1,
) -> Result<(), ReviewNoteCandidatePromotionEnvelopeBuildErrorV1> {
    ...
    {
        return Err(ReviewNoteCandidatePromotionEnvelopeBuildErrorV1::InvalidContext);
    }
    Ok(())
}

fn id16(value: &[u8]) -> Result<[u8; 16], ReviewNoteCandidatePromotionEnvelopeBuildErrorV1> {
    value
        .try_into()
        .ok()
        .filter(|value: &[u8; 16]| value.iter().any(|byte| *byte != 0))
        .ok_or(ReviewNoteCandidatePromotionEnvelopeBuildErrorV1::InvalidPayload)
}
...
fn timestamp(context: &ReviewNoteCandidatePromotionEnvelopeContextV1) -> Timestamp {
    Timestamp {
...
fn outbox_error(_: OutboxRecordError) -> ReviewNoteCandidatePromotionEnvelopeBuildErrorV1 {
    ReviewNoteCandidatePromotionEnvelopeBuildErrorV1::OutboxRejected
}
...
mod tests {
    ...
    fn exact_result_event_binds_causation_and_review_partition() {
        let record = build_review_note_candidate_promotion_result_outbox_record_v1(
            [9; 16],
            ReviewNoteCandidatePromotionResultV1 {
                ...
                outcome: ReviewNoteCandidatePromotionOutcomeV1::ReviewNoteCandidatePromotionOutcomeSucceeded as i32,
                note_id: Some(vec![4; 16]),
                failure_code: ReviewNoteCandidatePromotionFailureCodeV1::ReviewNoteCandidatePromotionFailureCodeUnspecified as i32,
