...
use makosh_knowledge_assembly::materialize_knowledge_release_assembly_v1;
...
fn main() -> ExitCode {
    let Some(arguments) = arguments(std::env::args().skip(1).collect()) else {
        return fail(
            "usage: makosh-knowledge-assembly --build-id <id> \
             --output-dir <absolute-path> --runtime <absolute-path>",
        );
    };
    if materialize_knowledge_release_assembly_v1(
    ...
    println!("knowledge-release-assembly: ok");
    ExitCode::SUCCESS
}
