...
use makosh_call_transcription_assembly::materialize_release_assembly_v1;
...
fn main() -> ExitCode {
    match arguments(std::env::args().skip(1).collect()) {
        Some(arguments) => {
            if materialize_release_assembly_v1(
            ...
            println!("call-transcription-release-assembly: ok");
            ExitCode::SUCCESS
        }
        None => fail(
            "usage: makosh-call-transcription-assembly --build-id <id> \
...
fn fail(message: &str) -> ExitCode {
    eprintln!("call-transcription-release-assembly: {message}");
    ExitCode::FAILURE
}
...
mod tests {
    ...
    fn parses_exact_options() {
        let parsed = arguments(vec![
            "--runtime".to_owned(),
            "/tmp/makosh-call-transcription-runtime".to_owned(),
            "--build-id".to_owned(),
            "build-1".to_owned(),
            "--output-dir".to_owned(),
            "/tmp/call-transcription-assembly".to_owned(),
        ])
        .expect("arguments");
        assert_eq!(parsed.build_id, "build-1");
        assert_eq!(
            parsed.runtime,
            PathBuf::from("/tmp/makosh-call-transcription-runtime")
        );
    }
}
