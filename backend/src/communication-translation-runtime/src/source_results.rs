use makosh_ai_contracts::{
    ...
    wire::{
        AiContextReceiptV1, AiEgressPolicyV1, AiPrivateSourceReceiptV1, AiTranslationLanguageV1,
...
use makosh_communication_translation_core::{
    ...
    CommunicationTranslationStateV1, CommunicationTranslationTransitionV1,
};
use makosh_communication_translation_persistence::{
    ...
    CommunicationTranslationSourceResultV1,
};
use makosh_communications_ai_source_api::{
    ...
    wire::{CommunicationTranslationSourcePreparedV1, CommunicationTranslationSourceRejectedV1},
};
...
use crate::{
    CommunicationTranslationBlobErrorV1, CommunicationTranslationInferenceErrorV1,
    blob_materialization::{
        CommunicationTranslationSourceBlobReceiptV1, materialize_translation_source_for_ai_v1,
        release_translation_source_blobs_v1,
    },
    inference::complete_communication_translation_inference_v1,
};

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum CommunicationTranslationSourceResultErrorV1 {
    InvalidEnvelope,
    InvalidPayload,
    Blob(CommunicationTranslationBlobErrorV1),
...
pub async fn consume_translation_source_prepared_once_v1(
...
) -> Result<bool, CommunicationTranslationSourceResultErrorV1> {
    let delivery = receive_runtime_pull_delivery(connection, permit)
        .await
        .map_err(event_error)?;
    let record = OutboxRecordV1::accept(delivery.exact_bytes().to_vec())
        .map_err(|_| CommunicationTranslationSourceResultErrorV1::InvalidEnvelope)?;
    let prepared = decode_prepared(&record, expected_logical_owner_id)?;
    let run = persistence
        .load_run(expected_logical_owner_id, &prepared.run_id)
        .await
        .map_err(CommunicationTranslationSourceResultErrorV1::Persistence)?;
    if run.draft.source_message_id.as_slice() != prepared.source_message_id {
        return Err(CommunicationTranslationSourceResultErrorV1::InvalidPayload);
    }
    if matches!(
        run.status.state,
        CommunicationTranslationStateV1::Ready | CommunicationTranslationStateV1::Rejected
    ...
    let source = prepared
        .source_content
        .as_ref()
        .ok_or(CommunicationTranslationSourceResultErrorV1::InvalidPayload)?;
    let materialized = materialize_translation_source_for_ai_v1(
        channel,
        dispatcher,
        prepared.run_id,
        &CommunicationTranslationSourceBlobReceiptV1 {
    ...
    .map_err(CommunicationTranslationSourceResultErrorV1::Blob)?;
    ...
    let persisted = persistence
        .persist_source_result(CommunicationTranslationSourceResultV1 {
            result_message_id: *record.message_id(),
            envelope_sha256: *record.envelope_sha256(),
            logical_owner_id: expected_logical_owner_id.to_owned(),
            run_id: prepared.run_id,
            transition: CommunicationTranslationTransitionV1::SourcePrepared {
    ...
    let persisted = match persisted {
        makosh_communication_translation_persistence::CommunicationTranslationInboxResultV1::Applied(
            value,
        )
        | makosh_communication_translation_persistence::CommunicationTranslationInboxResultV1::Duplicate(
            value,
        ) => value,
    };
    match persisted.status.state {
        CommunicationTranslationStateV1::AwaitingInference => {
            let persisted = persistence
                ...
                .await
                .map_err(CommunicationTranslationSourceResultErrorV1::Persistence)?;
            complete_communication_translation_inference_v1(
                persistence,
                channel,
                dispatcher,
                &persisted,
            ...
            .await
            .map_err(CommunicationTranslationSourceResultErrorV1::Inference)?;
        }
        CommunicationTranslationStateV1::Ready | CommunicationTranslationStateV1::Rejected => {}
        _ => return Err(CommunicationTranslationSourceResultErrorV1::InvalidPayload),
    }
    ...
    let accepted = match terminal.status.state {
        CommunicationTranslationStateV1::Ready => true,
        CommunicationTranslationStateV1::Rejected => false,
        _ => return Err(CommunicationTranslationSourceResultErrorV1::InvalidPayload),
    };
    release_translation_source_blobs_v1(
        channel,
...
pub async fn consume_translation_source_rejected_once_v1(
    ...
    persistence
        .persist_source_result(CommunicationTranslationSourceResultV1 {
            ...
            transition: CommunicationTranslationTransitionV1::Reject(rejected.rejection),
...
struct PreparedSourceV1 {
    ...
    source_content: Option<
        makosh_communications_ai_source_api::wire::CommunicationTranslationSourceContentReceiptV1,
    >,
}

struct RejectedSourceV1 {
    run_id: [u8; 16],
    rejection: CommunicationTranslationRejectionCodeV1,
}

fn decode_prepared(
    record: &OutboxRecordV1,
    expected_logical_owner_id: &str,
) -> Result<PreparedSourceV1, CommunicationTranslationSourceResultErrorV1> {
    ...
    let payload = CommunicationTranslationSourcePreparedV1::decode(envelope.payload.as_slice())
        .map_err(|_| CommunicationTranslationSourceResultErrorV1::InvalidPayload)?;
...
fn decode_rejected(
    record: &OutboxRecordV1,
    expected_logical_owner_id: &str,
) -> Result<RejectedSourceV1, CommunicationTranslationSourceResultErrorV1> {
    let envelope = decode_envelope_v1(record.exact_bytes())
        .map_err(|_| CommunicationTranslationSourceResultErrorV1::InvalidEnvelope)?;
    let run_id = validate_result_envelope(
        &envelope.contract,
    ...
    let payload = CommunicationTranslationSourceRejectedV1::decode(envelope.payload.as_slice())
        .map_err(|_| CommunicationTranslationSourceResultErrorV1::InvalidPayload)?;
    if payload.logical_owner_id != expected_logical_owner_id || payload.run_id.as_slice() != run_id
    {
        return Err(CommunicationTranslationSourceResultErrorV1::InvalidPayload);
    ...
    let rejection = match payload.code {
        1 => CommunicationTranslationRejectionCodeV1::InvalidRequest,
        7 => CommunicationTranslationRejectionCodeV1::Policy,
        2..=6 => CommunicationTranslationRejectionCodeV1::SourceRejected,
        _ => return Err(CommunicationTranslationSourceResultErrorV1::InvalidPayload),
    };
...
fn seal_request(
    draft: &makosh_communication_translation_core::CommunicationTranslationDraftV1,
    prepared: &PreparedSourceV1,
    source: AiPrivateSourceReceiptV1,
    logical_owner_id: &str,
) -> Result<CommunicationTranslationInferenceRequestV1, CommunicationTranslationSourceResultErrorV1>
{
    seal_translation_inference_request_v1(CommunicationTranslationInferenceRequestV1 {
        ...
        source: Some(source),
        target_language: ai_target_language(draft.target_language) as i32,
...
fn validate_result_envelope(
...
) -> Result<[u8; 16], CommunicationTranslationSourceResultErrorV1> {
    ...
    {
        return Err(CommunicationTranslationSourceResultErrorV1::InvalidEnvelope);
    }
...
fn context_id(run_id: &[u8; 16], evidence_id: &[u8; 16]) -> [u8; 16] {
    ...
    digest.update(b"makosh.communication_translation.context.v1\0");
    digest.update(run_id);
    digest.update(evidence_id);
    digest.finalize()[..16].try_into().expect("digest prefix")
}

const fn ai_target_language(value: CommunicationTranslationLanguageV1) -> AiTranslationLanguageV1 {
    match value {
...
fn id16(value: &[u8]) -> Result<[u8; 16], CommunicationTranslationSourceResultErrorV1> {
...
fn sha256(value: &[u8]) -> Result<[u8; 32], CommunicationTranslationSourceResultErrorV1> {
...
fn event_error(_: RuntimePullDeliveryErrorV1) -> CommunicationTranslationSourceResultErrorV1 {
...
mod tests {
    ...
    fn ai_mapping_is_fixed_and_provider_neutral() {
        assert_eq!(
            ai_target_language(CommunicationTranslationLanguageV1::Spanish),
            AiTranslationLanguageV1::AiTranslationLanguageSpanish
        );
        assert_ne!(
