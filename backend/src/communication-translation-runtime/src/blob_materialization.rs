...
use makosh_ai_contracts::{
    ...
    AI_OWNER_V1, encode_translation_source_content_v1,
    wire::{AiPrivateSourceReceiptV1, AiTranslationSourceContentV1},
};
...
use makosh_communication_translation_persistence::CommunicationTranslationBlobCleanupV1;
use makosh_communications_ai_source_api::decode_communication_translation_source_content_v1;
...
use crate::COMMUNICATION_TRANSLATION_BLOB_CAPABILITY_ID_V1;

const MAX_PROOF_BYTES_V1: usize = 2_048;

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct CommunicationTranslationSourceBlobReceiptV1 {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) struct CommunicationTranslationBlobMaterializationV1 {
    pub ai_source: AiPrivateSourceReceiptV1,
    pub source_cleanup: CommunicationTranslationBlobCleanupV1,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum CommunicationTranslationBlobErrorV1 {
    InvalidReceipt,
    Unavailable,
}
...
pub(crate) fn materialize_translation_source_for_ai_v1(
    ...
    run_id: [u8; 16],
    source: &CommunicationTranslationSourceBlobReceiptV1,
) -> Result<CommunicationTranslationBlobMaterializationV1, CommunicationTranslationBlobErrorV1> {
    validate_source(source)?;
    let transfer = request_managed_blob_custody_transfer_v2(
        channel,
        dispatcher,
        ManagedBlobCustodyTransferRequestV1 {
            capability_id: COMMUNICATION_TRANSLATION_BLOB_CAPABILITY_ID_V1,
    ...
    .map_err(|_| CommunicationTranslationBlobErrorV1::Unavailable)?;
    ...
    let source_content = decode_communication_translation_source_content_v1(&raw_source)
        .map_err(|_| CommunicationTranslationBlobErrorV1::InvalidReceipt)?;
    let encoded = Zeroizing::new(
        encode_translation_source_content_v1(&AiTranslationSourceContentV1 {
    ...
    if !(1..=AI_MAX_PRIVATE_SOURCE_BYTES_V1).contains(&declared_bytes) {
        return Err(CommunicationTranslationBlobErrorV1::InvalidReceipt);
    }
    ...
    Ok(CommunicationTranslationBlobMaterializationV1 {
...
pub(crate) fn release_translation_source_blobs_v1(
    ...
    source_cleanup: &CommunicationTranslationBlobCleanupV1,
    accepted: bool,
) -> Result<(), CommunicationTranslationBlobErrorV1> {
...
fn read_exact(
    ...
    declared_bytes: u64,
    sha256: &[u8; 32],
) -> Result<Zeroizing<Vec<u8>>, CommunicationTranslationBlobErrorV1> {
...
fn ai_reference_id(run_id: [u8; 16], local_reference: [u8; 16], sha256: [u8; 32]) -> [u8; 16] {
    ...
    digest.update(b"makosh.communication_translation.ai-source.v1\0");
...
fn release_operation_id(run_id: [u8; 16], label: &[u8]) -> [u8; 16] {
    let mut digest = Sha256::new();
    digest.update(b"makosh.communication_translation.blob-release.v1\0");
    digest.update(label);
    digest.update(run_id);
    digest.finalize()[..16].try_into().expect("digest prefix")
}

fn id16(value: &[u8]) -> Result<[u8; 16], CommunicationTranslationBlobErrorV1> {
    value
        .try_into()
        .ok()
        .filter(|value: &[u8; 16]| value.iter().any(|byte| *byte != 0))
        .ok_or(CommunicationTranslationBlobErrorV1::InvalidReceipt)
}

fn id32(value: &[u8]) -> Result<[u8; 32], CommunicationTranslationBlobErrorV1> {
    value
        .try_into()
        .ok()
