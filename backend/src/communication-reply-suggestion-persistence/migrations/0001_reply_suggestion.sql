CREATE TABLE makosh_data.communication_reply_suggestion_runs (
    ...
    requested_tone SMALLINT NOT NULL,
    ...
    CHECK (requested_tone BETWEEN 1 AND 4),
    ...
    CHECK (
        (state = 4 AND candidate_subject_utf8 IS NOT NULL
          AND length(candidate_subject_utf8) <= 998
          AND candidate_body_utf8 IS NOT NULL
          AND length(candidate_body_utf8) BETWEEN 1 AND 65536
          AND candidate_resolved_tone BETWEEN 1 AND 4
        ...
        OR
        (state = 5 AND candidate_subject_utf8 IS NULL
          AND candidate_body_utf8 IS NULL AND rejection_code BETWEEN 1 AND 4)
        OR
        (state IN (1, 2, 3) AND candidate_subject_utf8 IS NULL
          AND candidate_body_utf8 IS NULL AND rejection_code IS NULL)
    )
);
