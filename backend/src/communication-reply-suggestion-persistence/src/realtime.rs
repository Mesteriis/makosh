...
use crate::{
    ...
    model::{REPLY_SUGGESTION_REALTIME_LIMIT_V1, valid_identity, valid_timestamp},
    repository::state_from_code,
};

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct ReplySuggestionRealtimeTransitionV1 {
    pub sequence: u64,
    pub run_id: [u8; 16],
...
impl crate::CommunicationReplySuggestionPersistenceV1 {
    pub async fn client_realtime_window(
        &self,
        logical_owner_id: &str,
        after_sequence: Option<u64>,
        limit: u16,
    ) -> Result<Vec<ReplySuggestionRealtimeTransitionV1>, ReplySuggestionPersistenceErrorV1> {
        if !valid_identity(logical_owner_id)
            || after_sequence == Some(0)
            || !(1..=REPLY_SUGGESTION_REALTIME_LIMIT_V1).contains(&limit)
        {
...
pub(crate) async fn insert_realtime_transition(
    ...
    if inserted == 1 {
        ...
        Err(ReplySuggestionPersistenceErrorV1::InvalidRow)
    }
}

fn transition_from_row(
    row: sqlx::postgres::PgRow,
) -> Result<ReplySuggestionRealtimeTransitionV1, ReplySuggestionPersistenceErrorV1> {
    ...
    Ok(ReplySuggestionRealtimeTransitionV1 {
...
fn rejection_from_code(
    value: i16,
) -> Result<ReplySuggestionRejectionCodeV1, ReplySuggestionPersistenceErrorV1> {
    match value {
