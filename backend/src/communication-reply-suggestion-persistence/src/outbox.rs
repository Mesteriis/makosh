...
use crate::{
    ...
    model::{
        REPLY_SUGGESTION_MAX_EVENT_BYTES_V1, REPLY_SUGGESTION_OUTBOX_LIMIT_V1, nonzero,
...
impl CommunicationReplySuggestionPersistenceV1 {
    pub async fn unpublished_source_prepare_events(
        ...
        limit: u16,
    ) -> Result<Vec<UnpublishedReplySuggestionEventV1>, ReplySuggestionPersistenceErrorV1> {
        if !valid_identity(logical_owner_id)
            || !(1..=REPLY_SUGGESTION_OUTBOX_LIMIT_V1).contains(&limit)
        {
            return Err(ReplySuggestionPersistenceErrorV1::InvalidInput);
        }
        sqlx::query(
            "SELECT message_id, envelope_sha256, envelope_bytes
             FROM makosh_data.communication_reply_suggestion_outbox
        ...
        .map(|row| {
            ...
            let envelope_bytes: Vec<u8> = row
                ...
                .map_err(|_| ReplySuggestionPersistenceErrorV1::InvalidRow)?;
            let event = UnpublishedReplySuggestionEventV1 {
            ...
            if !nonzero(&event.message_id)
                ...
                || event.envelope_bytes.len() > REPLY_SUGGESTION_MAX_EVENT_BYTES_V1
            {
    ...
    pub async fn mark_source_prepare_published(
        ...
        let updated = sqlx::query(
            "UPDATE makosh_data.communication_reply_suggestion_outbox
