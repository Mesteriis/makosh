...
fn main() {
    ...
    let descriptor = output.join("communication-translation-v1.bin");
    ...
    std::fs::write(
        output.join("communication_translation_schema.rs"),
        format!("pub const COMMUNICATION_TRANSLATION_SCHEMA_SHA256: [u8; 32] = {digest:?};\n"),
    )
    .expect("Communication translation schema digest must be written");
}
