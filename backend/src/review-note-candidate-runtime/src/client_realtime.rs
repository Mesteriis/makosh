...
pub(crate) struct ReviewNoteCandidateClientRealtimePublisherV1 {
    last_sequence: u64,
}

impl ReviewNoteCandidateClientRealtimePublisherV1 {
    pub(crate) async fn publish_pending(
    ...
    ) -> Result<bool, ReviewNoteCandidateClientRealtimeErrorV1> {
        let transitions = persistence
            ...
            .await
            .map_err(ReviewNoteCandidateClientRealtimeErrorV1::Persistence)?;
        ...
        for transition in transitions {
            ...
            let response = channel
                ...
                .map_err(|_| ReviewNoteCandidateClientRealtimeErrorV1::Unavailable)?;
            if !response.error_code.is_empty() {
                return Err(ReviewNoteCandidateClientRealtimeErrorV1::Unavailable);
            }
...
fn publish_request(
    logical_owner_id: &str,
    transition: &ReviewNoteCandidateRealtimeTransitionV1,
) -> Result<ManagedRuntimeClientRealtimePublishRequestV1, ReviewNoteCandidateClientRealtimeErrorV1>
{
    let occurred_at_unix_millis = u64::try_from(transition.occurred_at_unix_millis)
        .map_err(|_| ReviewNoteCandidateClientRealtimeErrorV1::InvalidTransition)?;
    let request = ManagedRuntimeClientRealtimePublishRequestV1 {
        ...
        cursor: format!("review-note-candidate/{}", transition.sequence),
        event_kind: REVIEW_NOTE_CANDIDATE_REALTIME_EVENT_KIND_V1.to_owned(),
...
pub(crate) enum ReviewNoteCandidateClientRealtimeErrorV1 {
    InvalidTransition,
    ...
    fn transition_builds_shared_replayable_frame() {
        let request = publish_request(
            ...
            &ReviewNoteCandidateRealtimeTransitionV1 {
                ...
                review_id: [1; 16],
                candidate_id: [2; 16],
                state: ReviewNoteCandidateStateV1::Approved,
                promotion_status: ReviewNoteCandidatePromotionStatusV1::Pending,
                review_revision: 2,
                occurred_at_unix_millis: 1_800_000_000_000,
            },
        )
        .expect("frame");
        assert_eq!(request.cursor, "review-note-candidate/9");
        assert_eq!(
            validate_managed_client_realtime_publish_request_v1(&request),
            Ok(())
        );
