//! Focused managed conformance for reversible Mail-owned message location mutations.
...
fn managed_mail_message_location_is_exact_replay_safe_and_restart_safe() {
    assert_eq!(
        std::env::var("MAKOSH_STORAGE_AUTHENTICATED_TEST").as_deref(),
        Ok("1")
    );
    let imap = MailImapFixture::start();
    let root = unique_target_root("makosh-managed-mail-message-location");
    ...
    sync_mail(
        ...
        &mail,
        1,
        "managed-mail-message-location-sync",
    );
