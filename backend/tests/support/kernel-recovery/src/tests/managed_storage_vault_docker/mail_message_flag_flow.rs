//! Focused managed conformance for Mail-owned message flag mutations.
...
fn managed_mail_message_flags_reconcile_provider_and_projection() {
    assert_eq!(
        std::env::var("MAKOSH_STORAGE_AUTHENTICATED_TEST").as_deref(),
        Ok("1")
    );
    let imap = MailImapFixture::start();
    let root = unique_target_root("makosh-managed-mail-message-flags");
