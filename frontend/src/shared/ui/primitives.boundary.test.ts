...
describe('Макошь UI primitive component contracts', () => {
	it('keeps the primitive batch documented and exported through the UI kit', () => {
	...
	it('keeps primitive components UI-only', () => {
		const uiRoot = fileURLToPath(new URL('.', import.meta.url))
	...
	it('keeps primitive components represented in Storybook', () => {
		const storySource = readFileSync(
			fileURLToPath(new URL('../../../stories/ui/Primitives.stories.ts', import.meta.url)),
			'utf8'
		)
