...
describe('Mail operational active route boundary', () => {
	it('uses separate Mail read, composition, sync, delivery and status contracts without domain coupling', () => {
		...
		expect(route).toContain('mailConnectionCredentialRequired')
		expect(route).toContain("@open-settings=\"emit('openSettings')\"")
		expect(presentation).toContain('Mail account authorization required')
		expect(presentation).toContain('Open Mail settings')
		expect(appLayout).toContain('@open-settings="openMailSettings"')
		expect(appLayout).toContain(':initial-owner="settingsInitialOwner"')
		...
		expect(appLayout).toContain("'mail.operational.realtime.shared.v1'")
