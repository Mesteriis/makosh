...
import {
	...
	type WhatsAppOperationalReplayModel,
} from '../presentation/whatsAppOperationalReplayModel'
import type { WhatsAppOperationalReadState } from '../presentation/whatsAppOperationalReadModel'
import { whatsAppOperationalReplayAccounts } from './whatsAppOperationalAccounts'

export function useWhatsAppOperationalReplay(input: {
	canReplay: () => boolean
	...
	const frames = shallowRef<readonly WhatsAppOperationalReplayFrameV1[]>([])
	let generation = 0

	const accounts = computed(() => whatsAppOperationalReplayAccounts(input.modules()))
	const model = computed<WhatsAppOperationalReplayModel>(() => (
		buildWhatsAppOperationalReplayModel({
	...
	async function reconcile(): Promise<void> {
		...
		if (!input.canReplay()) {
			clear('WhatsApp operational realtime capability is not admitted.')
			return
		}
		if (available.length === 0) {
			clear('No admitted WhatsApp realtime account is available in effective integration settings.')
			state.value = 'empty'
			return
		}
	...
	async function refresh(): Promise<void> {
		...
		begin('Loading WhatsApp replay window…')
		resetReplay()
		try {
			const response = await replayWhatsAppOperationalEvents({
	...
	async function loadMore(): Promise<void> {
		...
		const token = ++generation
		begin('Loading more WhatsApp replay frames…')
		try {
			const response = await replayWhatsAppOperationalEvents({
				accountId: selectedAccountId.value,
	...
	function applyResponse(
		response: Awaited<ReturnType<typeof replayWhatsAppOperationalEvents>>,
		append: boolean,
	): void {
	...
	function readyForReplay(): boolean {
		...
		if (!selectedAccountId.value) {
			clear('Select an admitted WhatsApp realtime account.')
			state.value = 'empty'
			return false
		}
		return true
	...
	function finish(): void {
		...
		statusMessage.value = frames.value.length === 0
			? 'No WhatsApp realtime frames are available for this account.'
			: ''
	}
	...
	function fail(error: unknown, token: number): void {
		...
		statusMessage.value = error instanceof Error
			? error.message
			: 'WhatsApp operational replay is unavailable.'
	}
...
function appendUniqueFrames(
	current: readonly WhatsAppOperationalReplayFrameV1[],
	next: readonly WhatsAppOperationalReplayFrameV1[],
): readonly WhatsAppOperationalReplayFrameV1[] {
