...
describe('Telegram account Settings composition', () => {
	it('coordinates user-account setup into provider QR without a bot credential surface', () => {
		...
		expect(pairing).toContain('data-testid="telegram-qr-placeholder"')
		expect(pairing).toContain('<TelegramCloudPasswordForm')
		expect(pairing).toContain("pairing.state.value === 'waiting_password'")
		...
		for (const source of [settings, setup, pairing, cloudPassword, coordinator]) {
